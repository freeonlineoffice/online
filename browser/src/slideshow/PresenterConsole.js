...
class PresenterConsole {
		...
		return `
			...
			<body>
                                ...
                                <main id="main-content">
								  <div id="toolbar">
									<button type="button" id="close-slides" data-looltip="${this.labels.goBack}" aria-label="${this.labels.goBack}">
                                    ...
                                    <div id="first-presentation">
										<div id="timer-container">
											<div id="timer"></div>
											 <div id="timer-controls">
												<button type="button" id="pause" data-looltip="${this.labels.pause}" aria-label="${this.labels.pause}">
													<img src="${LOUtil.getImageURL('presenterscreen-ButtonPauseTimerNormal.svg')}">
												</button>
												<button type="button" id="restart" data-looltip="${this.labels.restart}" aria-label="${this.labels.restart}">
                                        ...
                                        <div id='current-slide-container'>
											...
											<div id="navigation-container">
												<button type="button" id="prev" data-looltip="${this.labels.previous}" aria-label="${this.labels.previous}">
												...
												<button type="button" id="next" data-looltip="${this.labels.next}" aria-label="${this.labels.next}">
													<img src="${LOUtil.getImageURL('presenterscreen-ButtonEffectNextSelected.svg')}">
												</button>
											</div>
											<div id="action-buttons-container">
												<button type="button" id="notes" data-looltip="${this.labels.notes}" aria-label=${this.labels.notes}">
													<img src="${LOUtil.getImageURL('presenterscreen-ButtonNotesNormal.svg')}">
												</button>
												<button type="button" id="slides" data-looltip="${this.labels.slides}" aria-label="${this.labels.slides}">
	...
	_onPresentInConsole() {
		...
		this.tooltip.style.boxShadow =
			'rgba(77, 77, 77, 0.5) 0px 0px 4px 0px';
					...
					function () {
						...
						if (!button.disable)
							button.style.backgroundColor =
								this.PresenterConsoleBtnHoverColor;

						button.style.borderRadius =
							this.PresenterConsoleBtnRadius;
						const tooltipText =
							button.getAttribute('data-looltip') ||
							'Button'; // Default text if no attribute
	...
	_createTextScalerContainer() {
		...
		plusButton.setAttribute('data-looltip', this.labels.zoomIn); // Set the tooltip text

		// Create the image for the plus button
		let plusImage = this._proxyPresenter.document.createElement('img');
		plusImage.src = LOUtil.getImageURL(
			'presenterscreen-ButtonPlusNormal.svg',
		);
		...
		minusButton.setAttribute('data-looltip', this.labels.zoomOut); // Set the tooltip text
		...
		let fontScalerButtons =
			fontChangeContainer.querySelectorAll('button');
	...
	_pauseButton() {
		...
		if (this._pause) {
			...
			pauseBtn.setAttribute('data-looltip', this.labels.resume); // Set the tooltip text
		} else {
			imgElem.src =
				'images/presenterscreen-ButtonPauseTimerNormal.svg';
			pauseBtn.setAttribute('data-looltip', this.labels.pause); // Set the tooltip text
		}
		...
		if (!this._timerReset) {
			this._showTooltip(
				pauseBtn,
				pauseBtn.getAttribute('data-looltip'),
			);
			return;
		}
	...
	_onToolbarClick(e) {
		...
		const isLastSlide =
			this._currentIndex + 1 == this._visibleSlidesCount;
		switch (target.id) {
			case 'prev': {
	...
	_onShowNotes() {
		...
		let textScaler =
			this._proxyPresenter.document.querySelector('#textScaler');
		...
		this._resizeTimeout = this._proxyPresenter.setTimeout(
			function () {
				...
				if (previews && typeof this._currentIndex !== 'undefined') {
					...
					if (
						preview.width >=
							this._proxyPresenter.innerWidth ||
						preview.height >=
							this._proxyPresenter.innerHeight ||
						preview.width < width
					) {
	...
	drawNext(elem) {
		...
		if (nextSlideIndex >= this._getSlidesCount()) {
			this.drawEnd(size, this._getRepeatDuration()).then(
				function (blob) {
