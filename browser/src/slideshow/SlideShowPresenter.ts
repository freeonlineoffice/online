...
class SlideShowPresenter {
	...
	private _initializeSlideNavWidget(container: HTMLDivElement): void {
		...
		animationsImage.addEventListener(
			...
			function (this: SlideShowPresenter, e: Event) {
				...
				animationsImage.setAttribute(
					'aria-label',
					slideshowAnimToggleText,
				);
				animationsImage.setAttribute(
					'data-looltip',
					slideshowAnimToggleText,
	...
	private startLoader(): void {
		try {
			this._canvasLoader = new CanvasLoaderGl(
				this._slideRenderer._context,
			);
		} catch (error) {
			this._canvasLoader = new CanvasLoader2d(
				this._slideRenderer._context,
			);
		}
	...
	onSlideShowInfo(data: PresentationInfo) {
		...
		if (!this._metaPresentation) {
			...
			this._slideShowHandler.setMetaPresentation(
				this._metaPresentation,
			);
			this._slideShowNavigator.setMetaPresentation(
				this._metaPresentation,
			);
		} else {
			...
			const currentSlideHash =
				this._metaPresentation.getCurrentSlideHash();
			if (this._presentationInfoChanged || currentSlideHash) {
				...
				const slideInfo =
					this._metaPresentation.getSlideInfo(currentSlideHash);
				this._startSlide = slideInfo
					? slideInfo.indexInSlideShow
					: 0;
				skipTransition = true;
			} else {
