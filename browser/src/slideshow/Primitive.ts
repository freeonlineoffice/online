...
class Primitive {
	...
	static cloneDeep(oldPrimitive: Primitive): Primitive {
		...
		newPrimitive.operations = oldPrimitive.operations.map((operation) => {
			...
			newOperation.interpolate =
				operation.interpolate.bind(newOperation);
			return newOperation;
		});
	...
	public pushTriangle(
		...
		verts.push([
			2 * slideLocation0[0] - 1,
			-2 * slideLocation0[1] + 1,
			0.0,
		]);
		verts.push([
			2 * slideLocation1[0] - 1,
			-2 * slideLocation1[1] + 1,
			0.0,
		]);
		verts.push([
			2 * slideLocation2[0] - 1,
			-2 * slideLocation2[1] + 1,
			0.0,
		]);
