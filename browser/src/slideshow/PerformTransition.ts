...
SlideShow.PerformTransition = function (
	...
	switch (stringToTransitionTypeMap[slideInfo.transitionType]) {
		...
		case TransitionType.BARWIPE:
			...
			new SlideShow.WipeTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
		...
		case TransitionType.PINWHEELWIPE:
			if (
				...
				if (
					transitionSubType == TransitionSubType.TWOBLADEVERTICAL
				) {
					subTypeIndex = 2;
				} else if (
					transitionSubType == TransitionSubType.THREEBLADE
				) {
					subTypeIndex = 3;
				} else if (
					transitionSubType == TransitionSubType.FOURBLADE
				) {
					subTypeIndex = 4;
				} else if (
					transitionSubType == TransitionSubType.EIGHTBLADE
				) {
					subTypeIndex = 8;
				} else {
					subTypeIndex = 1;
			...
			new SlideShow.WheelTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
		...
		case TransitionType.SLIDEWIPE:
			if (
				...
				if (transitionSubType == TransitionSubType.FROMTOP) {
					...
					transitionSubType == TransitionSubType.FROMLEFT
				) {
					subTypeIndex = 2;
				} else if (
					transitionSubType == TransitionSubType.FROMRIGHT
				) {
					subTypeIndex = 4;
				} else {
					subTypeIndex = 3;
		...
		case TransitionType.RANDOMBARWIPE:
			...
			new SlideShow.BarsTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
		...
		case TransitionType.IRISWIPE:
			...
			new SlideShow.BoxTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
		...
		case TransitionType.BLINDSWIPE:
			...
			new SlideShow.VenetianTransition(canvas, image1, image2).start(
				1,
			);
			break;
		...
		case TransitionType.PUSHWIPE:
			...
			new SlideShow.PushTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
		...
		case TransitionType.BARNDOORWIPE:
			...
			new SlideShow.SplitTransition(canvas, image1, image2).start(
				subTypeIndex,
			);
			break;
