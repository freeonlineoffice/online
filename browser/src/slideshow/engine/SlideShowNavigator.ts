...
class SlideShowNavigator {
	...
	skipEffect() {
		NAVDBG.print(
			'SlideShowNavigator.skipEffect: current index: ' +
				this.currentSlide,
		);
	...
	skipAllEffects() {
		NAVDBG.print(
			'SlideShowNavigator.skipAllEffects: current index: ' +
				this.currentSlide,
		);
	...
	goToLastSlide() {
		NAVDBG.print(
			'SlideShowNavigator.goToLastSlide: current index: ' +
				this.currentSlide,
		);
	...
	displaySlide(nNewSlide: number, bSkipTransition: boolean) {
		...
		if (!aNewMetaSlide) {
			...
			NAVDBG.print(
				'SlideShowNavigator.displaySlide: hidden slide: ' +
					nNewSlide,
			);
			slideAvailable = false;
		}
	...
	private clickHandler(aEvent: MouseEvent) {
		if (aEvent.button === 0) {
			...
			const y =
				(aEvent.offsetY / height) * this.theMetaPres.slideHeight;
