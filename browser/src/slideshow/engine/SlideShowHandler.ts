...
class SlideShowHandler {
	...
	skipNEffects(nEffectNumber: number) {
		for (let i = 0; i <= nEffectNumber; i++)
			if (!this.skipNextEffect()) break;
	}
	...
	cleanLeavingSlideStatus(nOldSlide: number, bSkipSlideTransition: boolean) {
		...
		if (nOldSlide !== undefined) {
			this.slideCompositor.pauseVideos(
				aMetaDoc.getSlideHash(nOldSlide),
			);
			...
			if (
				...
				if (
					...
					try {
						...
						this.enteringSlideTexture =
							transitionParameters.next;
						const aTransitionActivity =
