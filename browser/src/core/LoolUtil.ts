...
namespace lool {
	export namespace Util {
		...
		export function stamp(obj: IDAble): number {
			...
			obj._leaflet_id = lool.Util.nextId();
			return obj._leaflet_id;
		}
		...
		export function throttle(
			fn: lool.Util.CallbackFunctionVariadic,
			time: number,
		...
		export function trimEnd(str: string, suffix: string): string {
			...
			if (
				suffixIndex !== -1 &&
				str.length - suffix.length === suffixIndex
			)
		...
		export function trim(
			...
			if (!prefix)
				return str.trim
					? str.trim()
					: str.replace(/^\s+|\s+$/g, '');
			let result = lool.Util.trimStart(str, prefix);
			result = lool.Util.trimEnd(result, suffix);
			return result;
		}

		// split a string into words
		export function splitWords(str: string): string[] {
			return lool.Util.trim(str).split(/\s+/);
		}
		...
		export function template(str: string, data: any): string {
			return str.replace(lool.Util.templateRe, function (str, key) {
		...
		export function getTextWidth(text: string, font: string): number {
			const canvas = lool.Util.getTempCanvas();
		...
		export function getProduct(): string {
			...
			if (customWindow.feedbackUrl && customWindow.buyProductUrl) {
				const integratorUrl = encodeURIComponent(
					customWindow.buyProductUrl,
				);
				brandFAQURL = customWindow.feedbackUrl;
				brandFAQURL =
					brandFAQURL.substring(
						0,
						brandFAQURL.lastIndexOf('/'),
					) +
