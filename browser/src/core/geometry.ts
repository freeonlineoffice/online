...
namespace lool {
	...
	export class SimplePoint {
		...
		public equals(point: Array<number>): boolean {
			return (
				this._x === Math.round(point[0]) &&
				this._y === Math.round(point[1])
			);
		}
		public toArray(): number[] {
			return [this._x, this._y];
		}
		public distanceTo(point: number[]): number {
			return Math.sqrt(
				Math.pow(this._x - point[0], 2) +
					Math.pow(this._y - point[1], 2),
			);
		}

		// Core / canvas pixel.
		public get pX(): number {
			return Math.round(this._x * app.twipsToPixels);
		}
		public set pX(x: number) {
			this._x = Math.round(x * app.pixelsToTwips);
		}

		public get pY(): number {
			return Math.round(this._y * app.twipsToPixels);
		}
		public set pY(y: number) {
			this._y = Math.round(y * app.pixelsToTwips);
		}

		public pEquals(point: Array<number>): boolean {
			return (
				this.pX === Math.round(point[0]) &&
				this.pY === Math.round(point[1])
			);
		}
		public pToArray(): number[] {
			return [this.pX, this.pY];
		}
		public pDistanceTo(point: number[]): number {
			return Math.sqrt(
				Math.pow(this.pX - point[0], 2) +
					Math.pow(this.pY - point[1], 2),
			);
		}

		// CSS pixel.
		public get cX(): number {
			return Math.round((this._x * app.twipsToPixels) / app.dpiScale);
		}
		public set cX(x: number) {
			this._x = Math.round(x * app.dpiScale * app.pixelsToTwips);
		}

		public get cY(): number {
			return Math.round((this._y * app.twipsToPixels) / app.dpiScale);
		}
		public set cY(y: number) {
			this._y = Math.round(y * app.dpiScale * app.pixelsToTwips);
		}

		public cEquals(point: Array<number>): boolean {
			return (
				this.cX === Math.round(point[0]) &&
				this.cY === Math.round(point[1])
			);
		}
		public cToArray(): number[] {
			return [this.cX, this.cY];
		}
		public cDistanceTo(point: number[]): number {
			return Math.sqrt(
				Math.pow(this.cX - point[0], 2) +
					Math.pow(this.cY - point[1], 2),
			);
		}

		public clone(): SimplePoint {
			return new SimplePoint(this._x, this._y);
		}

		public static fromCorePixels(point: Array<number>): SimplePoint {
			return new SimplePoint(
				Math.round(point[0] * app.pixelsToTwips),
				Math.round(point[1] * app.pixelsToTwips),
			);
		}
	}
	...
	export class SimpleRectangle {
		...
		public get x2(): number {
			return this._x1 + this._width;
		}
		public set x2(x2: number) {
			this._width = Math.round(x2) - this._x1;
		}

		public get y2(): number {
			return this._y1 + this._height;
		}
		public set y2(y2: number) {
			this._height = Math.round(y2) - this._y1;
		}
		...
		public containsPoint(point: number[]): boolean {
			return (
				Math.round(point[0]) >= this.x1 &&
				Math.round(point[0]) <= this.x2 &&
				Math.round(point[1]) >= this.y1 &&
				Math.round(point[1]) <= this.y2
			);
		}
		public containsX(x: number): boolean {
			return Math.round(x) >= this.x1 && Math.round(x) <= this.x2;
		}
		public containsY(y: number): boolean {
			return Math.round(y) >= this.y1 && Math.round(y) <= this.y2;
		}
		public containsRectangle(rectangle: number[]): boolean {
			return (
				this.containsPoint([rectangle[0], rectangle[1]]) &&
				this.containsPoint([
		...
		public intersectsRectangle(rectangle: number[]): boolean {
			return app.LOUtil._doRectanglesIntersect(
				this.toArray(),
				rectangle,
			);
		}
		public equals(rectangle: Array<number>): boolean {
			return (
				this.x1 === Math.round(rectangle[0]) &&
				this.y1 === Math.round(rectangle[1]) &&
				this.width === Math.round(rectangle[2]) &&
				this.height === Math.round(rectangle[3])
			);
		}

		public moveTo(point: number[]): void {
			this._x1 = Math.round(point[0]);
			this._y1 = Math.round(point[1]);
		}
		public moveBy(point: number[]): void {
			this._x1 += Math.round(point[0]);
			this._y1 += Math.round(point[1]);
		}

		// Pixel.
		public get pX1(): number {
			return Math.round(this._x1 * app.twipsToPixels);
		}
		public set pX1(x1: number) {
			this._x1 = Math.round(x1 * app.pixelsToTwips);
		}

		public get pY1(): number {
			return Math.round(this._y1 * app.twipsToPixels);
		}
		public set pY1(y1: number) {
			this._y1 = Math.round(y1 * app.pixelsToTwips);
		}

		public get pX2(): number {
			return this.pX1 + this.pWidth;
		}
		public set pX2(x2: number) {
			this._width = Math.round(x2 * app.pixelsToTwips) - this._x1;
		}

		public get pY2(): number {
			return this.pY1 + this.pHeight;
		}
		public set pY2(y2: number) {
			this._height = Math.round(y2 * app.pixelsToTwips) - this._y1;
		}

		public get pWidth(): number {
			return Math.round(this._width * app.twipsToPixels);
		}
		public set pWidth(width: number) {
			this._width = Math.round(width * app.pixelsToTwips);
		}

		public get pHeight(): number {
			return Math.round(this._height * app.twipsToPixels);
		}
		public set pHeight(height: number) {
			this._height = Math.round(height * app.pixelsToTwips);
		}

		public get pArea(): number {
			return this.pWidth * this.pHeight;
		}
		public get pCenter(): number[] {
			return [(this.pX1 + this.pX2) / 2, (this.pY1 + this.pY2) / 2];
		}

		public pToArray(): number[] {
			return [this.pX1, this.pY1, this.pWidth, this.pHeight];
		}

		// Pixel checkers for coordinate match.
		public pContainsPoint(point: number[]): boolean {
			return (
				Math.round(point[0]) >= this.pX1 &&
				Math.round(point[0]) <= this.pX2 &&
				Math.round(point[1]) >= this.pY1 &&
				Math.round(point[1]) <= this.pY2
			);
		}
		public pContainsX(x: number): boolean {
			return Math.round(x) >= this.pX1 && Math.round(x) <= this.pX2;
		}
		public pContainsY(y: number): boolean {
			return Math.round(y) >= this.pY1 && Math.round(y) <= this.pY2;
		}
		public pContainsRectangle(rectangle: number[]): boolean {
			return (
				this.pContainsPoint([rectangle[0], rectangle[1]]) &&
				this.pContainsPoint([
					rectangle[0] + rectangle[2],
					rectangle[1] + rectangle[3],
				])
		...
		public pEquals(rectangle: Array<number>): boolean {
			return (
				this.pX1 === Math.round(rectangle[0]) &&
				this.pY1 === Math.round(rectangle[1]) &&
				this.pWidth === Math.round(rectangle[2]) &&
				this.pHeight === Math.round(rectangle[3])
			);
		}

		public pMoveTo(point: number[]): void {
			this._x1 = Math.round(point[0] * app.pixelsToTwips);
			this._y1 = Math.round(point[1] * app.pixelsToTwips);
		}
		public pMoveBy(point: number[]): void {
			this._x1 += Math.round(point[0] * app.pixelsToTwips);
			this._y1 += Math.round(point[1] * app.pixelsToTwips);
		}
		...
		public get cX1(): number {
			return Math.round((this._x1 * app.twipsToPixels) / app.dpiScale);
		}
		public set cX1(x1: number) {
			this._x1 = Math.round(x1 * app.dpiScale * app.pixelsToTwips);
		}

		public get cY1(): number {
			return Math.round((this._y1 * app.twipsToPixels) / app.dpiScale);
		}
		public set cY1(y1: number) {
			this._y1 = Math.round(y1 * app.dpiScale * app.pixelsToTwips);
		}

		public get cX2(): number {
			return this.cX1 + this.cWidth;
		}
		public set cX2(x2: number) {
			this._width = Math.round(x2 * app.dpiScale * app.pixelsToTwips);
		}

		public get cY2(): number {
			return this.cY1 + this.cHeight;
		}
		public set cY2(y2: number) {
			this._height = Math.round(y2 * app.dpiScale * app.pixelsToTwips);
		}

		public get cWidth(): number {
			return Math.round(
				(this._width * app.twipsToPixels) / app.dpiScale,
			);
		}
		public set cWidth(width: number) {
			this._width = Math.round(
				width * app.dpiScale * app.pixelsToTwips,
			);
		}

		public get cHeight(): number {
			return Math.round(
				(this._height * app.twipsToPixels) / app.dpiScale,
			);
		}
		public set cHeight(height: number) {
			this._height = Math.round(
				height * app.dpiScale * app.pixelsToTwips,
			);
		}

		public get cArea(): number {
			return this.cWidth * this.cHeight;
		}
		public get cCenter(): number[] {
			return [(this.cX1 + this.cX2) / 2, (this.cY1 + this.cY2) / 2];
		}

		public cToArray(): number[] {
			return [this.cX1, this.cY1, this.cWidth, this.cHeight];
		}

		// CSS pixel checkers for coordinate match.
		public cContainsPoint(point: number[]): boolean {
			return (
				Math.round(point[0]) >= this.cX1 &&
				Math.round(point[0]) <= this.cX2 &&
				Math.round(point[1]) >= this.cY1 &&
				Math.round(point[1]) <= this.cY2
			);
		}
		public cContainsX(x: number): boolean {
			return Math.round(x) >= this.cX1 && Math.round(x) <= this.cX2;
		}
		public cContainsY(y: number): boolean {
			return Math.round(y) >= this.cY1 && Math.round(y) <= this.cY2;
		}
		public cContainsRectangle(rectangle: number[]): boolean {
			return (
				this.cContainsPoint([rectangle[0], rectangle[1]]) &&
				this.cContainsPoint([
					rectangle[0] + rectangle[2],
					rectangle[1] + rectangle[3],
				])
		...
		public cEquals(rectangle: Array<number>): boolean {
			return (
				this.cX1 === Math.round(rectangle[0]) &&
				this.cY1 === Math.round(rectangle[this.y1]) &&
				this.cWidth === Math.round(rectangle[2]) &&
				this.cHeight === Math.round(rectangle[3])
			);
		}

		public cMoveTo(point: number[]): void {
			this._x1 = Math.round(
				point[0] * app.dpiScale * app.pixelsToTwips,
			);
			this._y1 = Math.round(
				point[1] * app.dpiScale * app.pixelsToTwips,
			);
		}
		public cMoveBy(point: number[]): void {
			this._x1 += Math.round(
				point[0] * app.dpiScale * app.pixelsToTwips,
			);
			this._y1 += Math.round(
		...
		public static fromCorePixels(
			rectangle: Array<number>,
		): SimpleRectangle {
			return new SimpleRectangle(
				Math.round(rectangle[0] * app.pixelsToTwips),
				Math.round(rectangle[1] * app.pixelsToTwips),
				Math.round(rectangle[2] * app.pixelsToTwips),
				Math.round(rectangle[3] * app.pixelsToTwips),
			);
		}
	}
...
*/
	export function rectanglesToPolygon(
		rectangles: Array<number[]>,
		converterConstant: number = 1,
		tolerance: number = 5,
	): number[] {
		/*
	...
	*/
		...
		for (let i = 0; i < finalRows.length; i++) {
			...
			if (i === 0) {
				...
				if (
					finalRows[i][0] !== polygon[polygon.length - 2] ||
					finalRows[i][1] !== polygon[polygon.length - 1]
				) {
					polygon.push(finalRows[i][0]);
					polygon.push(finalRows[i][1]);
				}
		...
		for (let i = finalRows.length - 1; i >= 0; i--) {
			if (
				finalRows[i][0] + finalRows[i][2] !==
					polygon[polygon.length - 2] ||
				finalRows[i][1] + finalRows[i][3] !==
