		...
		if (app.map._docLayer._docType === 'spreadsheet') {
			for (let i = 0; i < this.sectionProperties.handles.length; i++)
				app.map._docLayer.sheetGeometry.convertToTileTwips(
					this.sectionProperties.handles[i].point,
				);
		}
	}
	...
	onMouseMove(position: number[], dragDistance: number[]) {
		if (
			...
			this.sectionProperties.svg.style.left =
				String(
					(this.myTopLeft[0] + dragDistance[0]) / app.dpiScale,
				) + 'px';
			this.sectionProperties.svg.style.top =
				String(
					(this.myTopLeft[1] + dragDistance[1]) / app.dpiScale,
				) + 'px';
