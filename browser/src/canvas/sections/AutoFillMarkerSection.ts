...
class AutoFillMarkerSection extends CanvasSectionObject {
	...
	private getDocumentPositionFromLocal(point: lool.SimplePoint): lool.SimplePoint {
	...
	private getCenterRegardingDocument(): lool.SimplePoint {
		const p2 = new lool.SimplePoint(0, 0);
		p2.pX += this.position[0] + this.size[0] * 0.5;
		p2.pY += this.position[1] + this.size[1] * 0.5;
		return p2;
	}

	private autoScroll(point: lool.SimplePoint) {
