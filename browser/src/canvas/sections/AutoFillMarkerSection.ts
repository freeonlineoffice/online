...
app.definitions.AutoFillMarkerSection = class AutoFillMarkerSection extends (
	CanvasSectionObject
) {
	...
	constructor() {
		...
		this.backgroundColor = cursorColor
			? cursorColor
		...
		this.selectionBorderWidth = Math.round(
			window.devicePixelRatio *
				parseInt(
					selectionStyle.getPropertyValue('border-top-width'),
				),
		);
	}
	...
	private drawWhiteOuterBorders() {
		...
		if (!desktop) {
			...
			this.context.moveTo(
				transformX(
					this.size[0] - 0.5 - (desktop ? borderWidth : 0),
				),
				-0.5,
			);
			this.context.lineTo(
	...
	public onMouseMove(
		...
		if (!this.sectionProperties.draggingStarted) {
			...
			this.sectionProperties.dragStartPosition =
				this.sectionProperties.docLayer._cellAutoFillAreaPixels.getCenter();
			pos = new L.Point(
		...
		pos = this.sectionProperties.docLayer._corePixelsToTwips(
	...
	public onMouseUp(point: Array<number>, e: MouseEvent) {
		if (this.sectionProperties.draggingStarted) {
			...
			var pos = this.sectionProperties.docLayer._corePixelsToTwips(
				new L.Point(point[0], point[1]),
			);
			this.sectionProperties.docLayer._postMouseEvent(
				'buttonup',
	...
	public onMouseDown(point: Array<number>, e: MouseEvent) {
		if ((<any>window).mode.isDesktop()) {
			if (this.sectionProperties.inMouseDown) return;
			...
			var canvasClientRect =
				this.containerObject.getCanvasBoundingClientRect();
			point[0] =
				(this.myTopLeft[0] + this.size[0] * 0.5 + 1) /
					app.dpiScale +
				canvasClientRect.left;
			point[1] =
				(this.myTopLeft[1] + this.size[1] * 0.5 + 1) /
					app.dpiScale +
				canvasClientRect.top;
			...
			var newEvent =
				this.sectionProperties.docLayer._createNewMouseEvent(
					'mousedown',
					newPoint,
				);
