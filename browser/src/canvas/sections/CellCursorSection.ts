...
class CellCursorSection extends CanvasSectionObject {
	...
	public static adjustSizePos(defaultSizePos: number[]): number[] {
		...
		if (
			!splitPos ||
			(splitPos.x === 0 && splitPos.y === 0) ||
			!app.activeDocument ||
			(app.activeDocument.activeView.viewedRectangle.pX1 === 0 &&
				app.activeDocument.activeView.viewedRectangle.pY1 === 0)
		)
			return defaultSizePos;

		if (
			defaultSizePos[0] < splitPos.x &&
			defaultSizePos[0] + defaultSizePos[2] > splitPos.x
		)
			defaultSizePos[2] = Math.max(
				splitPos.x - defaultSizePos[0],
				defaultSizePos[2] -
		...
		if (
			defaultSizePos[0] >= splitPos.x &&
			app.activeDocument.activeView.viewedRectangle.pX1 + splitPos.x >
				defaultSizePos[0]
		) {
			defaultSizePos[2] =
				defaultSizePos[0] +
		...
		if (
			defaultSizePos[1] < splitPos.y &&
			defaultSizePos[1] + defaultSizePos[3] > splitPos.y
		)
			defaultSizePos[3] = Math.max(
				splitPos.y - defaultSizePos[1],
				defaultSizePos[3] -
		...
		if (
			defaultSizePos[1] >= splitPos.y &&
			app.activeDocument.activeView.viewedRectangle.pY1 + splitPos.y >
				defaultSizePos[1]
		) {
			defaultSizePos[3] =
				defaultSizePos[1] +
		...
		if (app.calc.cellCursorVisible) {
			...
			if (window.prefs.getBoolean('darkTheme')) {
				...
				this.context.strokeRect(
					x + -0.5 + diff,
					y - 0.5 + diff,
					tempSizePos[2] - 2 * diff,
					tempSizePos[3] - 2 * diff,
				);
				this.context.strokeRect(
					x + -0.5 + diff,
					y - 0.5 + diff,
