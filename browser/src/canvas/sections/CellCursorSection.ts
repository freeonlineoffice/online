...
class CellCursorSection extends CanvasSectionObject {
	...
	private getContainingPane() {
		...
		if (rectangles.length > 1) {
			...
			for (let i = 0; i < rectangles.length; i++) {
				if (
					rectangles[i].pContainsRectangle([
						this.position[0],
						this.position[1],
			...
			return null;
		} else return app.calc.cellCursorRectangle;
	}
	...
	public onDraw() {
		...
		if (app.calc.cellCursorVisible && pane) {
			...
			const movePen =
				this.sectionProperties.paneIndex !== -1 &&
				this.sectionProperties.paneCount - 1 !==
					this.sectionProperties.paneIndex;
			...
			if (movePen) {
				...
				this.context.translate(
					penX - this.myTopLeft[0],
					penY - this.myTopLeft[1],
				);
			}
			...
			for (let i: number = 0; i < this.sectionProperties.weight; i++)
				this.context.strokeRect(
					x + -0.5 - i,
					-0.5 - i,
					app.calc.cellCursorRectangle.pWidth + i * 2,
					app.calc.cellCursorRectangle.pHeight + i * 2,
				);
			...
			if (window.prefs.getBoolean('darkTheme')) {
				...
				this.context.strokeRect(
					x + -0.5 + diff,
					-0.5 + diff,
					app.calc.cellCursorRectangle.pWidth - 2 * diff,
	...
	onNewDocumentTopLeft(size: Array<number>): void {
		if (this.getContainingPane()) this.isVisible = true;
	}
