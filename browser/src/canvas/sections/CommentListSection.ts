...
export class CommentSection extends CanvasSectionObject {
	...
	constructor () {
		...
		public onInitialize(): void {
			...
			this.map.on(
				'AnnotationScrollUp',
				this.onAnnotationScrollUp,
				this,
			);
			this.map.on(
				'AnnotationScrollDown',
				this.onAnnotationScrollDown,
				this,
			);
			...
			this.map.on(
				...
				function (event: any) {
					if (
						...
						event.commandName === '.uno:ShowTrackedChanges' &&
						event.state === 'true'
					)
	...
	public static showCommentEditingWarning (): void {
		...
		public static showCommentEditingWarning(): void {
			L.Map.THIS.uiManager.showInfoModal(
				'annotation-editing',
				_('A comment is being edited'),
				_(
					'Please save or discard the comment currently being edited.',
				),
				null,
				_('Close'),
			);
		...
		private findNextPartWithComment(currentPart: number): number {
			for (
				var i = 0;
				i < this.sectionProperties.commentList.length;
				i++
			) {
				if (
					this.sectionProperties.commentList[i].sectionProperties
						.partIndex > currentPart
				) {
					return this.sectionProperties.commentList[i]
						.sectionProperties.partIndex;
				}
			}
			return -1;
		}
		...
		public onAnnotationScrollDown(): void {
			var index = this.findNextPartWithComment(
		...
		public onAnnotationScrollUp(): void {
			var index = this.findPreviousPartWithComment(
				app.map._docLayer._selectedPart,
			);
			if (index >= 0) {
				this.map.setPart(index);
		...
		private checkSize(): void {
			...
			if (
				...
				if (
					app.map._docLayer._docType === 'presentation' &&
					this.sectionProperties.scrollAnnotation
				) {
				...
				// If there are comments but none of them are on the selected part.
				if (!this.sectionProperties.scrollAnnotation) {
					this.sectionProperties.scrollAnnotation =
						L.control.scrollannotation();
					this.sectionProperties.scrollAnnotation.addTo(
						this.map,
					);
				}
			}
		...
		private isEditing(): boolean {
			const textBoxes = this.sectionProperties.commentList
				.flatMap((section: any) => [
					section.sectionProperties.nodeModifyText,
					section.sectionProperties.nodeReplyText,
				])
		...
		public shouldCollapse(): boolean {
			if (
				!this.containerObject.getDocumentAnchorSection() ||
				app.map._docLayer._docType === 'spreadsheet' ||
				(<any>window).mode.isMobile()
			)
				return false;
			...
			for (
				...
				if (app.map._docLayer._docType === 'spreadsheet') {
					...
					if (
						parseInt(
							this.sectionProperties.commentList[i]
								.sectionProperties.data.tab,
						) === part
					) {
						this.sectionProperties.commentList[
							i
						].drawingOrder = 2;
					} else {
						this.sectionProperties.commentList[
		...
		private getChildren(comment: any, array: Array<any>) {
			for (
				var i = 0;
				i < comment.sectionProperties.children.length;
				i++
			) {
				array.push(comment.sectionProperties.children[i]);
				if (
					comment.sectionProperties.children[i].sectionProperties
						.children.length > 0
				)
					this.getChildren(
						comment.sectionProperties.children[i],
						array,
					);
			}
		}
		...
		private getCommentListOneDimensionalArray() {
			...
			for (
				...
				if (this.sectionProperties.commentList[i].isRootComment()) {
					...
					if (
						this.sectionProperties.commentList[i]
							.sectionProperties.children.length > 0
					)
						this.getChildren(
							this.sectionProperties.commentList[i],
							openArray,
		...
		private createCommentStructureWriter(
			menuStructure: any,
			threadOnly: any,
		): void {
			...
			if (threadOnly) {
				if (
					!threadOnly.sectionProperties.data.trackchange &&
					threadOnly.sectionProperties.data.parent !== '0'
				)
					threadOnly =
						commentList[
							this.getIndexOf(
								threadOnly.sectionProperties.data
									.parent,
							)
						];
			}
			...
			for (var i = 0; i < commentList.length; i++) {
				if (
					commentList[i].isRootComment() ||
					commentList[i].sectionProperties.data.trackchange
				) {
					var commentThread = [];
					do {
						comment = {
							id:
								'comment' +
								commentList[i].sectionProperties.data
									.id,
							enable: true,
							data: commentList[i].sectionProperties.data,
							type: 'comment',
							text: commentList[i].sectionProperties.data
								.text,
						...
						commentList[i] &&
						commentList[i].sectionProperties.data.parent !==
							'0'
					);
					i--;
					...
					if (commentThread.length > 0) {
						rootComment = {
							id: commentThread[commentThread.length - 1]
								.id,
							enable: true,
							data: commentThread[commentThread.length - 1]
								.data,
							type: 'rootcomment',
							text: commentThread[commentThread.length - 1]
								.data.text,
							annotation:
								commentThread[commentThread.length - 1]
									.annotation,
						...
						var matchingThread =
							threadOnly &&
							threadOnly.sectionProperties.data.id ===
								commentThread[0].data.id;
						if (matchingThread)
		...
		public createCommentStructureImpress(
			menuStructure: any,
			threadOnly: any,
		): void {
			var rootComment;
					...
					rootComment = {
						id:
							...
							this.sectionProperties.commentList[i]
								.sectionProperties.data.id,
						enable: true,
						data: this.sectionProperties.commentList[i]
							.sectionProperties.data,
						type: threadOnly ? 'comment' : 'rootcomment',
						text: this.sectionProperties.commentList[i]
		...
		public createCommentStructureCalc(
			menuStructure: any,
			threadOnly: any,
		): void {
			var rootComment;
			...
			for (var i: number = 0; i < commentList.length; i++) {
				var matchingThread =
					...
					(threadOnly &&
						threadOnly.sectionProperties.data.id ===
							commentList[i].sectionProperties.data.id);
				if (
					parseInt(commentList[i].sectionProperties.data.tab) ===
						selectedTab &&
					matchingThread
				) {
					rootComment = {
		...
		public createCommentStructure(
			menuStructure: any,
			threadOnly: any,
		): void {
			if (app.map._docLayer._docType === 'text') {
				this.createCommentStructureWriter(
					menuStructure,
					threadOnly,
				);
			} else if (
				...
				this.createCommentStructureImpress(
					menuStructure,
					threadOnly,
				);
		...
		public newAnnotationMobile(
			...
			var callback = function (div: HTMLDivElement) {
				if (div.textContent || div.innerHTML) {
					...
					annotation.sectionProperties.data.text =
						div.textContent;
			...
			var json = this.map.uiManager._modalDialogJSON(
				...
				[
					{
						...
						type: 'multilineedit',
						text:
							commentData.text && isMod
								? commentData.text
								: '',
						html:
							commentData.html && isMod
								? commentData.html
								: '',
			...
			const mentionListCallback = function (
				objectType: any,
				eventType: any,
				object: any,
				index: number,
				...
				if (eventType === 'close') mention.closeMentionPopup(false);
				else if (
					eventType === 'select' ||
					eventType === 'activate'
				) {
			...
			this.map.uiManager.showModal(json, [
				{
					...
					func: function () {
						...
						this.map.uiManager.closeModal(
							this.mobileCommentModalId,
						);
					}.bind(this),
				},
			...
			app.layoutingService.appendLayoutingTask(() => {
				const multilineEditDiv =
					document.getElementById('input-modal-input');
				multilineEditDiv.addEventListener(
					'input',
					function (ev: any) {
						if (ev && app.map._docLayer._docType === 'text') {
							// special handling for mentions
							this.map?.mention.handleMentionInput(
								ev,
								comment.isNewPara(),
							);
						}
				...
				const contentAuthor = L.DomUtil.create(
					tagDiv,
					'lool-annotation-content-author',
					tdAuthor,
				);
				const contentDate = L.DomUtil.create(
					tagDiv,
				...
				if (user >= 0) {
					const color = app.LOUtil.rgbToHex(
						this.map.getViewColor(user),
					);
				...
				if (commentData.dateTime) {
					const d = new Date(
						commentData.dateTime.replace(/,.*/, 'Z'),
					);
					const dateOptions = {
						weekday: 'short',
						year: 'numeric',
				...
				const newAnnotationDialog = document.getElementById(
					this.mobileCommentId,
				);
		...
		public highlightComment(comment: any): void {
			...
			var lastChild: any = this.getLastChildIndexOf(
			...
			while (true && lastChild >= 0) {
				...
				if (commentList[lastChild].isRootComment()) break;

				lastChild = this.getIndexOf(
					commentList[lastChild].sectionProperties.data.parent,
				);
			}
		}
				...
				comment = {
					...
					annotation.sectionProperties.data.html
						? {
								Html: {
									type: 'string',
									value: annotation.sectionProperties
										.data.html,
								},
							}
						: {
		...
		public reply(annotation: any): void {
			...
			if ((<any>window).mode.isMobile()) {
				var avatar = undefined;
				var author = this.map.getViewName(
					app.map._docLayer._viewId,
				);
				if (author in this.map._viewInfoByUserName) {
					avatar =
						this.map._viewInfoByUserName[author].userextrainfo
							.avatar;
				}
				...
				if (
					...
					this.newAnnotationMobile(
						annotation,
						annotation.onReplyClick,
						/* isMod */ false,
					);
				} else {
					var replyAnnotation = {
						...
						anchorPos: [
							annotation.sectionProperties.data
								.anchorPos[0],
							annotation.sectionProperties.data
								.anchorPos[1],
						],
					...
					const name =
						lool.Comment.makeName(replyAnnotation) + '-reply';
					var replyAnnotationSection = new lool.Comment(
						name,
						replyAnnotation,
						replyAnnotation.id === 'new'
							? { noMenu: true }
							: {},
						this,
					);
		...
		private showCollapsedReplies(rootIndex: number) {
			if (!this.sectionProperties.commentList.length) return;
			var lastIndex = this.getLastChildIndexOf(
				this.sectionProperties.commentList[rootIndex]
					.sectionProperties.data.id,
			);
			...
			while (rootIndex <= lastIndex) {
				this.sectionProperties.commentList[
					rootIndex
				].sectionProperties.container.style.display = '';
				this.sectionProperties.commentList[
					rootIndex
				].sectionProperties.container.style.visibility = '';
				$(
					this.sectionProperties.commentList[rootIndex]
						.sectionProperties.container,
				).removeClass('collapsed-comment');
				rootIndex++;
			}
		...
		private collapseReplies(rootIndex: number, rootId: number) {
			...
			for (var i = lastChild; i > rootIndex; i--) {
				this.sectionProperties.commentList[
					i
				].sectionProperties.container.style.display = 'none';
				$(
					this.sectionProperties.commentList[i].sectionProperties
						.container,
				).addClass('collapsed-comment');
		...
		public select(annotation: Comment, force: boolean = false): void {
			if (
				force ||
				(annotation &&
					!annotation.pendingInit &&
					annotation !==
						this.sectionProperties.selectedComment &&
					(annotation.sectionProperties.data.resolved !==
						'true' ||
						this.sectionProperties.showResolved))
			) {
				// Select the root comment
				var idx = this.getRootIndexOf(
					annotation.sectionProperties.data.id,
				);
				...
				if (this.sectionProperties.selectedComment) this.unselect();
				...
				if (
					...
					!$(
						this.sectionProperties.selectedComment
							.sectionProperties.container,
					).hasClass('annotation-active')
				) {
					$(
						this.sectionProperties.selectedComment
							.sectionProperties.container,
					).addClass('annotation-active');
				}

				if (
					app.map._docLayer._docType === 'text' &&
					this.sectionProperties.showSelectedBigger
				) {
					this.setThreadPopup(
						this.sectionProperties.selectedComment,
						true,
		...
		private scrollCommentIntoView(comment: Comment) {
			if (CommentSection.importingComments) return;
			...
			switch (docType) {
				case 'text': {
					anchorPosition = this.numberArrayToCorePixFromTwips(
						rootComment.sectionProperties.data.anchorPos,
						0,
						2,
					);
					break;
			...
			if (anchorPosition && anchorPosition[1] > 0) {
				...
				const firstIdx = this.getIndexOf(
					rootComment.sectionProperties.data.id,
				);
				const lastIdx = this.getIndexOf(
					comment.sectionProperties.data.id,
				);
				for (let i = firstIdx; i < lastIdx; i++) {
					annotationTop += this.cssToCorePixels(
						this.sectionProperties.commentList[
							i
						].getCommentHeight(),
					);
				...
				const annotationBottom =
					annotationTop +
					this.cssToCorePixels(comment.getCommentHeight());
				...
				if (!this.isInViewPort([annotationTop, annotationBottom])) {
					...
					if (annotationTop < screenTopBottom[0]) {
						scrollSection.scrollVerticalWithOffset(
							annotationTop - screenTopBottom[0],
						);
					} else
						scrollSection.scrollVerticalWithOffset(
							annotationBottom - screenTopBottom[1],
						);
		...
		public getScreenTopBottom(): Array<number> {
			...
			const screenBottom =
				screenTop + this.cssToCorePixels($('#map').height());
		...
		private isInViewPort(positionTopBotton: Array<number>): boolean {
			...
			return screenTopBottom[0] <= top && screenTopBottom[1] >= bottom;
		}

		public unselect(): void {
			if (
				this.sectionProperties.selectedComment &&
				this.sectionProperties.selectedComment.sectionProperties
					.data.id != 'new'
			) {
				if (
					this.sectionProperties.selectedComment &&
		...
		private setThreadPopup(comment: Comment, popup: boolean) {
			if (
				popup &&
				!$(comment.sectionProperties.container).hasClass(
					'annotation-pop-up',
				)
			)
				$(comment.sectionProperties.container).addClass(
					'annotation-pop-up',
				);
			else if (
				!popup &&
				...
				$(comment.sectionProperties.container).removeClass(
					'annotation-pop-up',
				);
		...
		public toggleShowBigger(comment: Comment) {
			...
			const isSelected =
				this.sectionProperties.selectedComment === rootComment;
	...
	public isThreadResolved (annotation: any): boolean {
			...
			var comment = {
				...
				annotation.sectionProperties.data.html
					? {
							Html: {
								type: 'string',
								value: annotation.sectionProperties.data
									.html,
							},
						}
					: {
		...
		public remove(id: any): void {
			...
			removedComment.sectionProperties.selfRemoved = true;
			if (app.file.fileBasedView)
				// We have to set the part from which the comment will be removed as selected part before the process.
				this.map.setPart(app.map._docLayer._selectedPart, false);
			...
			if (app.file.fileBasedView) this.map.setPart(0, false);
		...
		public isThreadResolved(annotation: any): boolean {
			...
			if (annotation.sectionProperties.children.length > 0) {
				for (
					var i = 0;
					i < annotation.sectionProperties.children.length;
					i++
				) {
					if (
						annotation.sectionProperties.children[i]
							.sectionProperties.data.resolved !== 'true'
					)
						return false;
				}
				return true;
			...
			// If it has a parent.
				...
				var index = this.getSubRootIndexOf(
					annotation.sectionProperties.data.parent,
				);
				...
				if (comment.sectionProperties.data.resolved !== 'true')
					...
					for (
						...
						if (
							comment.sectionProperties.children[i]
								.sectionProperties.data.resolved !==
							'true'
						)
							return false;
					}
		...
		private initializeContextMenus(): void {
			...
			L.installContextMenu({
				...
				build: function ($trigger: any) {
					const blockChangeFromDifferentAuthor =
						this.map.isReadOnlyMode() &&
						docLayer._docType === 'text' &&
						this.map.getViewName(docLayer._viewId) !==
							$trigger[0].annotation.sectionProperties.data
								.author;
					const isShownBig =
						this.sectionProperties.showSelectedBigger &&
						this.sectionProperties.selectedComment ===
					...
					return {
						autoHide: true,
						items: {
							modify: blockChangeFromDifferentAuthor
								? undefined
								: {
										name: _('Modify'),
										callback: function (
							...
							remove: blockChangeFromDifferentAuthor
								? undefined
								: {
										name: _('Remove'),
										callback: function (
							...
							removeThread:
								docLayer._docType !== 'text' ||
								!$trigger[0].annotation.isRootComment() ||
								blockChangeFromDifferentAuthor
									? undefined
									: {
		...
		public showHideComment(annotation: any): void {
			...
			if (
				...
				if (
					annotation.isContainerVisible() &&
					annotation.sectionProperties.data.resolved === 'true'
				) {
					if (
						this.sectionProperties.selectedComment ==
						annotation
					...
					!annotation.isContainerVisible() &&
					annotation.sectionProperties.data.resolved === 'false'
				) {
					annotation.show();
					annotation.update();
				}
				...
				if (
					annotation.sectionProperties.partIndex ===
						app.map._docLayer._selectedPart ||
					app.file.fileBasedView
				) {
		...
		public add(comment: any): lool.Comment {
			...
			comment.sectionProperties.noMenu =
				comment.sectionProperties.data.id === 'new' ? true : false;
		...
		*/
		...
		public adjustRedLine(redline: any): boolean {
			...
			redline.anchorPos = this.stringToRectangles(
				redline.textRange,
			)[0];
			redline.anchorPix = this.numberArrayToCorePixFromTwips(
				redline.anchorPos,
				0,
				2,
			);
			...
			var rectangles = L.PolyUtil.rectanglesToPolygons(
				app.LOUtil.stringToRectangles(redline.textRange),
				app.map._docLayer,
			);
		...
		public getComment(id: any): any {
			...
			return index == -1
				? null
				: this.sectionProperties.commentList[index];
		}

		private checkIfCommentHasPreAssignedChildren(
			...
			for (
				...
				var possibleChild: Comment =
					this.sectionProperties.commentList[i];
				if (
					possibleChild.sectionProperties
						.possibleParentCommentId !== null
				) {
					if (
						possibleChild.sectionProperties
						...
						if (
							!comment.sectionProperties.children.includes(
								possibleChild,
							)
						)
							comment.sectionProperties.children.push(
								possibleChild,
							);
					}
				}
		...
		public adjustParentAdd(comment: any): void {
			...
			if (comment.sectionProperties.data.parent !== '0') {
				var parentIdx = this.getIndexOf(
					comment.sectionProperties.data.parent,
				);
				if (parentIdx === -1) {
					console.warn(
						'adjustParentAdd: No parent comment to attach received comment to. ' +
							'Parent comment ID sought is :' +
							comment.sectionProperties.data.parent +
							' for current comment with ID : ' +
							comment.sectionProperties.data.id,
					);
					comment.sectionProperties.possibleParentCommentId =
						comment.sectionProperties.data.parent; // Save the proposed parentId so we can remember if such parent appears.
					comment.setAsRootComment(); // Set this to default since there is no such parent at the moment.
				} else {
					var parentComment =
						this.sectionProperties.commentList[parentIdx];
					if (
						parentComment &&
						!parentComment.sectionProperties.children.includes(
							comment,
						)
					)
						parentComment.sectionProperties.children.unshift(
							comment,
						);
				}
			}
		...
		public adjustParentRemove(comment: any): void {
			...
			if (parentComment) {
				...
				if (index >= 0) parentComment.removeChildByIndex(index); // Removed comment has a parent. Remove the comment also from its parent's list.
			}

			// If a parent comment is removed.
			for (var i = 0; i < comment.getChildrenLength(); i++) {
				// Loop over removed comment's children.
				var childComment = comment.getChildByIndex(i);
				if (childComment) childComment.setAsRootComment(); // The children have no parent comment any more.
			}
		}
		...
		public handleCommentConflict(obj: any, editComment: Comment) {
			if (
				document.getElementById(
					this.map.uiManager.generateModalId('comments-update'),
				)
			)
				return;

			if (
				obj.comment.action === 'Remove' ||
				obj.comment.action === 'RedlinedDeletion'
			) {
				JSDialog.showInfoModalWithOptions('comments-update', {
					title: _('Comments Updated'),
					messages: [
						_('Another user has removed this comment.'),
					],
					buttons: [
						{
							text: _('OK'),
							callback: () => {
								this.overWriteCommentChanges(
									obj,
									editComment,
								);
							},
			...
			this.map.uiManager.showYesNoButton(
				...
				_('Comments Updated'),
				_(
					'Another user has updated the comment. Would you like to overwrite those changes?',
				),
				_('Overwrite'),
				_('Update'),
				null,
		...
		public checkIfOnlyAnchorPosChanged(
			obj: any,
			editComment: Comment,
		): boolean {
			if (obj.comment.action !== 'Modify') return false;

			var newComment = obj.comment;
			var editCommentData = editComment.sectionProperties.data;

			if (
				newComment.author !== editCommentData.author ||
				newComment.dateTime !== editCommentData.dateTime ||
				newComment.html !== editCommentData.html ||
				newComment.layoutStatus !==
					editCommentData.layoutStatus.toString() ||
			...
			if (
				newComment.anchorPos.replaceAll(' ', '') !==
				editCommentData.anchorPos.toString()
			)
				return true;
			return false;
		}
		...
		private actionPerformedByCurrentUser(obj: any): boolean {
			return (
				obj.comment.author ===
				this.map._viewInfo[this.map._docLayer._editorId].username
			);
		}
		...
		public onACKComment(obj: any): void {
			...
			if (!changetrack && obj.comment.parent === undefined) {
				...
				else obj.comment.parent = '0';
			}

			if (
				changetrack &&
				obj.redline.author in this.map._viewInfoByUserName
			) {
				obj.redline.avatar =
					this.map._viewInfoByUserName[
						obj.redline.author
					].userextrainfo.avatar;
			} else if (
				!changetrack &&
				obj.comment.author in this.map._viewInfoByUserName
			) {
				obj.comment.avatar =
					this.map._viewInfoByUserName[
						obj.comment.author
			...
			if ((<any>window).mode.isMobile()) {
				var annotation =
					this.sectionProperties.commentList[
						this.getRootIndexOf(obj[dataroot].id)
					];
				if (!annotation)
					annotation =
						this.sectionProperties.commentList[
							this.getRootIndexOf(obj[dataroot].parent)
						]; //this is required for reload after reply in writer
			}
			if (action === 'Add') {
				if (changetrack) {
					...
					const currentComment = this.getComment(
						obj[dataroot].id,
					);
					if (currentComment !== null) {
						if (obj[dataroot].layoutStatus !== undefined) {
							currentComment.sectionProperties.data.layoutStatus =
								parseInt(obj[dataroot].layoutStatus);
					...
					if (autoSavedComment) {
						...
						if (isOurComment) {
							if (
								app.definitions.CommentSection.needFocus
							) {
								app.definitions.CommentSection.needFocus =
									annotation;
							}
							annotation.sectionProperties.container.style.visibility =
								'visible';
							annotation.sectionProperties.autoSave.innerText =
								_('Autosaved');
							if (
								app.map._docLayer._docType ===
								'spreadsheet'
							)
								annotation.show();
							if (
								autoSavedComment.sectionProperties.data
									.id === 'new'
							)
								this.removeItem(
									autoSavedComment.sectionProperties
										.data.id,
								);
							annotation.edit();
							if (this.shouldCollapse())
				...
				if (
					this.sectionProperties.selectedComment &&
					!this.sectionProperties.selectedComment.isEdit()
				) {
					this.map.focus();
				}
				...
				if (removed) {
					...
					if (
						this.sectionProperties.selectedComment === removed
					) {
						this.unselect();
						this.removeItem(id);
					} else {
				...
				if (
					_redlined &&
					_redlined.sectionProperties.data.layoutStatus ===
						CommentLayoutStatus.INSERTED
				) {
				...
				if (modified) {
					...
					if (oldParent !== null && modified.isRootComment()) {
						...
						if (parentComment) {
							const index =
								parentComment.getIndexOfChild(modified);
							if (index >= 0)
					...
					if (CommentSection.autoSavedComment) {
						CommentSection.autoSavedComment.sectionProperties.autoSave.innerText =
							_('Autosaved');
						if (app.map._docLayer._docType === 'spreadsheet')
							modified.show();
						modified.edit();
			...
			if ((<any>window).mode.isMobile()) {
				...
				var wePerformedAction =
					obj.comment.author ===
					this.map.getViewName(app.map._docLayer._viewId);

				if (
					(<any>window).commentWizard ||
					(action === 'Add' && wePerformedAction)
				)
		...
		private numberArrayToCorePixFromTwips(
			numberArray: Array<number>,
			startIndex: number = 0,
			length: number = null,
		): Array<number> {
			if (!length) length = numberArray.length;
		...
		private adjustCommentFileBasedView(comment: any): void {
			...
			var index = app.impress.getIndexFromSlideHash(
				parseInt(comment.parthash),
			);
			...
			comment.rectangles = this.stringToRectangles(
				comment.textRange || comment.anchorPos || comment.rectangle,
			); // Simple array of point arrays [x1, y1, x2, y2].
			comment.rectangles[0][1] += yAddition; // There is only one rectangle for our case.

			comment.rectanglesOriginal = this.stringToRectangles(
				comment.textRange || comment.anchorPos || comment.rectangle,
			); // This unmodified version will be kept for re-calculations.
			comment.rectanglesOriginal[0][1] += yAddition;

			comment.anchorPos = this.stringToRectangles(
				comment.anchorPos || comment.rectangle,
			)[0];
			comment.anchorPos[1] += yAddition;

			if (comment.rectangle) {
				comment.rectangle = this.stringToRectangles(
					comment.rectangle,
				)[0]; // This is the position of the marker.
				comment.rectangle[1] += yAddition;
			}

			comment.anchorPix = this.numberArrayToCorePixFromTwips(
				comment.anchorPos,
				0,
				2,
			);
			...
			var color =
				viewId >= 0
					? app.LOUtil.rgbToHex(this.map.getViewColor(viewId))
					: '#43ACE8';
			comment.color = color;
		}
		...
		private adjustCommentNormal(comment: any): void {
			...
			if (comment.cellRange) {
				...
				comment.cellRange = app.map._docLayer._parseCellRange(
					comment.cellRange,
				);
			}
			...
			var cellPos = comment.cellRange
				? app.map._docLayer
						._cellRangeToTwipRect(comment.cellRange)
						.toRectangle()
				: null;
			comment.rectangles = this.stringToRectangles(
				comment.textRange ||
			...
			comment.anchorPos = this.stringToRectangles(
				comment.anchorPos || comment.rectangle || cellPos,
			)[0];
			comment.anchorPix = this.numberArrayToCorePixFromTwips(
				comment.anchorPos,
				0,
			...
			comment.parthash = comment.parthash ? comment.parthash : null;
			comment.tab =
				comment.tab || comment.tab === 0 ? comment.tab : null;
			comment.layoutStatus =
				comment.layoutStatus !== undefined
					? parseInt(comment.layoutStatus)
					: null;

			if (comment.parentId) comment.parent = String(comment.parentId);

			if (comment.rectangle) {
				comment.rectangle = this.stringToRectangles(
					comment.rectangle,
				)[0]; // This is the position of the marker (Impress & Draw).
			}
		...
		private adjustComment(comment: any): void {
			if (!app.file.fileBasedView) this.adjustCommentNormal(comment);
			else this.adjustCommentFileBasedView(comment);
		}
		...
		private getLastChildIndexOf(id: any): number {
			...
			index = this.getRootIndexOf(
				this.sectionProperties.commentList[index].sectionProperties
					.data.id,
			);
			...
			while (
				...
				this.sectionProperties.commentList[index + 1]
					.sectionProperties.data.parent !== '0'
			) {
				index++;
			}
		...
		private layoutUp(
			...
			for (var i = 0; i < subList.length; i++) {
				...
				lastY =
					subList[i].sectionProperties.data.anchorPix[1] +
						height <
					lastY
						? subList[i].sectionProperties.data.anchorPix[1]
						: lastY - height * app.dpiScale;

				subList[i].sectionProperties.container.style.left =
					String(
						Math.round(actualPosition[0] / app.dpiScale) +
							this.sectionProperties.canvasContainerLeft,
					) + 'px';
				subList[i].sectionProperties.container.style.top =
					String(
						Math.round(lastY / app.dpiScale) +
							this.sectionProperties.canvasContainerTop,
					) + 'px';

				if (!subList[i].isEdit()) subList[i].show();
			}
			return lastY;
		}
		...
		private loopUp(
			...
			for (var i = startIndex; i > -1; ) {
				...
				do {
					this.sectionProperties.commentList[
						tmpIdx
					].sectionProperties.data.anchorPix =
						this.numberArrayToCorePixFromTwips(
							this.sectionProperties.commentList[tmpIdx]
								.sectionProperties.data.anchorPos,
							0,
							2,
						);
					this.sectionProperties.commentList[
						tmpIdx
					].sectionProperties.data.anchorPix[1] -=
						app.activeDocument.activeView.viewedRectangle.pY1;
					...
					tmpIdx > -1 &&
					this.sectionProperties.commentList[tmpIdx]
						.sectionProperties.data.parent ===
						this.sectionProperties.commentList[tmpIdx + 1]
							.sectionProperties.data.id
				);

				if (subList.length > 0) {
					startY = this.layoutUp(
						subList,
						[
							x,
							subList[0].sectionProperties.data
								.anchorPix[1],
						],
						startY,
						relayout,
		...
		private layoutDown(
			...
			var selectedComment =
				subList[0] === this.sectionProperties.selectedComment;
			for (var i = 0; i < subList.length; i++) {
				lastY =
					subList[i].sectionProperties.data.anchorPix[1] > lastY
						? subList[i].sectionProperties.data.anchorPix[1]
						: lastY;
					...
					const posX = this.sectionProperties.showSelectedBigger
						? Math.round(
								(document
									...
									subList[
										i
									].sectionProperties.container.getBoundingClientRect()
										.width) /
									2,
							)
						: Math.round(actualPosition[0] / app.dpiScale) -
							this.sectionProperties
								.deflectionOfSelectedComment *
								(isRTL ? -1 : 1);
				...
				lastY +=
					subList[i].getCommentHeight(relayout) * app.dpiScale;
				if (!subList[i].isEdit()) subList[i].show();
			}
			return lastY;
		}
		...
		private loopDown(
			...
			for (
				...
				do {
					...
					tmpIdx < this.sectionProperties.commentList.length &&
					this.sectionProperties.commentList[tmpIdx]
						.sectionProperties.data.parent !== '0'
				);

				if (subList.length > 0) {
					startY = this.layoutDown(
						subList,
						[
							x,
							subList[0].sectionProperties.data
		...
		public hideArrow(): void {
			if (this.sectionProperties.arrow) {
				document
					...
					.removeChild(this.sectionProperties.arrow);
				this.sectionProperties.arrow = null;
		...
		private showArrow(
			...
			var anchorSection =
				this.containerObject.getDocumentAnchorSection();
			startPoint[0] -=
				anchorSection.myTopLeft[0] +
				app.activeDocument.activeView.viewedRectangle.pX1;
			startPoint[1] -=
				anchorSection.myTopLeft[1] +
				app.activeDocument.activeView.viewedRectangle.pY1;
			endPoint[1] -=
				anchorSection.myTopLeft[1] +
				app.activeDocument.activeView.viewedRectangle.pY1;
			...
			if (this.sectionProperties.arrow !== null) {
				var line: SVGLineElement = <SVGLineElement>(
					this.sectionProperties.arrow.firstElementChild
				);
				...
				svg.setAttribute(
					'width',
					String(this.context.canvas.width),
				);
				svg.setAttribute(
					'height',
					String(this.context.canvas.height),
				);
		...
		private layout(relayout: boolean = true): void {
			...
			this.sectionProperties.canvasContainerLeft = document
				.getElementById('document-container')
				.getBoundingClientRect().left;
			this.sectionProperties.canvasContainerTop = document
				.getElementById('document-container')
			...
			if (this.sectionProperties.commentList.length > 0) {
				this.orderCommentList();
				if (relayout) this.resetCommentsSize();

				var isRTL = document.documentElement.dir === 'rtl';

				var topRight: Array<number> = [
					this.myTopLeft[0],
					this.myTopLeft[1] +
				...
				if (isRTL)
					...
					x =
						(app.activeDocument.fileSize.cX -
							app.activeDocument.activeView.viewedRectangle
								.cX1 -
							app.sectionContainer.getCanvasBoundingClientRect()
								.x) *
						app.dpiScale;
					x += app.map.navigator
						? app.map.navigator.navigationPanel.offsetWidth *
							app.dpiScale
						: 0;
				}

				if (this.sectionProperties.selectedComment) {
					selectedIndex = this.getRootIndexOf(
						this.sectionProperties.selectedComment
							.sectionProperties.data.id,
					);
					this.sectionProperties.commentList[
					...
					yOrigin =
						this.sectionProperties.commentList[selectedIndex]
							.sectionProperties.data.anchorPix[1] -
						app.activeDocument.activeView.viewedRectangle.pY1;
					var tempCrd: Array<number> =
						this.sectionProperties.commentList[selectedIndex]
							.sectionProperties.data.anchorPix;
					var resolved: string =
						this.sectionProperties.commentList[selectedIndex]
			...
			if (relayout) this.resizeComments();
		...
		private updateDOM(): void {
			...
			app.layoutingService.appendLayoutingTask(() => {
				if (
					this.sectionProperties.reLayout &&
					app.map._docLayer._docType === 'text'
				)
					this.updateThreadInfoIndicator();
		...
		private updateThreadInfoIndicator(): void {
			for (
				...
				if (comment && comment.isRootComment()) {
					...
					var j = i;
					while (
						this.sectionProperties.commentList[j] &&
						j <= lastIndex
					) {
						anyEdit =
							this.sectionProperties.commentList[
				...
				if (anyEdit) comment.updateThreadInfoIndicator('!');
				else comment.updateThreadInfoIndicator(replyCount);
			}
		}
		...
		private getSubRootIndexOf(id: any): number {
			...
			if (index !== -1) {
				...
				while (index >= 0) {
					if (
						this.sectionProperties.commentList[index]
							.sectionProperties.data.id !== parentId &&
						this.sectionProperties.commentList[index]
							.sectionProperties.data.parent !== '0'
					)
						index--;
		...
		public setViewResolved(state: any): void {
			...
			for (
				var idx = 0;
				idx < this.sectionProperties.commentList.length;
				idx++
			) {
				if (
					this.sectionProperties.commentList[idx]
		...
		public setView(state: any): void {
			...
			for (
				...
				if (state == false)
					this.sectionProperties.commentList[idx].hide();
				else this.sectionProperties.commentList[idx].show();
			}
		}

		private orderCommentList(): void {
			this.sectionProperties.commentList.sort(function (
				a: any,
				b: any,
			) {
				return (
					Math.abs(a.sectionProperties.data.anchorPos[1]) -
						Math.abs(b.sectionProperties.data.anchorPos[1]) ||
					Math.abs(a.sectionProperties.data.anchorPos[0]) -
						Math.abs(b.sectionProperties.data.anchorPos[0])
				);
			});
		...
		private resetCommentsSize(): void {
			if (app.map._docLayer._docType === 'text') {
				for (
					...
					if (
						this.sectionProperties.commentList[i]
							.sectionProperties.contentNode.style
							.display !== 'none'
					) {
						const maxHeight =
							this.sectionProperties.commentList[i] ===
							this.sectionProperties.selectedComment
								? this.annotationMaxSize
								: this.annotationMinSize;
						this.sectionProperties.commentList[
							i
						].sectionProperties.contentNode.style.maxHeight =
							maxHeight + 'px';
					}
				}
			}
		...
		private resizeComments(): void {
			...
			var growUp = false;
			if (app.map._docLayer._docType === 'text') {
				const minMaxHeight = Number(
					getComputedStyle(
						document.documentElement,
					).getPropertyValue('--annotation-min-size'),
				);
				const maxMaxHeight = Number(
					getComputedStyle(
						document.documentElement,
					).getPropertyValue('--annotation-max-size'),
				);
				for (
					var i = 0;
					i < this.sectionProperties.commentList.length;
					...
					if (
						...
						var actHeight =
							this.sectionProperties.commentList[
								i
							].sectionProperties.contentText.getBoundingClientRect()
								.height;
							...
							if (
								...
								if (
									...
									maxSize =
										...
										this.sectionProperties.commentList[
											i
										].sectionProperties.author.getBoundingClientRect()
											.height -
										3 *
											this.sectionProperties
												.marginY - //top/bottom of comment window + space between comments
										2; // not sure why
								...
								if (maxSize > maxMaxHeight) {
									...
									if (spaceBefore > 0) {
										...
										this.sectionProperties.commentList[
											i
										].sectionProperties.container.style.left =
											Math.round(posX) + 'px';
										this.sectionProperties.commentList[
											i
										].sectionProperties.container.style.top =
											Math.round(posY) + 'px';
		...
		private updateIdIndexMap(): void {
			...
			for (var idx = 0; idx < commentList.length; idx++) {
				...
				console.assert(
					comment.sectionProperties &&
						comment.sectionProperties.data,
					'no sectionProperties.data!',
				);
		...
		private addUpdateChildGroups() {
			...
			for (
				...
				if (comment.sectionProperties.data.parent !== '0') {
					if (
						!parentCommentList.includes(
							comment.sectionProperties.data.parent,
						)
					)
						parentCommentList.push(
							comment.sectionProperties.data.parent,
						);
				}
			}
			...
			for (var i = 0; i < parentCommentList.length; i++) {
				...
				for (
					var j = 0;
					j < this.sectionProperties.commentList.length;
					j++
				) {
					if (
						this.sectionProperties.commentList[j]
				...
				if (parentComment) {
					for (
						...
						if (
							...
							parentComment.sectionProperties.children.push(
								this.sectionProperties.commentList[j],
							);
					}
				} else console.warn("Couldn't find parent comment.");
			}
		}

		private addChildrenCommentsToList(
			comment: any,
			newOrder: Array<any>,
		) {
			comment.sectionProperties.children.forEach(
		...
		public importComments(commentList: any): void {
			...
			if (commentList.length > 0) {
				for (var i = 0; i < commentList.length; i++) {
					...
					const name = lool.Comment.makeName(comment);
					var commentSection = new lool.Comment(
						name,
						comment,
						{},
						this,
					...
					if (!this.containerObject.addSection(commentSection))
						continue;
					this.sectionProperties.commentList.push(
						commentSection,
					);
					this.idIndexMap.set(
						commentSection.sectionProperties.data.id,
						i,
					);
				}
			...
			var show =
				this.map.stateChangeHandler.getItemValue('showannotations');
			this.setView(show === true || show === 'true');

			var showResolved = this.map.stateChangeHandler.getItemValue(
				'ShowResolvedAnnotations',
			);
			this.setViewResolved(
				showResolved === true || showResolved === 'true',
			);
		...
		public importChanges(changesList: any): void {
			...
			if (changesList.length > 0) {
				for (var i = 0; i < changesList.length; i++) {
					...
					if (
						changeComment.author in
						this.map._viewInfoByUserName
					) {
						changeComment.avatar =
							this.map._viewInfoByUserName[
								changeComment.author
							].userextrainfo.avatar;
					}
		...
		private clearChanges(): void {
			...
			for (
				var i: number =
					this.sectionProperties.commentList.length - 1;
				i > -1;
				i--
			) {
				if (
					...
					this.containerObject.removeSection(
						this.sectionProperties.commentList[i].name,
					);
					this.sectionProperties.commentList.splice(i, 1);
				}
			}
		...
		private clearList(): void {
			...
			for (
				...
				if (
					!this.sectionProperties.commentList[i]
						.sectionProperties.data.trackchange
				) {
					this.containerObject.removeSection(
						this.sectionProperties.commentList[i].name,
					);
		...
		public onCommentsDataUpdate(): void {
			for (
				...
				if (
					...
					comment.sectionProperties.commentListSection.removeItem(
						comment.sectionProperties.data.id,
					);
				}
		...
		public rejectAllTrackedCommentChanges(): void {
			for (
				...
				var comment = this.sectionProperties.commentList[i];
				if (
					comment.sectionProperties.data.layoutStatus ===
					CommentLayoutStatus.DELETED
				) {
					comment.sectionProperties.data.layoutStatus =
						CommentLayoutStatus.VISIBLE;
					comment.sectionProperties.container.classList.remove(
						'tracked-deleted-comment-show',
					);
				}
			}
		}
