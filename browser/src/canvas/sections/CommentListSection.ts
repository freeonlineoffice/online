...
namespace lool {
	export class CommentSection extends app.definitions.canvasSectionObject {
		...
		constructor() {
			...
			this.sectionProperties.commentsAreListed =
				(this.sectionProperties.docLayer._docType === 'text' ||
					this.sectionProperties.docLayer._docType ===
						'presentation' ||
					this.sectionProperties.docLayer._docType ===
						'drawing') &&
		...
		public newAnnotationMobile(
			...
			multilineEditDiv.addEventListener(
				...
				function (ev: any) {
					if (
						ev &&
						comment.sectionProperties.docLayer._docType ===
							'text'
					) {
			...
			var contentAuthor = L.DomUtil.create(
				tagDiv,
				'lool-annotation-content-author',
				tdAuthor,
			);
			var contentDate = L.DomUtil.create(
				tagDiv,
			...
			var newAnnotationDialog = document.getElementById(
				this.mobileCommentId,
			);
		...
		public save(annotation: any): void {
			...
			if (annotation.sectionProperties.data.id === 'new') {
				comment = {
					...
					...(this.sectionProperties.docLayer._docType ===
						'text' && annotation.sectionProperties.data.html
						? {
								Html: {
									type: 'string',
		...
		public reply(annotation: any): void {
			...
			if ((<any>window).mode.isMobile()) {
				...
				var author = this.map.getViewName(
					this.sectionProperties.docLayer._viewId,
				);
		...
		public getScreenTopBottom(): Array<number> {
			...
			const screenTop =
				scrollSection.containerObject.getDocumentTopLeft()[1];
			const screenBottom =
			...
			L.installContextMenu({
				...
				events: {
					show: function (options: any) {
						options.$trigger[0].annotation.sectionProperties.contextMenu =
							true;
						setTimeout(
							function () {
		...
		public add(comment: any): lool.Comment {
			var annotation = new lool.Comment(
				comment,
		...
		*/
			...
			if (
				!newComment &&
				authorName === annotation.sectionProperties.data.author
			) {
				this.select(annotation);
			}
		...
		public adjustRedLine(redline: any): boolean {
			...
			var rectangles = L.PolyUtil.rectanglesToPolygons(
				L.LOUtil.stringToRectangles(redline.textRange),
				this.sectionProperties.docLayer,
			);
		...
		public onACKComment(obj: any): void {
			...
			if ((<any>window).mode.isMobile()) {
				...
				if (shouldOpenWizard) {
					this.sectionProperties.docLayer._openCommentWizard(
						annotation,
					);
				}
			}
		...
		private adjustCommentFileBasedView(comment: any): void {
			...
			var partHeightTwips =
				this.sectionProperties.docLayer._partHeightTwips +
				this.sectionProperties.docLayer._spaceBetweenParts;
			...
			var color =
				viewId >= 0
					? L.LOUtil.rgbToHex(this.map.getViewColor(viewId))
					: '#43ACE8';
			comment.color = color;
		}
		...
		private adjustCommentNormal(comment: any): void {
			...
			if (comment.cellRange) {
				...
				comment.cellRange =
					this.sectionProperties.docLayer._parseCellRange(
						comment.cellRange,
					);
			}
		...
		private layoutUp(
			...
			for (var i = 0; i < subList.length; i++) {
				...
				new L.PosAnimation().run(
					subList[i].sectionProperties.container,
					{
						x: Math.round(actualPosition[0] / app.dpiScale),
						y: Math.round(lastY / app.dpiScale),
					},
		...
		private layoutDown(
			...
			for (var i = 0; i < subList.length; i++) {
				...
				if (selectedComment) {
					...
					new L.PosAnimation().run(
						subList[i].sectionProperties.container,
						{ x: posX, y: Math.round(lastY / app.dpiScale) },
						this.getAnimationDuration(),
					);
				} else
					new L.PosAnimation().run(
		...
		private showArrow(
			...
			var anchorSection =
				this.containerObject.getDocumentAnchorSection();
			startPoint[0] -=
				anchorSection.myTopLeft[0] + this.documentTopLeft[0];
			startPoint[1] -=
				anchorSection.myTopLeft[1] + this.documentTopLeft[1];
			endPoint[1] -=
				anchorSection.myTopLeft[1] + this.documentTopLeft[1];
			...
			if (this.sectionProperties.arrow !== null) {
				var line: SVGLineElement = <SVGLineElement>(
					(<any>document.getElementById('comment-arrow-line'))
				);
		...
		private doLayout(): void {
			...
			if (this.sectionProperties.commentList.length > 0) {
				...
				if (availableSpace > this.sectionProperties.commentWidth) {
					...
					x =
						-this.containerObject.getDocumentAnchorSection()
							.size[0];
				else x -= this.sectionProperties.collapsedMarginToTheEdge;
			...
			if (lastY > app.file.size.pixels[1]) {
				...
				this.containerObject.requestReDraw();
			} else app.view.size.pixels[1] = app.file.size.pixels[1];

			this.disableLayoutAnimation = false;
		}

		private layout(zoom: any = null): void {
			if (zoom) this.doLayout();
			...
			if (this.sectionProperties.docLayer._docType === 'text') {
				...
				if (this.sectionProperties.selectedComment) {
					if (
						...
						this.sectionProperties.selectedComment.sectionProperties.contentNode.style.maxHeight =
							maxMaxHeight + 'px';
					}
				}
			}
				...
				for (
					...
					if (
						...
						if (actHeight > minMaxHeight) {
							...
							if (
								typeof this.sectionProperties
									.commentList[i].sectionProperties
		...
		public importComments(commentList: any): void {
			...
			if (Comment.isAnyEdit()) {
				this.map.uiManager.showConfirmModal(
					...
					_('Comments Updated'),
					_(
						'Another user has updated comments. Would you like to proceed updating?',
					),
					_('OK'),
					() => {
						CommentSection.pendingImport = false;
		...
		public hasAnyComments(): boolean {
			return this.containerObject.sections.some(
				(x: any) => x.constructor.name == 'Comment',
			);
		}
	}
}
