...
namespace lool {
	export class ContentControlSection extends app.definitions
		.canvasSectionObject {
		...
		public onInitialize(): void {
			...
			this.sectionProperties.dropdownButton = L.marker(
				new L.LatLng(0, 0),
				{
					icon: L.divIcon({
			...
			this.sectionProperties.dropdownButton.addEventListener(
				'click',
		...
		public drawContentControl(json: any) {
			...
			if (json.action === 'show') {
				...
				if (this.sectionProperties.frame)
					this.sectionProperties.frame.setPointSet(
						new CPointSet(),
					);
		...
		public drawPolygon(): void {
			var rectArray = lool.Bounds.parseArray(
				this.sectionProperties.json.rectangles,
			);
			var rectangles = rectArray.map(function (rect: lool.Bounds) {
				return rect.getPointArray();
			});

			var docLayer = this.map._docLayer;
			this.sectionProperties.pointSet = CPolyUtil.rectanglesToPointSet(
				rectangles,
				function (twipsPoint) {
			...
			if (!this.sectionProperties.frame) {
				this.sectionProperties.frame = new CPolygon(
					this.sectionProperties.pointSet,
					this.sectionProperties.polyAttri,
				);
				this.changeBorderStyle();
				this.map._docLayer._canvasOverlay.initPath(
					this.sectionProperties.frame,
				);
			}
			this.sectionProperties.frame.setPointSet(
				this.sectionProperties.pointSet,
		...
		public onDraw(): void {
			...
			if (text) {
				...
				var ratio: number =
					app.tile.size.pixels[0] / app.tile.size.twips[0];
				var x: number =
					rectangles[rectangles.length - 1][0] * ratio;
				var y: number =
					rectangles[rectangles.length - 1][1] * ratio;
				...
				var textWidth: number =
					L.Util.getTextWidth(text, font) + padding;
			...
			if (
				this.sectionProperties.json.items ||
				this.sectionProperties.datePicker
			) {
				this.addDropDownBtn();
			}
		}
		...
		private addDropDownBtn(): void {
			var matches =
				this.sectionProperties.json.rectangles.match(/\d+/g);
			...
			var topLeftTwips = new lool.Point(rectangle[0], rectangle[1]);
			var offset = new lool.Point(rectangle[2], rectangle[3]);
			var bottomRightTwips = topLeftTwips.add(offset);
			var buttonAreaTwips = [topLeftTwips, bottomRightTwips];

			var frameArea = new lool.Bounds(
			...
			this.sectionProperties.framePos = new lool.Point(
				Math.round(frameArea.min.x + panePos.x - origin.x),
				Math.round(frameArea.min.y + panePos.y - origin.y),
			);
			...
			var buttonAreaLatLng = new L.LatLngBounds(
				this.map._docLayer._twipsToLatLng(
					buttonAreaTwips[0],
					this.map.getZoom(),
				),
				this.map._docLayer._twipsToLatLng(
		...
		private openDropdownJson(): any {
			...
			//add position
			json.posx =
				this.sectionProperties.framePos.x +
				this.sectionProperties.frameWidth;
			json.posy =
				this.sectionProperties.framePos.y +
				this.sectionProperties.frameHeight;
		...
		private changeBorderStyle(): void {
			var borderColor = (<any>window).prefs.getBoolean('darkTheme')
				? 'white'
				: 'black';
