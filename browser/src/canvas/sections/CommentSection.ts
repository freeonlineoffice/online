...
export class Comment extends CanvasSectionObject {
	...
	// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
	constructor (name: string, data: any, options: any, commentListSectionPointer: lool.CommentSection) {
		super(name);
	...
	public onInitialize (): void {
		...
		this.sectionProperties.nodeModifyText = L.DomUtil.create('div', 'lool-annotation-textarea', this.sectionProperties.nodeModify);
		...
		this.sectionProperties.nodeReplyText = L.DomUtil.create('div', 'lool-annotation-textarea', this.sectionProperties.nodeReply);
	...
	private createAuthorTable (): void {
		...
		this.sectionProperties.autoSave = L.DomUtil.create('div', 'lool-annotation-autosavelabel', tdAuthor);
	}
	...
	public highlight (): void {
		if (app.map._docLayer._docType === 'text') {
			...
			(this.containerObject.getSectionWithName(L.CSections.Scroll.name) as any as lool.ScrollSection).onScrollTo({x: x, y: y});
		}
		else if (app.map._docLayer._docType === 'spreadsheet') {
