...
DOMPurify.addHook('afterSanitizeAttributes', function (node: HTMLElement) {
	if (
		node.tagName === 'A' &&
		!node.classList.contains('context-menu-link')
	) {
...
export class Comment extends CanvasSectionObject {
	...
	public removeHighlight (): void {
		...
		private doPendingInitializationInView(force: boolean = false): void {
			...
			if (!force) {
				if (!this.convertRectanglesToViewCoordinates()) return;
	...
	private showWriter() {
		...
		this.cachedIsEdit = false;
			...
			if (
				(this.sectionProperties.commentMarkerSubSection === null &&
		...
		private updateContent(): void {
			if (this.sectionProperties.data.html)
				this.sectionProperties.contentText.innerHTML =
					app.LOUtil.sanitize(this.sectionProperties.data.html);
			else
			...
			this.sectionProperties.contentText.innerHTML =
				app.LOUtil.sanitize(linkedText);
			// Original unlinked text
			this.sectionProperties.contentText.origText = this
				.sectionProperties.data.text
				? this.sectionProperties.data.text
				: '';
			this.sectionProperties.contentText.origHTML = this
				.sectionProperties.data.html
				? this.sectionProperties.data.html
				: '';
			this.sectionProperties.nodeModifyText.innerText = this
				.sectionProperties.data.text
				? this.sectionProperties.data.text
				: '';
			if (this.sectionProperties.data.html) {
			...
			// dateTime is already in UTC, so we will not append Z that will create issues while converting date
			var d = new Date(
				this.sectionProperties.data.dateTime.replace(/,.*/, ''),
			);
			var dateOptions: any = {
				weekday: 'short',
				year: 'numeric',
		...
		private setPositionAndSize(): void {
			...
			if (rectangles && app.map._docLayer._docType === 'text') {
				...
				var position: Array<number> = [
					Math.round(cellPos[0] * app.twipsToPixels),
					Math.round(cellPos[1] * app.twipsToPixels),
				];
	...
	*/
		...
		private updatePosition(): void {
			...
			if (app.map._docLayer._docType === 'spreadsheet')
				...
				if (
					...
					this.sectionProperties.commentMarkerSubSection.sectionProperties.data =
						this.sectionProperties.data;
					this.sectionProperties.commentMarkerSubSection.setPosition(
						this.sectionProperties.data.anchorPos[0] *
							app.twipsToPixels,
						this.sectionProperties.data.anchorPos[1] *
							app.twipsToPixels,
					);
				}
			}
		...
		private createMarkerSubSection() {
			if (this.sectionProperties.data.rectangle === null) return;

			const showMarker =
				app.impress.partList[app.map._docLayer._selectedPart]
					.hash === this.sectionProperties.data.parthash ||
			...
			this.sectionProperties.commentMarkerSubSection =
				new CommentMarkerSubSection(
					this.name +
						this.sectionProperties.data.id +
						String(Math.random()), // Section name - only as a placeholder.
					28,
					28, // Width and height.
					new SimplePoint(
						this.sectionProperties.data.anchorPos[0],
	...
	public onLostFocus (e: any): void {
		...
		public positionCalcComment(): void {
			if (!(<any>window).mode.isMobile()) {
				var cellPos = app.map._docLayer
					...
					.toRectangle();
				var originalSize = [
					Math.round(cellPos[2] * app.twipsToPixels),
					Math.round(cellPos[3] * app.twipsToPixels),
				];
		...
		public hide(): void {
			...
			if (app.map._docLayer._docType === 'text') this.hideWriter();
			else if (app.map._docLayer._docType === 'spreadsheet')
				this.hideCalc();
			else if (
				app.map._docLayer._docType === 'presentation' ||
		...
		public handleCancelCommentButton(e: any): void {
			...
			if (this.sectionProperties.contentText.origHTML) {
				...
				this.sectionProperties.nodeModifyText.innerText =
					this.sectionProperties.contentText.origText;
			}
			this.sectionProperties.nodeReplyText.innerText = '';
		...
		public onCancelClick(e: any): void {
			...
			this.sectionProperties.nodeReplyText.innerText = '';
			if (app.map._docLayer._docType !== 'spreadsheet') this.show();
			this.sectionProperties.commentListSection.cancel(this);
		}
		...
		public onLostFocus(e: any): void {
			if (
				!this.isEdit() ||
				this.sectionProperties.container.contains(e.relatedTarget)
			)
				return;
			if (this.sectionProperties.nodeReply.contains(e.target)) {
				this.onLostFocusReply(e);
		...
		public focus(): void {
			...
			if (
				this.isModifying() &&
				this.sectionProperties.nodeModifyText.childNodes.length > 0
			) {
				var range = document.createRange();
				var sel = document.getSelection();
				range.setStartAfter(
					this.sectionProperties.nodeModifyText.lastChild,
				);
				range.collapse(true);
				sel.removeAllRanges();
				sel.addRange(range);
		...
		public static isAnyFocus(): boolean {
			...
			if (
				...
				(document.activeElement ===
					comment_.sectionProperties.nodeModifyText ||
					document.activeElement ===
						comment_.sectionProperties.nodeReplyText)
			)
				return true;
	...
	*/
			...
			if (this.sectionProperties.showSelectedCoordinate) {
				if (app.map._docLayer._docType === 'text') {
					...
					if (cellSize[0] !== 0 && cellSize[1] !== 0) {
						...
						var x = isRTL
							? margin
							: cellSize[0] - squareDim - margin;
						var commentColor = getComputedStyle(
							document.body,
						).getPropertyValue('--color-calc-comment');
		...
		public onMouseLeave(point: lool.SimplePoint): void {
			if (this.calcContinueWithMouseEvent()) {
				if (
					parseInt(this.sectionProperties.data.tab) ===
					app.map._docLayer._selectedPart
		...
		public onRemove(): void {
			...
			if (this.sectionProperties.commentMarkerSubSection !== null)
				app.sectionContainer.removeSection(
					this.sectionProperties.commentMarkerSubSection.name,
				);
		...
		public getCommentHeight(invalidateCache: boolean = true): number {
			if (invalidateCache) this.cachedCommentHeight = null;
			if (this.cachedCommentHeight === null)
				this.cachedCommentHeight =
					this.sectionProperties.container.getBoundingClientRect()
						.height -
