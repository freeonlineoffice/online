...
*/
	...
	export class Comment extends CanvasSectionObject {
		...
		constructor(
			data: any,
			options: any,
			commentListSectionPointer: lool.CommentSection,
		) {
			super();
			...
			this.sectionProperties.minWidth = options.minWidth
				? options.minWidth
				: 160;
			this.sectionProperties.maxHeight = options.maxHeight
				? options.maxHeight
				: 50;
			this.sectionProperties.imgSize = options.imgSize
				? options.imgSize
				: [32, 32];
			this.sectionProperties.margin = options.margin
				? options.margin
				: [40, 40];
			this.sectionProperties.noMenu = options.noMenu
				? options.noMenu
				: false;
		...
		*/
			...
			this.sectionProperties.usedTextColor =
			...
			this.name =
				data.id === 'new' ? 'new comment' : 'comment ' + data.id;
		...
		private doPendingInitializationInView(force: boolean = false): void {
			...
			if (!force && !this.convertRectanglesToViewCoordinates()) return;
			...
			this.createButton(
				button,
				'annotation-cancel-' + this.sectionProperties.data.id,
				'annotation-button button-secondary',
				_('Cancel'),
				this.handleCancelCommentButton,
			);
			this.createButton(
				button,
				'annotation-save-' + this.sectionProperties.data.id,
				'annotation-button button-primary',
				_('Save'),
				this.handleSaveCommentButton,
			);
			...
			this.createButton(
				button,
				'annotation-cancel-reply-' + this.sectionProperties.data.id,
				'annotation-button button-secondary',
				_('Cancel'),
				this.handleCancelCommentButton,
			);
			this.createButton(
				button,
				'annotation-reply-' + this.sectionProperties.data.id,
				'annotation-button button-primary',
				_('Reply'),
				this.handleReplyCommentButton,
			);
			...
			if (
				...
				this.sectionProperties.commentListSection.isCollapsed ===
					true
			)
			...
			L.DomEvent.on(
				this.sectionProperties.container,
				'wheel',
				app.sectionContainer.onMouseWheel,
				app.sectionContainer,
			);
			L.DomEvent.on(
				this.sectionProperties.contentNode,
				'wheel',
				this.onMouseWheel,
				this,
			);
		...
		onMouseWheel(
			...
			if (
				(e as any).currentTarget.clientHeight ===
				(e as any).currentTarget.scrollHeight
			)
				return;

			var _scrollTop = (e as any).currentTarget.scrollTop;
			if ((e as any).deltaY < 0 && _scrollTop > 0) e.stopPropagation();
			else if (
				(e as any).deltaY > 0 &&
				_scrollTop + $(e.currentTarget).height() <
					(e as any).target.scrollHeight
			)
				e.stopPropagation();
		}
		...
		public onInitialize(): void {
			...
			if (
				this.sectionProperties.noMenu !== true &&
			...
			if (this.sectionProperties.data.trackchange) {
				this.sectionProperties.captionNode = L.DomUtil.create(
					...
					this.sectionProperties.wrapper,
				);
				this.sectionProperties.captionText = L.DomUtil.create(
					'div',
					'',
					this.sectionProperties.captionNode,
				);
			}

			this.sectionProperties.contentNode = L.DomUtil.create(
				'div',
				'lool-annotation-content lool-dont-break',
				this.sectionProperties.wrapper,
			);
			this.sectionProperties.contentNode.id =
				'annotation-content-area-' + this.sectionProperties.data.id;
			this.sectionProperties.nodeModify = L.DomUtil.create(
				'div',
				'lool-annotation-edit' + ' modify-annotation',
				this.sectionProperties.wrapper,
			);
			this.sectionProperties.nodeModifyText = L.DomUtil.create(
				'div',
				'lool-annotation-textarea',
				this.sectionProperties.nodeModify,
			);
			this.sectionProperties.nodeModifyText.setAttribute(
				'contenteditable',
				'true',
			);
			this.sectionProperties.nodeModifyText.id =
				'annotation-modify-textarea-' +
				this.sectionProperties.data.id;
			this.sectionProperties.contentText = L.DomUtil.create(
				'div',
				'',
				this.sectionProperties.contentNode,
			);
			this.sectionProperties.nodeReply = L.DomUtil.create(
				'div',
				'lool-annotation-edit' + ' reply-annotation',
				this.sectionProperties.wrapper,
			);
			this.sectionProperties.nodeReplyText = L.DomUtil.create(
				'div',
				'lool-annotation-textarea',
				this.sectionProperties.nodeReply,
			);
			this.sectionProperties.nodeReplyText.setAttribute(
				'contenteditable',
				'true',
			);
			this.sectionProperties.nodeReplyText.id =
				'annotation-reply-textarea-' +
				this.sectionProperties.data.id;
		...
		private createContainerAndWrapper(): void {
			...
			this.sectionProperties.container = L.DomUtil.create(
				'div',
				'lool-annotation' + (isRTL ? ' rtl' : ''),
			);
			this.sectionProperties.container.id =
				'comment-container-' + this.sectionProperties.data.id;
			this.sectionProperties.container.addEventListener(
				'focusin',
				this.onContainerGotFocus.bind(this),
			);
			this.sectionProperties.container.addEventListener(
				'focusout',
				this.onContainerLostFocus.bind(this),
			);

			var mobileClass = (<any>window).mode.isMobile()
				? ' wizard-comment-box'
				: '';

			if (this.sectionProperties.data.trackchange) {
				this.sectionProperties.wrapper = L.DomUtil.create(
					'div',
				...
				this.sectionProperties.wrapper = L.DomUtil.create(
					'div',
					'lool-annotation-content-wrapper' + mobileClass,
					this.sectionProperties.container,
				);
			}
		...
		private createAuthorTable(): void {
			this.sectionProperties.author = L.DomUtil.create(
				'table',
				'lool-annotation-table',
				this.sectionProperties.wrapper,
			);

			var tbody = L.DomUtil.create(
				'tbody',
				'',
				this.sectionProperties.author,
			);
			var rowResolved = L.DomUtil.create('tr', '', tbody);
			var tdResolved = L.DomUtil.create(
				'td',
				'lool-annotation-resolved',
				rowResolved,
			);
			var pResolved = L.DomUtil.create(
				'div',
			...
			this.sectionProperties.contentAuthor = L.DomUtil.create(
				'div',
				'lool-annotation-content-author',
				tdAuthor,
			);
			this.sectionProperties.contentDate = L.DomUtil.create(
				'div',
				'lool-annotation-date',
				tdAuthor,
			);
			this.sectionProperties.autoSave = L.DomUtil.create(
				'div',
		...
		public updateChildLines(): void {
			...
			for (
				...
				if (this.sectionProperties.children[i].isContainerVisible())
					childPositions.push({
						...
						posY: this.sectionProperties.children[i]
							.sectionProperties.container._leaflet_pos.y,
					});
			}
			...
			let lastPosY =
				this.sectionProperties.container._leaflet_pos.y +
		...
		private createTrackChangeButtons(): void {
			var tdAccept = L.DomUtil.create(
				'td',
				'lool-annotation-menubar',
				this.sectionProperties.authorRow,
			);
			var acceptButton = (this.sectionProperties.acceptButton =
				L.DomUtil.create(
					'button',
			...
			var tdReject = L.DomUtil.create(
				'td',
				'lool-annotation-menubar',
				this.sectionProperties.authorRow,
			);
			var rejectButton = (this.sectionProperties.rejectButton =
				L.DomUtil.create(
					'button',
			...
			L.DomEvent.on(
				...
				'click',
				function () {
					this.map.fire('RedlineAccept', {
						id: this.sectionProperties.data.id,
					});
				},
				this,
			);
		...
		private updateContent(): void {
			if (this.sectionProperties.data.html)
				this.sectionProperties.contentText.innerHTML =
					this.sanitize(this.sectionProperties.data.html);
			else
				this.sectionProperties.contentText.innerText = this
					.sectionProperties.data.text
					? this.sectionProperties.data.text
					: '';
			// Get the escaped HTML out and find for possible, useful links
			var linkedText = Autolinker.link(
				this.sectionProperties.contentText.outerHTML,
			);
			this.sectionProperties.contentText.innerHTML =
				this.sanitize(linkedText);
			// Original unlinked text
			this.sectionProperties.contentText.origText = this
				.sectionProperties.data.text
				? this.sectionProperties.data.text
				: '';
			this.sectionProperties.contentText.origHTML =
				this.sectionProperties.data.html;
			this.sectionProperties.nodeModifyText.textContent = this
				.sectionProperties.data.text
				? this.sectionProperties.data.text
				: '';
			if (this.sectionProperties.data.html) {
			...
			this.sectionProperties.contentAuthor.innerText =
				this.sectionProperties.data.author;

			this.updateResolvedField(this.sectionProperties.data.resolved);
			if (this.sectionProperties.data.avatar) {
				this.sectionProperties.authorAvatarImg.setAttribute(
					'src',
					this.sectionProperties.data.avatar,
				);
			} else {
				$(this.sectionProperties.authorAvatarImg).css(
					'padding-top',
					'4px',
				);
			}
			var user = this.map.getViewId(
				this.sectionProperties.data.author,
			);
			if (user >= 0) {
				var color = L.LOUtil.rgbToHex(this.map.getViewColor(user));
				this.sectionProperties.authorAvatartdImg.style.borderColor =
					color;
			}

			var d = new Date(
				this.sectionProperties.data.dateTime.replace(/,.*/, 'Z'),
			);
			var dateOptions: any = {
				weekday: 'short',
				year: 'numeric',
			...
			this.sectionProperties.contentDate.innerText = isNaN(d.getTime())
				? this.sectionProperties.data.dateTime
				: d.toLocaleDateString((<any>String).locale, dateOptions);

			if (this.sectionProperties.data.trackchange) {
				this.sectionProperties.captionText.innerText =
					this.sectionProperties.data.description;
			}
		}
		...
		private setPositionAndSize(): void {
			...
			if (
				...
				this.size = [
					Math.round(
						this.sectionProperties.imgSize[0] * app.dpiScale,
					),
					Math.round(
						this.sectionProperties.imgSize[1] * app.dpiScale,
					),
				];
				this.setPosition(
					Math.round(
						this.sectionProperties.data.rectangle[0] * ratio,
					),
					Math.round(
						this.sectionProperties.data.rectangle[1] * ratio,
					),
				);
			}
		}
		...
		public removeHighlight(): void {
			if (this.sectionProperties.docLayer._docType === 'text') {
				this.sectionProperties.usedTextColor =
					this.sectionProperties.data.color;
		...
		public highlight(): void {
			if (this.sectionProperties.docLayer._docType === 'text') {
				this.sectionProperties.usedTextColor =
					this.sectionProperties.highlightedTextColor;
		...
		// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
		private static doesRectIntersectView(
			pos: number[],
			size: number[],
			viewContext: any,
		): boolean {
			...
			for (var i = 0; i < paneBoundsList.length; ++i) {
				...
				var rectInvisible =
					endPos[0] < paneBounds.min.x ||
					endPos[1] < paneBounds.min.y ||
					pos[0] > paneBounds.max.x ||
					pos[1] > paneBounds.max.y;
				if (!rectInvisible) return true;
			}
			return false;
		}
	...
	*/
		private convertRectanglesToCoreCoordinates() {
			...
			if (originals) {
				...
				this.sectionProperties.pixelBasedOrgRectangles =
					pixelBasedOrgRectangles;
			}
		}
		...
		private convertRectanglesToViewCoordinates(): boolean {
			...
			if (rectangles) {
				var documentAnchorSection =
					this.containerObject.getDocumentAnchorSection();
				var diff = [
					documentAnchorSection.myTopLeft[0] -
						this.documentTopLeft[0],
					documentAnchorSection.myTopLeft[1] -
				...
				for (var i = 0; i < rectangles.length; i++) {
					...
					if (
						!intersectsVisibleArea &&
						Comment.doesRectIntersectView(
							pos,
							size,
							viewContext,
						)
				...
				this.sectionProperties.data.trackchange &&
				this.sectionProperties.data.anchorPos
			) {
				...
				intersectsVisibleArea = Comment.doesRectIntersectView(
					pos,
					size,
					viewContext,
				);
		...
		private updateAnnotationMarker(): void {
			...
			if (this.sectionProperties.data == null) return;

			if (this.sectionProperties.annotationMarker === null) {
				this.sectionProperties.annotationMarker = L.marker(
					new L.LatLng(0, 0),
					{
						icon: L.divIcon({
				...
				if (
					app.impress.partList[
						...
						parseInt(this.sectionProperties.data.parthash) ||
					app.file.fileBasedView
				)
					this.map.addLayer(
						this.sectionProperties.annotationMarker,
					);
			}
			if (this.sectionProperties.data.rectangle != null) {
				this.sectionProperties.annotationMarker.setLatLng(
					this.sectionProperties.docLayer._twipsToLatLng(
						new L.Point(
							this.sectionProperties.data.rectangle[0],
							this.sectionProperties.data.rectangle[1],
						),
					),
				);
				this.sectionProperties.annotationMarker.on(
		...
		public isContainerVisible(): boolean {
			return (
				this.sectionProperties.container.style &&
		...
		private showWriter() {
			...
			if (
				this.sectionProperties.data.resolved !== 'true' ||
				this.sectionProperties.commentListSection.sectionProperties
					.showResolved
			) {
				L.DomUtil.addClass(
					this.sectionProperties.container,
					'lool-annotation-collapsed-show',
				);
		...
		private getCommentWidth() {
			...
			return (
				parseFloat(
					getComputedStyle(this.sectionProperties.container)
						.width,
				) * app.dpiScale
			);
		}
		...
		public positionCalcComment(): void {
			if (!(<any>window).mode.isMobile()) {
				...
				var cellPos = this.map._docLayer
					...
					.toRectangle();
				var originalSize = [
					Math.round(cellPos[2] * ratio),
					Math.round(cellPos[3] * ratio),
				];

				const startX = this.isCalcRTL()
					? this.myTopLeft[0] - this.getCommentWidth()
					: this.myTopLeft[0] + originalSize[0] - 3;

				var pos: Array<number> = [
					Math.round(startX / app.dpiScale),
					Math.round(this.myTopLeft[1] / app.dpiScale),
				];
				this.sectionProperties.container.style.transform =
					'translate3d(' + pos[0] + 'px, ' + pos[1] + 'px, 0px)';
			}
		}
		...
		public setLayoutClass(): void {
			this.sectionProperties.container.classList.remove(
				'tracked-deleted-comment-show',
			);
			this.sectionProperties.container.classList.remove(
				'tracked-deleted-comment-hide',
			);

			var showTrackedChanges: boolean =
				this.map['stateChangeHandler'].getItemValue(
					'.uno:ShowTrackedChanges',
				) === 'true';
			var layoutClass: string = showTrackedChanges
				? 'tracked-deleted-comment-show'
				: 'tracked-deleted-comment-hide';
			if (
				this.sectionProperties.data.layoutStatus ===
		...
		public show(): void {
			...
			if (
				...
				this.sectionProperties.container.parentElement ===
					document.getElementById('document-container')
			)
				this.sectionProperties.container.style.visibility =
					'hidden';

			if (lool.CommentSection.commentWasAutoAdded) return;
			if (this.sectionProperties.docLayer._docType === 'text')
				this.showWriter();
			else if (
				this.sectionProperties.docLayer._docType ===
		...
		private hideCalc() {
			...
			if (
				...
				this.sectionProperties.commentListSection.sectionProperties.selectedComment =
					null;
		}
		...
		public isAutoSaved(): boolean {
			...
			if (!autoSavedComment) return false;
		...
		public hide(): void {
			...
			if (this.sectionProperties.data.id === 'new') {
				this.sectionProperties.commentListSection.removeItem(
					this.sectionProperties.data.id,
				);
				return;
			}
		...
		public onReplyClick(e: any): void {
			L.DomEvent.stopPropagation(e);
			if ((<any>window).mode.isMobile()) {
				this.sectionProperties.data.reply =
					this.sectionProperties.data.text;
				...
				this.sectionProperties.data.html =
					this.sectionProperties.nodeReplyText.innerHTML;
		...
		public handleCancelCommentButton(e: any): void {
			if (lool.CommentSection.commentWasAutoAdded) {
				app.sectionContainer
					...
					.remove(this.sectionProperties.data.id);
			}

			if (lool.CommentSection.autoSavedComment) {
				this.sectionProperties.contentText.origText =
					this.sectionProperties.contentText.uneditedText;
				this.sectionProperties.contentText.uneditedText = null;
				this.sectionProperties.contentText.origHTML =
					this.sectionProperties.contentText.uneditedHTML;
			...
			if (this.sectionProperties.contentText.origHTML) {
				...
				this.sectionProperties.nodeModifyText.textContent =
					this.sectionProperties.contentText.origText;
			}
			this.sectionProperties.nodeReplyText.textContent = '';
		...
		public onSaveComment(e: any): void {
			...
			this.sectionProperties.data.html =
				this.sectionProperties.nodeModifyText.innerHTML;
			this.updateContent();
			if (!lool.CommentSection.autoSavedComment) this.show();
		...
		public onLostFocus(e: any): void {
			if (
				this.sectionProperties.docLayer._docType === 'text' &&
				this.map.mention?.isTypingMention()
			) {
				return;
			}
			if (!this.sectionProperties.isRemoved) {
				$(this.sectionProperties.container).removeClass(
					'annotation-active reply-annotation-container modify-annotation-container',
				);
				this.removeBRTag(this.sectionProperties.nodeModifyText);
				if (
					this.sectionProperties.contentText.origText !==
					...
					var insertButton =
						document.getElementById('menu-insertcomment');
					if (insertButton) {
						if (
							window.getComputedStyle(insertButton)
								.display === 'none'
						) {
							this.onCancelClick(e);
						}
		...
		public focus(): void {
			this.sectionProperties.container.classList.add(
				'annotation-active',
			);
		...
		private onMarkerDrag(event: any): void {
			if (this.sectionProperties.annotationMarker == null) return;

			if (event.type === 'dragend') {
				var pointTwip =
					this.sectionProperties.docLayer._latLngToTwips(
						this.sectionProperties.annotationMarker.getLatLng(),
					);
		...
		// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
		private doesRectangleContainPoint(
			rectangle: any,
			point: Array<number>,
		): boolean {
			if (
	...
	*/
		private checkIfCursorIsOnThisCommentWriter(
			rectangles: any,
			point: Array<number>,
		) {
			for (var i: number = 0; i < rectangles.length; i++) {
				if (this.doesRectangleContainPoint(rectangles[i], point)) {
					if (!this.isSelected()) {
						this.sectionProperties.commentListSection.selectById(
							this.sectionProperties.data.id,
						);
					}
			...
			if (this.isSelected()) {
				if (this.isCollapsed) this.setCollapsed();
		...
		public onCellAddressChanged(): void {
			if (this.sectionProperties.data.rectangles) {
				var midX =
					...
					Math.round(app.calc.cellCursorRectangle.pCenter[0]);
				var midY =
					this.containerObject.getDocumentAnchor()[1] +
					Math.round(app.calc.cellCursorRectangle.pCenter[1]);

				if (
					midX > this.sectionProperties.data.rectangles[0][0] &&
					midX <
						this.sectionProperties.data.rectangles[0][0] +
							this.sectionProperties.data
								.rectangles[0][2] &&
					midY > this.sectionProperties.data.rectangles[0][1] &&
					midY <
						this.sectionProperties.data.rectangles[0][1] +
							this.sectionProperties.data.rectangles[0][3]
				) {
					this.sectionProperties.commentListSection.sectionProperties.calcCurrentComment =
						this;
					...
					this.sectionProperties.commentListSection
						.sectionProperties.calcCurrentComment == this
				)
					this.sectionProperties.commentListSection.sectionProperties.calcCurrentComment =
						null;
			}
		...
		public onDraw(): void {
			if (this.sectionProperties.showSelectedCoordinate) {
				if (this.sectionProperties.docLayer._docType === 'text') {
					var rectangles: Array<any> =
						this.sectionProperties.data.rectangles;
					if (rectangles) {
					...
					this.sectionProperties.docLayer._docType ===
						'spreadsheet' &&
					parseInt(this.sectionProperties.data.tab) ===
						this.sectionProperties.docLayer._selectedPart
				) {
					var cellSize = this.calcCellSize();
					if (cellSize[0] !== 0 && cellSize[1] !== 0) {
						// don't draw notes in hidden cells
						...
						var x = this.isCalcRTL()
							? margin
							: cellSize[0] - (margin + squareDim);
		...
		private calcContinueWithMouseEvent(): boolean {
			if (this.sectionProperties.docLayer._docType === 'spreadsheet') {
				...
				if (conditions) {
					var sc =
						this.sectionProperties.commentListSection
							.sectionProperties.selectedComment;
					if (sc)
						conditions =
							sc.sectionProperties.data.id !== 'new';
				}
				return conditions;
			} else {
				return false;
		...
		public calcCellSize(): number[] {
			var ratio: number =
				app.tile.size.pixels[0] / app.tile.size.twips[0];
			var cellPos = this.map._docLayer
				._cellRangeToTwipRect(this.sectionProperties.data.cellRange)
				.toRectangle();
			return [
				Math.round(cellPos[2] * ratio),
				Math.round(cellPos[3] * ratio),
				...
				if (
					...
					var sc =
						this.sectionProperties.commentListSection
							.sectionProperties.selectedComment;
					if (sc) {
						if (!sc.isEdit()) sc.hide();
						else return; // Another comment is being edited. Return.
					}

					var containerWidth: number =
						this.sectionProperties.container.getBoundingClientRect()
							.width;
					var ratio: number =
						app.tile.size.pixels[0] / app.tile.size.twips[0];
					var cellPos = this.map._docLayer
						...
						.toRectangle();
					this.size = [
						Math.round(cellPos[2] * ratio + containerWidth),
						Math.round(cellPos[3] * ratio),
					];
					this.sectionProperties.commentListSection.selectById(
						this.sectionProperties.data.id,
		...
		public getParentCommentId(): string {
			if (
				this.sectionProperties.data.parent &&
				this.sectionProperties.data.parent !== '0'
			)
		...
		public getChildLevel(): number {
			...
			const parentComment =
				this.sectionProperties.commentListSection.getComment(
		...
		public getCommentHeight(): number {
			return (
				this.sectionProperties.container.getBoundingClientRect()
					.height -
				this.sectionProperties.childLinesNode.getBoundingClientRect()
					.height
			);
		}
		...
		public updateThreadInfoIndicator(
			replycount: number | string = -1,
		): void {
			if (this.sectionProperties.docLayer._docType === 'spreadsheet')
				return;
			...
			if (this.isEdit())
				...
				this.sectionProperties.collapsedInfoNode.innerText =
					replycount;
			else this.sectionProperties.collapsedInfoNode.innerText = '';
		...
		public autoCompleteMention(
			...
			const mentionStart = containerText.lastIndexOf(
				replacement,
				cursorPosition,
			);
			...
			if (mentionStart !== -1) {
				...
				const beforeMention = containerText.substring(
					0,
					mentionStart,
				);
				...
				hyperlink.parentNode?.insertBefore(
					extraSpaceNode,
					hyperlink.nextSibling,
				);
				hyperlink.parentNode?.insertBefore(
					afterTextNode,
					extraSpaceNode.nextSibling,
				);
...
app.definitions.Comment = lool.Comment;
