...
class OtherViewCursorSection extends HTMLObjectSection {
	...
	constructor(
		...
		super(
			...
			new lool.SimplePoint(rectangle.x1, rectangle.y1),
		);
		...
		this.getHTMLObject().style.backgroundColor =
	...
	public static addOrUpdateOtherViewCursor(
		...
		const documentPosition = new lool.SimplePoint(
	...
	public static updateVisibilities(hideCursors: boolean = false) {
		for (
			let i = 0;
			i < OtherViewCursorSection.sectionPointers.length;
			i++
		) {
