...
class OtherViewCellCursorSection extends CanvasSectionObject {
	...
	constructor(
		viewId: number,
		rectangle: lool.SimpleRectangle,
		part: number,
	) {
		...
		this.sectionProperties.color = app.LOUtil.rgbToHex(
			app.LOUtil.getViewIdColor(viewId),
		);
	...
	onDraw(frameCount?: number, elapsedTime?: number): void {
		...
		const tempSizePos = CellCursorSection.adjustSizePos([
			this.position[0],
			this.position[1],
			this.size[0],
			this.size[1],
	...
	adjustPopUpPosition() {
		const width =
			this.sectionProperties.popUpContainer.getBoundingClientRect()
				.width;
		const height =
			this.sectionProperties.popUpContainer.getBoundingClientRect()
				.height;
		...
		const pos = [
			this.myTopLeft[0] / app.dpiScale +
				(this.size[0] * 0.5) / app.dpiScale -
				width * 0.5,
			this.myTopLeft[1] / app.dpiScale - (height + 15),
		];
		this.sectionProperties.popUpContainer.style.left = pos[0] + 'px';
		this.sectionProperties.popUpContainer.style.top = pos[1] + 'px';

		if (!this.showSection) this.hideUsernamePopUp();
	}
	...
	prepareUsernamePopUp() {
		if (this.sectionProperties.popUpContainer === null) {
			...
			arrowDiv.style.backgroundColor =
				nameParagraph.style.backgroundColor =
					this.sectionProperties.color;
	...
	showUsernamePopUp() {
		const textCursorSectionName =
			CursorHeaderSection.namePrefix + this.sectionProperties.viewId;
    ...
    public static addOrUpdateOtherViewCellCursor(viewId: number, username: string, rectangleData: Array<string>, part: number) {
		...
		if (app.sectionContainer.doesSectionExist(sectionName)) {
			...
			section = new OtherViewCellCursorSection(
				viewId,
				rectangle,
				part,
			);
		...
		if (section.showSection && !newSection) section.showUsernamePopUp();

		if (!section.showSection) section.hideUsernamePopUp();
	...
	public static removeView(viewId: number) {
		...
		if (app.sectionContainer.doesSectionExist(sectionName)) {
			...
			OtherViewCellCursorSection.sectionPointers.splice(
				OtherViewCellCursorSection.sectionPointers.indexOf(section),
				1,
			);
	...
	public static updateVisibilities() {
		for (
			let i = 0;
			i < OtherViewCellCursorSection.sectionPointers.length;
			i++
		) {
			...
			if (newState !== section.showSection) {
				...
				if (newState === false) section.hideUsernamePopUp();
			}
		}
		app.sectionContainer.requestReDraw();
	}
