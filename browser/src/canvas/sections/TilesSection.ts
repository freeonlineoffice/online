...
namespace lool {
	export class TilesSection extends app.definitions.canvasSectionObject {
		...
		private makeCheckPattern() {
			...
			for (var y = 0; y < 256; y += 32) {
				for (var x = 0; x < 256; x += 32) {
					if (patternOn) drawctx.fillStyle = 'darkgray';
					else drawctx.fillStyle = 'gray';
					patternOn = !patternOn;
		...
		paintWithPanes(tile: any, ctx: any, async: boolean, now: Date): void {
			...
			for (var i = 0; i < ctx.paneBoundsList.length; ++i) {
				...
				if (paneBounds.intersects(tileBounds)) {
					...
					paneOffset.x = Math.min(
						paneOffset.x,
						viewBounds.min.x,
					);
					paneOffset.y = Math.min(
						paneOffset.y,
						viewBounds.min.y,
					);
		...
		public getSubsetBounds(
			canvasCtx: CanvasRenderingContext2D,
			tileSubset: Set<any>,
		): lool.Bounds {
			// don't do anything for this atypical varient
			if (app.file.fileBasedView) return null;
			...
			for (const coords of Array.from(tileSubset)) {
				...
				var rightBottom = new L.Point(
					topLeft.x + ctx.tileSize.x,
					topLeft.y + ctx.tileSize.y,
				);
		...
		public clipSubsetBounds(
			...
			canvasCtx.rect(
				rect[0] - ctx.viewBounds.min.x,
				rect[1] - ctx.viewBounds.min.y,
				rect[2],
				rect[3],
			);
		...
		drawTileInPane(
			...
			if (cropWidth && cropHeight) {
				if (
					clearBackground ||
					this.containerObject.isZoomChanged() ||
					canvasCtx !== this.context
				) {
					// Whole canvas is not cleared after zoom has changed, so clear it per tile as they arrive.
					canvasCtx.fillStyle =
						this.containerObject.getClearColor();
					this.beforeDraw(canvasCtx);
					canvasCtx.fillRect(
		...
		pdfViewDrawTileBorders(
			...
			this.context.fillText(
				tile.coords.x +
					...
					' ' +
					tile.coords.part,
				Math.round(offset.x + tileSize * 0.5),
				Math.round(offset.y + tileSize * 0.5),
			);
		}
		...
		paintSimple(tile: any, ctx: any, async: boolean, now: Date): void {
			...
			var offset = new L.Point(
				tile.coords.getPos().x - ctx.viewBounds.min.x,
				tile.coords.getPos().y - ctx.viewBounds.min.y,
			);

			if (
				(async || this.containerObject.isZoomChanged()) &&
			...
			if (app.file.fileBasedView) {
				tileSizeX = tileSizeY =
					this.sectionProperties.docLayer._tileSize;
				var ratio =
					tileSizeX /
					this.sectionProperties.docLayer._tileHeightTwips;
				...
				offset.y =
					tile.coords.part * partHeightPixels +
					tile.coords.y -
		...
		public paint(tile: any, ctx: any, async: boolean, now: Date): void {
			...
			if (!ctx) ctx = this.sectionProperties.tsManager._paintContext();

			this.containerObject.setPenPosition(this);

			if (ctx.paneBoundsActive === true)
				this.paintWithPanes(tile, ctx, async, now);
			else this.paintSimple(tile, ctx, async, now);
		}

		private forEachTileInView(
			zoom: number,
			part: number,
			mode: number,
			ctx: any,
			...
			if (app.file.fileBasedView) {
				var coordList: Array<any> =
					this.sectionProperties.docLayer._updateFileBasedView(
						true,
					);
				...
				for (var k: number = 0; k < coordList.length; k++) {
					...
					if (!callback(tile, coordList[k])) return;
				}
			} else {
				for (
					var rangeIdx = 0;
					rangeIdx < tileRanges.length;
					++rangeIdx
				) {
					...
					for (
						...
						for (
							...
							if (!callback(tile, coords)) return;
						}
					}
				}
			}
		...
		public haveAllTilesInView(
			...
			this.forEachTileInView(
				zoom,
				part,
				mode,
				ctx,
				function (tile: any): boolean {
		...
		private drawPageBackgroundWriter(
			ctx: any,
			rectangle: any,
			pageNumber: number,
		) {
			...
			this.context.fillStyle =
				this.containerObject.getDocumentBackgroundColor(); // used to be pageBackgroundFillColorWriter (see below)
			this.context.fillRect(
				rectangle[0] -
					ctx.viewBounds.min.x +
					this.sectionProperties.pageBackgroundInnerMargin,
				rectangle[1] -
					ctx.viewBounds.min.y +
					this.sectionProperties.pageBackgroundInnerMargin,
				rectangle[2] -
					this.sectionProperties.pageBackgroundInnerMargin,
				rectangle[3] -
		...
		private drawPageBackgroundFileBasedView(
			ctx: any,
			top: number,
			bottom: number,
		) {
			var partHeightPixels: number = Math.round(
				this.map._docLayer._partHeightTwips * app.twipsToPixels,
			);
			var gap: number = Math.round(
				this.map._docLayer._spaceBetweenParts * app.twipsToPixels,
			);
			var partWidthPixels: number = Math.round(
				this.map._docLayer._partWidthTwips * app.twipsToPixels,
			);
			var startY: number =
				(partHeightPixels + gap) * (top > 0 ? top - 1 : 0);
			...
			for (var i: number = 0; i <= bottom - top; i++) {
				...
				this.context.fillText(
					String(i + top + 1),
					Math.round((2 * rectangle[0] + rectangle[2]) * 0.5) -
						ctx.viewBounds.min.x,
					Math.round((2 * rectangle[1] + rectangle[3]) * 0.5) -
						ctx.viewBounds.min.y,
					rectangle[2] * 0.4,
				);
		...
		private drawPageBackgrounds(ctx: any) {
			if (
				this.map._docLayer._docType !== 'text' &&
				!app.file.fileBasedView
			)
			...
			if (!this.containerObject.getDocumentAnchorSection()) return;

			this.context.fillStyle =
				this.sectionProperties.pageBackgroundTextColor;
			this.context.strokeStyle =
				this.sectionProperties.pageBackgroundBorderColor;
			...
			if (this.map._docLayer._docType === 'text') {
				...
				viewRectangleTwips = viewRectangleTwips.map(function (
					element: number,
				) {
				...
				for (
					var i: number = 0;
					i < app.file.writer.pageRectangleList.length;
					i++
				) {
					var rectangle: any =
						app.file.writer.pageRectangleList[i];
					if (
						(rectangle[1] > viewRectangleTwips[1] &&
							rectangle[1] <
								viewRectangleTwips[1] +
									viewRectangleTwips[3]) ||
				...
				// Writer and fileBasedView can not be "true" at the same time.
				// PDF view supports only same-sized pages for now. So we can use simple math instead of a loop.
				var partHeightPixels: number = Math.round(
					(this.map._docLayer._partHeightTwips +
						this.map._docLayer._spaceBetweenParts) *
						app.twipsToPixels,
				);
				var visibleBounds: Array<number> =
					this.containerObject.getDocumentBounds();
				var topVisible: number = Math.floor(
					visibleBounds[1] / partHeightPixels,
				);
				var bottomVisible: number = Math.ceil(
					visibleBounds[3] / partHeightPixels,
				);

				// Check existence of pages.
				topVisible = topVisible >= 0 ? topVisible : 0;
				bottomVisible =
					bottomVisible < this.map._docLayer._parts
						? bottomVisible
						: this.map._docLayer._parts - 1;

				if (!isNaN(partHeightPixels) && partHeightPixels > 0)
					this.drawPageBackgroundFileBasedView(
						ctx,
						topVisible,
						bottomVisible,
					);
		...
		public onDraw(
			...
			if (this.sectionProperties.tsManager.waitForTiles()) {
				if (!this.haveAllTilesInView(zoom, part, mode, ctx)) return;
			...
			this.forEachTileInView(
				...
				mode,
				ctx,
				function (tile: any, coords: any): boolean {
					if (doneTiles.has(coords.key())) return true;

					// We can skip this tile if we only want to draw a subset, and it falls outside that
					if (
						subsetBounds &&
						!subsetBounds.intersects(
							docLayer._coordsToPixBounds(coords),
						)
					)
						return true;
					...
					if (tile && docLayer._isValidTile(coords)) {
						if (!this.isJSDOM) {
							// perf-test code
							if (
								docLayer._isTileReadyToDraw(tile) ||
								this.map._debug.tileOverlaysOn
							) {
		...
		public onClick(point: Array<number>, e: MouseEvent): void {
			...
			if (
				this.map._docLayer._docType === 'presentation' ||
				this.map._docLayer._docType === 'drawing'
			)
		...
		// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
		static getTileIntersectionAreaFraction(
			tileBounds: any,
			viewBounds: any,
		): number {
			...
			return (
				(Math.max(0, interSize.x) * Math.max(0, interSize.y)) /
				(size.x * size.y)
			);
		}

		private forEachTileInArea(
			area: any,
			zoom: number,
			part: number,
			mode: number,
			...
			if (app.file.fileBasedView) {
				var coordList: Array<any> = docLayer._updateFileBasedView(
					true,
					area,
					zoom,
				);
				...
				for (var k: number = 0; k < coordList.length; k++) {
					...
					var key = coords.key();
					var tile = docLayer._tiles[key];
					if (tile) callback(tile, coords, this);
				}
		 ...
		 */
		private zoomLevelWithMaxContentInArea(
			area: any,
			areaZoom: number,
			part: number,
			mode: number,
			...
			var frameScale =
				this.sectionProperties.tsManager._zoomFrameScale;
			var docLayer = this.sectionProperties.docLayer;
			var targetZoom = Math.round(
				this.map.getScaleZoom(frameScale, areaZoom),
			);
			var bestZoomLevel = targetZoom;
			var availAreaScoreAtBestZL = -Infinity; // Higher the better.
			var area = area.clone();
			if (area.min.x < 0) area.min.x = 0;
			if (area.min.y < 0) area.min.y = 0;
			...
			for (var zoom = minZoom; zoom <= maxZoom; ++zoom) {
				...
				var dimensionCorrection = this.map.zoomToFactor(
					maxZoom - zoom + this.map.options.zoom,
				);

				// Compute area for zoom-level 'zoom'.
				var areaAtZoom = this.scaleBoundsForZoom(
					area,
					zoom,
					areaZoom,
				);
				...
				this.forEachTileInArea(
					areaAtZoom,
					zoom,
					part,
					mode,
					ctx,
					function (tile, coords, section) {
						if (tile && tile.canvas) {
							var tilePos = coords.getPos();

							if (app.file.fileBasedView) {
								var ratio =
									(ctx.tileSize.y * relScale) /
									docLayer._tileHeightTwips;
								var partHeightPixels = Math.round(
									(docLayer._partHeightTwips +
										docLayer._spaceBetweenParts) *
										ratio,
								);
								tilePos.y =
									coords.part * partHeightPixels +
									tilePos.y;
							}

							var tileBounds = new L.Bounds(
								tilePos,
								tilePos.add(ctx.tileSize),
							);
							var interFrac =
								TilesSection.getTileIntersectionAreaFraction(
									tileBounds,
									areaAtZoom,
								);
				...
				if (
					availAreaScore > availAreaScoreAtBestZL ||
					(availAreaScore == availAreaScoreAtBestZL &&
						Math.abs(targetZoom - bestZoomLevel) >
							Math.abs(targetZoom - zoom))
				) {
		...
		private drawDebugHistogram(
			...
			if (rowLeft > 0)
				canvas.fillRect(
					x,
					y + rowBlock * deltaSize + deltaSize,
					rowLeft * deltaSize,
					deltaSize,
				);
		}
		...
		public drawZoomFrame(ctx?: any): void {
			...
			if (!tsManager._inZoomAnim) return;

			var scale = tsManager._zoomFrameScale;
			if (!scale || !tsManager._newCenter) return;
			...
			for (var k = 0; k < paneBoundsList.length; ++k) {
				...
				if (!freezeX) {
					tsManager._zoomAtDocEdgeX =
						docPos.topLeft.x == splitPos.x;
				}

				if (!freezeY) {
					tsManager._zoomAtDocEdgeY =
						docPos.topLeft.y == splitPos.y;
				}

				var docRange = new L.Bounds(
					docPos.topLeft,
					docPos.topLeft.add(docAreaSize),
				);
				if (tsManager._calcGridSection) {
					tsManager._calcGridSection.onDrawArea(
						docRange,
						docRange.min.subtract(destPos),
						this.context,
					);
				}
				...
				if (scale < 1.0) {
					...
					bestZoomSrc = this.zoomLevelWithMaxContentInArea(
						docRange,
						zoom,
						part,
						mode,
				...
				var relScale =
					bestZoomSrc == zoom
						? 1
						: this.map.getZoomScale(bestZoomSrc, zoom);
				...
				this.forEachTileInArea(
					...
					mode,
					ctx,
					function (tile, coords, section): boolean {
						if (
							!tile ||
							!docLayer._isTileReadyToDraw(tile) ||
							!docLayer._isValidTile(coords)
						)
							return false;
						...
						var tileBounds = new L.Bounds(
							tileCoords,
							tileCoords.add(ctx.tileSize),
						);

						var crop = new L.Bounds(
							tileBounds.min,
							tileBounds.max,
						);
						crop.min.x = Math.max(
							docRangeScaled.min.x,
							tileBounds.min.x,
						);
						crop.min.y = Math.max(
							docRangeScaled.min.y,
							tileBounds.min.y,
						);
						crop.max.x = Math.min(
							docRangeScaled.max.x,
							tileBounds.max.x,
						);
						crop.max.y = Math.min(
							docRangeScaled.max.y,
							tileBounds.max.y,
						);
						...
						var tileOffset = crop.min.subtract(
							tileBounds.min,
						);
						var paneOffset = crop.min.subtract(
							docRangeScaled.min.subtract(destPosScaled),
						);
						if (cropWidth && cropHeight) {
							section.drawTileToCanvasCrop(
								...
								canvasContext,
								tileOffset.x,
								tileOffset.y, // source x, y
								cropWidth,
								cropHeight, // source size
								// Destination x, y, w, h (In non-Chrome browsers it leaves lines without the 0.5 correction).
								Math.floor(
									(paneOffset.x / relScale) * scale,
								) + 0.5, // Destination x
								Math.floor(
									(paneOffset.y / relScale) * scale,
								) + 0.5, // Destination y
								Math.floor(
									(cropWidth / relScale) * scale,
								) + 1.5, // Destination width
								Math.floor(
									(cropHeight / relScale) * scale,
								) + 1.5,
							); // Destination height
						}
		...
		private scalePosForZoom(
			...
			if (docLayer.sheetGeometry) {
				var toScale =
					(convScale * docLayer._tileSize * 15.0) /
					docLayer._tileWidthTwips;
				toScale =
					(docLayer._tileSize * 15.0) /
					Math.round((15.0 * docLayer._tileSize) / toScale);
				var posScaled = docLayer.sheetGeometry.getCorePixelsAtZoom(
					pos,
					toScale,
				);
				return posScaled;
		...
		private scaleBoundsForZoom(
			...
			if (docLayer.sheetGeometry) {
				var topLeft = this.scalePosForZoom(
					corePxBounds.min,
					toZoom,
					fromZoom,
				);
