...
namespace lool {
	export class TilesSection extends CanvasSectionObject {
		...
		public getSubsetBounds(
			...
			for (const coords of Array.from(tileSubset)) {
				...
				var rightBottom = new L.Point(
					topLeft.x + TileManager.tileSize,
					topLeft.y + TileManager.tileSize,
				);
		...
		public paint(tile: any, ctx: any, async: boolean): void {
			...
			else this.paintSimple(tile, ctx, async);
		}
		...
		private drawPageBackgroundWriter(ctx: any) {
			let viewRectangleTwips = [
				...
				this.containerObject.getDocumentAnchorSection().size[1],
			];
			...
			this.context.fillStyle =
				this.containerObject.getDocumentBackgroundColor();
			for (
				let i: number = 0;
				...
				if (
					...
					(rectangle[1] + rectangle[3] > viewRectangleTwips[1] &&
		...
		private drawPageBackgroundFileBasedView(ctx: any) {
			...
			var bottomVisible: number = Math.ceil(
				(visibleBounds[1] + visibleBounds[3]) / partHeightPixels,
			);
			...
			if (!isNaN(partHeightPixels) && partHeightPixels > 0) {
				...
				var startY: number =
					(partHeightPixels + gap) *
					(topVisible > 0 ? topVisible : 0);
				var rectangle: Array<number>;

				for (
					var i: number = 0;
					i <= bottomVisible - topVisible;
					i++
				) {
					rectangle = [
						0,
					...
					this.context.fillText(
						String(i + topVisible + 1),
						Math.round(
							(2 * rectangle[0] + rectangle[2]) * 0.5,
						) - ctx.viewBounds.min.x,
						Math.round(
							(2 * rectangle[1] + rectangle[3]) * 0.5,
						) - ctx.viewBounds.min.y,
						rectangle[2] * 0.4,
					);
		...
		private drawPageBackgroundsMultiPageView() {
			...
			for (
				let i = 0;
				i < app.activeDocument.activeView.layoutRectangles.length;
				i++
			) {
				const rectangle =
					app.activeDocument.activeView.layoutRectangles[i];
				const coords = [
					rectangle.layoutX -
						app.activeDocument.activeView.viewedRectangle.pX1,
					rectangle.layoutY -
		...
		private drawForMultiPageView() {
			const visibleCoordList: Array<TileCoordData> =
				TileManager.getVisibleCoordList(
					app.activeDocument.activeView.getVisibleAreaRectangle(),
				);
			...
			for (let i = 0; i < visibleCoordList.length; i++) {
				...
				for (
					let j = 0;
					j <
					app.activeDocument.activeView.layoutRectangles.length;
					j++
				) {
					const layoutRectangle =
						app.activeDocument.activeView.layoutRectangles[j];
		...
		public onDraw(
			...
			this.forEachTileInView(
				...
				mode,
				ctx,
				function (tile: any, coords: TileCoordData): boolean {
		...
		private forEachTileInArea(
			...
			if (app.file.fileBasedView) {
				const coordList = TileManager.updateFileBasedView(
					true,
					area,
					zoom,
				);
		...
		private scalePosForZoom(
			...
			if (docLayer.sheetGeometry) {
				var toScale =
					(convScale * TileManager.tileSize * 15.0) /
					app.tile.size.x;
				toScale =
					(TileManager.tileSize * 15.0) /
					Math.round((15.0 * TileManager.tileSize) / toScale);
