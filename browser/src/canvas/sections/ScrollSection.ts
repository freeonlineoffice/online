...
namespace lool {
	...
	public onHandleAutoScroll (e :any): void {
		...
		public onInitialize(): void {
			...
			this.sectionProperties.defaultCursorStyle =
				this.sectionProperties.mapPane.style.cursor;

			this.sectionProperties.previousDragDistance = null;

			this.sectionProperties.scrollBarThickness =
				6 * app.roundedDpiScale;

			this.sectionProperties.drawScrollBarRailway = true;
			this.sectionProperties.scrollBarRailwayThickness =
				6 * app.roundedDpiScale;
			this.sectionProperties.scrollBarRailwayAlpha =
				this.map._docLayer._docType === 'spreadsheet' ? 1.0 : 0.5;
			this.sectionProperties.scrollBarRailwayColor = '#EFEFEF';

			this.sectionProperties.drawVerticalScrollBar = (<any>(
				window
			)).mode.isDesktop()
				? true
				: false;
			this.sectionProperties.drawHorizontalScrollBar = (<any>(
				window
			)).mode.isDesktop()
				? true
				: false;
	...
	private hideVerticalScrollBar (): void {
		if (this.sectionProperties.mouseIsOnVerticalScrollBar) {
			...
			this.sectionProperties.minimumScrollSize =
				80 * app.roundedDpiScale;

			this.sectionProperties.circleSliderRadius =
				24 * app.roundedDpiScale; // Radius of the mobile vertical circular slider.
			this.sectionProperties.arrowCornerLength =
				10 * app.roundedDpiScale; // Corner length of the arrows inside circular slider.
			...
			this.sectionProperties.idleDuration = 2000; // In milliseconds. Scroll bar will be visible for this period of time after being used.
			this.sectionProperties.fadeOutStartingTime = 1800; // After this period, scroll bar starts to disappear. This duration is included in "idleDuration".
			this.sectionProperties.fadeOutDuration =
				this.sectionProperties.idleDuration -
				this.sectionProperties.fadeOutStartingTime;

			this.sectionProperties.animatingScroll = false;

			this.sectionProperties.animateWheelScroll = (<any>(
				window
			)).mode.isDesktop();
			this.sectionProperties.lastElapsedTime = 0;
			this.sectionProperties.scrollAnimationDelta = [0, 0];
	...
	*/
	...
	private clearQuickScrollTimeout() {
				...
				this.autoScrollTimer = setInterval(
					L.bind(function () {
						...
						if (
							...
							this.containerObject.targetBoundSectionListContains(
								L.CSections.Tiles.name,
							) &&
							(<any>window).mode.isDesktop() &&
							this.containerObject.isDraggingSomething() &&
							L.Map.THIS._docLayer._docType ===
								'spreadsheet'
						) {
							var temp = [e.pos.x, e.pos.y];
							var tempPos = [
								(this.isRTL()
									? this.map._size.x - temp[0]
									: temp[0]) * app.dpiScale,
								temp[1] * app.dpiScale,
							];
							tempPos = [
							...
							tempPos = [
								Math.round(
									tempPos[0] * app.pixelsToTwips,
								),
								Math.round(
									tempPos[1] * app.pixelsToTwips,
								),
							];
							L.Map.THIS._docLayer._postMouseEvent(
								'move',
								tempPos[0],
								tempPos[1],
								1,
		...
		public onHandleAutoScroll(e: any): void {
			...
			const mousePosX: number = this.isRTL()
				? e.map._size.x - e.pos.x
				: e.pos.x;
			const mapLeft: number = this.isRTL()
				? e.map._size.x - e.map._getTopLeftPoint().x
				: e.map._getTopLeftPoint().x;
		...
		private DrawVerticalScrollBarMobile(): void {
			const scrollProps: ScrollProperties = (
				app.activeDocument as DocumentBase
			).activeView.scrollProperties;

			this.context.globalAlpha = this.sectionProperties
				.clickScrollVertical
				? this.sectionProperties.alphaWhenBeingUsed
				: this.sectionProperties.alphaWhenVisible;
			...
			var x: number =
				circleStartX -
				this.sectionProperties.arrowCornerLength * 0.5;
		...
		private drawVerticalScrollBar(): void {
			...
			var startX = this.isRTL()
				? scrollProps.edgeOffset
				: this.size[0] -
					this.sectionProperties.scrollBarThickness -
					scrollProps.edgeOffset;
			...
			if (this.containerObject.testing) {
				var element: HTMLDivElement = <HTMLDivElement>(
					document.getElementById('test-div-vertical-scrollbar')
				);
				if (!element) {
					element = document.createElement('div');
					element.id = 'test-div-vertical-scrollbar';
		...
		private drawHorizontalScrollBar(): void {
			...
			const sizeX =
				...
				this.sectionProperties.scrollBarThickness;
			const docWidth: number = this.map
				.getPixelBoundsCore()
				.getSize().x;
			const startX = this.isRTL()
				? docWidth - scrollProps.startX - sizeX
				: scrollProps.startX;
		...
		private calculateCurrentAlpha(elapsedTime: number): void {
			if (elapsedTime >= this.sectionProperties.fadeOutStartingTime) {
				this.sectionProperties.currentAlpha = Math.max(
					(1 -
						(elapsedTime -
							this.sectionProperties.fadeOutStartingTime) /
							this.sectionProperties.fadeOutDuration) *
				...
				this.sectionProperties.currentAlpha =
					this.sectionProperties.alphaWhenVisible;
			}
		}
		...
		private doMove() {
			...
			app.layoutingService.appendLayoutingTask(() => {
				this.map.panBy(
					new L.Point(
						scrollProps.moveBy[0] / app.dpiScale,
						scrollProps.moveBy[1] / app.dpiScale,
					),
				);
				scrollProps.moveBy = null;
				this.onUpdateScrollOffset();
		...
		public onDraw(frameCount: number, elapsedTime: number): void {
			if (
				app.activeDocument.activeView.scrollProperties.moveBy !==
				null
			)
				this.doMove();
			else app.activeDocument.activeView.refreshScrollProperties();

			if (this.isAnimating && frameCount >= 0)
				this.calculateCurrentAlpha(elapsedTime);

			if (
				this.sectionProperties.drawVerticalScrollBar ||
				this.sectionProperties.alwaysDrawVerticalScrollBar
			) {
				if ((<any>window).mode.isMobile())
					this.DrawVerticalScrollBarMobile();
				else this.drawVerticalScrollBar();
			}
		...
		public onAnimate(frameCount: number, elapsedTime: number): void {
			const timeDelta =
				(elapsedTime - this.sectionProperties.lastElapsedTime) /
				(1000 / 60);
			if (this.sectionProperties.animatingScroll) {
				const lineHeight =
					this.containerObject.getScrollLineHeight();
				// Smoothness will be affected by Firefox bug #1967935
				// Note that we should really use geometric series to calculate acceleration when frames
				// are skipped, but given we expect consistent performance, this shouldn't make a
				// noticeable difference and isn't worth the added complication.
				const accel =
					lineHeight *
					ScrollSection.scrollAnimationAcceleration *
					timeDelta *
					app.dpiScale;
				...
				for (let i = 0; i < 2; ++i) {
					const sign =
						this.sectionProperties.scrollAnimationDirection[
							i
						];
					...
					// we ought to multiply the scrollAnimationDelta here by some factor that increases with
					// elapsedTime.
					this.sectionProperties.scrollAnimationAcc[i] +=
						this.sectionProperties.scrollAnimationDelta[i];
					this.sectionProperties.scrollAnimationDelta[i] = 0;

					this.sectionProperties.scrollAnimationVelocity[i] +=
						this.sectionProperties.scrollAnimationAcc[i] != 0
							? accel
							: -accel;
					this.sectionProperties.scrollAnimationVelocity[i] =
						Math.max(
							...
							this.sectionProperties
								.scrollAnimationVelocity[i],
						);

					deltas[i] =
						this.sectionProperties.scrollAnimationVelocity[
							i
						] * sign;
				...
				if (this.sectionProperties.scrollWheelDelta[0] !== 0) {
					const delta =
						this.sectionProperties.scrollWheelDelta[0];
					this.sectionProperties.scrollWheelDelta[0] = 0;
					this.scrollHorizontalWithOffset(delta);
				}
				if (this.sectionProperties.scrollWheelDelta[1] !== 0) {
					const delta =
						this.sectionProperties.scrollWheelDelta[1];
					this.sectionProperties.scrollWheelDelta[1] = 0;
					this.scrollVerticalWithOffset(delta);
				}
			}
			...
			const animatingScrollbar =
				elapsedTime &&
				elapsedTime < this.sectionProperties.fadeOutDuration;
			const animatingScroll =
				this.sectionProperties.animatingScroll &&
				(timeDelta <= 0 ||
					this.sectionProperties.scrollAnimationVelocity.reduce(
						(a: number, x: number) => a + x,
						0,
					) !== 0);
			if (!animatingScrollbar && !animatingScroll)
		...
		private hideVerticalScrollBar(): void {
			...
			if (
				...
				// On desktop, we don't want to hide the vertical scroll bar.
		...
		private isMouseOnScrollBar(point: lool.SimplePoint): void {
			...
			const documentAnchor: CanvasSectionObject =
			...
			let temp =
				point.pY >= this.size[1] - scrollProps.usableThickness;
			temp =
				temp &&
				((!mirrorX &&
					point.pX <=
						this.size[0] -
							scrollProps.horizontalScrollRightOffset &&
					point.pX >= scrollProps.xOffset) ||
					(mirrorX &&
						point.pX >=
							scrollProps.horizontalScrollRightOffset &&
						point.pX >= scrollProps.xOffset));
			...
			if (
				app.activeDocument.activeView.canScrollHorizontal(
					documentAnchor,
				)
			) {
				if (
					(<any>window).mode.isDesktop() ||
					this.sectionProperties.mouseIsOnHorizontalScrollBar
				)
					this.showHorizontalScrollBar();
				else this.hideHorizontalScrollBar();
			} else this.hideHorizontalScrollBar();
			...
			temp =
				(temp &&
					!mirrorX &&
					point.pX >=
						this.size[0] - scrollProps.usableThickness) ||
				(mirrorX && point.pX <= scrollProps.usableThickness);
			this.sectionProperties.mouseIsOnVerticalScrollBar = temp;

			if (
				app.activeDocument.activeView.canScrollVertical(
					documentAnchor,
				)
			) {
				if (
					(<any>window).mode.isDesktop() ||
					this.sectionProperties.mouseIsOnVerticalScrollBar
				)
					this.showVerticalScrollBar();
				else this.hideVerticalScrollBar();
			} else this.hideVerticalScrollBar();
		...
		private isMouseInsideDocumentAnchor(
			point: lool.SimplePoint,
		): boolean {
			var docSection = this.containerObject.getDocumentAnchorSection();
			return this.containerObject.doesSectionIncludePoint(
				docSection,
				point.pToArray(),
			);
		}
		...
		// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
		private isMousePointerSyncedWithVerticalScrollBar(
			scrollProps: any,
			position: lool.SimplePoint,
		): boolean {
			// Keep this desktop-only for now.
			if (!(<any>window).mode.isDesktop()) return true;
			...
			var pointerIsSyncWithScrollBar = false;
			if (this.sectionProperties.pointerSyncWithVerticalScrollBar) {
				pointerIsSyncWithScrollBar =
					scrollProps.startY < position.pX &&
					scrollProps.startY +
						scrollProps.scrollSize -
						this.sectionProperties.scrollBarThickness >
						position.pY;
				pointerIsSyncWithScrollBar =
					pointerIsSyncWithScrollBar ||
					(this.isMouseInsideDocumentAnchor(position) &&
						spacer === 0);
			} else {
				// See if the scroll bar is on top or bottom.
				var docAncSectionY =
				...
				if (
					scrollProps.startY <
					30 * window.app.roundedDpiScale + docAncSectionY
				) {
					pointerIsSyncWithScrollBar =
						scrollProps.startY + spacer < position.pY;
				} else {
					pointerIsSyncWithScrollBar =
						scrollProps.startY + spacer > position.pY;
				}
			...
			this.sectionProperties.pointerSyncWithVerticalScrollBar =
				pointerIsSyncWithScrollBar;
			return pointerIsSyncWithScrollBar;
		}

		// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
		private isMousePointerSyncedWithHorizontalScrollBar(
			scrollProps: any,
			position: lool.SimplePoint,
		): boolean {
			// Keep this desktop-only for now.
			...
			if (this.sectionProperties.pointerSyncWithHorizontalScrollBar) {
				pointerIsSyncWithScrollBar =
					position.pX > startX && position.pX < endX;
				pointerIsSyncWithScrollBar =
					pointerIsSyncWithScrollBar ||
					(this.isMouseInsideDocumentAnchor(position) &&
						spacer === 0);
				...
				if (
					startX <
					30 * window.app.roundedDpiScale + docAncSectionX
				) {
					pointerIsSyncWithScrollBar =
						startX + spacer < position.pX;
				} else {
					pointerIsSyncWithScrollBar =
						startX + spacer > position.pX;
				}
			...
			this.sectionProperties.pointerSyncWithHorizontalScrollBar =
				pointerIsSyncWithScrollBar;
			return pointerIsSyncWithScrollBar;
		}
		...
		public onMouseMove(
			...
			if (
				this.sectionProperties.clickScrollVertical &&
				this.containerObject.isDraggingSomething()
			) {
				if (!this.sectionProperties.previousDragDistance) {
					this.sectionProperties.previousDragDistance = [0, 0];
				...
				var diffY: number =
					dragDistance[1] -
					this.sectionProperties.previousDragDistance[1];

				if (
					this.isMousePointerSyncedWithVerticalScrollBar(
						scrollProps,
						position,
					)
				)
					this.scrollVerticalWithOffset(
						diffY * scrollProps.verticalScrollRatio,
					);
				...
				this.sectionProperties.clickScrollHorizontal &&
				this.containerObject.isDraggingSomething()
			) {
				if (!this.sectionProperties.previousDragDistance) {
					this.sectionProperties.previousDragDistance = [0, 0];
				...
				var diffX: number =
					dragDistance[0] -
					this.sectionProperties.previousDragDistance[0];
				var actualDistance =
					scrollProps.horizontalScrollRatio * diffX;

				if (
					this.isMousePointerSyncedWithHorizontalScrollBar(
						scrollProps,
						position,
					)
				)
		...
		we quickly scroll the document to that position.
	*/
		private quickScrollVertical(
			point: lool.SimplePoint,
			originalSign?: number,
		): void {
			// Desktop only for now.
			if (!(<any>window).mode.isDesktop()) return;

			L.DomUtil.addClass(document.documentElement, 'prevent-select');
			const scrollProps: ScrollProperties =
				app.activeDocument.activeView.scrollProperties;
			...
			if (this.stepByStepScrolling) {
				...
				if (this.sectionProperties.quickScrollVerticalTimer)
					clearTimeout(
						this.sectionProperties.quickScrollVerticalTimer,
					);
				if (this.sectionProperties.clickScrollVertical)
					this.sectionProperties.quickScrollVerticalTimer =
						setTimeout(() => {
				...
				offset =
					(point.pY -
						(scrollProps.startY +
							scrollProps.verticalScrollSize * 0.5)) *
	...
	*/
		private quickScrollHorizontal(
			...
			if (this.stepByStepScrolling) {
				...
				if (this.sectionProperties.quickScrollHorizontalTimer)
					clearTimeout(
						this.sectionProperties.quickScrollHorizontalTimer,
					);
				if (this.sectionProperties.clickScrollHorizontal)
					this.sectionProperties.quickScrollHorizontalTimer =
						setTimeout(() => {
				...
				offset =
					(point.pX -
						(scrollProps.startX +
							scrollProps.horizontalScrollSize * 0.5)) *
		...
		private getLocalYOnVerticalScrollBar(
			point: lool.SimplePoint,
		): number {
			return (
				point.pY -
				app.activeDocument.activeView.scrollProperties.startY
			);
		}

		private getLocalXOnHorizontalScrollBar(
			point: lool.SimplePoint,
		): number {
			return (
				point.pX -
				app.activeDocument.activeView.scrollProperties.startX
			);
		}
		...
		public onMouseDown(point: lool.SimplePoint, e: MouseEvent): void {
			...
			if (app.activeDocument.activeView.viewedRectangle.pX1 >= 0) {
				if (
					point.pY >=
					this.size[1] - scrollProps.usableThickness
				) {
					if (
						(!mirrorX &&
							point.pX >= scrollProps.xOffset &&
	...
	private animateScroll(delta: [number, number]): void {
		...
		for (let i = 0; i < 2; ++i) {
			...
			// We will keep this until we remove leaflet.
				...
				tempPos = [
					Math.round(tempPos[0] * app.pixelsToTwips),
					Math.round(tempPos[1] * app.pixelsToTwips),
				];
				this.onScrollVelocity({ vx: 0, vy: 0 }); // Cancel auto scrolling.
		...
		public onClick(point: lool.SimplePoint, e: MouseEvent): void {
			if (
				this.isAnimating &&
				this.sectionProperties.animatingWheelScrollVertical
			)
				this.containerObject.stopAnimating();
		}
		...
		private animateScroll(delta: [number, number]): void {
			...
			for (let i = 0; i < 2; ++i) {
				...
				if (
					sign !==
					this.sectionProperties.scrollAnimationDirection[i]
				) {
					// Stop animation on scroll change direction
					this.sectionProperties.scrollAnimationVelocity[i] = 0;
					this.sectionProperties.scrollAnimationAcc[i] = 0;
				...
				this.sectionProperties.scrollAnimationDelta[i] =
					lineHeight *
					ScrollSection.scrollWheelDelta *
					sign *
					app.dpiScale;
			}
		...
		public onMouseWheel(
			...
			if (Math.abs(delta[1]) > Math.abs(delta[0])) {
				if (e.shiftKey) hscroll = delta[1];
				else vscroll = delta[1];
			} else hscroll = delta[0];
			...
			if (shouldAnimate) {
				...
				if (e.deltaMode !== WheelEvent.DOM_DELTA_PIXEL) {
					...
					if (
						Math.abs((e as any).wheelDeltaY) <= 32 &&
						Math.abs((e as any).wheelDeltaX) <= 32
					)
						shouldAnimate = false;
				...
				if (e.deltaX !== 0 && e.deltaY !== 0) {
					...
					const hasFractionalComponent =
						e.deltaX % 1 !== 0 || e.deltaY % 1 !== 0;
					const deltaMaybeDiscrete =
						Math.abs((e as any).wheelDelta) % 60 === 0;

					if (
						hasFractionalComponent &&
						(!nowIsAccurate || !deltaMaybeDiscrete)
					) {
			...
			if (shouldAnimate) this.animateScroll([hscroll, vscroll]);
			else {
				this.sectionProperties.animatingScroll = false;
