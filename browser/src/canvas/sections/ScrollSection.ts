...
namespace lool {
	export class ScrollSection extends app.definitions.canvasSectionObject {
		...
		public onInitialize(): void {
			...
			this.sectionProperties.usableThickness =
				20 * app.roundedDpiScale;
			...
			this.sectionProperties.horizontalScrollRightOffset =
				this.sectionProperties.usableThickness * 2; // To prevent overlapping of the scroll bars.
		...
		public onScrollBy(e: any): void {
			if (this.map._docLayer._docType !== 'spreadsheet') {
				...
				// Some early exits.
				if (e.y < 0 && docTopLef[1] === 0)
					// Don't scroll to negative values.
					return;

				if (e.x < 0 && docTopLef[0] === 0) return;
				...
				this.autoScrollTimer = setInterval(
					L.bind(function () {
						...
						if (
							...
							var docTopLeft =
								app.sectionContainer.getDocumentTopLeft();
							tempPos = [
								tempPos[0] + docTopLeft[0],
								tempPos[1] + docTopLeft[1],
							];
							tempPos = [
								Math.round(
		...
		private getVerticalScrollLength(): number {
			...
			this.sectionProperties.yOffset =
				this.containerObject.getDocumentAnchorSection().myTopLeft[1];

			if (this.map._docLayer._docType !== 'spreadsheet') {
				return result;
			} else {
				var splitPanesContext: any =
					this.map.getSplitPanesContext();
		...
		private calculateVerticalScrollSize(scrollLength: number): number {
			var scrollSize = Math.round(
				(scrollLength * scrollLength) / app.view.size.pixels[1],
			);
			return Math.round(scrollSize);
		}

		private calculateYMinMax() {
			var diff: number = Math.round(
				app.view.size.pixels[1] -
		...
		public getVerticalScrollProperties(): any {
			...
			result.scrollSize = this.calculateVerticalScrollSize(
				result.scrollLength,
			...
			result.startY = Math.round(
				this.documentTopLeft[1] / result.ratio +
					this.sectionProperties.yOffset,
			);
		...
		private getHorizontalScrollLength(): number {
			...
			this.sectionProperties.xOffset =
				this.containerObject.getDocumentAnchorSection().myTopLeft[0];
		...
		private calculateHorizontalScrollSize(scrollLength: number): number {
			var scrollSize = Math.round(
				(scrollLength * scrollLength) / app.view.size.pixels[0],
			);
			return scrollSize;
		}
			...
			if (diff >= 0) {
				...
				if (app.view.size.pixels[0] > 0) {
					if (
						...
						this.sectionProperties.drawHorizontalScrollBar =
							false;
				}
			}
		}
		...
		public getHorizontalScrollProperties(): any {
			...
			result.scrollSize = this.calculateHorizontalScrollSize(
				result.scrollLength,
			...
			result.startX = Math.round(
				this.documentTopLeft[0] / result.ratio +
					this.sectionProperties.xOffset,
			);
		...
		private drawVerticalScrollBar(): void {
			...
			var startX = this.isRTL()
				? this.sectionProperties.edgeOffset
				: this.size[0] -
					this.sectionProperties.scrollBarThickness -
					this.sectionProperties.edgeOffset;
			...
			if (this.sectionProperties.drawScrollBarRailway) {
				...
				this.context.fillRect(
					startX,
					this.sectionProperties.yMin +
						this.sectionProperties.yOffset,
					this.sectionProperties.scrollBarRailwayThickness,
					this.sectionProperties.yMax -
						this.sectionProperties.yMin -
						this.sectionProperties.yOffset,
				);
			}
		...
		private drawHorizontalScrollBar(): void {
			...
			if (this.sectionProperties.drawScrollBarRailway) {
				...
				this.context.fillStyle =
					this.sectionProperties.scrollBarRailwayColor;
				this.context.fillRect(
					this.sectionProperties.xMin +
						this.sectionProperties.xOffset,
					startY,
					this.sectionProperties.xMax -
						this.sectionProperties.xMin -
						this.sectionProperties.xOffset,
					this.sectionProperties.scrollBarRailwayThickness,
				);
		...
		private isMouseOnScrollBar(point: Array<number>): void {
			...
			if (this.documentTopLeft[0] >= 0) {
				if (
					point[1] >=
					this.size[1] - this.sectionProperties.usableThickness
				) {
					if (
						(!mirrorX &&
							point[0] <=
						...
						(mirrorX &&
							point[0] >=
								this.sectionProperties
									.horizontalScrollRightOffset &&
							point[0] >= this.sectionProperties.xOffset)
					) {
		...
		public scrollVerticalWithOffset(offset: number): void {
			...
			if (offset > 0) {
				if (
					this.documentTopLeft[1] + offset >
					this.sectionProperties.yMax
				)
					offset =
						this.sectionProperties.yMax -
						this.documentTopLeft[1];
				if (offset <= 0) go = false;
			} else {
				if (
					this.documentTopLeft[1] + offset <
					this.sectionProperties.yMin
		...
		private isMousePointerSyncedWithVerticalScrollBar(
			...
			if (this.sectionProperties.pointerSyncWithVerticalScrollBar) {
				...
				if (
					...
					pointerIsSyncWithScrollBar =
						scrollProps.startY + spacer < position[1];
				} else {
					pointerIsSyncWithScrollBar =
						scrollProps.startY + spacer > position[1];
				}
		...
		private isMousePointerSyncedWithHorizontalScrollBar(
			...
			if (this.sectionProperties.pointerSyncWithHorizontalScrollBar) {
				pointerIsSyncWithScrollBar =
					position[0] > startX && position[0] < endX;
				pointerIsSyncWithScrollBar =
					pointerIsSyncWithScrollBar ||
					(this.isMouseInsideDocumentAnchor(position) &&
						spacer === 0);
	...
	*/
		private quickScrollVertical(
			...
			if (this.stepByStepScrolling) {
				var sign =
					point[1] - (props.startY + props.scrollSize) > 0
						? 1
						: point[1] - props.startY < 0
							? -1
							: 0;
	...
	*/
		private quickScrollHorizontal(
			...
			const docWidth: number = this.map
				...
				.getSize().x;
			const startX = this.isRTL()
				? docWidth - props.startX - sizeX
				: props.startX;
			var midX = startX + sizeX * 0.5;

			if (this.stepByStepScrolling) {
				var sign =
					point[0] - (startX + sizeX) > 0
						? 1
						: point[0] - startX < 0
							? -1
							: 0;
		...
		private performVerticalScroll(delta: number): void {
			...
			if (!this.sectionProperties.drawVerticalScrollBar) {
				if (this.isAnimating) {
					...
					this.sectionProperties.animatingVerticalScrollBar =
						true;
				} else this.fadeOutVerticalScrollBar();
			}
		}
		...
		private performHorizontalScroll(delta: number): void {
			...
			if (!this.sectionProperties.drawHorizontalScrollBar) {
				if (this.isAnimating) {
					...
					this.sectionProperties.animatingHorizontalScrollBar =
						true;
				} else this.fadeOutHorizontalScrollBar();
			}
		}
		...
		public onMouseWheel(
			...
			if (Math.abs(delta[1]) > Math.abs(delta[0])) {
				if (!e.shiftKey) this.performVerticalScroll(delta[1]);
				else this.performHorizontalScroll(delta[1]);
			} else {
