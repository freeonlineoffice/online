...
class OtherViewGraphicSelectionSection extends CanvasSectionObject {
	...
	constructor(
		...
		this.name =
			OtherViewGraphicSelectionSection.sectionNamePrefix + viewId;
	...
	public static addOrUpdateGraphicSelectionIndicator(
		viewId: number,
		...
		if (app.sectionContainer.doesSectionExist(sectionName)) {
			...
			section = new OtherViewGraphicSelectionSection(
				viewId,
				rectangle,
				part,
				mode,
	...
	public static removeView(viewId: number) {
		...
		if (app.sectionContainer.doesSectionExist(sectionName)) {
			...
			OtherViewGraphicSelectionSection.sectionPointers.splice(
				OtherViewGraphicSelectionSection.sectionPointers.indexOf(
					section,
				),
				1,
			);
	...
	public static updateVisibilities() {
		for (
			let i = 0;
			i < OtherViewGraphicSelectionSection.sectionPointers.length;
			i++
		) {
			const section =
				OtherViewGraphicSelectionSection.sectionPointers[i];
			section.setShowSection(section.checkMyVisibility());
		}
		app.sectionContainer.requestReDraw();
	}
...
app.definitions.otherViewGraphicSelectionSection =
