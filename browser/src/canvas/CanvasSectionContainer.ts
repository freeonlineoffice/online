...
class CanvasSectionContainer {
	...
	*/
	...
	private propagateOnClick(
		...
		var propagate: boolean = true;
		var windowPosition: Array<number> = position
			? [
	...
	*/
	...
	public doesSectionIncludePoint(
		section: any,
		point: Array<number>,
	): boolean {
		...
		for (var i: number = 0; i < this.sections.length - 1; i++) {
			for (var j = i + 1; j < this.sections.length; j++) {
				if (
					...
					(this.sections[i].zIndex === this.sections[j].zIndex &&
						this.sections[i].processingOrder >
							this.sections[j].processingOrder)
				) {
	...
	public applyDrawingOrders() {
		...
		for (var i: number = 0; i < this.sections.length - 1; i++) {
			...
			for (
				...
				this.sections[j].zIndex === zIndex;
				j++
			) {
				if (
					this.sections[i].drawingOrder >
					this.sections[j].drawingOrder
				) {
	...
	private drawSections(
		...
		if (tileSubset) {
			const tileSection: lool.TilesSection = this.getSectionWithName(
				L.CSections.Tiles.name,
			) as any as lool.TilesSection;
			if (
				tileSection &&
				this.shouldDrawSection(
					tileSection as any as CanvasSectionObject,
				)
			) {
				subsetBounds = tileSection.getSubsetBounds(
					this.context,
					tileSubset,
				);
			}
	...
	addSectionFunctions(section: CanvasSectionObject) {
		...
		section.setPosition = function (x: number, y: number) {
			...
			y = Math.round(y);
			let sectionXcoord =
				x - this.containerObject.getDocumentTopLeft()[0];
			if (this.isCalcRTL()) {
	...
	private animate(timeStamp: number) {
		...
		if (this.continueAnimating) {
			...
			for (
				var i: number = 0;
				i < this.stoppingFunctionList.length;
				i++
			) {
		...
		*/
		...
		if (!this.getAnimatingSectionName()) {
			...
			if (options.stoppingEvents) {
				for (
					var i: number = 0;
					i < options.stoppingEvents.length;
					i++
				) {
					this.stoppingEventTypes.push(
						options.stoppingEvents[i],
					);
					this.stoppingFunctionList.push(
						this.createStoppingFunction(),
					);
