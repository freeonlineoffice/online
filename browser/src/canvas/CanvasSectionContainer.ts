...
class CanvasSectionContainer {
	...
	*/
	...
	private paintOnResumeOrEnable() {
		...
		var scrollSection = <any>(
			this.getSectionWithName(L.CSections.Scroll.name)
		);
	...
	public targetBoundSectionListContains(sectionName: string): boolean {
		if (!this.targetSection) return false;
			...
			var section: CanvasSectionObject = this.getSectionWithName(
				this.targetSection,
			);
			if (section && section.boundsList) {
				for (
					var i: number = 0;
					i < section.boundsList.length;
					i++
				) {
	...
	private updateBoundSectionList(
		section: CanvasSectionObject,
		sectionList: Array<CanvasSectionObject> = null,
		...
		if (section.boundToSection) {
			var tempSection = this.getSectionWithName(
	...
	private orderBoundsList(section: CanvasSectionObject) {
		...
		for (var i: number = 0; i < section.boundsList.length - 1; i++) {
			for (var j = i + 1; j < section.boundsList.length; j++) {
				if (
					section.boundsList[i].zIndex >
						section.boundsList[j].zIndex ||
					(section.boundsList[i].zIndex ===
						section.boundsList[j].zIndex &&
		...
		for (var j: number = 0; j < this.windowSectionList.length; j++) {
			...
			if (windowSection.interactable)
				windowSection.onCursorPositionChanged(
					app.file.textCursor.rectangle.clone() as lool.SimpleRectangle,
				);
	...
	private propagateOnClick(
		...
		var propagate: boolean = true;
		var windowPosition: lool.SimplePoint = position
			? lool.SimplePoint.fromCorePixels([
					position[0] + section.myTopLeft[0],
					position[1] + section.myTopLeft[1],
				])
			: null;
		for (var j: number = 0; j < this.windowSectionList.length; j++) {
			var windowSection = this.windowSectionList[j];
		...
		if (propagate) {
			for (
				var i: number = section.boundsList.length - 1;
				i > -1;
				i--
			) {
	...
	*/
		...
		if (!this.draggingSomething) {
			...
			if (
				...
				this.positionOnClick =
					this.convertPositionToCanvasLocale(e);
				var s1 = this.findSectionContainingPoint(
					this.positionOnMouseDown,
				);
				var s2 = this.findSectionContainingPoint(
					this.positionOnMouseUp,
				);
				if (s1 && s2 && s1 == s2) {
					// Allow click event if only mouse was above same section while clicking.
					var section: CanvasSectionObject =
						this.findSectionContainingPoint(
							this.positionOnClick,
						);
					if (section) {
		...
		if (!this.isLongPressActive()) {
			if (!this.touchEventInProgress) {
				...
				if (
					...
					var dragDistance = [
						this.mousePosition[0] -
							this.positionOnMouseDown[0],
						this.mousePosition[1] -
							this.positionOnMouseDown[1],
					];
					if (
						Math.abs(dragDistance[0]) >=
							this.draggingTolerance ||
						Math.abs(dragDistance[1]) >=
							this.draggingTolerance
					) {
				...
				if (this.draggingSomething) {
					...
					section = this.getSectionWithName(
						this.sectionOnMouseDown,
					);
				} else {
					section = this.findSectionContainingPoint(
						this.mousePosition,
					);
				}
				...
				if (section && section.boundsList) {
					if (section.name !== this.sectionUnderMouse) {
						if (this.sectionUnderMouse !== null) {
							var previousSection: CanvasSectionObject =
	...
	public onMouseDown(e: MouseEvent) {
		...
		if (e.button === 0 && !this.touchEventInProgress) {
			...
			var section: CanvasSectionObject =
				this.findSectionContainingPoint(this.positionOnMouseDown);
			if (section) {
	...
	public onMouseWheel(e: WheelEvent) {
		...
		var section: CanvasSectionObject =
			this.findSectionContainingPoint(point);
		if (section)
		...
		if (!this.multiTouch) {
			...
			if (section) {
				...
				if (!this.touchCenter) {
					...
					this.touchCenter = this.convertPointToCanvasLocale(
						this.touchCenter,
					);
				}
	...
	private findSectionContainingPoint(
		point: Array<number>,
		interactable = true,
	): any {
		for (var i: number = this.sections.length - 1; i > -1; i--) {
			...
			if (
				...
				(!this.sections[i].documentObject ||
					this.sections[i].isVisible) &&
	...
	public doesSectionIncludePoint(section: any, point: number[]): boolean {
	...
	private hitLeft(section: CanvasSectionObject): number {
		...
		for (var i: number = 0; i < this.sections.length; i++) {
			if (
				...
				if (
					currentLeft > maxX &&
					currentLeft < section.myTopLeft[0]
				) {
					if (
						this.doSectionsIntersectOnYAxis(
							this.sections[i],
							section,
						)
					) {
	...
	private hitRight(section: CanvasSectionObject): number {
		...
		for (var i: number = 0; i < this.sections.length; i++) {
			if (
				...
				if (
					currentRight < minX &&
					currentRight > section.myTopLeft[0]
				) {
					if (
						this.doSectionsIntersectOnYAxis(
							this.sections[i],
	...
	private hitTop(section: CanvasSectionObject): number {
		...
		for (var i: number = 0; i < this.sections.length; i++) {
			if (
				...
				if (
					currentTop > maxY &&
					currentTop < section.myTopLeft[1]
				) {
					if (
						this.doSectionsIntersectOnXAxis(
							this.sections[i],
							section,
						)
					) {
	...
	private hitBottom(section: CanvasSectionObject): number {
		...
		for (var i: number = 0; i < this.sections.length; i++) {
			if (
				...
				if (
					currentBottom < minY &&
					currentBottom > section.myTopLeft[1]
				) {
					if (
						this.doSectionsIntersectOnXAxis(
							this.sections[i],
	...
	public createUpdateSingleDivElement(section: CanvasSectionObject) {
		...
		if (
			...
			element.style.left =
				String(
					bcr.left +
						Math.round(section.myTopLeft[0] / app.dpiScale),
				) + 'px';
			element.style.top =
				String(
					bcr.top +
						Math.round(section.myTopLeft[1] / app.dpiScale),
				) + 'px';
			element.style.width =
	...
	public reNewAllSections(redraw: boolean = true) {
		...
		if (redraw && this.drawingAllowed()) this.requestReDraw();
		else if (
			window.L &&
			window.L.Browser.safari &&
		...
		if (
			...
			if (index === 0)
				return anchor === 'top'
					? section.position[1]
					: this.height -
							(section.position[1] + section.size[1]);
			else
				return anchor === 'left'
					? section.position[0]
					: this.width - (section.position[0] + section.size[0]);
		} else {
			...
			if (section.anchor[index].length % 2 === 0) {
				...
				for (var i: number = 0; i < count - 1; i++) {
					targetSection = this.getSectionWithName(
	...
	private locateSections() {
		...
		for (var i: number = 0; i < this.sections.length; i++) {
			...
			if (section.documentObject === true) {
				...
				var parentSection = this.getSectionWithName(
					section.boundToSection,
				);
				if (parentSection) {
	...
	private orderSections() {
		...
		function compareSections(
			a: CanvasSectionObject,
			b: CanvasSectionObject,
		) {
	...
	public applyDrawingOrders() {
		...
		function compareSections(
			...
			if (drawingOrderDiff != 0) return drawingOrderDiff;
			return a.processingOrder - b.processingOrder;
		}
		this.sections.sort(compareSections);
	}
			...
			if (this.shouldDrawSection(this.sections[i])) {
				...
				if (this.sections[i].backgroundColor) {
					...
					this.context.fillStyle =
						this.sections[i].backgroundColor;
				...
				if (this.sections[i].borderColor) {
					...
					this.context.lineWidth =
						this.sections[i].getLineWidth();
					this.context.strokeStyle =
						this.sections[i].borderColor;
	...
	private animate(timeStamp: number) {
		...
		if (this.continueAnimating) {
			if (section)
				section.onAnimate(this.frameCount, this.elapsedTime);
			this.frameCount++;
	...
	public startAnimating(sectionName: string, options: any): boolean {
		...
		*/
		...
		if (!this.getAnimatingSectionName()) {
			...
			if (options.defer)
				requestAnimationFrame(this.animate.bind(this));
			else this.animate(document.timeline.currentTime as number);
			return true;
		} else {
			return false;
		}
