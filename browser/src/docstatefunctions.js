...
window.addEventListener('load', function () {
	app.calc.cellCursorRectangle = new lool.SimpleRectangle(0, 0, 0, 0);
	app.calc.cellAddress = new lool.SimplePoint(0, 0);
	app.calc.splitCoordinate = new lool.SimplePoint(0, 0);
	app.file.textCursor.rectangle = new lool.SimpleRectangle(0, 0, 0, 0);
	app.tile.size = new lool.SimplePoint(0, 0);
...
app.isXVisibleInTheDisplayedArea = function (twipsX) {
	if (app.map._docLayer._splitPanesContext) {
		...
		return app.activeDocument.activeView.viewedRectangle.containsX(
			twipsX,
		);
	}
};
...
app.isYVisibleInTheDisplayedArea = function (twipsY) {
	if (app.map._docLayer._splitPanesContext) {
		...
		return app.activeDocument.activeView.viewedRectangle.containsY(
			twipsY,
		);
	}
};
...
app.showAsyncDownloadError = function (response, initialMsg) {
	...
	reader.onload = function () {
		if (reader.result === 'wrong server') {
			initialMsg += _(
				', cluster configuration error: mis-matching serverid',
			);
...
app.calc.isAnyPartHidden = function () {
	...
	for (let i = 0; i < app.map._docLayer._lastStatusJSON.parts.length; i++) {
		if (app.map._docLayer._lastStatusJSON.parts[i].visible === 0)
			return true;
	}
	return false;
};
