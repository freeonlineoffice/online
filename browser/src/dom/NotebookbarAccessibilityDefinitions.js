...
var NotebookbarAccessibilityDefinitions = function () {
		...
		if (Array.isArray(rawList)) {
			for (var i = 0; i < rawList.length; i++) {
				if (rawList[i].accessibility) {
					var combination =
						language && rawList[i].accessibility[language]
							? rawList[i].accessibility[language]
							: rawList[i].accessibility.combination;
					var id = this.cleanMenuName(rawList[i].id);

					var overflow = document.querySelector(
						'#' + id + '.ui-overflow-group',
					);
					...
					if (overflow) {
						// overflow button
						if (
							typeof overflow.isCollapsed === 'function' &&
							overflow.isCollapsed()
						) {
							list.push({
								id: id,
								focusBack:
									rawList[i].accessibility.focusBack,
								combination: combination,
							});
						} else if (rawList[i].children) {
							this.getContentListRecursive(
								rawList[i].children,
								list,
								language,
							);
							if (
								rawList[i].more &&
								rawList[i].more.accessibility
							) {
								const moreAccessibility =
									rawList[i].more.accessibility;
								combination =
									language &&
									moreAccessibility[language]
										? moreAccessibility[language]
										: moreAccessibility.combination;
								list.push({
									id: id + '-more-button',
									focusBack:
										moreAccessibility.focusBack,
									combination: combination,
								});
							}
						}
					...
					rawList[i].children &&
					Array.isArray(rawList[i].children) &&
					rawList[i].children.length > 0
				) {
					this.getContentListRecursive(
						rawList[i].children,
						list,
			...
			rawList.children &&
			Array.isArray(rawList.children) &&
			rawList.children.length > 0
		) {
	...
	this.getTabsAndContents = function () {
		...
		while (
			rawDefinitions.children &&
			Array.isArray(rawDefinitions.children) &&
			rawDefinitions.children[0] &&
			...
			else rawDefinitions = rawDefinitions.children[0];
		}

		if (!contextContainer || !Array.isArray(tabs)) return;
		else {
			for (var i = 0; i < tabs.length; i++) {
				...
				for (var j = 0; j < contextContainer.children.length; j++) {
					if (
						contextContainer.children[j].children[0].id ===
						tabName + '-container'
					) {
						tabs[i].rawContentList =
							contextContainer.children[
								j
							].children[0].children;
						break;
					}
		...
		for (i = 0; i < tabs.length; i++) {
			if (
				tabs[i].accessibility &&
				tabs[i].accessibility.focusBack !== undefined
			) {
				defs[tabs[i].id] = tabs[i];
				defs[tabs[i].id].focusBack =
					tabs[i].accessibility.focusBack;
				defs[tabs[i].id].combination =
					language && tabs[i].accessibility[language]
						? tabs[i].accessibility[language]
						: tabs[i].accessibility.combination;
				defs[tabs[i].id].contentList = [];
				this.getContentListRecursive(
					defs[tabs[i].id].rawContentList,
					defs[tabs[i].id].contentList,
					language,
				);
	...
	this.optionsToolButtons = function (selectedDefinitions) {
		...
		const optionsToolSectionData =
			app.map.uiManager.notebookbar.getDefaultToolItems();
		const language = this.getLanguage();

		for (
			let option = 0;
			option < optionsToolSectionData.length;
			option++
		) {
			let toolOption = optionsToolSectionData[option];
			toolOption.id =
				(toolOption.id == null
					? toolOption.command.replace('.uno:', '')
					: toolOption.id) + '-button';
			selectedDefinitions[toolOption.id] = {
				focusBack: toolOption.accessibility.focusBack,
				combination:
					language && toolOption.accessibility[language]
