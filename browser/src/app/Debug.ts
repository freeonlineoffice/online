...
class DebugManager {
		...
		*/
		...
		this._addDebugTool({
			...
			onRemove: function () {
				...
				if (self._eventDelayTimeout)
					clearTimeout(self._eventDelayTimeout);
		...
		this._addDebugTool({
			...
			category: 'Functionality',
			startsOn: !!window.loolParams.get('randomUser'),
			onAdd: function () {
	...
	private _automatedUserTypeCellFormula(phase: number): number {
		...
		switch (phase) {
			case 0:
				{
					window.app.console.log(
						'Automated User: Click in center',
					);
					const pos = this._docLayer._latLngToTwips(
						this._map.getCenter(),
					);
					this._docLayer._postMouseEvent(
						'buttondown',
			...
			case 2:
				{
					window.app.console.log(
						'Automated User: Click formula bar',
					);
	...
	private _automatedUserResizeRowsColumns(phase: number): number {
		...
		switch (phase) {
			...
			case 1:
				{
					window.app.console.log(
						'Automated User: Resize row larger',
					);
					// Not necessary here, but nice to highlight the row being changed
					app.sectionContainer
						.getSectionWithName('row header')
			...
			case 2:
				{
					window.app.console.log(
						'Automated User: Resize row auto',
					);
	...
	public addTileInvalidationMessage(message: string): void {
		...
		this._tileInvalidationMessages.set(
			this._tileInvalidationId - 1,
			message,
		);
		...
		for (
			...
			if (i >= 0 && this._tileInvalidationMessages.has(i)) {
				messages +=
					...
					i +
					': ' +
					this._tileInvalidationMessages.get(i) +
					'\n';
			}
		}
	...
	public timeEventDelay(): void {
		...
		this._eventDelayTimeout = setTimeout(() => {
			...
			this.reportEventDelay(
				performance.now() - this._eventDelayWatchStart,
			);
		}, 0);
	}
	...
	public reportEventDelay(delayMs: number): void {
		...
		if (
			...
			currentTime - this._lastEventDelayTime >
				slow_time_display_timeout
		) {
