...
class Tile {
	...
	/// Demand a whole tile back to the keyframe from loolwsd.
...
class TileManager {
	...
	private static garbageCollect(discardAll = false) {
		...
		for (const tile of this.tiles.values()) {
			...
			if (tile.distanceFromView !== 0) {
				totalSize += tile.rawDeltas.reduce(
					(a, c) => a + c.length,
					0,
				);
				tileCount++;
	...
	private static setBitmapOnTile(tile: Tile, bitmap: ImageBitmap) {
		...
		if (tile.image) {
			...
			if (assertChecks)
				window.app.console.assert(
					!!this.tileBitmapList.find((i) => i == tile),
				);
			tile.image.close();
			tile.image = bitmap;
			return;
	...
	private static sortTileBitmapList() {
		...
		this.tileBitmapList.sort(
			(a, b) => a.distanceFromView - b.distanceFromView,
		);
	}
	...
	private static getVisibleRanges(): Array<lool.Bounds> {
		var zoom = Math.round(app.map.getZoom());
		var pixelBounds = app.map.getPixelBoundsCore(
			app.map.getCenter(),
			zoom,
		);
		return app.map._docLayer._splitPanesContext
			? app.map._docLayer._splitPanesContext.getPxBoundList(
					pixelBounds,
				)
			: [pixelBounds];
	}
	...
	private static tileReady(
		coords: TileCoordData,
		visibleRanges: Array<lool.Bounds>,
	) {
		var key = coords.key();
	...
	private static initPreFetchAdjacentTiles() {
		...
		this._adjacentTilePreFetcher = setTimeout(
			function () {
				...
				var queue = this.getMissingTiles(
					this._pixelBounds,
					this._zoom,
				);
	...
	private static sendTileCombineRequest(
		...
		for (var pmKey in partMode) {
			...
			if (added.size) app.socket.sendMessage(msg, '');
				...
				window.app.console.log(
					'Skipped empty (too fast) tilecombine',
				);
		}
	}
	...
	private static pxBoundsToTileRanges(bounds: any) {
		...
		var boundList =
			app.map._docLayer._splitPanesContext.getPxBoundList(bounds);
	...
	private static updateTileDistance(
		...
		for (let i = 1; i < visibleRanges.length; ++i) {
			...
			if (distance < tile.distanceFromView)
				tile.distanceFromView = distance;
		}
	}
		...
		while (coordsQueue.length > 0) {
			...
			while (i < coordsQueue.length) {
				...
				if (!rowLocked) {
					if (
						current.y === bound.y &&
						current.x === bound.x + this.tileSize
					) {
	...
	public static overlapInvalidatedRectangleWithView(
		...
		mode: number,
		wireId: number,
		invalidatedRectangle: lool.SimpleRectangle,
		textMsg: string,
	) {
		...
		if (
			...
			if (needsNewTiles && mode === app.map._docLayer._selectedMode)
				app.map._docLayer._debug.addTileInvalidationMessage(
					textMsg,
				);
		}
	}
	...
	public static preFetchTiles(forceBorderCalc: boolean) {
		...
		var validTileRange = new lool.Bounds(
			...
			new lool.Point(
				...
				Math.floor(
					(app.activeDocument.fileSize.y - 1) / app.tile.size.y,
				),
			),
		);
	...
	public static onTileMsg(textMsg: string, img: any) {
		...
		if (img && !img.isKeyframe && !tile.hasKeyframe()) {
			window.app.console.debug(
				'Unusual: Delta sent - but we have no keyframe for ' +
					coords.key(),
			);
			// force keyframe
			tile.forceKeyframe();
		...
		if (hasContent) {
			...
			const rawDelta = new lool.RawDelta(
				img.rawData,
				++tile.deltaId,
				img.isKeyframe,
			);
	...
	public static update(center: any = null, zoom: number = null) {
		...
		if (
			app.map._docLayer.isCalc() &&
			!app.map._docLayer._gotFirstCellCursor
		)
			return;
		...
		switch (e.data.message) {
			case 'endTransaction':
				...
				for (const x of e.data.deltas) {
					...
					let rawDeltas: any[] = [];
					const firstDelta = tile.rawDeltas.findIndex(
						(d) => d.id === x.ids[0],
					);
					const lastDelta = tile.rawDeltas.findIndex(
						(d) => d.id === x.ids[1],
					);
					if (firstDelta !== -1 && lastDelta !== -1)
						rawDeltas = tile.rawDeltas.slice(
							firstDelta,
							lastDelta + 1,
						);
					else
					...
					if (!x.bitmap) {
						...
						x.deltas = (window as any).fzstd.decompress(
							x.rawDelta,
						);
						if (x.isKeyframe) {
	...
	*/
	...
	public static getVisibleCoordList(
		rectangle: lool.SimpleRectangle = app.activeDocument.activeView
			.viewedRectangle,
	) {
	...
	public static updateFileBasedView(
		...
		var partWidthPixels = Math.round(
			app.map._docLayer._partWidthTwips * ratio,
		);
		...
		var intersectionAreaRectangle = app.LOUtil._getIntersectionRectangle(
			...
			[
				...
				0,
				partWidthPixels,
				partHeightPixels * app.map._docLayer._parts,
			],
		);
		...
		if (intersectionAreaRectangle) {
			var minLocalX =
				Math.floor(
					intersectionAreaRectangle[0] / app.tile.size.pX,
				) * app.tile.size.pX;
			var maxLocalX =
				Math.floor(
				...
				for (
					...
					for (
						...
						if (
							...
							queue.push(
								new TileCoordData(j, k, zoom, i, mode),
							);
				}
			}
	...
	public static invalidateTile(key: string, wireId: number) {
		...
		if (!tile) {
			window.app.console.warn(
				'invalidateTile called with invalid key',
				key,
			);
			return;
		}
