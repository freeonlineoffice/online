...
class TileManager {
	...
	public static updateOverlayMessages() {
		...
		app.map._debug.setOverlayMessage(
			...
			'Tiles: ' +
				...
				' = ' +
				String(this.canvasCache.size + n_canvases).padStart(
					3,
					' ',
				) +
				' current ' +
	...
	private static garbageCollect(discardAll = false) {
		...
		if (this.debugDeltas)
			window.app.console.log(
				'Garbage collect! iter: ' + this.gcCounter,
			);
	...
	private static initPreFetchAdjacentTiles() {
		...
		this._adjacentTilePreFetcher = setTimeout(
			function () {
				...
				const direction =
					app.sectionContainer.getLastPanDirection();
	...
	private static updateTileDistance(
		...
		const tileBounds = new L.Bounds(
			...
			new L.Point(
				tile.coords.x + this.tileSize,
				tile.coords.y + this.tileSize,
			),
		);
		...
		tile.distanceFromView = tileCenter.distanceTo(
			visibleRanges[0].getCenter(),
		);
		for (let i = 1; i < visibleRanges.length; ++i) {
			const distance = tileCenter.distanceTo(
				visibleRanges[i].getCenter(),
			);
			...
			for (var j = tileRange.min.y; j <= tileRange.max.y; ++j) {
				for (var i = tileRange.min.x; i <= tileRange.max.x; ++i) {
					...
					if (!tile || tile.needsFetch()) queue.push(coords);
						...
						didRehydrate =
							this.makeTileCurrent(tile) || didRehydrate;
				}
			}
		}
		this.endTransaction(
	...
	private static addTiles(
		...
		for (let i = 0; i < coordsQueue.length; i++) {
			...
			if (!tile) {
				...
				if (!currentBounds) {
					const zoom = Math.round(app.map.getZoom());
					currentBounds = app.map.getPixelBoundsCore(
						app.map.getCenter(),
						zoom,
					);
				}
	...
	public static updateFileBasedView(
		...
		if (intersectionAreaRectangle) {
			...
			for (i = 0; i < queue.length; i++) {
				...
				if (tempTile)
					redraw = redraw || this.makeTileCurrent(tempTile);
			}
			this.endTransaction(
