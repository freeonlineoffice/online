...
class TilesPreFetcher {
	...
	private static computeBorders() {
		...
		const tileRanges = this._docLayer._pxBoundsToTileRanges(
			this._pixelBounds,
		);
	...
	public static initPreFetchAdjacentTiles() {
		...
		this._adjacentTilePreFetcher = setTimeout(
			function () {
				...
				pixelTopLeft.y =
					Math.floor(pixelTopLeft.y / tileSize) * tileSize;
				pixelTopLeft.y -= 1;
				...
				if (this._docLayer.isCalc() || queue.length === 0) {
					...
					queue = queue.concat(
						this._docLayer._getMissingTiles(
							this._pixelBounds,
							this._zoom,
						),
					);
				...
				if (queue.length !== 0)
					this._docLayer._addTiles(queue, true);
			}.bind(this),
			50 /*ms*/,
		);
	}
