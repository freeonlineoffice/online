...
class GraphicSelection {
	...
	static renderDarkOverlay() {
		var topLeft = new lool.Point(this.rectangle.pX1, this.rectangle.pY1);
		var bottomRight = new lool.Point(
			this.rectangle.pX2,
			this.rectangle.pY2,
		);
		...
		var bounds = new lool.Bounds(topLeft, bottomRight);
	...
	static extractAndSetGraphicSelection(messageJSON: any) {
		...
		if (hasExtraInfo) {
			...
			if (extraInfo.gridOffsetX || extraInfo.gridOffsetY) {
				app.map._docLayer._shapeGridOffset = new lool.SimplePoint(
		...
		this.rectangle = new lool.SimpleRectangle(
			signX * messageJSON[0],
			messageJSON[1],
			signX * messageJSON[2],
			messageJSON[3],
				...
				if (
					...
					dragInfo.dragDirection =
						app.map._docLayer._twipsToPixels(
							new lool.Point(dragDir[0], dragDir[1]),
						);
