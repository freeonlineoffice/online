...
class LayoutPageRectangle extends lool.SimpleRectangle {
	...
	private static resetViewLayout() {
		...
		for (let i = 0; i < this.layoutRectangles.length; i++) {
			...
			while (
				...
				const addition =
					this.layoutRectangles[j].pWidth + this.gapBetweenPages;
			...
			if (x < canvasSize[0]) {
				...
				for (let k = i; k < j; k++) {
					...
					currentX +=
						this.layoutRectangles[k].pWidth +
						this.gapBetweenPages;
					maxY = Math.max(
						maxY,
						this.layoutRectangles[k].pHeight,
					);
				}
				...
				lastY +=
					this.layoutRectangles[i].pHeight +
					this.gapBetweenPages;
			}
