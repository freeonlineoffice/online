...
function toPoint(x: PointConvertable | number, y?: number, round?: boolean): Point {
	return lool.Point.toPoint(x, y, round);
		...
		constructor(
			...
			if (b) {
				this.min =
					a instanceof Point
						? a.clone()
						: toPoint(<PointConvertable>a);
				const maybeMax =
					b instanceof Point ? b.clone() : toPoint(b);
		...
		public static parse(rectString: string): Bounds {
			...
			var refPoint1 = PointConstruct(
				parseInt(rectParts[0]),
				parseInt(rectParts[1]),
			);
			var offset = PointConstruct(
				parseInt(rectParts[2]),
				parseInt(rectParts[3]),
			);
	...
	public clamp(obj: Point | Bounds): Point | Bounds {
		...
		public contains(obj: Bounds | PointConvertable): boolean {
			...
			if (
				Array.isArray(obj) ||
				obj instanceof L.Point ||
				obj instanceof SimplePoint
			) {
				var point: Point = toPoint(<PointConvertable>obj);
				min = max = point;
			} else {
		...
		public equals(bounds: Bounds): boolean {
			return (
				this.min.equals(bounds.min) && this.max.equals(bounds.max)
			);
		}
		...
		public static toBounds(
			a: Bounds | PointConvertable | PointConvertable[],
			b?: PointConvertable,
		): Bounds {
