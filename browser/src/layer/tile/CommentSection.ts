...
export class Comment extends CanvasSectionObject {
	...
	public onInitialize (): void {
		...
		if (this.sectionProperties.data.trackchange) {
			this.sectionProperties.captionNode = L.DomUtil.create('div', 'lool-annotation-caption', this.sectionProperties.wrapper);
			this.sectionProperties.captionText = L.DomUtil.create('div', '', this.sectionProperties.captionNode);
		}

		this.sectionProperties.contentNode = L.DomUtil.create('div', 'lool-annotation-content lool-dont-break', this.sectionProperties.wrapper);
		this.sectionProperties.contentNode.id = 'annotation-content-area-' + this.sectionProperties.data.id;
		this.sectionProperties.nodeModify = L.DomUtil.create('div', 'lool-annotation-edit' + ' modify-annotation', this.sectionProperties.wrapper);
		this.sectionProperties.nodeModifyText = L.DomUtil.create('textarea', 'lool-annotation-textarea', this.sectionProperties.nodeModify);
		...
		this.sectionProperties.nodeReply = L.DomUtil.create('div', 'lool-annotation-edit' + ' reply-annotation', this.sectionProperties.wrapper);
		this.sectionProperties.nodeReplyText = L.DomUtil.create('textarea', 'lool-annotation-textarea', this.sectionProperties.nodeReply);
	...
	private createContainerAndWrapper (): void {
		...
		this.sectionProperties.container = L.DomUtil.create('div', 'lool-annotation' + (isRTL ? ' rtl' : ''));
		...
		if (this.sectionProperties.data.trackchange) {
			this.sectionProperties.wrapper = L.DomUtil.create('div', 'lool-annotation-redline-content-wrapper' + mobileClass, this.sectionProperties.container);
		} else {
			this.sectionProperties.wrapper = L.DomUtil.create('div', 'lool-annotation-content-wrapper' + mobileClass, this.sectionProperties.container);
		}

		this.sectionProperties.collapsed = L.DomUtil.create('div', 'lool-annotation-collapsed', this.sectionProperties.container);

		document.getElementById('document-container').appendChild(this.sectionProperties.container);
	}

	private createAuthorTable (): void {
		this.sectionProperties.author = L.DomUtil.create('table', 'lool-annotation-table', this.sectionProperties.wrapper);

		var tbody = L.DomUtil.create('tbody', '', this.sectionProperties.author);
		var rowResolved = L.DomUtil.create('tr', '', tbody);
		var tdResolved = L.DomUtil.create('td', 'lool-annotation-resolved', rowResolved);
		var pResolved = L.DomUtil.create('div', 'lool-annotation-content-resolved', tdResolved);
		...
		var tdAuthor = L.DomUtil.create('td', 'lool-annotation-author', tr);
		...
		this.sectionProperties.replyCountNode = L.DomUtil.create('div', 'lool-annotation-reply-count-collapsed', this.sectionProperties.collapsed);
		...
		this.sectionProperties.contentAuthor = L.DomUtil.create('div', 'lool-annotation-content-author', tdAuthor);
		this.sectionProperties.contentDate = L.DomUtil.create('div', 'lool-annotation-date', tdAuthor);
	}

	private createMenu (): void {
		var tdMenu = L.DomUtil.create('td', 'lool-annotation-menubar', this.sectionProperties.authorRow);
		this.sectionProperties.menu = L.DomUtil.create('div', this.sectionProperties.data.trackchange ? 'lool-annotation-menu-redline' : 'lool-annotation-menu', tdMenu);
	...
	private createTrackChangeButtons (): void {
		var tdAccept = L.DomUtil.create('td', 'lool-annotation-menubar', this.sectionProperties.authorRow);
		var acceptButton = this.sectionProperties.acceptButton = L.DomUtil.create('button', 'lool-redline-accept-button', tdAccept);

		var tdReject = L.DomUtil.create('td', 'lool-annotation-menubar', this.sectionProperties.authorRow);
		var rejectButton = this.sectionProperties.rejectButton = L.DomUtil.create('button', 'lool-redline-reject-button', tdReject);
	...
	public highlight (): void {
		if (this.sectionProperties.docLayer._docType === 'text') {
			...
			(this.containerObject.getSectionWithName(L.CSections.Scroll.name) as lool.ScrollSection).onScrollTo({x: x, y: y});
		}
		else if (this.sectionProperties.docLayer._docType === 'spreadsheet') {
