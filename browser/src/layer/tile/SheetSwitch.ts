	...
	export class SheetSwitchViewRestore {
		...
		public save(toPart: number): void {
			if (!this.currentSheetIndexReassigned) {
				this.centerOfSheet.set(
					this.docLayer._selectedPart as number,
					this.map.getCenter(),
				);
			} else {
				this.currentSheetIndexReassigned = false;
		...
		public updateOnSheetMoved(oldIndex: number, newIndex: number): void {
			...
			if (oldIndex < newIndex) {
				for (let i = oldIndex; i < newIndex; ++i) {
					...
					if (center) this.centerOfSheet.set(i, center);
					else this.centerOfSheet.delete(i);
				}
			} else {
				for (let i = oldIndex; i > newIndex; --i) {
					const center = this.centerOfSheet.get(i - 1);
			...
			else this.centerOfSheet.delete(newIndex);
		}
		...
		public updateOnSheetInsertion(index: number): void {
			...
			const currentSheetNumber: number =
				this.map.getCurrentPartNumber();
		...
		private restoreView(): void {
			...
			if (this.setPartRecvd) this.reset();
		}
		...
		public tryRestore(
			duplicateCursor: boolean,
			currentPart: number,
		): boolean {
			let shouldScrollToCursor = false;
			const attemptRestore =
				this.mayRestore && currentPart === this.restorePart;

			if (attemptRestore) {
				if (this.setPartRecvd && duplicateCursor) this.reset();
				if (!this.setPartRecvd) this.restoreView();
			}
