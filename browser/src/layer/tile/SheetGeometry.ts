...
namespace lool {
	...
	export class SheetGeometry {
		...
		constructor(
			sheetGeomJSON: SheetGeometryCoreData,
			...
			if (
				...
				console.error(
					'Incorrect constructor argument types or missing required arguments',
				);
				return;
			}
		...
		*/
			...
			const hiddenInfo = column
				? sheetGeomJSON.columns.hidden.trim()
				: sheetGeomJSON.rows.hidden.trim();

			const isFirstHidden = hiddenInfo.indexOf('1') === 0;
			const splitted = hiddenInfo.split(' ');
			const isLastHidden =
				(splitted.length % 2 === 0 && !isFirstHidden) ||
				(splitted.length % 2 === 1 && isFirstHidden);
		...
		*/
				...
				// Last rows / columns are hidden and there are more than 2 items.
		...
		public update(
			...
			if (sheetGeomJSON.columns) {
				if (!this._columns.update(sheetGeomJSON.columns)) {
					console.error(
						this._unoCommand + ': columns update failed.',
					);
					updateOK = false;
				}

				if (sheetGeomJSON.columns.hidden)
					this.maxVisibleColumnIndex = this.checkMaxIndex(
						sheetGeomJSON,
						true,
					);
				else
					this.maxVisibleColumnIndex = parseInt(
						sheetGeomJSON.maxtiledcolumn,
					);
			}
			...
			if (sheetGeomJSON.rows) {
				if (!this._rows.update(sheetGeomJSON.rows)) {
					console.error(
						this._unoCommand + ': rows update failed.',
					);
					updateOK = false;
				}

				if (sheetGeomJSON.rows.hidden)
					this.maxVisibleRowIndex = this.checkMaxIndex(
						sheetGeomJSON,
						false,
					);
				else
					this.maxVisibleRowIndex = parseInt(
						sheetGeomJSON.maxtiledrow,
					);
			}
		...
		public setTileGeometryData(
			...
			this._columns.setTileGeometryData(
				tileWidthTwips,
				tileSizePixels,
				updatePositions,
			);
			this._rows.setTileGeometryData(
				tileHeightTwips,
				tileSizePixels,
				updatePositions,
			);
		...
		public getTileTwipsAtZoom(point: Point, zoomScale: number): Point {
			...
			return new L.Point(
				this._columns.getTileTwipsAtZoom(point.x, zoomScale),
		...
		public getCorePixelsAtZoom(point: Point, zoomScale: number): Point {
			...
			return new L.Point(
				this._columns.getCorePixelsAtZoom(point.x, zoomScale),
		...
		public getCorePixelsFromZoom(point: Point, zoomScale: number): Point {
			...
			return new L.Point(
				this._columns.getCorePixelsFromZoom(point.x, zoomScale),
		...
		public getTileTwipsPointFromPrint(point: Point): Point {
			...
			return new L.Point(
				this._columns.getTileTwipsPosFromPrint(point.x),
				this._rows.getTileTwipsPosFromPrint(point.y),
			);
		}

		public convertToTileTwips(simplePoint: lool.SimplePoint): void {
			simplePoint.x = this._columns.getTileTwipsPosFromPrint(
				simplePoint.x,
			);
			simplePoint.y = this._rows.getTileTwipsPosFromPrint(
				simplePoint.y,
			);
		}

		public convertRectangleToTileTwips(
			simpleRectangle: lool.SimpleRectangle,
		): void {
			simpleRectangle.x1 = this._columns.getTileTwipsPosFromPrint(
				simpleRectangle.x1,
			);
			simpleRectangle.y1 = this._rows.getTileTwipsPosFromPrint(
				simpleRectangle.y1,
			);
		}
		...
		public getPrintTwipsPointFromTile(point: Point): Point {
			...
			return new L.Point(
				this._columns.getPrintTwipsPosFromTile(point.x),
		...
		public getTileTwipsSheetAreaFromPrint(rectangle: Bounds): Bounds {
			...
			var horizBounds = this._columns.getTileTwipsRangeFromPrint(
				topLeft.x,
				bottomRight.x,
			);
			var vertBounds = this._rows.getTileTwipsRangeFromPrint(
				topLeft.y,
				bottomRight.y,
			);
		...
		public getSize(unit: GeometryUnit): Point {
			return new L.Point(
				this._columns.getSize(unit),
				this._rows.getSize(unit),
			);
		}

		// Returns the cell rectangle in SimpleRectangle.
		public getCellSimpleRectangle(
			columnIndex: number,
			rowIndex: number,
			zoomScale: number = null,
		): lool.SimpleRectangle {
			var horizPosSize = this._columns.getElementData(
				columnIndex,
				zoomScale,
			);
		...
		*/
		...
		private _testValidity(
			...
			if (!sheetGeomJSON.commandName) {
				console.error(
					this._unoCommand +
						' response has no property named "commandName".',
				);
				return false;
			}
			...
			if (sheetGeomJSON.commandName !== this._unoCommand) {
				console.error(
					'JSON response has wrong commandName: ' +
						sheetGeomJSON.commandName +
						' expected: ' +
						this._unoCommand,
				);
				return false;
			...
			if (
				typeof sheetGeomJSON.maxtiledcolumn !== 'string' ||
				!/^\d+$/.test(sheetGeomJSON.maxtiledcolumn)
			) {
				console.error(
					'JSON is missing/unreadable maxtiledcolumn property',
				);
				return false;
			}

			if (
				typeof sheetGeomJSON.maxtiledrow !== 'string' ||
				!/^\d+$/.test(sheetGeomJSON.maxtiledrow)
			) {
				console.error(
					'JSON is missing/unreadable maxtiledrow property',
				);
				return false;
			}
			...
			if (checkCompleteness) {
				if (!sheetGeomJSON.rows || !sheetGeomJSON.columns) {
					console.error(
						this._unoCommand + ' response is incomplete.',
					);
					return false;
				}

				if (
					typeof sheetGeomJSON.rows !== 'object' ||
					typeof sheetGeomJSON.columns !== 'object'
				) {
					console.error(
						this._unoCommand +
							' response has invalid rows/columns children.',
					);
					return false;
				}
				...
				for (var idx = 0; idx < expectedFields.length; idx++) {
					...
					var encodingForCols =
						SheetGeometry.getDimensionDataField(
							sheetGeomJSON.columns,
							fieldName,
						);
					var encodingForRows =
						SheetGeometry.getDimensionDataField(
		...
		private static getDimensionDataField(
			dimData: SheetDimensionCoreData,
			fieldName: string,
		): string {
			switch (fieldName) {
				case 'sizes':
	...
	export class SheetDimension {
		...
		public setTileGeometryData(
			...
			this._coreZoomFactor =
				(this._tileSizePixels * 15.0) / this._tileSizeTwips;
			this._twipsPerCorePixel =
				this._tileSizeTwips / this._tileSizePixels;
		...
		private _updatePositions() {
			...
			var posPrintTwips = 0;
			this._visibleSizes.addCustomDataForEachSpan(
				function (
					index: number,
					...
					var sizeCorePxOne = Math.floor(
						size / this._twipsPerCorePixel,
					);
					posCorePx += sizeCorePxOne * spanLength;
					// position in core-pixel aligned twips.
					var posTileTwips = Math.floor(
						posCorePx * this._twipsPerCorePixel,
					);
		...
		public getElementData(
			...
			if (zoomScale !== undefined) {
				var startpos = 0;
				var size = 0;
				this._visibleSizes.forEachSpanInRange(
					0,
					index,
					function (spanData) {
						var count = spanData.end - spanData.start + 1;
						var sizeOneCorePx = Math.floor(
							(spanData.size * zoomScale) / 15.0,
						);
						if (index > spanData.end) {
							startpos += sizeOneCorePx * count;
						} else if (
							index >= spanData.start &&
							index <= spanData.end
						) {
							// final span
							startpos +=
		...
		public getElementDataAny(
			index: number,
			unitName: GeometryUnit,
		): DimensionPosSize {
			var span = this._visibleSizes.getSpanDataByIndex(index);
			if (span === undefined) {
				return undefined;
			...
			return this._getElementDataAnyFromSpanByIndex(
				index,
				span,
				unitName,
			);
		...
		private _getElementDataFromSpanByIndex(
			index: number,
			span: any,
		): DimensionPosSize {
			return this._getElementDataAnyFromSpanByIndex(
		...
		private _getElementDataAnyFromSpanByIndex(
			index: number,
			span: any,
			unitName: GeometryUnit,
		): DimensionPosSize {
			...
			if (unitName === 'printtwips') {
				return {
					startpos:
						span.data.posprinttwips - span.size * numSizes,
					size: span.size,
				};
			}
			...
			return {
				startpos: Math.floor(
					(span.data.poscorepx - span.data.sizecore * numSizes) *
						twipsPerCorePixel,
				),
		...
		public forEachInRange(
			...
			callback: (dimIndex: number, posSize: DimensionPosSize) => void,
		): void {
			this._visibleSizes.forEachSpanInRange(
				start,
				end,
				function (span: SpanViewData) {
					var first = Math.max(span.start, start);
					var last = Math.min(span.end, end);
					for (var index = first; index <= last; ++index) {
						callback(
							index,
							this._getElementDataFromSpanByIndex(
								index,
								span,
							),
						);
		...
		public forEachInCorePixelRange(
			startPix: number,
			endPix: number,
			callback: (startPosCorePx: number, index: number) => void,
		): void {
			this._visibleSizes.forEachSpan(function (spanData: any) {
				...
				var spanFirstCorePx =
					...
					spanData.data.sizecore *
						(spanData.end - spanData.start + 1);
				if (
					spanFirstCorePx < endPix &&
					spanData.data.poscorepx > startPix
				) {
		...
		private _getSpanAndIndexFromTileTwipsPos(pos: number): IndexAndSpan {
			...
			var span = this._visibleSizes.getSpanDataByCustomDataField(
				pos,
				'postiletwips',
			);
			result.span = span;
			if (span === undefined) {
				...
				result.span = this._visibleSizes.getSpanDataByIndex(
					result.index,
				);
				return result;
			}
			...
			var posStart =
				(span.data.poscorepx - span.data.sizecore * elementCount) *
				this._twipsPerCorePixel;
		...
		private _getSpanAndIndexFromPrintTwipsPos(pos: number): IndexAndSpan {
			...
			var elementCount = span.end - span.start + 1;
			var posStart =
				span.data.posprinttwips - span.size * elementCount;
		...
		public setViewLimits(
			startPosTileTwips: number,
			endPosTileTwips: number,
		): void {
			this._viewStartIndex = Math.max(
				0,
				this._getIndexFromTileTwipsPos(startPosTileTwips),
			);
			this._viewEndIndex = Math.min(
				this._maxIndex,
		...
		public getGroupsDataInView(): GroupData[] {
			...
			this._outlines.forEachGroupInRange(
				0,
				this._viewEndIndex,
				function (
					levelIdx: number,
					...
					groupsData.push({
						...
						endPos: (
							endElementData.startpos + endElementData.size
						).toString(),
						hidden: hidden ? '1' : '0',
					});
				}.bind(this),
		...
		public getCorePixelsFromZoom(
			posCPZ: number,
			zoomScale: number,
		): number {
			if (
				typeof posCPZ !== 'number' ||
		...
		public getTileTwipsPosFromPrint(
			posPT: number,
			zoomScale?: number,
		): number {
			...
			if (typeof zoomScale === 'number') {
				...
				this._visibleSizes.forEachSpan(function (spanData) {
					...
					var sizeSpanTT = Math.floor(
						(sizeOneCorePx * count * 15) / zoomScale,
					);
					...
					var elemCountFinalSpan = Math.floor(
						remainingPT / spanData.size,
					);
					var extra =
						remainingPT - elemCountFinalSpan * spanData.size;
					posTT +=
						Math.floor(
							(elemCountFinalSpan * sizeSpanTT) / count,
						) + extra;
					posPTInc = posPT;
				});
			...
			var elementDataTT = this._getElementDataAnyFromSpanByIndex(
				element.index,
				element.span,
				'tiletwips',
			);
			var elementDataPT = this._getElementDataAnyFromSpanByIndex(
				element.index,
				element.span,
				'printtwips',
			);
		...
		public getTileTwipsRangeFromPrint(
			posStartPT: number,
			posEndPT: number,
		): DimensionStartEndPos {
			var startElement =
				this._getSpanAndIndexFromPrintTwipsPos(posStartPT);
			var startData = this._getElementDataAnyFromSpanByIndex(
				startElement.index,
				startElement.span,
				'tiletwips',
			);
			...
			var endData = this._getElementDataAnyFromSpanByIndex(
				endElement.index,
				endElement.span,
				'tiletwips',
			);
		...
		public getSnapPos(pos: number, unit: GeometryUnit): number {
			...
			console.assert(
				unit === 'tiletwips' || unit === 'printtwips',
	...
	class SpanList {
		...
		public load(encoding: string): boolean {
			...
			var result = <ParsedSpanList>(
				parseSpanListEncoding(encoding, false /* boolean value ? */)
			);
			if (result === undefined) {
				return false;
			}
		...
		public applyZeroValues(other: BoolSpanList) {
			...
			if (
				this._spanlist[this._spanlist.length - 1].index !==
			...
			while (
				thisIdx < this._spanlist.length &&
				otherIdx < other._spanlist.length
			) {
		...
		public addCustomDataForEachSpan(
			getCustomDataCallback: (
				index: number,
				value: number,
				count: number,
			) => any,
		...
		public getSpanDataByCustomDataField(
			value: number,
			fieldName: string,
		): SpanViewData {
			if (
		...
		public forEachSpanInRange(
			start: number,
			end: number,
			callback: (span: SpanViewData) => void,
		) {
			if (
				typeof start != 'number' ||
				typeof end != 'number' ||
		...
		private _getSpanData(spanid: number): SpanViewData {
			...
			var clone: SpanViewData = {
				...
				start:
					spanid > 0 ? this._spanlist[spanid - 1].index + 1 : 0,
				end: span.index,
				size: span.value,
			};
		...
		private _searchByIndex(index: number): number {
			return binarySearch(
				...
				function directionProvider(
					...
					var spanStart = prevSpan ? prevSpan.index + 1 : 0;
					var spanEnd = curSpan.index;
					return testIndex < spanStart
						? -1
						: spanEnd < testIndex
							? 1
							: 0;
		...
		public _searchByCustomDataField(
			value: number,
			fieldName: string,
		): number {
			...
			return binarySearch(
				...
				function directionProvider(
					...
					var valueStart = prevSpan
						? prevSpan.data[fieldName]
						: 0;
					var valueEnd =
						curSpan.data[fieldName] - (nextSpan ? 1 : 0);
					if (
	...
	class BoolSpanList {
		...
		public load(encoding?: string): boolean {
			...
			var result = <ParsedBoolSpanList>(
				parseSpanListEncoding(encoding, true /* boolean value ? */)
			);
			if (result === undefined) {
				return false;
			}
		...
		public union(other: BoolSpanList) {
			...
			// Ensure both spanlists have the same total range.
			if (
				this._spanlist[this._spanlist.length - 1] !==
				other._spanlist[other._spanlist.length - 1]
			) {
				return undefined;
			}
	...
	function parseSpanListEncoding(
		encoding: string,
		booleanValue: boolean,
	): ParsedSpanList | ParsedBoolSpanList {
	...
	export class DimensionOutlines {
		...
		public load(encoding: string): boolean {
			...
			for (
				...
				for (
					var collIdx = 0;
					collIdx < collectionSplits.length - 1;
					++collIdx
				) {
		...
		public forEachGroupInRange(
			...
			var directionProvider = function (
				elementIdx: number,
				prevGroup: OutlineEntry,
				curGroup: OutlineEntry /*, nextGroup*/,
			): SearchDirection {
				var direction: SearchDirection =
					elementIdx < curGroup.start
						? -1
						: curGroup.end < elementIdx
							? 1
							: 0;
			...
			for (
				...
				for (
					var groupIdx = startGroupIdx;
					groupIdx < groupsInLevel.length;
					++groupIdx
				) {
	...
	function binarySearch(
		array: any[],
		key: any,
		directionProvider: DirectionProviderType,
		...
		if (typeof directionProvider != 'function') {
			directionProvider = function (key, prevvalue, testvalue) {
				return key === testvalue ? 0 : key < testvalue ? -1 : 1;
			};
		}
		...
		if (endDir >= 0) {
			...
			return firstMatch
				? _findFirstMatch(array, key, directionProvider, end)
				: end;
		}
		...
		while (start <= end) {
			...
			var direction = directionProvider(
				key,
				array[mid - 1],
				array[mid],
				array[mid + 1],
		...
		return start > end
			? -1
			: firstMatch
				? _findFirstMatch(array, key, directionProvider, mid)
				: mid;
	}
