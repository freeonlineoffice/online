...
L.CalcTileLayer = L.CanvasTileLayer.extend({
	...
	_onInvalidateTilesMsg: function (textMsg) {
		...
		if (
			this._debug.tileInvalidationsOn &&
			command.part === this._selectedPart
		) {
		...
		for (var key in this._tiles) {
			...
			if (
				...
				var intersectsVisible = visibleArea
					...
					: bounds.intersectsAny(visiblePaneAreas);
	...
	_restrictDocumentSize: function () {
		...
		if (!limitWidth && maxDocSize.x > this._docWidthTwips) {
			newDocWidth = Math.min(
				this._docWidthTwips + mapSizeTwips.x,
				maxDocSize.x,
			);
			extendedLimit = true;
		}
		...
		if (!limitHeight && maxDocSize.y > this._docHeightTwips) {
			newDocHeight = Math.min(
				this._docHeightTwips + mapSizeTwips.y,
				maxDocSize.y,
			);
			extendedLimit = true;
		}
		...
		var shouldRestrict =
			newDocWidth !== this._docWidthTwips ||
	...
	_onStatusMsg: function (textMsg) {
		...
		if (
			...
			var sizePx = this._twipsToPixels(
				new L.Point(this._docWidthTwips, this._docHeightTwips),
			);
			var width = sizePx.x;
			var height = sizePx.y;
		...
		if (values.commandName === '.uno:ViewRowColumnHeaders') {
			...
			for (var index in values.commentsPos) {
				...
				if (section) {
					...
					for (
						var i = 0;
						i < section.sectionProperties.commentList.length;
						i++
					) {
						if (
							parseInt(
								section.sectionProperties.commentList[i]
			...
			if (section) section.onCommentsDataUpdate();
		} else {
			...
			if (textMsg.trim() !== 'EMPTY' && textMsg.trim() !== '') {
				...
				this._cellSelections = this._cellSelections.map(
					function (element) {
						...
						element = L.LOUtil.createRectangle(
							boundsTwips.min.x * ratio,
							boundsTwips.min.y * ratio,
							boundsTwips.getSize().x * ratio,
							boundsTwips.getSize().y * ratio,
						);
						return element;
					},
				);
	...
	_calculateScrollForNewCellCursor: function () {
		...
		if (contained) return new L.LatLng(0, 0); // No scroll needed.

		var noSplit = this._splitPanesContext
			.getSplitPos()
			.equals(new L.Point(0, 0));
