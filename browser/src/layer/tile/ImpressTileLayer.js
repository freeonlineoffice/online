...
L.ImpressTileLayer = L.CanvasTileLayer.extend({
	...
	onResizeImpress: function () {
		...
		if (window.mode.isMobile()) {
			...
			if (container && slideSorter && toolbar) {
				$(slideSorter).height(
					$(container).height() - toolbar.outerHeight(),
				);
			}
		}
	},
	...
	_onInvalidateTilesMsg: function (textMsg) {
		...
		var preview = this._map._docPreviews
	...
	_onStatusMsg: function (textMsg) {
		...
		if (app.file.fileBasedView) this._updateFileBasedView();
	},

	_addHighlightSelectedWizardComment: function (annotation) {
		if (this.lastWizardCommentHighlight) {
			this.lastWizardCommentHighlight.removeClass(
				'impress-comment-highlight',
			);
		}
		if (annotation._annotationMarker) {
			this.lastWizardCommentHighlight = $(
				this._map._layers[annotation._annotationMarker._leaflet_id]
					._icon,
			);
			this.lastWizardCommentHighlight.addClass(
				'impress-comment-highlight',
			);
		}
	},
