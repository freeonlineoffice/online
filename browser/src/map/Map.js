...
L.Map = L.Evented.extend({
		...
		this.on(
			...
			function (e) {
				...
				if (this._docLayer && !this._docLoadedOnce) {
					...
					setTimeout(
						this.uiManager.initializeSidebar.bind(
							this.uiManager,
						),
						200,
					);
	...
	addView: function (viewInfo) {
		...
		this.fire('postMessage', {
			...
			args: {
				...
				Color: L.LOUtil.rgbToHex(viewInfo.color),
	...
	getDesktopCalcZoomCenter: function () {
		...
		if (app.calc.cellCursorRectangle) {
			...
			const cursorInBounds =
				app.file.viewedRectangle.containsPoint(twipsTopLeft);
		...
		if (docLayer._cellSelectionArea) {
			...
			const selectionInBounds =
				app.file.viewedRectangle.containsPoint(twipsCenter);
	...
	setZoom: function (zoom, options, animate) {
		...
		if (
			...
			app.file.viewedRectangle.containsPoint(
	 ...
	 * X coordinates.
	 */
	negateLatLng: function (latlng, zoom) {
	...
	_isMouseEnteringLeaving: function (e) {
		...
		return (
			L.DomUtil.hasClass(target, 'leaflet-tile') &&
			!(
				related &&
