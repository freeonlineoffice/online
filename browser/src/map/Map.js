...
/* global app _ Cursor JSDialog TileManager lool */
...
window.L.Map = window.L.Evented.extend({
	...
	initialize: function (id, options) { // (HTMLElement or String, Object)
		...
		this._fatal = false;
		this._enabled = true;
		this._debugAlwaysActive = false; // disables the dimming / document inactivity when trueq
		...
		this._progressBar = window.L.progressOverlay(new lool.Point.toPoint(150, 25));
	...
	panBy: function (offset) {
		offset = lool.Point.toPoint(offset).round();
	...
	getDesktopCalcZoomCenter: function () {
		...
		if (app.calc.cellCursorRectangle) {
			...
			if (cursorInBounds) {
				return new lool.Point(...twipsTopLeft);
			}
		}
		...
		if (docLayer._cellSelectionArea) {
			...
			if (selectionInBounds) {
				return new lool.Point(...twipsCenter);
			}
		}
	...
	setDesktopCalcViewOnZoom: function (zoom, animate) {
		...
		const freePaneBounds = new lool.Bounds(viewBounds.min.add(splitPos), viewBounds.max);

		const zoomCenter = docLayer._twipsToCorePixels(
			this.getDesktopCalcZoomCenter(),
		);

		tsManager._offset = new lool.Point(0, 0);
	...
	setZoomAround: function (latlng, zoom) {
		var scale = this.getZoomScale(zoom),
		    ...
		    containerPoint = latlng instanceof lool.Point ? latlng : this.latLngToContainerPointIgnoreSplits(latlng),
	...
	getCorePxDocBounds: function () {
		...
		return new lool.Bounds(this._docLayer._cssPixelsToCore(topleft),
	...
	getLayerMaxBounds: function () {
		return lool.Bounds.toBounds(this.latLngToLayerPoint(this.options.maxBounds.getNorthWest()),
			this.latLngToLayerPoint(this.options.maxBounds.getSouthEast()));
	},

	getLayerDocBounds: function () {
		return lool.Bounds.toBounds(this.latLngToLayerPoint(this.options.docBounds.getNorthWest()),
			this.latLngToLayerPoint(this.options.docBounds.getSouthEast()));
	},

	getSize: function () {
		if (!this._size || this._sizeChanged) {
			this._size = new lool.Point(
	...
	getPixelBounds: function (center, zoom) {
		...
		return new lool.Bounds(topLeftPoint, topLeftPoint.add(this.getSize()));
	},
	...
	project: function (latlng, zoom) {
		...
		return new lool.Point(app.util.round(projectedPoint.x, 1e-6), app.util.round(projectedPoint.y, 1e-6));
	},

	unproject: function (point, zoom) {
		// (Point[, Number]) -> LatLng
		zoom = zoom === undefined ? this.getZoom() : zoom;
		return this.options.crs.pointToLatLng(new lool.Point(point.x, point.y), zoom);
	},
	...
	layerPointToLatLng: function (point) { // (Point)
		var projectedPoint = lool.Point.toPoint(point).add(this.getPixelOrigin());
	...
	containerPointToLayerPoint: function (point) {
		...
		var result = lool.Point.toPoint(point).clone();
		var pointX = point.x;
	...
	containerPointToLayerPointIgnoreSplits: function (point) { // (Point)
		return lool.Point.toPoint(point).subtract(this._getMapPanePos());
	},
	...
	layerPointToContainerPoint: function (point) {
		...
		var pixelOrigin = this.getPixelOrigin();
		var mapPanePos = this._getMapPanePos();
		var result = lool.Point.toPoint(point).add(pixelOrigin);
	...
	layerPointToContainerPointIgnoreSplits: function (point) { // (Point)
		return lool.Point.toPoint(point).add(this._getMapPanePos());
	},

	containerPointToLatLngIgnoreSplits: function (point) {
		var layerPoint = this.containerPointToLayerPointIgnoreSplits(lool.Point.toPoint(point));
	...
	_resetView: function (center, zoom, preserveMapOffset, afterZoomAnim) {
		...
		if (!preserveMapOffset) {
			window.L.DomUtil.setPosition(this._mapPane, new lool.Point(0, 0));
		}
	...
	_getMapPanePos: function () {
		return window.L.DomUtil.getPosition(this._mapPane) || new lool.Point(0, 0);
	},
	...
	_limitCenter: function (center, zoom, bounds) {
		...
		var centerPoint = this.project(center, zoom),
		    viewHalf = this.getSize().divideBy(2),
		    viewBounds = new lool.Bounds(centerPoint.subtract(viewHalf), centerPoint.add(viewHalf)),
