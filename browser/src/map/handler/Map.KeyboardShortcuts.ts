...
class KeyboardShortcuts {
	...
	private findShortcut(
		...
		const viewType = this.map.isEditMode()
			? ViewType.Edit
			: ViewType.ReadOnly;

		const shortcuts = descriptors.filter(
			(descriptor: ShortcutDescriptor) => {
				const keyMatches = descriptor.key === key;
				const keyCodeMatches = Array.isArray(descriptor.keyCode)
					? descriptor.keyCode.includes(keyCode)
					: descriptor.keyCode === keyCode;

				return (
					(!descriptor.docType ||
						descriptor.docType === docType) &&
					(Array.isArray(descriptor.eventType)
						? descriptor.eventType.includes(eventType)
						: descriptor.eventType === eventType) &&
					descriptor.modifier === modifier &&
					(descriptor.viewType === null ||
	...
	public verifyShortcuts(): void {
		...
		this.definitions.forEach((shortcuts, language) => {
			shortcuts.forEach((shortcut) => {
				...
				const shortcutEventTypes = Array.isArray(shortcut.eventType)
					? shortcut.eventType
					: [shortcut.eventType];
				const shortcutKeyCodes = Array.isArray(shortcut.keyCode)
					? shortcut.keyCode
					: [shortcut.keyCode];
...
// Default shortcuts.
keyboardShortcuts.definitions.set(
	'default',
	new Array<ShortcutDescriptor>(
		/*
    ...
    */
		...
		new ShortcutDescriptor({
			...
			modifier: Mod.CTRL,
			key: ',',
			unoAction: '.uno:InsertCurrentDate',
		}),
		new ShortcutDescriptor({
			docType: 'spreadsheet',
			eventType: 'keydown',
		...
		new ShortcutDescriptor({
			...
			eventType: 'keydown',
			key: 'F5',
			dispatchAction: 'presentation',
		}),
		new ShortcutDescriptor({
			docType: 'presentation',
			eventType: 'keydown',
			key: 'PageUp',
			dispatchAction: 'previouspart',
		...
		new ShortcutDescriptor({
			...
			eventType: 'keydown',
			key: 'Home',
			dispatchAction: 'firstpart',
			viewType: ViewType.ReadOnly,
		}),

		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.ALT | Mod.CTRL,
			key: 'p',
		...
		}), // Open browser developer tools on Non-MacOS - shift means the I here is capital
		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.CTRL | Mod.ALT,
			keyCode: 73 /* keyCode('I') === 73 */,
			preventDefault: false,
			platform: Platform.MAC,
		}), // Open browser developer tools on MacOS - registered with keyCode as alt+i triggers a dead key on MacOS
		new ShortcutDescriptor({
			eventType: ['keydown', 'keypress'],
			modifier: Mod.CTRL | Mod.MACCTRL,
			key: ' ',
			preventDefault: false,
			platform: Platform.MAC | Platform.IOSAPP,
		}), // On MacOS, open system emoji picker - bound to keypress as well as keydown since as that is needed on webkit browsers (such as Safari or Orion)
		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.CTRL,
			key: 'r',
		...
		new ShortcutDescriptor({
			...
			preventDefault: false,
			platform: Platform.WINDOWS | Platform.LINUX,
		}), // Refresh browser tab & clear cache
		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.CTRL,
			key: 'm',
...
keyboardShortcuts.definitions.set(
	...
	new Array<ShortcutDescriptor>(
		...
		new ShortcutDescriptor({
			...
			modifier: Mod.SHIFT,
			key: 'F3',
			unoAction: '.uno:ChangeCaseRotateCase',
		}),
		new ShortcutDescriptor({
			docType: 'presentation',
			eventType: 'keydown',
			...
			key: 'F5',
			dispatchAction: 'presentation',
		}), // Already available without this shortcut.
		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.SHIFT | Mod.CTRL,
			key: 'F',
			unoAction: '.uno:Bold',
		}),
		new ShortcutDescriptor({
			...
			modifier: Mod.SHIFT | Mod.CTRL,
			key: 'K',
			unoAction: '.uno:Italic',
		}),
		new ShortcutDescriptor({
			eventType: 'keydown',
			modifier: Mod.SHIFT | Mod.CTRL,
			key: 'U',
			unoAction: '.uno:Underline',
		}),
		...
		new ShortcutDescriptor({
			...
			modifier: Mod.SHIFT,
			key: 'F2',
			dispatchAction: 'insertcomment',
		}),
		new ShortcutDescriptor({
			docType: 'spreadsheet',
			eventType: 'keydown',
			key: 'F4',
			dispatchAction: 'togglerelative',
		}),
		new ShortcutDescriptor({
			docType: 'spreadsheet',
			eventType: 'keydown',
			key: 'F9',
			unoAction: '.uno:Calculate',
		}),
		new ShortcutDescriptor({
			docType: 'spreadsheet',
			eventType: 'keydown',
			key: 'F5',
			dispatchAction: 'focusonaddressinput',
		}),
		new ShortcutDescriptor({
			docType: 'spreadsheet',
			eventType: 'keydown',
			modifier: Mod.ALT,
			key: '0',
			unoAction: '.uno:FormatCellDialog',
		}),
