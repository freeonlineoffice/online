...
L.Map.Keyboard = L.Handler.extend({
	...
	_ignoreKeyEvent: function (ev) {
		...
		if (
			shift &&
			(ev.keyCode === this.keyCodes.INSERT ||
				ev.keyCode === this.keyCodes.DELETE)
		) {
	...
	_onKeyDown: function (ev) {
		if (
			this._map.uiManager.isUIBlocked() ||
			(this._map._docLayer &&
				(this._map._docLayer._docType === 'presentation' ||
					this._map._docLayer._docType === 'drawing') &&
				this._map._docLayer._preview.partsFocused === true)
		)
			return;
		...
		if (this._map._debug.logKeyboardEvents) {
			window.app.console.log(
				...
				ev.key,
				ev.charCode,
				this._expectingInput,
				ev,
			);
		}

		var completeEvent = app.socket.createCompleteTraceEvent(
			'L.Map.Keyboard._onKeyDown',
			{ type: ev.type, charCode: ev.charCode },
		);
		...
		if (this._map._docLayer)
			if (ev.shiftKey && ev.type === 'keydown')
				...
				ev.keyCode === this.keyCodes.SHIFT &&
				ev.type === 'keyup'
			)
	...
	_globalKeyEvent: function (ev) {
		...
		if (ev.shortCutActivated === true) {
			window.app.console.log(
				'Shortcut for: ' + ev.code + ' already handled',
			);
			return;
		}

		if (
			window.KeyboardShortcuts.processEvent(
		...
		if (
			...
			(this._map.jsdialog.hasDialogOpened() ||
				this._map.jsdialog.hasSnackbarOpened() ||
				this._map.jsdialog.hasDropdownOpened()) &&
			...
			if (
				!this.modifier &&
				(ev.keyCode === this.keyCodes.DOWN ||
					ev.keyCode === this.keyCodes.UP ||
					ev.keyCode === this.keyCodes.RIGHT ||
					ev.keyCode === this.keyCodes.LEFT ||
					ev.keyCode === this.keyCodes.PAGEDOWN ||
					ev.keyCode === this.keyCodes.PAGEUP ||
					ev.keyCode === this.keyCodes.DELETE ||
					ev.keyCode === this.keyCodes.BACKSPACE) &&
				ev.type === 'keydown'
			) {
				var deletePart =
					ev.keyCode === this.keyCodes.DELETE ||
	...
	_handleKeyEvent: function (ev, keyEventFn) {
		...
		if (
			(this._map.slideShow && this._map.slideShow.fullscreen) ||
			(this._map.slideShowPresenter &&
				this._map.slideShowPresenter.isFullscreen())
		) {
			return;
		}

		if (ev.code === 'NumpadDecimal')
			this._map.numPadDecimalPressed = true;
		...
		if (
			(this.modifier == UNOModifier.ALT ||
				this.modifier == UNOModifier.SHIFT + UNOModifier.ALT) &&
		...
		if (
			this._map.stateChangeHandler._items['.uno:SlideMasterPage'] ===
			'true'
		) {
			ev.preventDefault();
			return;
		...
		if (this._map.isEditMode()) {
			...
			if (this._ignoreKeyEvent(ev)) {
				...
				if (this.handleOnKeyDownKeys[keyCode] && charCode === 0) {
					if (keyEventFn) {
						...
						if (
							!(
								this._map._textInput.hasAccessibilitySupport() &&
								this._map._textInput.hasFocus() &&
							...
							window.app.console.log(
								'Map.Keyboard._handleKeyEvent: default not prevented for left or right arrow.',
							);
						}
					}
				}
				...
				if (
					...
					window.app.console.log(
						'Ignore backspace/delete/enter keypress',
					);
					return;
				}
				if (
					charCode === keyCode &&
					charCode !== this.keyCodes.enter
				) {
			...
			if (
				...
				map.setZoom(
					map.getZoom() +
						(ev.shiftKey ? 3 : 1) * this._zoomKeys[key],
					null,
					true /* animate? */,
				);
			}
	...
	_isCtrlKey: function (e) {
		if (window.ThisIsTheiOSApp || L.Browser.mac) return e.metaKey;
	...
	_handleCtrlCommand: function (e) {
		...
		// Control
		if (e.keyCode == this.keyCodes.CTRL) return true;

		if (
			e.type !== 'keydown' &&
			e.keyCode !== this.keyCodes.C[DEFAULT] &&
			e.keyCode !== this.keyCodes.V[DEFAULT] &&
			e.keyCode !== this.keyCodes.X[DEFAULT] &&
			/* Safari */ e.keyCode !== this.keyCodes.C[MAC] &&
			e.keyCode !== this.keyCodes.V[MAC] &&
			e.keyCode !== this.keyCodes.X[MAC]
		) {
			e.preventDefault();
			return true;
		}
		...
		if (this._isCtrlKey(e) && e.altKey && e.keyCode === this.keyCodes.N) {
			this._map.fire('postMessage', {
				msgId: 'UI_CreateFile',
				args: { DocumentType: this._map.getDocType() },
			});
			e.preventDefault();
			return true;
		}
		...
		if (
			...
			e.shiftKey &&
			e.altKey &&
			this.keyCodes.V.includes(e.keyCode)
		) {
		...
		if (this._isCtrlKey(e) && e.keyCode === this.keyCodes.F1) {
			if (this._map.uiManager.getCurrentMode() === 'notebookbar') {
				...
				else this._map.uiManager.collapseNotebookbar();
			}
			return true;
		}
		...
		if (e.altKey || e.shiftKey) {
			...
			if (
				(this.keyCodes.C.includes(e.keyCode) ||
					e.keyCode === this.keyCodes.R) &&
				e.altKey
			) {
		...
		if (e.type === 'keydown' && window.ThisIsAMobileApp) {
			...
			if (window.ThisIsTheAndroidApp) e.preventDefault();
		}
		...
		switch (e.keyCode) {
			case this.keyCodes.NUM3[DEFAULT]: // 3
				if (this._map.getDocType() === 'spreadsheet') {
					...
					app.socket.sendMessage(
						'commandvalues command=.uno:ViewRowColumnHeaders',
					);
					return true;
				}
				return false;
		...
		if (
			...
			(this.keyCodes.C.includes(e.keyCode) ||
				this.keyCodes.V.includes(e.keyCode) ||
				this.keyCodes.X.includes(e.keyCode))
		) {
	...
	readOnlyAllowedShortcuts: function (e) {
		...
		if (this._isCtrlKey(e) && e.shiftKey && e.key === '?') return true;
		...
		// comment insert
			...
			e.altKey &&
			e.ctrlKey &&
			this._map.isPermissionEditForComments()
		)
			return true;
		// full-screen presentation
		else if (
			e.type === 'keydown' &&
			e.keyCode === this.keyCodes.F5 &&
			this._map._docLayer._docType === 'presentation'
		)
			return true;
		// moving around
			...
			!this.modifier &&
			(e.keyCode === this.keyCodes.pageUp ||
				e.keyCode === this.keyCodes.pageDown) &&
			e.type === 'keydown'
		)
			return true;
		else if (
			!this.modifier &&
			(e.keyCode === this.keyCodes.END ||
				e.keyCode === this.keyCodes.HOME) &&
			e.type === 'keydown'
		)
			return true;
