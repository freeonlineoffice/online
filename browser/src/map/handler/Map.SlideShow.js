...
L.Map.SlideShow = L.Handler.extend({
	...
	initialize: function (map) {
		...
		window.app.console.log(
			'L.Map.SlideShow: Cypress in window: ' + ('Cypress' in window),
		);
		this._cypressSVGPresentationTest =
			L.Browser.cypressTest &&
			'Cypress' in window &&
			window.Cypress.spec.name ===
				'impress/fullscreen_presentation_spec.js';
	},
	...
	_onFullScreen: function (e) {
		...
		if (
			...
			this._slideShow = L.DomUtil.create(
				'iframe',
				'leaflet-slideshow',
	...
	_startPlaying: function () {
		if (this._presentInWindow) {
			...
			this._slideShowWindowProxy.document.body.style.overflow =
				'hidden'; // Prevent scrollbars.

			const iFrame =
				this._slideShowWindowProxy.document.createElement('iframe');
			...
			this._slideShowWindowProxy.onload =
				this._handleSlideWindowLoaded.bind(this);
			...
			this._slideShowWindowProxy.addEventListener(
				'keydown',
				this._onSlideWindowKeyPress.bind(this),
			);
		...
		this._slideShow.src =
			...
			separator +
			'StartSlideNumber=' +
			this._startSlideNumber;
		this._slideShow.contentWindow.focus();
	},

	_handleSlideWindowLoaded: function () {
		const iframe =
			this._slideShowWindowProxy.document.querySelector('iframe');
	...
	_processSlideshowLinks: function () {
		this._slideShow.addEventListener(
			'load',
			function onLoadSlideshow() {
				var linkElements = [].slice
					.call(
						this._slideShow.contentDocument.querySelectorAll(
							'a',
						),
					)
					.filter(function (el) {
				...
				linkElements.forEach(function (item) {
					...
					var parentLink =
						item.parent.getAttribute('xlink:href');

					if (parentLink) {
						setAttributeNs(item.parent, parentLink);
					}

					item.parent.parentNode.insertBefore(
						item.parent.cloneNode(true),
	...
	_processSlideshowVideoForSafari: function () {
		...
		if (!this._slideShow) {
			console.error(
				'In Safari without fixing slideshow videos, this may cause videos to be offset',
			);
			return;
		}
			...
			function onLoadSlideshow() {
				...
				var fixSVGPos = function (video) {
					...
					return function () {
						...
						var leftRightBorders =
							svg.width.baseVal.value -
							minRatio * svg.viewBox.baseVal.width;
						var topBottomBorders =
							svg.height.baseVal.value -
							minRatio * svg.viewBox.baseVal.height;

						// revert the scaling positioning to center (back to top-left) by subtracting at 1/2 of width
						var offsetX =
							-foreignObject.width.baseVal.value / 2.0;
						var offsetY =
							-foreignObject.height.baseVal.value / 2.0;

						// revert the object position
						offsetX =
							offsetX -
							foreignObject.x.baseVal.value +
							leftRightBorders / 4.0;
						offsetY =
							offsetY -
							foreignObject.y.baseVal.value +
							topBottomBorders / 4.0;
						...
						offsetX =
							offsetX +
							foreignObject.x.baseVal.value * widthRatio;
						offsetY =
							offsetY +
							foreignObject.y.baseVal.value * heightRatio;
				...
				for (var i = 0; i < videos.length; i++) {
					...
					this._slideShow.contentDocument.defaultView.addEventListener(
						'resize',
						fixThisVideoPos,
					);
				}
