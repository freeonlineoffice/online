...
window.L.Map.WOPI = window.L.Handler.extend({
	...
	addHooks: function () {
		...
		this._map.on(
			'updateviewslist',
			function () {
				this._postViewsMessage('Views_List');
			},
			this,
		);
		...
		if (!window.ThisIsAMobileApp) {
			...
			window.open = (function (open) {
				return function (url, name, features) {
					...
					const eSignInProgress =
						eSignature && eSignature.signInProgress;
					if (eSignInProgress) {
	...
	_setWopiProps: function (wopiInfo) {
		...
		this.DownloadAsPostMessage = Object.prototype.hasOwnProperty.call(
			overridenFileInfo,
			'DownloadAsPostMessage',
		)
			? overridenFileInfo.DownloadAsPostMessage
		...
		if (this.UserCanWrite && !app.isReadOnly())
			// There are 2 places that set the file permissions, WOPI and URI. Don't change permission if URI doesn't allow.
		...
		if ('TemplateSaveAs' in wopiInfo) {
			this._map.showBusy(
				_('Creating new file from template...'),
				false,
			);
	...
	setupImageInsertionMenu: function () {
		...
		var menuEntriesImage =
			JSDialog.MenuDefinitions.get('InsertImageMenu');
		var menuEntriesMultimedia = JSDialog.MenuDefinitions.get(
			'InsertMultimediaMenu',
		);

		if (this.EnableInsertRemoteImage) {
			menuEntriesImage.push({
				action: 'remotegraphic',
				text: _UNO('.uno:InsertGraphic', '', true),
			});
		}

		if (this.EnableInsertRemoteFile) {
			/* Separate, because needs explicit integration support */
			menuEntriesMultimedia.push({
				action: 'remotemultimedia',
				text: _UNO('.uno:InsertAVMedia', '', true),
			});
		}
	...
	_postLoaded: function(e) {
		...
		for (var key in this._appLoadedConditions) {
			if (!this._appLoadedConditions[key]) return;
		}
	...
	_allowMessageOrigin: function (e) {
		...
		try {
			if (e.origin === window.parent.origin) return true;
		} catch (secErr) {
			// security error de-referencing window.parent.origin.
		}
	...
	_postMessageListener: function (e) {
		if (!this._allowMessageOrigin(e)) {
			window.app.console.error(
				'PostMessage not allowed due to incorrect origin.',
			);
			return;
		}
		...
		if (
			msg.MessageId === 'Show_Button' ||
			msg.MessageId === 'Hide_Button' ||
			msg.MessageId === 'Remove_Button'
		) {
			if (!msg.Values) {
			...
			msg.MessageId === 'Show_Command' ||
			msg.MessageId === 'Hide_Command'
		) {
			if (!msg.Values) {
				window.app.console.error('Property "Values" not set');
				return;
			...
			msg.MessageId === 'Show_NotebookTab' ||
			msg.MessageId === 'Hide_NotebookTab'
		) {
			if (!msg.Values) {
				window.app.console.error('Property "Values" not set');
				return;
			...
			msg.MessageId === 'Show_Menu_Item' ||
			msg.MessageId === 'Hide_Menu_Item'
		) {
			if (!msg.Values) {
				window.app.console.error('Property "Values" not set');
				return;
			...
			if (
				!this._map.menubar ||
				!this._map.menubar.hasItem(msg.Values.id)
			) {
				window.app.console.error(
					'Menu item with id "' + msg.Values.id + '" not found.',
				);
				if (
					this._map.uiManager.getCurrentMode() === 'notebookbar'
				) {
					window.app.console.error(
						'No menu items in notebookbar',
					);
				}
				return;
			}
			...
			msg.MessageId === 'Insert_ContextualButton' &&
			msg.Values &&
			msg.Values.id
		) {
			this._map.uiManager.map.contextToolbar.insertAdditionalContextButton(
				msg.Values,
			);
			return;
		} else if (
			msg.MessageId === 'Send_UNO_Command' &&
			msg.Values &&
			msg.Values.Command
		) {
			...
			if (
				msg.Values &&
				msg.Values.action &&
				msg.Values.disable !== undefined
			) {
				this._map._disableDefaultAction[msg.Values.action] =
					msg.Values.disable;
			}
			return;
		...
		if (msg.MessageId === 'Host_PostmessageReady') {
			// We already have a listener for this in lool.html, so ignore it here
			return;
		}
		...
		if (!this._appLoaded) {
			window.app.console.error(
				"Free Online Office not loaded yet. Listen for App_LoadingStatus (Document_Loaded) event before using PostMessage API. Ignoring post message '" +
					msg.MessageId +
					"'.",
			);
			return;
		...
		if (msg.MessageId === 'Set_Settings') {
			...
			var dontTerminateEdit =
				msg.Values && msg.Values['DontTerminateEdit'];
			var dontSaveIfUnmodified =
				msg.Values && msg.Values['DontSaveIfUnmodified'];
			...
			msg.MessageId === 'Action_FullscreenPresentation' &&
			...
			if (preview) {
				...
				if (
					preview.nextSibling &&
					preview.nextSibling.innerText !== msg.Values.url
				)
					return;
			...
			if (msg.Values && msg.Values.Mimetype && msg.Values.Data) {
				var blob = new Blob([
					'paste mimetype=' + msg.Values.Mimetype + '\n',
					msg.Values.Data,
				]);
			...
			msg.MessageId === 'CallPythonScript' &&
			...
			if (
				msg.Values &&
				msg.Values.ViewId !== null &&
				msg.Values.ViewId !== undefined
			) {
				app.socket.sendMessage(
					'removesession ' + msg.Values.ViewId,
				);
			}
		} else if (msg.MessageId === 'Action_ChangeUIMode') {
			this._map.uiManager.onChangeUIMode({
				mode: msg.Values.Mode,
				force: true,
			});
