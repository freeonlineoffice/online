...
L.Map.TouchGesture = L.Handler.extend({
	...
	initialize: function (map) {
		...
		if (!this._hammer) {
			...
			L.DomEvent.on(
				...
				window.touch.touchOnly(L.DomEvent.preventDefault),
			);
			L.DomEvent.on(
				this._map._mapPane,
				'touchend',
				window.touch.touchOnly(function (e) {
		...
		if (this._hammer.input instanceof Hammer.TouchInput) {
			...
			$doc.on('click.contextMenu', '.context-menu-item', function (e) {
				...
				if (
					$elem.data().contextMenu.selector === '.leaflet-layer'
				) {
	...
	addHooks: function () {
		this._hammer.on(
			'hammer.input',
			window.memo.bind(window.touch.touchOnly(this._onHammer), this),
		);
		this._hammer.on(
			'tap',
			window.memo.bind(window.touch.touchOnly(this._onTap), this),
		);
		this._hammer.on(
			'panstart',
			window.memo.bind(window.touch.touchOnly(this._onPanStart), this),
		);
		this._hammer.on(
			'pan',
			window.memo.bind(window.touch.touchOnly(this._onPan), this),
		);
		this._hammer.on(
			'panend',
			window.memo.bind(window.touch.touchOnly(this._onPanEnd), this),
		);
		this._hammer.on(
			'pinchstart',
			window.memo.bind(
				window.touch.touchOnly(this._onPinchStart),
				this,
			),
		);
		this._hammer.on(
			'pinchmove',
			window.memo.bind(window.touch.touchOnly(this._onPinch), this),
		);
		this._hammer.on(
			'pinchend',
			window.memo.bind(window.touch.touchOnly(this._onPinchEnd), this),
		);
		this._hammer.on(
			'tripletap',
			window.memo.bind(
				window.touch.touchOnly(this._onTripleTap),
				this,
			),
		);
		this._hammer.on(
			'swipe',
			window.memo.bind(window.touch.touchOnly(this._onSwipe), this),
		);
	...
	removeHooks: function () {
		...
		this._hammer.off(
			'doubletap',
			window.memo.bind(
				window.touch.touchOnly(this._onDoubleTap),
				this,
			),
		);
		this._hammer.off(
			'press',
			window.memo.bind(window.touch.touchOnly(this._onPress), this),
		);
		this._hammer.off(
			'tripletap',
			window.memo.bind(
	...
	_onPermission: function (e) {
		if (e.detail.perm == 'edit') {
			...
			this._hammer.on(
				'press',
				window.memo.bind(
					window.touch.touchOnly(this._onPress),
					this,
				),
			);
		} else {
	...
	_onHammer: function (e) {
		...
		if (e.isFirst) {
			...
			if (
				...
				app.calc.cellCursorVisible &&
				funcWizardRangeBounds &&
				funcWizardRangeBounds.contains(latlng)
			) {
				this._state = L.Map.TouchGesture.CURSOR;
			} else {
				this._state = L.Map.TouchGesture.MAP;
	...
	_onPress: function (e) {
		...
		var waitForSelectionMsg = function () {
			...
			if (
				!docLayer._cursorAtMispelledWord &&
				(!graphicSelection ||
					!graphicSelection.containsPoint(
						posInTwips.toArray(),
					)) &&
				(!app.calc.cellCursorVisible ||
					!app.calc.cellCursorRectangle.containsPoint(
			...
			that._map._contextMenu._onMouseDown({
		...
		var textSelection;
		if (
			docLayer._selectionHandles.start.rectangle &&
			docLayer._selectionHandles.end.rectangle
		) {
			// Oversimplication. See "inBand" function.
			textSelection = new app.definitions.simpleRectangle(
				0,
				docLayer._selectionHandles.end.rectangle.y1,
				app.file.size.twips[0],
				0,
			);
			textSelection.height =
				docLayer._selectionHandles.end.rectangle.y2 -
				docLayer._selectionHandles.start.rectangle.y1;
		}

		if (
			(textSelection &&
				textSelection.containsPoint(posInTwips.toArray())) ||
			(graphicSelection &&
				graphicSelection.containsPoint(posInTwips.toArray())) ||
			(app.calc.cellCursorVisible &&
				app.calc.cellCursorRectangle.containsPoint(
					posInTwips.toArray(),
				)) ||
			...
			// send right click to trigger context menus
			this._map._contextMenu._onMouseDown({
				originalEvent: e.srcEvent,
			});
			rightClick();
		} else {
	...
	_onTap: function (e) {
		...
		if (this._map._docLayer.isWriter()) {
			dropDownMarkers = document.getElementsByClassName(
				'leaflet-marker-icon writer-drop-down-marker',
			);
		} else if (this._map._docLayer.isCalc()) {
			dropDownMarkers = document.getElementsByClassName(
				'leaflet-marker-icon spreadsheet-drop-down-marker',
			);
		}
		if (
			dropDownMarkers &&
			dropDownMarkers.length == 1 &&
			dropDownMarkers[0] &&
			e.target &&
			e.target == dropDownMarkers[0]
		) {
	...
	_onPanStart: function (e) {
		...
		if (app.calc.cellCursorVisible) {
			...
			increasedCellCursor.y1 -=
				increasedCellCursor.height * increaseRatio;
			increasedCellCursor.y2 +=
				increasedCellCursor.height * increaseRatio;
		}

		if (
			increasedCellCursor &&
			increasedCellCursor.containsPoint(posInTwips.toArray())
		) {
			if (
				!app.calc.cellCursorRectangle.containsPoint(
					posInTwips.toArray(),
				...
				let heightBuffer =
					Math.abs(app.calc.cellCursorRectangle.pHeight) *
					increaseRatio;
		...
		if (this._state === L.Map.TouchGesture.CURSOR) {
			...
			this._map.dragging._draggable._onDown(
				this._constructFakeEvent(point, 'mousedown'),
			);
		}
	...
	_onPan: function (e) {
		...
		if (this._state === L.Map.TouchGesture.CURSOR) {
			...
			this._map.dragging._draggable._onMove(
				this._constructFakeEvent(point, 'mousemove'),
			);
		}
	},
	...
	_onPanEnd: function (e) {
		...
		if (this._state === L.Map.TouchGesture.CURSOR) {
			...
			this._map.dragging._draggable._onUp(
				this._constructFakeEvent(point, 'mouseup'),
			);
		}
	...
	_onPinchStart: function (e) {
		...
		const _pinchStartLatLng = this._map.mouseEventToLatLng({
			clientX: e.center.x,
			clientY: e.center.y,
		});
	...
	_onPinchEnd: function () {
		...
		if (!this._pinchStartCenter) return;

		var oldZoom = this._map.getZoom();
		var zoomDelta = this._zoom - oldZoom;
		var finalZoom = this._map._limitZoom(
			zoomDelta > 0 ? Math.ceil(this._zoom) : Math.floor(this._zoom),
		);
	...
	_onSwipe: function (e) {
		...
		let velocityX = this._map._docLayer.isCalcRTL()
			? -e.velocityX
			: e.velocityX;
	...
	_cancelAutoscrollRAF: function () {
		...
		if (app.file.fileBasedView) this._map._docLayer._checkSelectedPart();
		L.Util.cancelAnimFrame(this.autoscrollAnimReq);
		return;
	},
	...
	_autoscroll: function () {
		...
		if (
			...
			if (this._map.getDocSize().x < this._map.getSize().x) {
				...
				horizontalEnd =
					Math.max(
						Math.min(org.x, this._newPos.x),
						org.x - Math.max(docSize.x, 0),
					) !== this._newPos.x;
			}
			...
			if (this._map.getDocSize().y < this._map.getSize().y) {
				...
				verticalEnd =
					Math.max(
						Math.min(org.y, this._newPos.y),
						org.y - Math.max(docSize.y, 0),
					) !== this._newPos.y;
			}
