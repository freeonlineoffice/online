...
L.Map.TouchGesture = L.Handler.extend({
	...
	_onTap: function (e) {
		...
		if (this._map._docLayer.isWriter()) {
			dropDownMarkers = document.getElementsByClassName(
				'html-object-section writer-drop-down-marker',
			);
		...
		if (
			...
			if (e.target == dropDownMarkers[0])
				...
				let section = app.sectionContainer.getSectionWithName(
				...
				if (section) {
					...
					if (
						section &&
						section.containsPoint(posInTwips.pToArray())
					) {
						section.onClick();
						return; // don't send the mouse-event to core
					}
	...
	_onSwipe: function (e) {
		...
		this.autoscrollAnimReq = app.util.requestAnimFrame(
			this._autoscroll,
			this,
		);
