...
L.Map.include({
	...
	createFontSelector: function (nodeSelector) {
		...
		var createSelector = function () {
			var commandValues =
				that.getToolbarCommandValues('.uno:CharFontName');
		...
		var onCommandStateChanged = function (e) {
			...
			if (commandName !== '.uno:CharFontName') return;
			...
			if (!found && state) {
				fontcombobox.append($('<option></option>').text(state));
			}
		...
		var onFontListChanged = function (e) {
			if (e.commandName === '.uno:CharFontName') createSelector();
		};
	...
	createFontSizeSelector: function (nodeSelector) {
		var data = [
			6, 7, 8, 9, 10, 10.5, 11, 12, 13, 14, 15, 16, 18, 20, 22, 24, 26,
		...
		fontsizecombobox
			.off('select2:select', this.onFontSizeSelect.bind(this))
			.on('select2:select', this.onFontSizeSelect.bind(this));

		var onCommandStateChanged = function (e) {
			var commandName = e.commandName;

			if (commandName !== '.uno:FontHeight') return;
			...
			if (!found) {
				...
				fontsizecombobox.append(
					$('<option>').text(state).val(state),
				);
			}
	...
	downloadAs: function (name, format, options, id) {
		...
		if (
			(id === 'print' && this['wopi'].DisablePrint) ||
	...
	exportAs: function (url) {
		...
		app.socket.sendMessage(
			'exportas url=wopi:' + encodeURIComponent(url),
		);
	},
	...
	renameFile: function (filename) {
		...
		app.socket.sendMessage(
			'renamefile filename=' + encodeURIComponent(filename),
		);
	},
	...
	applyLayout: function (layout) {
		...
		if (this.isEditMode()) {
			var msg =
				...
				'"WhatPage":{"type":"unsigned short", "value": "' +
				this.getCurrentPartNumber() +
				'"},' +
				'"WhatLayout":{"type":"unsigned short", "value": "' +
				layout +
				'"}' +
				'}';
			app.socket.sendMessage(msg);
	...
	sendUnoCommand: function (command, json, force) {
		...
		if (command.startsWith('.uno:SpellOnline')) {
			var val =
				map['stateChangeHandler'].getItemValue('.uno:SpellOnline');
	...
	onHelpOpen: function (id, map, productName) {
		...
		if (id === 'keyboard-shortcuts-content') {
			...
			if (window.socketProxy) {
				...
				for (var p = 0; p < imgList.length; p++) {
					...
					imgList[p].src = window.makeWsUrl(
						'/browser/dist' + imgSrc,
					);
				}
			}
		...
		if (supportedLanguage.indexOf(currentLanguage) >= 0) {
			translatableContent = $(
				contentElement.querySelectorAll('.screenshot img'),
			);

			for (i = 0, max = translatableContent.length; i < max; i++) {
				translatableContent[i].src = translatableContent[
					i
				].src.replace('/en/', '/' + currentLanguage + '/');
			}
		}
		...
		if (id === 'online-help-content') {
			var productNameContent =
				contentElement.querySelectorAll('span.productname');
			for (i = 0, max = productNameContent.length; i < max; i++) {
				productNameContent[i].innerHTML = productNameContent[
					i
				].innerHTML.replace('{productname}', productName);
			}
			document.getElementById('online-help-content').innerHTML =
				app.util.replaceCtrlAltInMac(
					document.getElementById('online-help-content')
						.innerHTML,
				);
		}
		if (id === 'keyboard-shortcuts-content') {
			document.getElementById('keyboard-shortcuts-content').innerHTML =
				app.util.replaceCtrlAltInMac(
					document.getElementById('keyboard-shortcuts-content')
						.innerHTML,
				);
		}
		...
		searchInput.addEventListener(
			...
			function () {
				...
				if (!startFilter || !isAnyMatchingContent) {
					...
					document
						.querySelectorAll(
							'#online-help-content > *:not(a), .link-section p, .product-header',
						)
						.forEach(
							function (element) {
	...
	filterResults: function (searchTerm, isAnyMatchingContent, id) {
		...
		var mainSectionsQuery =
			'.section:not(div.text .section, div.spreadsheet .section, div.presentation .section)';
		var docType =
		...
		mainSections.forEach(
			function (mainSection) {
				...
				var headerText = mainSection
					.querySelector('.section-header')
					.textContent.toLowerCase();
				var containsTermInHeader = headerText.includes(
					searchTerm.toLowerCase(),
				);
				// check main section text matches or not
				var sectionText = mainSection.textContent.toLowerCase();
				var containsTerm = sectionText.includes(
					searchTerm.toLowerCase(),
				);

				//sub-section text matches or not
				var subSections =
					mainSection.querySelectorAll('.sub-section');
		...
		if (!isAnyMatchingContent) {
			...
			setTimeout(function () {
				$('#online-help-search-input').removeClass(
					'search-not-found',
				);
			}, 800);
		}
	...
	resetFilterResults: function () {
		...
		document
			.querySelectorAll('.m-v-0, .product-header, .help-dialog-header')
			.forEach(
				function (element) {
					this.show(element);
	...
	extractContent: function (html) {
		...
		return parser
			...
			.documentElement.getElementsByTagName('body')[0].textContent;
	},
	...
	makeURLFromStr: function (str) {
		...
		if (
			!(
				...
				lowerStr.startsWith('https://') ||
				lowerStr.startsWith('ftp://') ||
				lowerStr.startsWith('mailto:')
			)
		) {
			// Regular expression to test if the string is an email address
			const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
			if (emailPattern.test(str)) str = 'mailto:' + str;
			else str = 'http://' + str;
		}
		return str;
	},
	...
	_createAndRunHyperlinkDialog: function (defaultText, defaultLink) {
		...
		var json = map.uiManager._modalDialogJSON(
			id,
			title,
			true,
			[
		...
		if (this.hyperlinkUnderCursor && this.hyperlinkUnderCursor.text) {
			...
			if (
				...
				if (L.Browser.clipboardApiAvailable) {
					...
					text = this.extractContent(
						this._clip._selectionContent,
					);
				}
			}
	...
	formulabarBlur: function () {
		if (!this.uiManager.isAnyDialogOpen()) this.focus();
	},
	...
	formulabarSetDirty: function () {
		if (this.formulabar) this.formulabar.dirty = true;
	},
});
