...
L.Control.JSDialogBuilder = L.Control.extend({
	...
	_stressAccessKey: function (element, accessKey) {
		if (!accessKey || window.mode.isMobile()) return;
	...
	_expanderHandler: function (
		...
		if (data.children.length > 0) {
			...
			if (expanded) $(expanderChildren).addClass('expanded');
				...
				var findNextVisibleTab = function (tab, backwards) {
					...
					while (!isTabVisible(nextTab) && nextTab != tab) {
						if (backwards && tabs.indexOf(nextTab) == 0)
							...
							!backwards &&
							tabs.indexOf(nextTab) == tabs.length - 1
						)
							nextTab = tabs[0];
						else nextTab = tabs[tabs.indexOf(nextTab) + diff];
					}
	...
	_radiobuttonControl: function (parentContainer, data, builder) {
		var container = L.DomUtil.createWithId(
			'div',
			data.id,
			parentContainer,
		);
		...
		var radiobuttonLabel = L.DomUtil.createWithId(
			'label',
			data.id + '-label',
			container,
		);
	...
	_editControl: function (parentContainer, data, builder, callback) {
		...
		edit.addEventListener('keyup', function (e) {
			var callbackToUse =
				e.key === 'Enter' && data.changedCallback
					? data.changedCallback
					: null;
	...
	_pushbuttonControl: function (
		...
		if (customCallback) pushbutton.onclick = customCallback;
			...
			pushbutton.onclick = builder.callback.bind(
				...
				'click',
				{ id: pushbutton.id },
				builder._responses[pushbutton.id],
				builder,
			);
		else
			pushbutton.onclick = builder.callback.bind(
	...
	_valuesetControl: function (parentContainer, data, builder) {
		...
		for (var index in data.entries) {
			...
			if (image) {
				...
				image =
					'url("' + L.LOUtil.getImageURL(image + '.svg') + '")';
			}
	...
	_createComment: function (container, data, isRoot) {
		...
		var annotation = new app.definitions.Comment(
			data.data,
			data.id === 'new' ? { noMenu: true } : {},
			this,
		);
		annotation.context = data.annotation.containerObject.context;
		annotation.documentTopLeft =
			data.annotation.containerObject.documentTopLeft;
		annotation.containerObject = data.annotation.containerObject;
		annotation.sectionProperties.section = annotation;
		annotation.sectionProperties.commentListSection =
			data.annotation.sectionProperties.commentListSection;
	...
	_unoToolButton: function (parentContainer, data, builder, options) {
		...
		var div;
		if (
			data.command === '.uno:Paste' ||
			data.command === '.uno:Cut' ||
			data.command === '.uno:Copy'
		) {
			var hyperlink = L.DomUtil.create('a', '', parentContainer);
			div = this._createIdentifiable(
				'div',
				'unotoolbutton ' +
					builder.options.cssClass +
					' ui-content unospan',
		...
		if (data.command || data.postmessage === true) {
			...
			if (data.text) button.setAttribute('aria-label', data.text);
			else if (data.aria)
			...
			if (hasPopUp) button.setAttribute('aria-haspopup', true);
			...
			if (data.w2icon) {
				...
				if (data.icon) {
					...
					L.LOUtil.setImage(
						buttonImage,
						builder._createIconURL(data.command),
						builder.map,
					);
				}
			} else {
			...
			if (data.command) {
				var updateFunction = function () {
					...
					if (disabled) div.setAttribute('disabled', '');
					else div.removeAttribute('disabled');
				};
		...
		if (options && options.hasDropdownArrow) {
			...
			var arrowbackground = L.DomUtil.create(
				'div',
				'arrowbackground',
				div,
			);
