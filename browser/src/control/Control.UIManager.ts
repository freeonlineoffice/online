...
class UIManager extends window.L.Control {
	...
	initializeSpecializedUI(docType: string): void {
		...
		var startFolloMePresntationGet = this.map.isPresentationOrDrawing() && window.loolParams.get('startFollowMePresentation');
		var presentationLeaderIdGet = this.map.isPresentationOrDrawing() && window.loolParams.get('presentationLeaderId');
