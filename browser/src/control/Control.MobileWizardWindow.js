...
L.Control.MobileWizardWindow = L.Control.extend({
	...
	_applySpecialClasses: function () {
		if (this.isShapesWizard) this.mobileWizard.addClass('shapeswizard');
		if (this.isHamburgerMenu) this.mobileWizard.addClass('menuwizard');
		if (this.isFunctionMenu) this.mobileWizard.addClass('funcwizard');
		if (
			this.isPopup &&
			!this.isAutoCompletePopup &&
			!this.isPopupPartialScreen
		)
			this.mobileWizard.addClass('popup');
		if (this.isSnackBar) this.mobileWizard.addClass('snackbar');
		if (this.isBusyPopUp) this.mobileWizard.addClass('busypopup');
	},
	...
	_createScrollIndicator() {
		...
		container.style.boxShadow =
			'0 -8px 20px 4px #0b87e770, 0 1px 10px 6px #0b87e7';
		return container;
	},

	_showWizard: function () {
		if (this.snackBarTimout) clearTimeout(this.snackBarTimout);
		...
		wizard.on('scroll', function () {
			...
			if (scrollLeft < contentHeight + 1 || !that.isVisible) {
				that.scrollIndicator.style.display = 'none';
			} else {
				that.scrollIndicator.style.display = 'block';
			}
		});
		$('#toolbar-down').hide();
		if (window.ThisIsTheAndroidApp)
			window.postMobileMessage('MOBILEWIZARD show');
		if (window.mobileMenuWizard) this.map.showSidebar = false;

		if (this.map.uiManager.sheetsBar) this.map.uiManager.sheetsBar.hide();
	...
	restoreTabs: function () {
		if (this.tabs) this.setTabs(this.tabs);
	},
	...
	goLevelDown: function (contentToShow, options) {
		var animate =
			options && options.animate != undefined ? options.animate : true;
		...
		var nodesToHide = $(contentToShow)
			.siblings(':visible')
			.not('.mobile-wizard-scroll-indicator');

		var parent = $(contentToShow).parent();
		if (parent.hasClass('toolbox'))
			nodesToHide = nodesToHide.add(
				parent.siblings(
					':visible:not(.mobile-wizard-scroll-indicator)',
				),
			);
		var duration = 10;
		if (animate) {
		...
		} else nodesToHide.hide();

		$(contentToShow).children('.ui-header').hide();

		$('#mobile-wizard.funcwizard div#mobile-wizard-content').removeClass(
			'hideHelpBG',
		);
		$('#mobile-wizard.funcwizard div#mobile-wizard-content').addClass(
			'showHelpBG',
		);
		...
		else $(contentToShow).children('.ui-content').first().show();
		...
		if (!this._inBuilding)
			history.pushState(
				{ context: 'mobile-wizard', level: this._currentDepth },
				'mobile-wizard-level-' + this._currentDepth,
			);

		var content = $(contentToShow).children('.ui-content').get(0);
		var title = content ? content.title : '';

		if (this._customTitle) this._setCustomTitle(this._customTitle);
		else this._setTitle(title);
	...
	goLevelUp: function () {
		...
		if (
			...
			if (this._currentDepth > 0 && parent)
				this._customTitle
					? this._setCustomTitle(parent.get(0).customTitleBar)
			...
			var headers = currentNode.siblings().filter(function () {
				...
				return (
					styleAttributeValue &&
					styleAttributeValue.includes('display: none;')
				);
			});
			...
			$(
				'#mobile-wizard.funcwizard div#mobile-wizard-content',
			).removeClass('showHelpBG');
			$(
				'#mobile-wizard.funcwizard div#mobile-wizard-content',
			).addClass('hideHelpBG');
			...
			if (
				this._currentDepth == 0 ||
				(this._isTabMode && this._currentDepth == 1)
			) {
				this._inMainMenu = true;
				this.backButton.addClass('close-button');
				if (this._isTabMode) {
	...
	restoreTitle: function () {
		if (this.cusomTitle) this._setCustomTitle(this.cusomTitle);
		else this._setTitle(this.title);
	},
		...
		if (this.tabs && tabId) {
			for (var index in this.tabs.children) {
				if (this.tabs.children[index].id === tabId) {
					$(this.tabs.children[index]).trigger('click', {
						animate: false,
					});
					break;
				}
	...
	_onMobileWizard: function (data, callback) {
		if (data) {
			...
			var isDocumentAreaPopup =
				data.popupParent === '_POPOVER_' &&
				data.posx !== undefined &&
				data.posy !== undefined;
			var isCalc = this.map._docLayer
			...
			if (data.children) {
				for (var i in data.children) {
					if (data.children[i].type == 'deck') isSidebar = true;
				}
			}

			if (!this._isActive && isSidebar) {
				if (this.map.showSidebar == false) return;
			}
			...
			if (isMobileDialog) {
				...
				if (data.dialogid) this._dialogid = data.dialogid;
			}
			...
			if (this._currentPath) currentPath = this._currentPath;
			if (this._currentScrollPosition)
			...
			if (isPopup) {
				...
				if (popupContainer.length) {
					...
					if (
						...
						this._builder.build(popupContainer.get(0), [
							data,
						]);
					}
			...
			if (isSidebar) this._modifySidebarLayout(data);

			if (!alreadyOpen) {
				history.pushState(
					{ context: 'mobile-wizard' },
					'mobile-wizard-opened',
				);
				history.pushState(
					{ context: 'mobile-wizard', level: 0 },
					'mobile-wizard-level-0',
				);
				this.mobileWizard.scrollTop(0);
			}
			...
			if (data.id === 'menubar' || data.id === 'insertshape') {
				document.getElementById('mobile-wizard').style.height =
					'100vh';
				if (data.id === 'menubar') this.isHamburgerMenu = true;
				...
				data.id === 'modal-dialog-about-dialog-box' ||
				data.id === 'modal-dialog-unlock-features-popup'
			) {
				isPopup = true;
			} else {
				document.getElementById('mobile-wizard').style.height =
			...
			if (isPopup) {
				...
				if (data.type === 'snackbar') {
					...
					this.snackBarTimout = setTimeout(
						function () {
							that.parent.removeWindow(that);
						},
						data.timeout
	...
	_isSlidePropertyPanel: function (data) {
		var backgroundPanel = L.LOUtil.findItemWithAttributeRecursive(
			data,
			'id',
			'SlideBackgroundPanel',
		);
		var layoutPanel = L.LOUtil.findItemWithAttributeRecursive(
			data,
			'id',
			'SdLayoutsPanel',
		);
	...
	_insertCalcBorders: function (deck) {
		var replaceMe = L.LOUtil.findItemWithAttributeRecursive(
			deck,
			'id',
			'cellbordertype',
		);
	...
	_modifySidebarLayout: function (data) {
		...
		var deck = L.LOUtil.findItemWithAttributeRecursive(
			data,
			'type',
			'deck',
		);
		if (deck) {
			...
			var textIdx = L.LOUtil.findIndexInParentByAttribute(
				deck,
				'id',
				'TextPropertyPanel',
			);
	...
	onJSUpdate: function (e) {
		...
		var newControl = container.querySelector(
			"[id='" + data.control.id + "']",
		);
		if (newControl) {
			if (wasHidden) newControl.style.display = 'none';
		...
		$(this.content).animate(
			{ scrollTop: this._currentScrollPosition },
			0,
		);
