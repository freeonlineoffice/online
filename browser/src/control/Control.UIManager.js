...
L.Control.UIManager = L.Control.extend({
	...
	onAdd: function (map) {
		...
		$('#toolbar-wrapper').on('click', function (event) {
			if (event.target.parentElement.id === 'toolbar-up')
		...
		$('.main-nav').on('click', function (event) {
			if (
				event.target.parentElement.nodeName === 'NAV' || // checks if clicked on an container of an element
				event.target.nodeName === 'NAV' || // checks if clicked on navigation bar itself
				event.target.parentElement.id === 'document-titlebar'
			) {
	...
	initializeSpecializedUI: function (docType) {
		...
		if (docType === 'spreadsheet') {
			...
			let spreadsheetToolbar = document.getElementById(
				'spreadsheet-toolbar',
			);
			...
			document
				.getElementById('selectbackground')
		...
		if (window.mode.isDesktop() && !window.ThisIsAMobileApp) {
			...
			if (this.map.getDocType() === 'presentation') {
				if (
					...
					this.getBooleanDocTypePref('SdMasterPagesDeck', false)
				) {
					app.socket.sendMessage('uno .uno:SidebarShow');
					...
					this.getBooleanDocTypePref('NavigatorDeck', false)
				) {
					app.socket.sendMessage('uno .uno:SidebarShow');
			...
			if (!showSidebar) app.socket.sendMessage('uno .uno:SidebarHide');
	...
	showButtonInClassicToolbar: function (buttonId, show) {
		...
		if (!found) {
			window.app.console.error(
				'Toolbar button with id "' + buttonId + '" not found.',
			);
			return;
		}
	},
	...
	showCommandInClassicToolbar: function (command, show) {
		...
		if (!found) {
			window.app.console.error(
				'Toolbar item with command "' + command + '" not found.',
			);
			return;
		}
	},
	...
	isStatusBarVisible: function () {
		return (
			document.getElementById('toolbar-down').style.display !== 'none'
		);
	},
			...
			[
				{
					...
					func: function () {
						...
						if (typeof callback === 'function')
							dontClose = callback();
						if (!dontClose) that.closeModal(dialogId);
					},
				},
			],
			cancelButtonId,
	...
	showModalWithCustomButtons: function (
		id,
		title,
		message,
		cancellable,
	...
	enableTooltip: function (element) {
		...
		if (window.mode.isDesktop()) {
			...
			new Hammer(elem.get(0), { recognizers: [[Hammer.Press]] }).on(
				'press',
				function () {
					elem.tooltip('enable');
					elem.tooltip('open');
