...
L.Control.MenubarShortcuts = {
	...
	addShortcut: function (text, shortcut) {
		...
		var newText =
			_(text).replace('~', '') +
			' (' +
			L.Util.replaceCtrlAltInMac(shortcut) +
			')';
...
L.Control.Menubar = L.Control.extend({
	...
	options: {
		...
		text: [
			{
				...
				menu: [
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Save', 'text'),
							L.Control.MenubarShortcuts.shortcuts.SAVE,
						),
						id: 'save',
						type: 'action',
					},
					...
					{ type: 'separator' },
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Print', 'text'),
							L.Control.MenubarShortcuts.shortcuts.PRINT,
						),
						id: 'print',
						type: 'action',
					},
				...
				menu: [
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Undo', 'text'),
							L.Control.MenubarShortcuts.shortcuts.UNDO,
						),
						uno: '.uno:Undo',
					},
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Redo', 'text'),
							L.Control.MenubarShortcuts.shortcuts.REDO,
						),
						uno: '.uno:Redo',
					},
					...
					{ type: 'separator' },
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Cut', 'text'),
							L.Control.MenubarShortcuts.shortcuts.CUT,
						),
						uno: '.uno:Cut',
					},
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Copy', 'text'),
							L.Control.MenubarShortcuts.shortcuts.COPY,
						),
						uno: '.uno:Copy',
					},
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:Paste', 'text'),
							L.Control.MenubarShortcuts.shortcuts.PASTE,
						),
						uno: '.uno:Paste',
					},
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							...
							L.Control.MenubarShortcuts.shortcuts
								.PASTE_SPECIAL,
						),
				...
				menu: [
					...
					},
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							...
							L.Control.MenubarShortcuts.shortcuts.COMMENT,
						),
						id: 'insertcomment',
						type: 'action',
					},
					...
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:InsertEndnote', 'text'),
							L.Control.MenubarShortcuts.shortcuts.ENDNOTE,
						),
						...
						menu: [
							...
							},
							{
								name: L.Control.MenubarShortcuts.addShortcut(
									...
									L.Control.MenubarShortcuts
										.shortcuts.DOUBLE_UNDERLINE,
								),
								uno: '.uno:UnderlineDouble',
							},
							{
								name: L.Control.MenubarShortcuts.addShortcut(
									_UNO('.uno:Strikeout', 'text'),
									L.Control.MenubarShortcuts
		...
		presentation: [
			...
			},
			{
				...
				menu: [
					...
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:PasteSpecial', 'presentation'),
							L.Control.MenubarShortcuts.shortcuts
								.PASTE_SPECIAL,
						),
						uno: '.uno:PasteSpecial',
					...
					{
						name: L.Control.MenubarShortcuts.addShortcut(
							_UNO('.uno:SelectAll', 'presentation'),
							L.Control.MenubarShortcuts.shortcuts
								.SELECT_ALL,
						),
						uno: '.uno:SelectAll',
		...
		mobilespreadsheet: [
			{ name: _('Search'), id: 'searchdialog', type: 'action' },
			{
				...
				menu: [
					...
					{
						name: _('Define print area', 'spreadsheet'),
						uno: '.uno:DefinePrintArea',
					},
					{
						name: _('Remove print area', 'spreadsheet'),
						uno: '.uno:DeletePrintArea',
					},
				],
			},
		...
		allowedViewModeActions: [
			...
			!(L.Browser.ie || L.Browser.edge) ? 'fullscreen' : undefined,
			'zoomin',
			'zoomout',
			'zoomreset',
			'showstatusbar',
			...
			if (self._map.isEditMode()) {
				if (type === 'unocommand') {
					...
					if (unoCommand.startsWith(constLanguage)) {
						...
						languageAndCode =
							self._map[constState].getItemValue(
								unoCommand,
							);
					...
					if (id === 'fullscreen') {
						...
						if (
							self._map.uiManager.shouldUseNotebookbarMode()
						) {
						...
						if (
							!app.impress.isSlideHidden(
								self._map.getCurrentPartNumber(),
							)
						)
							$(aItem).hide();
						else $(aItem).show();
						...
						self._map.getDocType() === 'presentation' &&
						(id === 'deletepage' ||
							id === 'insertpage' ||
							id === 'duplicatepage')
					) {
						...
						var command =
							id === 'acceptalltrackedchanges'
								...
								: '.uno:RejectAllTrackedChanges';
						itemState =
							self._map['stateChangeHandler'].getItemValue(
								command,
							);
						if (itemState === 'disabled') {
							$(aItem).addClass('disabled');
				...
				if (type === 'unocommand') {
					...
					let allowed =
						self.options.allowedViewModeCommands.includes(
							uno,
						);
					if (!allowed) {
						$(aItem).addClass('disabled');
					...
					for (var i in self.options.allowedViewModeActions) {
						let action =
							self.options.allowedViewModeActions[i];
	...
	_getItems: function () {
		return $(this._menubarCont)
			.children()
			.children('ul')
			.children('li')
			.add($(this._menubarCont).children('li'));
	},
	...
	showUnoItem: function (targetId) {
		...
		if (items) {
			$(items).each(function () {
				if ($.inArray(this.id, menubar._hiddenItems) !== -1) {
					menubar._hiddenItems.splice(
						menubar._hiddenItems.indexOf(this.id),
						1,
					);
				}
			});
