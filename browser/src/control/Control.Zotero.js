...
L.Control.Zotero = L.Control.extend({
	...
	onFieldValue: function (fields) {
		...
		for (var index = 0; index < fields.length; index++) {
			...
			var fieldString;
			if (this.getFieldType() === 'Field') fieldString = field.command;
			...
			if (
				fieldString.startsWith('ZOTERO_BIBL') ||
				fieldString.startsWith('ADDIN ZOTERO_BIBL')
			) {
			...
			var citationString = app.util.trim(
				values.properties.plainCitation,
				this.settings.layout.prefix,
				this.settings.layout.suffix,
			);
			var citations = citationString.split(
				this.settings.layout.delimiter,
			);
			var itemUriList = [];
			values.citationItems.forEach(function (item, i) {
				...
				that.citations[citationId] = app.util.trim(
					citations[i],
					that.settings.group.prefix,
					that.settings.group.suffix,
				);
		...
		if (
			this.pendingCitationUpdate ||
			(this.previousNumberOfFields &&
	...
	askForApiKey: function () {
		...
		if (this.apiKey !== '')
			this.map.uiManager.showSnackbar(
				_('Zotero API key is not configured'),
			);
	},
	...
	updateUserID: function () {
		...
		fetch('https://api.zotero.org/keys/' + this.apiKey)
			...
			.then(
				...
				function () {
					that.map.uiManager.showSnackbar(
						_('Zotero API key is incorrect'),
					);
				},
			);
	},
	...
	showUnsupportedItemWarning: function (uriList) {
		if (this.showUnsupportedWarning === false || !this.userID) return;

		for (var i in uriList) {
			var uriObject = new URL(uriList[i]);

			var catagory = uriObject.pathname.startsWith('/groups/')
				? '/groups/'
				: '/users/';
			...
			if (!this.groupIdList.has(id) && id !== this.userID.toString()) {
				...
				this.map.uiManager.showInfoModal(
					'zoterounreachablewarn',
					_('Zotero Warning'),
					_(
						'The document contains some citations which may be unreachable through web API. It may cause some problems while editing citations or bibliography.',
					),
					null,
					_('Close'),
					function () {},
			...
			children: [
				...
				},
				{
					...
					entries: Array.from(
						...
						function (langCode) {
							return that.availableLanguages[langCode][0];
						},
					),
					selectedCount: '1',
					selectedEntries: [
						Object.keys(this.availableLanguages).indexOf(
			...
			return {
				data: {
					...
					control: {
						id: 'zoterolist',
						type: 'treelistbox',
						headers: headerArray.map(function (item) {
							return { text: item };
						}),
						entries: this.items,
					},
	...
	getZoteroItemQuery: function (limit, start) {
		var result =
			'?v=3&key=' +
			this.apiKey +
			'&include=data,citation,csljson&style=' +
			this.settings.style +
			'&locale=' +
			this.settings.locale;
	...
	_getDefaultSubCollections: function () {
		return [
			{
				columns: [{ text: _('My Publications') }],
				row:
					'https://api.zotero.org/users/' +
					this.userID +
					'/publications/items/top' +
					this.getZoteroItemQuery(),
			},
		];
	},
	...
	getDefaultCategories: function () {
		return [
			{
				columns: [{ text: _('My Library') }],
				row:
					'https://api.zotero.org/users/' +
					this.userID +
					'/items/top' +
					this.getZoteroItemQuery(this.itemsPerPage, 0),
				children: this._getDefaultSubCollections(),
			},
	...
	resetCitation: function () {
		...
		if (this.getFieldType() === 'Bookmark') this.bookmarksOrder = [];
	},

	setCitationNumber: function (number) {
		if (this.settings.citationFormat !== 'numeric') return;

		if (!this.settings.citationNumber) this.settings.citationNumber = 1;

		this.settings.citationNumber =
			this.settings.citationNumber <= parseInt(number)
				? parseInt(number) + 1
				: this.settings.citationNumber;
	},
	...
	getCitationText: function (citationId, text) {
		...
		if (this.settings.citationFormat === 'numeric') {
			...
			this.citations[citationId] = app.util.trim(
				text,
				this.settings.layout.prefix,
				this.settings.layout.suffix,
			);
		...
		return (
			this.settings.group.prefix +
			this.citations[citationId] +
			this.settings.group.suffix
		);
	},
	...
	handleCitationText: function (citationItems) {
		...
		var citationString = '';
		var citationNode = new DOMParser().parseFromString(
			citationItems[0].citation,
			'text/html',
		).body;

		var that = this;
		citationItems.forEach(function (item) {
			var itemHTML = new DOMParser().parseFromString(
				item.citation,
				'text/html',
			).body;
			...
			var keys = item['key']
				? [item['key']]
				: Object.keys(item['citationItems']).map(
						function (citationitem) {
							return that.extractItemKeyFromLink(
								item['citationItems'][citationitem].id,
							);
						},
					);
			keys.forEach(function (key) {
				citationString += innerMostNode
					? that.getCitationText(key, innerMostNode.textContent)
		...
		if (!innerText) citationNode.textContent = citationString;
		else innerText.textContent = citationString;
		return citationNode;
	},
	...
	getJSONfromCitationString: function (string) {
		...
		if (
			firstBraceIndex < 0 ||
			lastBraceIndex > string.length ||
			lastBraceIndex < 0
		)
			return {};

		var values = JSON.parse(
			string.substring(firstBraceIndex, lastBraceIndex + 1),
		);
	...
	getCitationJSONString: function (items) {
		...
		items.forEach(function (item) {
			if (item['citationItems']) {
				resultJSON['citationItems'] = resultJSON[
					'citationItems'
				].concat(item['citationItems']);
				return;
			}
			var citationItems = {
				id: item['csljson'].id,
				uris: Object.keys(item['links']).map(function (link) {
					return item['links'][link]['href'];
				}),
		...
		return {
			jsonString: JSON.stringify(resultJSON),
	...
	fillCategories: function () {
		...
		this.categories[0].children = this.categories[0].children.concat(
			this.collections.slice(),
		);
	...
	fillItems: function (items) {
		...
		for (var iterator = 0; iterator < items.length; ++iterator) {
			...
			for (
				var creator = 0;
				items[iterator].data.creators &&
				creator < items[iterator].data.creators.length;
				++creator
			) {
				creatorArray.push(
					items[iterator].data.creators[creator].name
						? items[iterator].data.creators[creator].name
						: items[iterator].data.creators[creator]
								.firstName +
								' ' +
								items[iterator].data.creators[creator]
									.lastName,
			...
			this.createEntry(
				index++,
				[
					items[iterator].csljson.title,
					creatorString,
					items[iterator].data.date
						? items[iterator].data.date
						: items[iterator].data.dateDecided,
				],
				{
					type: 'item',
					itemType: items[iterator].data.itemType,
					item: items[iterator],
				},
				true,
			);
	...
	fillStyles: function (styles) {
		...
		for (var iterator = 0; iterator < styles.length; ++iterator) {
			this.createEntry(
				iterator,
				[styles[iterator].title],
				Object.assign(
					{ name: styles[iterator].name, type: 'style' },
					styleToSelect === styles[iterator].name
						? { selected: true }
						: null,
				),
			);
	...
	showItemList: function () {
			...
			function (data) {
				for (var i = 0; i < data.length; i++) {
					that.groups.push({
						...
						id: data[i].data.id,
						row:
							data[i].links.self.href +
							'/items/top' +
							that.getZoteroItemQuery(),
					});
					that.fillCategories();
					that.updateCategories();
				}
		...
		fetch(
			...
			.then(function (response) {
				that.map.uiManager.showProgressBar(
					_('Loading references'),
					undefined,
					undefined,
					10000,
				);
				that.showItemsForUrl(
					'https://api.zotero.org/users/' +
						that.userID +
						'/items/top' +
						that.getZoteroItemQuery(that.itemsPerPage, 0),
					true,
				);

				that.totalItems = parseInt(
					response.headers.get('Total-Results'),
				);
				that.totalPages = Math.ceil(
					that.totalItems / that.itemsPerPage,
				);
				that._currentPage = 0;
	...
	asyncItemLoading: function () {
		...
		for (
			...
			that.getCachedOrFetch(
				'https://api.zotero.org/users/' +
					that.userID +
					'/items/top' +
					that.getZoteroItemQuery(that.itemsPerPage, i),
				true,
			).then(function () {
				++pagesLoaded;
	...
	showStyleList: function () {
		...
		this.getCachedOrFetch(
			...
			function (data) {
				...
				var dialogUpdateEvent = that.updateList(
					[_('Styles')],
					_('An error occurred while fetching style list'),
				);

				if (window.mode.isMobile())
					window.mobileDialogId = dialogUpdateEvent.data.id;
				that.map.fire('jsdialogupdate', dialogUpdateEvent);
				var styleToBeSelected =
					that.settings.style && that.settings.style !== ''
						? that.settings
						: {
								name: window.prefs.get(
									'Zotero_LastUsedStyle',
									'',
								),
							};
	...
	fetchCustomProperty: function (prefix) {
		app.socket.sendMessage(
			'commandvalues command=.uno:SetDocumentProperties?namePrefix=' +
				prefix,
		);
	},

	setFetchedCitationFormat: function (style) {
		if (!style) style = this.settings.style;
			...
			.then(function (html) {
				...
				for (var i = 0; i < categories.length; i++) {
					if (categories[i].getAttribute('citation-format')) {
						that.settings.citationFormat =
							categories[i].getAttribute(
								'citation-format',
							);
						break;
					}
				...
				if (citation) {
					...
					for (var i = 0; i < link.length; i++) {
						if (
							link[i].getAttribute('rel') ===
							'independent-parent'
						) {
							that.setFetchedCitationFormat(
								that.extractItemKeyFromLink(
	...
	checkStyleTypeAndEnableOK: function (selectedStyle) {
		...
		fetch('https://www.zotero.org/styles/' + style)
			...
			.then(function (html) {
				...
				if (style.length) {
					var defaultLocale =
						style[0].getAttribute('default-locale');
					if (defaultLocale && defaultLocale !== '')
						defaultLocale =
							that.makeLanguageAvailable(defaultLocale);
				}

				var availableLocale =
					Object.keys(that.availableLanguages).indexOf(
						that.settings.locale,
					) >= 0;
				...
				that.enableDialogFieldTypeCombobox(
					selectedStyle.citationFormat,
				);
	...
	setCitationLayout: function (ciatationNode) {
		...
		this.settings.layout['prefix'] =
			layout && layout.getAttribute('prefix')
				? layout.getAttribute('prefix')
				: '';
		this.settings.layout['suffix'] =
			layout && layout.getAttribute('suffix')
				? layout.getAttribute('suffix')
				: '';
		this.settings.layout['delimiter'] =
			layout && layout.getAttribute('delimiter')
				? layout.getAttribute('delimiter')
				: '';

		var group = layout.getElementsByTagName('group')[0];
		this.settings.group = {};
		this.settings.group['prefix'] =
			group && group.getAttribute('prefix')
				? group.getAttribute('prefix')
				: '';
		this.settings.group['suffix'] =
			group && group.getAttribute('suffix')
				? group.getAttribute('suffix')
				: '';
		this.settings.group['delimiter'] =
			group && group.getAttribute('delimiter')
				? group.getAttribute('delimiter')
				: '';
	},
	...
	setFetchedStyle: function (valueString) {
		...
		if (locale) this.settings.locale = locale;
		...
		else this.settings.fieldType = this.getFieldType();

		var noteTypeNode = value.getElementsByName('noteType');
		if (noteTypeNode && noteTypeNode[0])
			this.settings.wrapper =
				noteTypeNode[0].getAttribute('value') === '1'
					? 'Footnote'
					: 'Endnote';

		this.settings.hasBibliography =
			styleNode.getAttribute('hasBibliography');
		this.settings.bibliographyStyleHasBeenSet = styleNode.getAttribute(
	...
	getStyleXml: function () {
		...
		styleNode.setAttribute(
			'id',
			'http://www.zotero.org/styles/' + this.settings.style,
		);
		...
		styleNode.setAttribute(
			'bibliographyStyleHasBeenSet',
			this.settings.bibliographyStyleHasBeenSet,
		);
		...
		if (this.settings.wrapper) {
			...
			else prefNotedNode.setAttribute('value', '2');
	...
	markBibliographyStyleHasBeenSet: function (unset) {
		if (unset) this.settings.bibliographyStyleHasBeenSet = '0';
		else this.settings.bibliographyStyleHasBeenSet = '1';
	...
	getFieldType: function () {
		if (this.settings.fieldType) return this.settings.fieldType;

		var fileExtension = this.map['wopi'].BaseFileName.substring(
			this.map['wopi'].BaseFileName.lastIndexOf('.') + 1,
		);
		this.settings.fieldType = fileExtension.startsWith('doc')
			? 'Field'
			: 'ReferenceMark';
	...
	setFieldType: function (field, format) {
		this.settings.wrapper = '';
		if (field === 'Bookmarks') this.settings.fieldType = 'Bookmark';
		else {
			var fileExtension = this.map['wopi'].BaseFileName.substring(
				this.map['wopi'].BaseFileName.lastIndexOf('.') + 1,
			);
			...
			if (format === 'note') {
				...
				if (field === 'Endnotes') this.settings.wrapper = 'Endnote';
			}
		}
	},

	_findEntryWithUrlImpl: function (entry, url) {
		if (entry.row === url) return entry;
	...
	_fetchCollectionAndPushTo: function (collectionId, targetCollection) {
		...
		this.getCachedOrFetch(
			...
			function (data) {
				...
				if (!data.length) targetCollection.children = undefined;
				that.fillCategories();
				that.updateCategories();
			},
			function () {
	...
	showItemsForUrl: function (url, checkState) {
		var that = this;
		that.items = [];
		this.getCachedOrFetch(url).then(
			function (data) {
				that.fillItems(data);
				var dialogUpdateEvent = that.updateList(
					[_('Title'), _('Creator(s)'), _('Date')],
	...
	_onAction: function (element, action, data, index) {
		...
		if (element === 'treeview') {
			...
			if (data.id === 'zoterocategory') {
				...
				if (this.dialogType === 'itemlist') {
					...
					this.selected = data.entries.filter(function (e) {
						return e.state === true;
					});
		...
		if (data.id == 'ok') {
			...
			if (!this.selected || this.selected.type === 'style') {
				var citationFormat = this.selected
					? this.selected.citationFormat
					: this.settings.citationFormat;
				var parameters = this.selected
					? this.selected
					: {
							name: window.prefs.get(
								'Zotero_LastUsedStyle',
		...
		if (element === 'pushbutton' && data.id === 'zoterorefresh') {
			this._cachedURL = [];
			if (this.dialogType === 'itemlist') this.showItemList();
			else if (this.dialogType === 'insertnote')
				this.handleInsertNote();
			else this.showStyleList();
			return;
		}
		if (element === 'combobox') {
			if (data.id === 'zoterolocale') {
				this.selectedCitationLangCode = Object.keys(
					this.availableLanguages,
				)[parseInt(index)];
				if (this.settings.style) this.enableDialogOKButton();
			} else if (data.id === 'zoterotype') {
				this.selectedFieldType = index.substring(
					index.indexOf(';') + 1,
				);
			}
			return;
	...
	_onOk: function (selected) {
		if (this.dialogType === 'itemlist') {
			...
			for (var i in selected) {
				citationsToInsert = citationsToInsert.concat([
					selected[i].item,
				]);
			}
			...
			if (this.map._clip) {
				this.map._clip.dataTransferToDocumentFallback(
					null,
					selected.item.data.note,
				);
			} else console.warn('zotero: cannot paste a note');
		}
	},
	...
	handleInsertNote: function () {
		...
		var updateDialog = function () {
			var dialogUpdateEvent = that.updateList(
				[_('Notes')],
				_('An error occurred while fetching notes'),
			);
			if (window.mode.isMobile())
				window.mobileDialogId = dialogUpdateEvent.data.id;
			that.map.fire('jsdialogupdate', dialogUpdateEvent);
			...
			that.getCachedOrFetch(
				...
				for (var i = 0; i < data.length; i++) {
					var groupUrl =
						...
						'/items' +
						that.getZoteroItemQuery();
					that.getCachedOrFetch(groupUrl).then(function (data) {
						notesCount += that.fillNotes(data, notesCount);
						updateDialog();
					});
				}
	...
	handleInsertBibliography: function () {
		...
		if (!(this.citations && Object.keys(this.citations))) return;

		if (this.getCitationKeys().length === 0) return;
		...
		if (this.settings.citationFormat === 'numeric') {
			this.getCachedOrFetch(
				'https://api.zotero.org/users/' +
					this.userID +
					'/items?include=bib&itemKey=' +
					this.getCitationKeysForBib().join(',') +
					'&v=3&key=' +
					this.apiKey +
					'&style=' +
					this.settings.style +
				...
				that.getCitationKeysForBib().forEach(function (key) {
					var bib = new DOMParser().parseFromString(
						bibList[key],
						'text/html',
					).body;
					var numberNode =
						bib.getElementsByClassName('csl-entry')[0]
							.firstElementChild;
					var number = parseInt(
						numberNode.textContent.substring(
							numberNode.textContent.search(/[0-9]/),
						),
					).toString();
					numberNode.textContent =
						numberNode.textContent.replace(
							number,
							that.citations[key],
						);
					html += bib.innerHTML;
		...
		fetch(
			'https://api.zotero.org/users/' +
				this.userID +
				'/items?format=bib&itemKey=' +
				this.getCitationKeys().join(',') +
				'&v=3&key=' +
				this.apiKey +
				'&style=' +
				this.settings.style +
	...
	updateFieldsList: function () {
		if (this.getFieldType() === 'Field')
			app.socket.sendMessage(
				'commandvalues command=.uno:TextFormFields?type=vnd.oasis.opendocument.field.UNHANDLED&commandPrefix=ADDIN%20ZOTERO_ITEM%20CSL_CITATION',
			);
		else if (this.getFieldType() === 'ReferenceMark')
			app.socket.sendMessage(
				'commandvalues command=.uno:Fields?typeName=SetRef&namePrefix=ZOTERO_ITEM%20CSL_CITATION',
			);
		else if (this.getFieldType() === 'Bookmark')
			app.socket.sendMessage(
				'commandvalues command=.uno:Bookmarks?namePrefix=ZOTERO_BREF_',
			);
	},
	...
	updateBibList: function () {
		if (this.getFieldType() === 'Field')
			app.socket.sendMessage(
				'commandvalues command=.uno:TextFormFields?type=vnd.oasis.opendocument.field.UNHANDLED&commandPrefix=ADDIN%20ZOTERO_BIBL',
			);
		else if (this.getFieldType() === 'ReferenceMark')
			app.socket.sendMessage(
				'commandvalues command=.uno:Sections?typeName=SetRef&namePrefix=ZOTERO_BIBL',
			);
		else if (this.getFieldType() === 'Bookmark')
			app.socket.sendMessage(
				'commandvalues command=.uno:SetDocumentProperties?namePrefix=' +
					this.bibBookmarkName,
			);
	},
		...
		this.getCachedOrFetch(
			...
			function (data) {
				...
				Object.keys(citationCluster).forEach(function (clusterKey) {
					var itemList = citationCluster[clusterKey].reduce(
						function (list, item) {
					...
					var citationData =
						that.getCitationJSONString(itemList);
					var updateParameter = that.getFieldParameters(
						citationData.jsonString,
						citationData.citationString,
					);
					newValues.push(
						that.getFieldType() !== 'Bookmark'
							? updateParameter
							: {
									parameter: updateParameter,
									cslJSON: citationData.jsonString,
								},
					);
				});
	...
	unlinkCitations: function () {
		this.map.uiManager.showInfoModal(
			...
			_('Unlink Citations'),
			_(
				'Unlinking citations will prevent Free Online Office from updating citations and bibliography in this document.',
			),
			null,
			_('Confirm'),
			this.handleUnlinkCitations.bind(this),
		);
	},
	...
	handleInsertCitation: function () {
		...
		if (!this.fetchCitationUnderCursor()) this.insertCitation({}); //temp fix until exitsting citations can be fetched for every field type
	},

	fetchCitationUnderCursor: function () {
		if (this.getFieldType() === 'Field')
			app.socket.sendMessage(
				'commandvalues command=.uno:TextFormField?type=vnd.oasis.opendocument.field.UNHANDLED&commandPrefix=ADDIN%20ZOTERO_ITEM',
			);
		else if (this.getFieldType() === 'ReferenceMark')
			app.socket.sendMessage(
				'commandvalues command=.uno:Field?typeName=SetRef&namePrefix=ZOTERO_ITEM%20CSL_CITATION',
			);
		else if (this.getFieldType() === 'Bookmark')
			app.socket.sendMessage(
				'commandvalues command=.uno:Bookmark?namePrefix=ZOTERO_BREF_',
			);
		else return false;
	...
	handleFieldUnderCursor: function (field) {
		if (!this.pendingItemListing) return;
		...
		var existingCitationUnderCursor =
			this.getJSONfromCitationString(cslString);
		this.insertNewCitation = !Object.keys(existingCitationUnderCursor)
			.length;
		...
		var dialogUpdateEvent = this.updateList(
			[_('Title'), _('Creator(s)'), _('Date')],
			_('Your library is empty'),
		);
	...
	insertCitation: function (citationsToInsert) {
		if (!(citationsToInsert && citationsToInsert.length)) return;

		var command = '';
		if (this.getFieldType() === 'Field') {
			command = this.insertNewCitation
				? '.uno:TextFormField'
				: '.uno:UpdateTextFormField';
		} else if (this.getFieldType() == 'Bookmark') {
			command = this.insertNewCitation
				? '.uno:InsertBookmark'
				: '.uno:UpdateBookmark';
		...
		var parameters = this.insertNewCitation
			? this.getFieldParameters(
					citationData.jsonString,
					citationData.citationString,
				)
			: this.getFieldUpdateParameters(
					citationData.jsonString,
					citationData.citationString,
				);

		if (this.getFieldType() == 'Bookmark') {
			var prefix = this.insertNewCitation
				? parameters['Bookmark'].value
				: parameters['Bookmark']['value']['Bookmark'].value;
			this.setCustomProperty(
				prefix + '_',
				'ZOTERO_ITEM CSL_CITATION ' + citationData.jsonString,
			);
		}
	...
	sendUpdateCitationCommand: function (newValueArray) {
		...
		if (this.getFieldType() === 'Field') {
			...
			updatedCitations = {
				...
				Bookmarks: {
					type: '[][]com.sun.star.beans.PropertyValue',
					value: newValueArray.map(function (value) {
			...
			newValueArray.forEach(function (value) {
				that.setCustomProperty(
					value.parameter['Bookmark'].value + '_',
					'ZOTERO_ITEM CSL_CITATION ' + value.cslJSON,
				);
			});
			this.map.sendUnoCommand(
				'.uno:UpdateBookmarks',
	...
	getBibParameters: function (html) {
		...
		if (this.getFieldType() === 'Field') {
			...
			field['FieldCommand'] = {
				...
				value:
					'ADDIN ZOTERO_BIBL ' +
					JSON.stringify(this.settings.bib) +
					' CSL_BIBLIOGRAPHY',
			};
	...
	setCustomProperty: function (prefix, string) {
		...
		for (
			var start = 0, end = 1;
			end * 255 < string.length + 255;
			start++, end++
		) {
			property['UpdatedProperties']['value']['UserDefinedProperties'][
				'value'
			][prefix + end] = {
				type: 'string',
	...
	handleCustomProperty: function (userDefinedProperties) {
		...
		for (var i = 0; i < userDefinedProperties.length; i++) {
			prefixList.add(
				userDefinedProperties[i].name.substring(
					0,
					userDefinedProperties[i].name.lastIndexOf('_'),
				),
			);
			nameValueMap[userDefinedProperties[i].name] =
				userDefinedProperties[i].value;
		}
		...
		prefixList.forEach(function (prefix) {
			for (i = 1; nameValueMap[prefix + '_' + i]; i++) {
				...
				else resultMap[prefix] = nameValueMap[prefix + '_' + i];
			}
		});
		...
		// update existing bookmark citation
			...
			resultKeys.length &&
			resultKeys[0].startsWith('ZOTERO_BREF')
		) {
			this.handleFieldUnderCursor({
				bookmark: resultMap[resultKeys[0]],
			});
	...
	setBibCustomProperty: function (bibProperty) {
		...
		if (
			firstBraceIndex < 0 ||
			lastBraceIndex > bibProperty.length ||
			lastBraceIndex < 0
		)
			return {};

		var values = JSON.parse(
			bibProperty.substring(firstBraceIndex, lastBraceIndex + 1),
		);
