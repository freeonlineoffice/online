...
L.Map.include({
	...
	disableLockedItem: function (item, DOMParentElement, buttonToDisable) {
		if (this.isLockedUser() && this.isLockedItem(item)) {
			...
			if (window.mode.isMobile()) {
				...
				var lock = L.DomUtil.create(
					'img',
					'locking-overlay-lock',
					overlay,
				);
	...
	openUnlockPopup: function (cmd) {
		if (
			(this.isRestrictedUser() && this.isRestrictedItem(cmd)) ||
			this.uiManager.isAnyDialogOpen()
		)
			return;
		var message = [
			'<div class="container">',
		...
		highlights.forEach(function (highlight) {
			if (highlight) message.push('<li>' + highlight + '</li>');
		});
		...
		this.uiManager.showInfoModal(
			...
			function () {
				...
				this.uiManager.closeModal(
					this.uiManager.generateModalId(
						'unlock-features-popup',
					),
				);
			}.bind(this),
		...
		var paraTag = document
			.getElementById('unlock-features-popup')
			.querySelectorAll('p')[0];
		if (paraTag) paraTag.outerHTML = message;
		else {
	...
	_setRestrictions: function (restrictionInfo) {
		this.Restriction.isRestrictedUser =
			!!restrictionInfo['IsRestrictedUser'];
		this.Restriction.restrictedCommandList =
			restrictionInfo['RestrictedCommandList'];
	},
	...
	isRestrictedItem: function (item) {
		...
		for (var i in commands) {
			if (
				this.Restriction.restrictedCommandList.indexOf(
					commands[i],
				) >= 0
			)
				return true;
	...
	_extractCommand: function (item) {
		...
		var commandArray = [];
		if (item.lockUno || item.uno) {
			// in classic mode uno commands are stored as uno in menus
			var uno = item.lockUno ? item.lockUno : item.uno;
			if (typeof uno === 'string') commandArray.push(uno);
			else {
				// some unos have multiple commands
				commandArray.push(uno.textCommand);
				commandArray.push(uno.objectCommand);
				if (item.unosheet) commandArray.push(item.unosheet);
			}
		} else if (item.command)
			// in notebookbar uno commands are stored as command
			commandArray.push(item.command);
		else if (item.id) commandArray.push(item.id);
		else if (typeof item === 'string') commandArray.push(item);
