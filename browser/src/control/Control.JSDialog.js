...
L.Control.JSDialog = L.Control.extend({
	...
	close: function (id, sendCloseEvent) {
		if (id && this.dialogs[id]) {
			if (
				!sendCloseEvent &&
				this.dialogs[id].overlay &&
				!this.dialogs[id].isSubmenu
			)
				L.DomUtil.remove(this.dialogs[id].overlay);
	...
	closePopover: function (id, sendCloseEvent) {
		...
		if (sendCloseEvent) {
			...
			else console.warn('closePopover: no builder');
		} else {
			this.clearDialog(id);
		}
	...
	createContainer: function (instance, parentContainer) {
		...
		instance.form.addEventListener('submit', function (event) {
	...
	createDialog: function (instance) {
		instance.builder = new L.control.jsDialogBuilder({
			...
			cssClass:
				'jsdialog' +
				(instance.isAutofilter ? ' autofilter' : '') +
	...
	addHandlers: function (instance) {
		...
		if (tabControlWidget) {
			...
			focusWidget = instance.init_focus_id
		...
		if (
			focusWidget &&
			document.activeElement !== focusWidget &&
			instance.canHaveFocus
		) {
	...
	calculateAutoFilterPosition: function (instance) {
		...
		var offsetY = app.sectionContainer.getSectionWithName(
		...
		if (splitPanesContext) splitPos = splitPanesContext.getSplitPos();

		var newLeft = left + panePos.x - origin.x;
		if (left >= splitPos.x && newLeft >= 0) left = newLeft;

		var newTop = top + panePos.y - origin.y;
		if (top >= splitPos.y && newTop >= 0) top = newTop;

		if (isSpreadsheetRTL) left = this.map._size.x - left;
		...
		var autoFilterBottom =
			instance.posy +
			canvasEl.top +
			instance.form.getBoundingClientRect().height;
		...
		if (autoFilterBottom > canvasBottom)
			instance.posy =
				instance.posy - (autoFilterBottom - canvasBottom);
	...
	calculateSubmenuAutoFilterPosition: function (instance, parentAutofilter) {
		...
		instance.posy =
			parentAutofilter.top -
			this.map._docLayer._canvas.getBoundingClientRect().top;
	...
	onJSDialog: function (e) {
		...
		*/
		...
		instance.isPopup =
			...
			instance.isDocumentAreaPopup ||
			instance.isSnackbar;
		instance.containerParent = instance.isDocumentAreaPopup
		...
		instance.haveTitlebar =
			(!instance.isModalPopUp && !instance.isSnackbar) ||
			(instance.hasClose && instance.title && instance.title !== '');
		instance.nonModal =
			!instance.isModalPopUp &&
			!instance.isDocumentAreaPopup &&
			!instance.isSnackbar;
		...
		if (instance.action === 'fadeout') {
			...
			if (dialogs.length) {
				...
				var initialFocusElement =
					JSDialog.GetFocusableElements(container);
