...
L.Control.AlertDialog = L.Control.extend({
	...
	_onError: function (e) {
		if (!this._map._fatal && e.cmd !== 'notasync' && e.type !== 'warn') {
		...
		if (e.msg) {
			...
			this._map.uiManager.showInfoModal(
				...
				messageText,
				url,
				isLinkValid ? _('Open link') : _('OK'),
				function () {
					if (!isLinkValid) return;
					if ('procesLoolUrl' in window) {
			...
			if (e.cmd == 'paste') {
				...
				var message1 = _(
					'It seems you have copied a selection that includes external images.',
				);
				var message2 = _(
					'Downloading external resources is forbidden but pasting images is still possible. Please right click in the image, choose "Copy Image" and paste it into the document instead.',
				);
			} else {
				...
				message1 = _(
					'It seems you have tried to insert external data.',
				);
				message2 = _(
					'Selected external data source is forbidden. Please contact the system administrator.',
				);
			}
		...
		} else if (e.cmd == 'notasync') {
			// developer only no translation needed
			this._map.uiManager.showInfoModal(
				'lool_alert',
				'',
				'This dialog is non-async',
				'',
				_('Close'),
				function () {
					/* Do nothing. */
