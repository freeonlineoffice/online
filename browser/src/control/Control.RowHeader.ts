	...
	drawHeaderEntry (entry: HeaderEntryData): void {
		...
		onInitialize(): void {
			...
			this._menuItem = {
				...
				'.uno:InsertRowsAfter': {
					name: app.IconUtil.createMenuItemLink(
						_UNO('.uno:InsertRowsAfter', 'spreadsheet', true),
						'InsertRowsAfter',
					),
					isHtmlName: true,
					callback: this._insertRowBelow.bind(this),
				...
				'.uno:DeleteRows': {
					name: app.IconUtil.createMenuItemLink(
						_UNO('.uno:DeleteRows', 'spreadsheet', true),
						'DeleteRows',
					),
					isHtmlName: true,
					callback: this._deleteSelectedRow.bind(this),
				...
				'.uno:RowHeight': {
					name: app.IconUtil.createMenuItemLink(
						_UNO('.uno:RowHeight', 'spreadsheet', true),
						'RowHeight',
					),
					isHtmlName: true,
					callback: this._rowHeight.bind(this),
				...
				'.uno:HideRow': {
					name: app.IconUtil.createMenuItemLink(
						_UNO('.uno:HideRow', 'spreadsheet', true),
						'HideRow',
					),
					isHtmlName: true,
					callback: this._hideRow.bind(this),
				...
				'.uno:ShowRow': {
					name: app.IconUtil.createMenuItemLink(
						_UNO('.uno:ShowRow', 'spreadsheet', true),
						'ShowRow',
					),
					isHtmlName: true,
					callback: this._showRow.bind(this),
		...
		drawHeaderEntry(entry: HeaderEntryData): void {
			...
			this.context.fillText(
				content.toString(),
				this.size[0] / 2,
				entry.pos - entry.size / 2 + app.roundedDpiScale,
			);
		...
		getHeaderEntryBoundingClientRect(index: number): Partial<DOMRect> {
			...
			if (index) entry = this._headerInfo.getRowData(index);
		...
		onDragEnd(dragDistance: number[]): void {
			...
			const nextRow = this._headerInfo.getNextIndex(
			...
			height /= app.dpiScale;
			height = this._map._docLayer._pixelsToTwips({
				x: 0,
				y: height,
			}).y;
...
app.definitions.rowHeader = lool.RowHeader;
