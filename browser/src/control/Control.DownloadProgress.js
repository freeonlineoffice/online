...
L.Control.DownloadProgress = L.Control.extend({
	...
	_getLargeCopyPasteMessage: function () {
		return this._map._clip._substProductName(
			_(
				"If you want to share large elements outside of {productname} it's necessary to first download them.",
			),
		);
	},
	...
	_showLargeCopyPasteWarning: function (inSnackbar) {
		...
		var buttonText =
			_('Download') + L.Util.replaceCtrlAltInMac(' (Ctrl + C)');

		if (inSnackbar) {
			this._map.uiManager.showSnackbar(
				msg,
				buttonText,
				this._onStartDownload.bind(this),
	...
	_showDownloadProgress: function (inSnackbar) {
		...
		if (inSnackbar) {
			...
			buttonText =
				_('Copy') + L.Util.replaceCtrlAltInMac(' (Ctrl + C)');

			this._map.uiManager.showProgressBarDialog(
				modalId,
				this._getDialogTitle(),
				msg,
				buttonText,
				this._onConfirmCopyAction.bind(this),
				0,
				this._onClose.bind(this),
			);
	...
	_closeDownloadProgressDialog: function () {
		...
		if (!this._userAlreadyWarned())
			this._map.uiManager.closeModal(
				this._map.uiManager.generateModalId(modalId),
			);
	},
	...
	_showDownloadComplete: function (inSnackbar) {
		var modalId = this._getDownloadProgressDialogId();
		var snackbarMsg = _(
			'Download completed and ready to be copied to clipboard.',
		);
		var dialogMsg =
			snackbarMsg +
			' ' +
	...
	show: function (isLargeCopy) {
		...
		else this._showDownloadProgress(this._userAlreadyWarned());
	},
	...
	currentStatus: function () {
		...
		if (!this._started && !this._complete) return 'downloadButton';
		if (this._started) return 'progress';
		if (this._complete) return 'confirmPasteButton';
	},
	...
	setValue: function (value) {
		if (this._userAlreadyWarned())
			...
			var modalId = this._getDownloadProgressDialogId();
			this._map.uiManager.setDialogProgress(
				modalId,
				Math.round(value),
			);
		}
	...
	_onComplete: function () {
		...
		else this._closeDownloadProgressDialog();
	},
	...
	_onClose: function () {
		if (this._userAlreadyWarned()) this._map.uiManager.closeSnackbar();

		if (this._started) this._closeDownloadProgressDialog();
	...
	_cancelDownload: function () {
		this._setNormalCursor();
		if (!this._started || this._complete) return;
