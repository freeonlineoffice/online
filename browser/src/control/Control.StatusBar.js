...
class StatusBar extends JSDialog.Toolbar {
	...
	onPermissionChanged(event) {
		...
		if (
			...
			NotEditDocMode =
				app.map['stateChangeHandler'].getItemValue('EditDoc') ===
				'false'; // can be true, false or disabled
			if (NotEditDocMode)
	...
	onCommandStateChanged(e) {
		...
		if (commandName === '.uno:StatusDocPos') {
			...
			state = state !== 'false';
			$('#permissionmode-container').attr(
				'default-state',
				this.map.isEditMode() || null,
			);
	...
	updateDefaultStateAttribute() {
		...
		if (
			...
			(!documentStatus.textContent ||
				documentStatus.textContent.trim() === '')
		) {
