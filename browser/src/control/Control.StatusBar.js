...
class StatusBar extends JSDialog.Toolbar {
	constructor(map) {
		...
		app.events.on(
			'updatepermission',
			this.onPermissionChanged.bind(this),
		);
		...
		map.on(
			...
			this.onInitModificationIndicator,
			this,
		);
		map.on(
			'updatemodificationindicator',
			this.onUpdateModificationIndicator,
			this,
		);
	}
	...
	localizeStateTableCell(text) {
		...
		for (var i = 0; i < stateArrayLength; i++) {
			...
			localizedText +=
				_(labelValuePair[0].trim()) + ':' + labelValuePair[1];
	...
	toLocalePattern(pattern, regex, text, sub1, sub2) {
		...
		if (matches) {
			text = pattern
				...
				.replace(
					sub1,
					parseInt(matches[1].replace(/,/g, '')).toLocaleString(
						String.locale,
					),
				)
				.replace(
					sub2,
					parseInt(matches[2].replace(/,/g, '')).toLocaleString(
						String.locale,
					),
				);
		}
	...
	callback(objectType, eventType, object, data, builder) {
		if (object.id === 'search-input' || object.id === 'search') {
			...
			var selected = this._generateZoomItems().filter((item) => {
				return item.id === data;
			});
			if (selected.length)
				this.map.setZoom(
			...
			var selected = [];
			if (data === '1') {
				// 'None' was clicked, remove all other options
				selected = ['1'];
			} else {
				// Something else was clicked, remove the 'None' option from the array
				selected = [data];
			}
	...
	onZoomEnd() {
		...
		switch (this.map.getZoom()) {
			...
			case 10:
				zoomPercent = 100;
				zoomSelected = 'zoom100';
				break; // 1
			case 11:
				zoomPercent = 120;
				zoomSelected = 'zoom120';
				break; // 1.1892
			// Why do we call this 150% even if it is actually closer to 140%
			case 12:
				zoomPercent = 150;
				zoomSelected = 'zoom150';
				break; // 1.4142
			case 13:
				zoomPercent = 170;
				zoomSelected = 'zoom170';
				break; // 1.6818
			case 14:
				zoomPercent = 200;
				zoomSelected = 'zoom200';
				break; // 2
			case 15:
				zoomPercent = 235;
				zoomSelected = 'zoom235';
				break; // 2.3784
			case 16:
				zoomPercent = 280;
				zoomSelected = 'zoom280';
				break; // 2.8284
			case 17:
				zoomPercent = 335;
				zoomSelected = 'zoom335';
				break; // 3.3636
			case 18:
				zoomPercent = 400;
				zoomSelected = 'zoom400';
				break; // 4
			default:
				var zoomRatio = this.map.getZoomScale(
					this.map.getZoom(),
	...
	_generateStateTableCellMenuItem(value, visible) {
		...
		var selected = submenu.filter((item) => {
			return item.id === value;
		});
		var text = selected.length ? selected[0].text : _('None');
		return {
			type: 'menubutton',
			id: 'StateTableCellMenu',
			text: text,
			image: false,
			menu: submenu,
			visible: visible,
	...
	onDocLayerInit() {
		...
		var language = app.map['stateChangeHandler'].getItemValue(
			'.uno:LanguageStatus',
		);
		if (language)
			this.updateLanguageItem(
				this.extractLanguageFromStatus(language),
			);
	...
	updateLanguageItem(language) {
		...
		this.builder.updateWidget(this.parentContainer, {
			type: 'menubutton',
			id: 'languagestatus:LanguageStatusMenu',
			noLabel: false,
			text: language,
		});
		JSDialog.RefreshScrollables();
	...
	showSigningItem(icon, text) {
		this.builder.updateWidget(this.parentContainer, {
			...
			command: '.uno:Signature',
			w2icon: icon,
			text: text ? text : _UNO('.uno:Signature'),
		});
		JSDialog.RefreshScrollables();
	}
	...
	onPermissionChanged(event) {
		...
		var canUserWrite = window.ThisIsAMobileApp
			? !app.isReadOnly()
			: this.map['wopi'].UserCanWrite;
		var EditDocMode = true;
		if (
			app.map['stateChangeHandler'].getItemValue('EditDoc') !==
			undefined
		) {
			EditDocMode =
				app.map['stateChangeHandler'].getItemValue('EditDoc') ===
			...
			if (!EditDocMode)
				app.map.uiManager.showSnackbar(
					_(
						'To prevent accidental changes, the author has set this file to open as view-only',
					),
				);
		}
		...
		var permissionContainer = document.getElementById(
			'permissionmode-container',
		);
		if (permissionContainer) {
			while (permissionContainer.firstChild)
				permissionContainer.removeChild(
					permissionContainer.firstChild,
				);
			permissionContainer.appendChild(
				getPermissionModeElements(isReadOnlyMode, canUserWrite),
			);
		}
	...
	extractLanguageFromStatus(state) {
		...
		if (split.length > 1) language = _(split[0]);
		return language;
	}
		...
		if (commandName === '.uno:StatusDocPos') {
			...
			this.updateHtmlItem(
				...
				state
					...
					: _('Insert mode: inactive'),
				!state,
			);

			$('#InsertMode').removeClass();
			$('#InsertMode').addClass(
				'jsdialog ui-badge insert-mode-' + state,
			);

			if (
				(state === 'false' || !state) &&
				app.definitions.urlPopUpSection.isOpen()
			) {
			...
			this.builder.updateWidget(
				this.parentContainer,
				this._generateStateTableCellMenuItem(state, true),
			);
			JSDialog.RefreshScrollables();
	...
	onLanguagesUpdated() {
		...
		for (var lang in languages) {
			...
			menuEntries.push({
				...
				uno:
					constLang +
					encodeURIComponent('Default_' + splitNeutral[0]),
			});
		}
		...
		menuEntries.push({
			id: 'reset',
			text: resetLang,
			uno: constLang + constDefault,
		});
		menuEntries.push({
			id: 'morelanguages',
			action: 'morelanguages-all',
			text: _('Set Language for All text'),
		});
		...
		if (this.map.getDocType() === 'text') {
			menuEntries.push({
				...
				text: _('Set Language for Paragraph'),
			});
			menuEntries.push({
				id: 'langselection',
				action: 'morelanguages-selection',
				text: _('Set Language for Selection'),
			});
		}
	...
	onInitModificationIndicator(lastmodtime) {
		if (!this.isSaveIndicatorActive()) return;

		const docstatcontainer = document.getElementById(
			'documentstatus-container',
		);
		if (lastmodtime == null) {
			if (
				docstatcontainer !== null &&
				docstatcontainer !== undefined
			) {
