...
JSDialog.comboboxEntry = function (parentContainer, data, builder) {
	...
	if (data.icon) {
		...
		builder._isStringCloseToURL(data.icon)
			...
			: app.LOUtil.setImage(
					icon,
					app.LOUtil.getIconNameOfCommand(data.icon),
					builder.map,
				);
	}
...
JSDialog.combobox = function (parentContainer, data, builder) {
	...
	content.addEventListener('keyup', function (event) {
		const shouldTriggerChange = data.changeOnEnterOnly
			? event.key === 'Enter'
			: true;
		...
		if (
			...
			const selectedElement =
				comboboxEntries.querySelector('.selected');
			if (selectedElement) {
