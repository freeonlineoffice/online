...
class TreeViewControl {
	...
	fillHeaders(headers: Array<TreeHeaderJSON>, builder: any) {
		...
		this._thead.style.gridColumn =
			'1 / ' + (this._columns + dummyCells + 1);
	...
	preprocessColumnData(entires: Array<TreeEntryJSON>) {
		...
		entires.forEach((entry: TreeEntryJSON) => {
			...
			const missingColumns =
				columnTypes.length - existingColumns.length;
			...
			while (targetIndex < columnTypes.length) {
				const isExistingColumn =
					existingIndex < existingColumns.length;
				...
				if (
					!isExistingColumn ||
					currentType !== columnTypes[targetIndex]
				) {
