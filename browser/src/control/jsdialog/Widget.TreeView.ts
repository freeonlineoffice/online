...
class TreeViewControl {
	...
	changeCheckboxStateOnClick(
		...
		if (checkbox.checked) {
			foundEntry = this.findEntryWithRow(
				treeViewData.entries,
				entry.row,
			);
	...
	fillCells(
		...
		for (const index in entry.columns) {
			...
			if (img) {
				...
				link.href =
					entry.columns[index].link || entry.columns[index].text;
				...
				innerText.innerText =
					entry.columns[index].text || entry.text;
			}
...
class TreeViewFactory {
	...
	fillHeaders(headers: Array<TreeHeaderJSON>, builder: any) {
		...
		for (const index in headers) {
			...
			var clickFunction = (
				...
				var clearSorting = () => {
					var icons =
						this._implementation._thead.querySelectorAll(
			...
			const last = this._implementation._thead
		...
		for (const index in entries) {
			...
			if (entries[index].children && entries[index].children.length) {
				...
				subGrid.style.gridColumn =
					'1 / ' +
					(this._implementation._columns + dummyColumns + 1);
	...
	build(data: TreeWidget, builder: any, parentContainer: HTMLElement) {
		const container = this._implementation
			? this._implementation._tbody
			: null;
