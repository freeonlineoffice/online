...
function _hasDrawingAreaInside(children) {
	...
	for (var i in children) {
		if (children[i].type === 'drawingarea') return true;
		if (_hasDrawingAreaInside(children[i].children)) return true;
	}
...
function _scrolledWindowControl(parentContainer, data, builder) {
	var scrollwindow = L.DomUtil.create(
		'div',
		builder.options.cssClass + ' ui-scrollwindow',
		parentContainer,
	);
	if (data.id) scrollwindow.id = data.id;
	...
	var content = L.DomUtil.create(
		'div',
		builder.options.cssClass + ' ui-scrollwindow-content',
		scrollwindow,
	);

	builder.build(content, data.children, false);

	if (!data.vertical && !data.horizontal) return false;

	var noVertical = data.vertical.policy === 'never';
	if (noVertical) scrollwindow.style.overflowY = 'hidden';
	if (data.vertical.policy === 'always')
		scrollwindow.style.overflowY = 'scroll';

	var noHorizontal = data.horizontal.policy === 'never';
	if (noHorizontal) scrollwindow.style.overflowX = 'hidden';
	...
	var verticalSteps =
		(data.vertical.upper -
			data.vertical.lower -
			data.vertical.page_size) *
		10;
	if (verticalSteps < 0 || noVertical) verticalSteps = 0;

	var horizontalSteps =
		(data.horizontal.upper -
			data.horizontal.lower -
			data.horizontal.page_size) *
		10;
	if (horizontalSteps < 0 || noHorizontal) horizontalSteps = 0;
	...
	var updateSize = function () {
		...
		if (realContentHeight === 0 || realContentWidth === 0) {
			if (timeoutLimit--) setTimeout(updateSize, 100);
			return;
		}
	...
	if (data.user_managed_scrolling !== false) setTimeout(updateSize, 0);
		...
		scrollwindow.addEventListener('scroll', function () {
			...
			if (data.user_managed_scrolling !== false) {
				...
				content.style.height =
					realContentHeight - scrollTop + verticalSteps + 'px';
				content.style.width =
					realContentWidth - scrollLeft + horizontalSteps + 'px';
			}
...
JSDialog.scrolledWindow = function (parentContainer, data, builder) {
	var buildInnerData = _scrolledWindowControl(
		parentContainer,
		data,
		builder,
	);
