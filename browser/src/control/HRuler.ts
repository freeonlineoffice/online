...
class HRuler extends Ruler {
	...
	_updateParagraphIndentations() {
		...
		const navigationsidebarWidth =
			this._map.uiManager.getBooleanDocTypePref('ShowNavigator')
	...
	_moveIndentationEnd(e: Event) {
		...
		if (element.id === 'lo-fline-marker') {
			...
			leftValue =
				parseFloat(
					this._firstLineMarker.style.left.replace('px', ''),
				) -
