	...
	export class RowGroup extends GroupBase {
		...
		update(): void {
			...
			this._collectGroupsData(
				this._sheetGeometry.getRowGroupsDataInView(),
			);
		}
		...
		getEndPosition(endPos: number): number {
			if (endPos <= this._splitPos.y) return endPos;
			else {
				return Math.max(
					endPos +
						this._cornerHeaderHeight -
						this.documentTopLeft[1],
					this._splitPos.y + this._cornerHeaderHeight,
				);
			}
		}
		...
		drawGroupControl(group: GroupEntry): void {
			...
			if (this.isGroupHeaderVisible(startY, group.startPos)) {
				...
				if (!group.hidden) {
					...
					this.context.moveTo(
						...
						startY + this._groupHeadSize / 2 + 0.5,
					);
					this.context.lineTo(
						this.transformX(
							startX +
			...
			if (
				!group.hidden &&
				endY > this._cornerHeaderHeight + this._groupHeadSize &&
				endY > startY
			) {
				//draw tail
				this.context.beginPath();
				...
				startY =
					startY >=
					this._cornerHeaderHeight + this._groupHeadSize
						? startY
						: this._cornerHeaderHeight + this._groupHeadSize;
				...
				this.context.lineTo(
					this.transformX(startX),
					endY - app.roundedDpiScale,
				);
				this.context.stroke();
				this.context.lineTo(
					Math.round(
						this.transformX(startX + this._groupHeadSize / 2),
					),
					endY - app.roundedDpiScale,
				);
				this.context.stroke();
		...
		drawLevelHeader(level: number): void {
			...
			const startY = Math.round(
				(this._cornerHeaderHeight - ctrlHeadSize) * 0.5,
			);
		...
		findClickedLevel(point: number[]): number {
			if (point[1] < this._cornerHeaderHeight) {
				let index =
					(this.transformX(point[0]) / this.size[0]) * 100; // Percentage.
						...
						if (
							...
							const startX =
								...
								(this._groupHeadSize +
									this._levelSpacing) *
									group_.level;
							const startY = this.getRelativeY(
								group_.startPos,
							);
