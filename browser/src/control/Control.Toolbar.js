...
(function (global) {
	...
	function getBorderStyleMenuElements(closeCallback) {
		...
		for (let i = 0; i < 12; i++) {
			...
			for (let j = 0; j < 4; j++) {
				...
				td.className =
					'w2ui-tb-image w2ui-icon ' +
					(i + j < 9
						? 'frame0' + String(i + j + 1)
						: 'frame' + String(i + j + 1));
				td.onclick = function () {
					setBorderStyle(parseInt(num));
				};
			}
			i += 3;
		}
	...
	function setupSearchInput() {
		$('#search-input')
			.off('input', onSearchInput)
			.on('input', onSearchInput);
		$('#search-input')
			.off('keydown', onSearchKeyDown)
			.on('keydown', onSearchKeyDown);
		$('#search-input')
			.off('focus', onSearchFocus)
