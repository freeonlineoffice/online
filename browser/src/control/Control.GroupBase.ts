...
*/
	...
	export abstract class GroupBase extends app.definitions
		.canvasSectionObject {
		_map: any;
		_textColor: string;
		_getFont: () => string;
			...
			for (let i = 0; i < groups.length; ++i) {
				...
				if (!isHidden) {
					...
					if (
						...
						const childGroupEntry =
							this._groups[lastLevel][
								firstChildGroupIndex[lastLevel]
							];
						if (childGroupEntry.hidden) {
							if (
								startPos > childGroupEntry.startPos &&
								startPos < childGroupEntry.endPos
							) {
								startPos = childGroupEntry.startPos;
								moved = true;
							}
				...
				if (i == level - 1) {
					for (const group in this._groups[i]) {
						if (
							...
							if (
								startPos != endPos &&
								startPos >= group_.startPos &&
								endPos <= group_.endPos
							) {
								return this._isParentGroupVisible(
									group_,
								);
							}
						...
						for (const group in this._groups[i]) {
							if (
								...
								if (this._groups[i][group].level == 0) {
									this.drawGroupControl(
										this._groups[i][group],
									);
								} else if (
									this._isPreviousGroupVisible(
										this._groups[i][group].level,
										this._groups[i][group]
											.startPos,
										this._groups[i][group].endPos,
										this._groups[i][group].hidden,
									)
								) {
									this.drawGroupControl(
										this._groups[i][group],
		...
		transformRectX(xcoord: number, rectWidth: number): number {
			return this.isCalcRTL()
				? this.size[0] - xcoord - rectWidth
				: xcoord;
		}
		...
		onMouseMove(point: number[]): void {
			...
			if (
				this.findClickedGroup(point) !== null ||
				this.findClickedLevel(point) !== -1
			)
		...
		onDoubleClick(point: number[]): void {
			...
			if (group) this._updateOutlineState(group);
		}
