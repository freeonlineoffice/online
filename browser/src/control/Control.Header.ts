...
namespace lool {
		...
		drawResizeLineIfNeeded(): void {
			...
			this.context.lineTo(
				this._isColumn
					...
					: isRTL
						? -this.myTopLeft[0]
						: this.containerObject.getWidth(),
				this._isColumn ? this.containerObject.getHeight() : y,
			);
			this.context.stroke();
		}
	...
	export class HeaderInfo {
		...
		update(section: CanvasSectionObject): void {
			...
			const maxIndex = this._isColumn
				? this._map._docLayer.sheetGeometry.maxVisibleColumnIndex
				: this._map._docLayer.sheetGeometry.maxVisibleRowIndex;
