...
namespace lool {
	...
	export class Header extends app.definitions.canvasSectionObject {
		...
		_initHeaderEntryStyles(className: string): void {
			...
			this._backgroundColor = getComputedStyle(
				document.body,
			).getPropertyValue('--color-background-darker');
		...
		_getFontSize(): number {
			const map = this._map;
			const zoomScale = map.getZoomScale(
				map.getZoom(),
				map.options.defaultZoom,
			);
			if (zoomScale < 0.68) return Math.round(8 * app.dpiScale);
			else if (zoomScale < 0.8) return Math.round(10 * app.dpiScale);
			else return Math.round(12 * app.dpiScale);
		}

		_initHeaderEntryHoverStyles(className: string): void {
			const baseElem = document.getElementsByTagName('body')[0];
			const elem = L.DomUtil.create('div', className, baseElem);
			this._hoverColor = getComputedStyle(
				document.body,
			).getPropertyValue('--color-background-lighter');
			L.DomUtil.remove(elem);
		}
		...
		_initHeaderEntrySelectedStyles(className: string): void {
			...
			let gradientColors: string = L.DomUtil.getStyle(
				elem,
				'background-image',
			);
			...
			while (gradientColors) {
				...
				gradientColors = gradientColors.substr(
					gradientColors.indexOf('r'),
				); // remove ', ' stuff
			}
		...
		_isRowColumnInSelectedRange(index: number): boolean {
			return (
				!!this._headerInfo.getElementData(index).isCurrent ||
		...
		_selectRow(row: number, modifier: number): void {
			...
			if (
				this._map.dialog.hasOpenedDialog() &&
				this._map.dialog.getCurrentDialogContainer()
			) {
		...
		_updateColumnHeader(): void {
			this._map._docLayer.refreshViewData({
		...
		_colIndexToAlpha(columnNumber: number): string {
			...
			while (dividend > 0) {
				...
				columnName =
					String.fromCharCode(offset + modulo) + columnName;
		...
		_entryAtPoint(point: number[]): PointEntryQueryResult {
			if (!this._headerInfo) return undefined;
			...
			this._headerInfo.forEachElement(
				function (entry: HeaderEntryData): boolean {
					const end = isRTL
						? this.size[0] - entry.pos + entry.size
						: entry.pos;
					const start = end - entry.size;
					if (position >= start && position < end) {
						// NOTE: From a geometric perspective resizeAreaStart is really "resizeAreaEnd" in RTL case.
						let resizeAreaStart = isRTL
							? Math.min(start + 3 * app.dpiScale, end)
							: Math.max(start, end - 3 * app.dpiScale);
						if (
							entry.isCurrent ||
							(window as any).mode.isMobile()
						) {
							resizeAreaStart = isRTL
								? start + this._resizeHandleSize
								: end - this._resizeHandleSize;
						}
						const isMouseOverResizeArea = isRTL
							? position < resizeAreaStart
							: position > resizeAreaStart;
						result = {
							entry: entry,
							hit: isMouseOverResizeArea,
						};
						return true;
					}
				}.bind(this),
		...
		onDraw(): void {
			this._headerInfo.forEachElement(
				function (elemData: HeaderEntryData): boolean {
		...
		onMouseLeave(point: number[]): void {
			if (point === null) {
				// This means that the mouse pointer is outside the canvas.
				if (
					this.containerObject.isDraggingSomething() &&
					this._dragEntry
				) {
					// Were we resizing a row / column before mouse left.
		...
		drawResizeLineIfNeeded(): void {
			...
			const x = this._isColumn
				? (isRTL
						? this.size[0] - this._dragEntry.pos
						: this._dragEntry.pos) + this._dragDistance[0]
				: isRTL
					? 0
					: this.size[0];
			const y = this._isColumn
				? this.size[1]
				: this._dragEntry.pos + this._dragDistance[1];
			...
			this.context.lineTo(
				this._isColumn
					...
					: isRTL
						? -this.myTopLeft[0]
						: this.containerObject.getCanvasRight(),
				this._isColumn ? this.containerObject.getCanvasBottom() : y,
			);
			this.context.stroke();
		}
		...
		onMouseMove(point: number[], dragDistance?: number[]): void {
			...
			if (!this.containerObject.isDraggingSomething()) {
				...
				if (
					this._prevMouseOverEntry &&
					result &&
					result.entry.index !== this._prevMouseOverEntry.index
				) {
					this.containerObject.setPenPosition(this);
				...
				if (L.Browser.cypressTest) return;

				if (isMouseOverResizeArea !== this._hitResizeArea) {
					// Do we need to change cursor (to resize or pointer).
					const cursor = isMouseOverResizeArea
						? this._resizeCursor
						: this._cursor;
				...
				if (
					...
					this._lastSelectedIndex ==
						this._prevMouseOverEntry.index
				)
					return;
		...
		onDoubleClick(): void {
			this._isColumn
				? this.setOptimalWidthAuto()
				: this.setOptimalHeightAuto();
		}
		...
		onMouseDown(point: number[]): void {
			...
			if (this._hitResizeArea) {
				...
				this._dragEntry = {
					// In case dragging takes place, we will remember this entry.
	...
	export class HeaderInfo {
		...
		constructor(map: any, _isColumn: boolean) {
			window.app.console.assert(
				map && _isColumn !== undefined,
				'map and isCol required',
			);
			this._map = map;
			this._isColumn = _isColumn;
			window.app.console.assert(
				this._map._docLayer.sheetGeometry,
				'no sheet geometry data-structure found!',
			);
			const sheetGeom = this._map._docLayer
				.sheetGeometry as lool.SheetGeometry;
			this._dimGeom = this._isColumn
				? sheetGeom.getColumnsGeometry()
				: sheetGeom.getRowsGeometry();
		}

		findXInCellSelections(
			cellSelections: lool.Rectangle[],
			ordinate: number,
		): boolean {
		...
		isHeaderEntryHighLighted(
			cellSelections: lool.Rectangle[],
			ordinate: number,
		): boolean {
			if (this._isColumn && this._map.wholeRowSelected) return true;
		...
		update(section: CanvasSectionObject): void {
			const cellSelections: lool.Rectangle[] =
				this._map._docLayer._cellSelections;

			let currentIndex: number;
			if (app.calc.cellCursorVisible) {
				currentIndex = this._isColumn
					? app.calc.cellAddress.x
					: app.calc.cellAddress.y;
			} else {
				currentIndex = -1;
			}
			...
			if (tsManager._inZoomAnim) {
				...
				const freePaneBounds = new L.Bounds(
					viewBounds.min.add(ctx.splitPos),
					viewBounds.max,
				);
			...
			let startIdx = this._dimGeom.getIndexFromPos(
				startPx,
				'corepixels',
			);
			const endIdx = Math.min(
				this._dimGeom.getIndexFromPos(endPx - 1, 'corepixels'),
				1048576 - 1,
			);
			this._elements = [];
			...
			if (splitPos) {
				const splitIndex = this._dimGeom.getIndexFromPos(
					splitPos + 1,
					'corepixels',
				);
				...
				if (splitIndex) {
					...
					this._dimGeom.forEachInRange(
						...
						(idx: number, data: DimensionPosSize) => {
							this._elements[idx] = {
								index: idx,
								pos:
									(data.startpos + data.size) *
									scale, // end position on the header canvas
								size: data.size * scale,
								origsize: data.size,
								isHighlighted:
									this.isHeaderEntryHighLighted(
										cellSelections,
										data.startpos +
											data.size * 0.5,
									),
					...
					const freeStartIndex = this._dimGeom.getIndexFromPos(
						freeStartPos + 1,
						'corepixels',
					);
			...
			this._elements[startIdx] = {
				...
				isHighlighted: this.isHeaderEntryHighLighted(
					cellSelections,
					dataFirstFree.startpos + dataFirstFree.size * 0.5,
				),
		...
		isZeroSize(i: number): boolean {
			...
			window.app.console.assert(
				elem,
				'queried a non existent row/col in the header : ' + i,
			);
			return elem.size === 0;
		}
		...
		getRowData(index: number): HeaderEntryData {
			window.app.console.assert(
				!this._isColumn,
				'this is a column header instance!',
			);
