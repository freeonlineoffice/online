...
var AdminSocketAnalytics = AdminSocketBase.extend({
	...
	_initStatsData: function (option, size, interval, reset) {
		...
		if (option === 'mem') this._memStatsData = actualData;
		else if (option === 'cpu') this._cpuStatsData = actualData;
		else if (option === 'sent') this._sentStatsData = actualData;
		else if (option === 'recv') this._recvStatsData = actualData;
		else if (option === 'conn') this._connStatsData = actualData;
	},
	...
	onSocketOpen: function () {
		...
		this.socket.send(
			'subscribe mem_stats cpu_stats sent_activity recv_activity connection_activity settings',
		);
	...
	_setUpAxis: function (option) {
		...
		if (option === 'mem') data = this._memStatsData;
		else if (option === 'cpu') data = this._cpuStatsData;
		else if (option === 'net')
		...
		xScale = d3
			.scaleLinear()
			.range([
				this._graphMargins.left,
				this._graphWidth - this._graphMargins.right,
			])
			.domain([
		...
		yScale = d3
			.scaleLinear()
			.range([
				this._graphHeight - this._graphMargins.bottom,
				this._graphMargins.top,
			])
			.domain([
				d3.min(data, function (d) {
					return d.value;
		...
		d3XAxis = d3.axisBottom(xScale).tickFormat(function (d) {
			d = Math.abs(d / 1000);
			var sUnit = 0;
			var i = 0;
			var units = ['s', 'min', 'hr'];
		...
		if (option === 'mem') {
			...
			this._d3NetYAxis = d3
				.axisLeft(this._yNetScale)
				.tickFormat(function (d) {
					return Util.humanizeMem(d / 1000) + '/sec';
				});
			this._d3NetYAxis2 = d3.axisRight(this._yNetScale2).tickFormat(
				function (d) {
	...
	_createGraph: function (option) {
		...
		if (option === 'mem') {
			...
			for (var i = legendData.length - 1; i >= 0; i--) {
				legend
					...
					.attr(
						'x',
						this._graphWidth - this._graphMargins.right - 120,
					)
		...
		vis.append('svg:g')
			...
			.attr(
				'transform',
				'translate(' + this._graphMargins.left + ',0)',
			)
			.call(yAxis);
	...
	_addNewData: function (oldData, newData, option) {
		...
		else if (option === 'connect') size = this._connStatsSize;
	...
	_updateMemGraph: function () {
		...
		svg.select('.line').attr('d', this._d3MemLine(this._memStatsData));

		svg.select('.x-axis').call(this._d3MemXAxis);
	...
	_updateCpuGraph: function () {
		...
		svg.select('.line').attr('d', this._d3CpuLine(this._cpuStatsData));

		svg.select('.x-axis').call(this._d3CpuXAxis);
	...
	_updateNetGraph: function () {
		...
		svg.select('.lineSent').attr(
			'd',
			this._d3NetSentLine(this._sentStatsData),
		);
		svg.select('.lineRecv').attr(
			'd',
			this._d3NetRecvLine(this._recvStatsData),
		);
		svg.select('.lineConn').attr(
			'd',
			this._d3NetConnLine(this._connStatsData),
		);

		svg.select('.x-axis').call(this._d3NetXAxis);
