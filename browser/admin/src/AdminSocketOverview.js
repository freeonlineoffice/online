...
function createDocumentUserListElement(cell, doc) {
	var collapsable = createCollapsable(
		cell,
		'ucontainer' + doc['pid'],
		String(doc['views'].length) + _(' user(s).'),
	);
	for (var i = 0; i < doc['views'].length; i++) {
		collapsable.addItem(
			'user' + doc['views'][i]['sessionid'],
			doc['views'][i]['userName'],
		);
	}
}
...
function upsertDocsTable(doc, sName, socket, wopiHost) {
	...
	if (add === true) {
		...
		if (add === true) {
			...
			row.cells[1] = userInfoCell;
		}
		createDocumentUserListElement(userInfoCell, doc);
	} else {
		var collapsable = getCollapsibleClass('ucontainer' + doc['pid']);
		collapsable.addItem(
			'user' + doc['views'][0]['sessionid'],
			doc['views'][0]['userName'],
		);
		collapsable.setText(
			String(parseInt(collapsable.getText().split(' ')[0]) + 1) +
				_(' user(s).'),
		);
	}
	...
	for (var i = 0; i < userList.length; i++) {
		...
		if (row === undefined || row === null) {
			...
			collapsable = createCollapsable(
				docInfoCell,
				'docListContainer_' + encodedUId,
				'1' + ' document(s) open.',
			);
			collapsable.addItem(
				userList[i]['sessionid'] + '_' + docPid,
				sName,
			);
		} else {
			collapsable = getCollapsibleClass(
				'docListContainer_' + encodedUId,
			);
			collapsable.setText(
...
var AdminSocketOverview = AdminSocketBase.extend({
	...
	onSocketOpen: function () {
		...
		this.socket.send(
			'subscribe adddoc rmdoc resetidle propchange modifications uploaded',
		);

		this._getBasicStats();
		var socketOverview = this;
		this._basicStatsIntervalId = setInterval(function () {
			return socketOverview._getBasicStats();
		}, 5000);

		this._docElapsedTimeIntervalId = setInterval(function () {
	...
	onSocketMessage: function (e) {
		...
		if (textMsg.startsWith('documents')) {
			...
			var docList = JSON.parse(textMsg.substr(jsonStart).trim())[
				'documents'
			];
			...
			document.getElementById('docidle' + sPid).innerText =
				Util.humanizeSecs(0);
			...
			document.getElementById('active_docs_count').innerText = String(
				parseInt(
					document.getElementById('active_docs_count').innerText,
				) + 1,
			);
			document.getElementById('active_users_count').innerText = String(
				parseInt(
					document.getElementById('active_users_count')
						.innerText,
				) + 1,
			);
		} else if (
			textMsg.startsWith('mem_consumed') ||
			...
			if (doc !== undefined && doc !== null) {
				...
				var viewerCount =
					parseInt(collapsable.getText().split(' ')[0]) - 1;
				if (viewerCount === 0) {
					document
						.getElementById('docview')
						.deleteRow(doc.rowIndex);
				} else {
					collapsable.setText(
						String(viewerCount) + _(' user(s).'),
					);
				}
			...
			if (docEntry !== null) {
				...
				var userDocListCell =
					docEntry.parentNode.parentNode.parentNode;
				if (docCount === 1) {
					document
						.getElementById('userview')
						.deleteRow(userDocListCell.parentNode.rowIndex);
				} else {
					docEntry = null;
					userDocListCell.children[0].innerText = String(
						parseInt(userDocListCell.children[0].innerText) -
							1,
					);
				}
			}
			...
			e.data == 'InvalidAuthToken' ||
			e.data == 'NotAuthenticated'
		) {
			var msg;
			if (window.location.protocol === 'http:') {
				// Browsers refuse to overwrite the jwt cookie in this case.
				msg = _(
					'Failed to set jwt authentication cookie over insecure connection',
				);
			} else {
				msg = _(
					'Failed to authenticate this session over protocol {0}',
				);
