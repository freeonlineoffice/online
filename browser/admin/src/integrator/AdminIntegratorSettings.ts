		...
		for (const key in data) {
			if (Object.prototype.hasOwnProperty.call(data, key)) {
				...
				if (
					...
					const childContent = this.renderSettingsOption(
						value,
						uniqueId,
					);
		...
		for (const key in data) {
			if (typeof data[key] === 'boolean') {
				...
				if (key === 'accessibilityState') {
					checkbox.checked =
						checkbox.checked && window.enableAccessibility;
				...
				if (checkbox.disabled) {
					if (key === 'accessibilityState') {
						// This has no effect if loolwsd accessibility.enable is not set. So present as disabled,
						// and warn that it cannot be toggled unless loolwsd accessibility is on so we don't have
