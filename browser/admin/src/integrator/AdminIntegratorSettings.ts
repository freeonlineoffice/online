...
class SettingIframe {
			...
			if (fileInput && button) {
				...
				fileInput.addEventListener('change', async () => {
					if (fileInput.files?.length) {
						if (
							cfg.uploadPath === this.PATH.wordBookUpload()
						) {
	...
	private async fetchAndPopulateSharedConfigs(): Promise<void> {
		if (!window.wopiSettingBaseUrl) {
			console.error(
				_('Shared Config URL is missing in initial variables.'),
			);
			return;
		}
		...
		if (!window.accessToken) {
			console.error(
				_('Access token is missing in initial variables.'),
			);
			return;
		}
	...
	private createBrowserSettingForm(
		...
		editorContainer.appendChild(
			this.createHeading(_('Interface Settings')),
		);
		editorContainer.appendChild(
		...
		const navContainer =
			this.createBrowserSettingTabsNav(editorContainer);
		...
		const contentsContainer =
			this.createBrowserSettingContentsContainer();
	...
	private createBrowserSettingContentsContainer(): HTMLDivElement {
		...
		contentsContainer.textContent = _(
			'Select a tab to browser settings.',
		);
		...
		const saveButton = this.createButtonWithText(
			...
			async (button) => {
				...
				this.collectBrowserSettingsFromUI(
					sharedConfigsContainer.querySelector(
						'#browser-setting',
					)!,
				);
		...
		for (const key in data) {
			if (Object.prototype.hasOwnProperty.call(data, key)) {
				...
				if (
					...
					const customElement = this.customRenderers[
						value.customType
					](key, value, uniqueId);
				...
				if (
					...
					container.appendChild(
						this.createFieldset(key, value, uniqueId),
					);
				} else {
					container.appendChild(
	...
	private createCheckbox(
		...
		const inputCheckbox = this.createCheckboxInput(
			id,
			isChecked,
			isDisabled,
		);
	...
	private collectBrowserSettingsFromUI(
		...
		const inputs =
			browserSettingSection.querySelectorAll<HTMLInputElement>(
		...
		inputs.forEach((input) => {
			...
			if (sectionRaw === 'common') {
				...
				(
					this.browserSettingOptions[sectionRaw] as Record<
						string,
						boolean
					>
				)[settingKey] = value;
	...
	private generateViewSettingUI(data: any) {
		...
		viewContainer.appendChild(
			this.createParagraph(_('Adjust view settings.')),
		);
		...
		for (const key in data) {
			...
			if (typeof data[key] === 'boolean') {
				...
				fieldset.appendChild(
					this.createViewSettingsTextBox(key, data),
				);
			}
		}
	...
	private async populateSharedConfigUI(data: ConfigData): Promise<void> {
		...
		if (data.kind === 'user') {
			...
			if (data.browsersetting && data.browsersetting.length > 0) {
				...
				const browserSettingContent =
					await this.fetchSettingFile(fileId);
		...
		if (data.xcu && data.xcu.length > 0) {
			...
			const existingXcuSection =
				document.getElementById('xcu-section');
