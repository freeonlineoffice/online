...
var AdminClusterOverview = AdminSocketBase.extend({
	...
	createCard: function (server) {
		...
		if (server.podname) {
			...
			console.warn(
				'podname does not exist, using serverId instead of podname on card title',
			);
		...
		var serverIdTile = this.createTile(
			'ServerId',
			server.serverId,
			'serverId',
		);
	...
	updateCardContent: function (cardId, server) {
		...
		card.querySelector('#mem-con').textContent = Util.humanizeMem(
			server.memory,
		);
		...
		var innerWidth =
			this._graphDimensions.x -
			this._graphMargins.left -
			this._graphMargins.right;

		var yAxisMemGenerator = d3
			.axisLeft(memObj.yScale)
		...
		var yAxisCpuGenerator = d3
			.axisLeft(cpuObj.yScale)
			.tickFormat(function (d) {
				return d + '%';
			});
	...
	getScaleAndLine: function (data) {
		...
		var xScale = d3.scaleTime().domain(xDomain).range([0, width]);
	...
	createDocumentTable: function (documents) {
		...
		documents.forEach(function (doc) {
			var row = that.createRow(
				doc.documentName,
				Util.humanizeMem(doc.memoryConsumed),
				doc.pid,
			);
	...
	createAnchor: function (server) {
		...
		anchor.href =
			server.ingressUrl +
			'/browser/dist/admin/admin.html?RouteToken=' +
			server.routeToken;
		...
		if (server.podname) {
			...
			console.warn(
				'podname does not exist, using serverId instead of podname on anchor tag',
			);
	...
	onSocketMessage: function (e) {
		...
		if (textMsg.startsWith('stats')) {
			...
			var srvList = document.querySelector(
				'#column-admin-panel .list',
			);
			var that = this;
			stats.forEach(function (srvStat) {
				var anchor = document.getElementById(
					'anchor' + srvStat.serverId,
				);
				if (anchor) {
			...
			if (card) {
				...
				if (!table) {
					table = this.createDocumentTable([
						{
							documentName: decodeURI(filename),
							memoryConsumed: mem,
							pid: pid,
						},
					]);
					...
					var row = this.createRow(
						decodeURI(filename),
						Util.humanizeMem(Number.parseInt(mem)),
						pid,
					);
			...
			if (card) {
				...
				if (row) {
					row.querySelector('.docmem').textContent =
						Util.humanizeMem(Number.parseInt(newMem));
				}
			}
			...
			if (msg == 'true') {
				var dialog = new DlgLoading().text(
					_('Please wait kubernetes cluster is scaling...'),
				);
				dialog.open();
			} else {
			...
			textMsg == 'InvalidAuthToken' ||
			textMsg == 'NotAuthenticated'
		) {
			var msg;
	...
	onSocketOpen: function () {
		this.socket.send(
			'auth jwt=' + window.jwtToken + ' routeToken=' + this.routeToken,
		);
		this.socket.send('stats');
