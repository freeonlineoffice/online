...
var w2utils = (function ($) {
	...
	function isFloat(val) {
		if (typeof val == 'string')
			val = val
				.replace(/\s+/g, '')
				.replace(w2utils.settings.groupSymbol, '')
	...
	function isMoney(val) {
		...
		var re = new RegExp(
			'^' +
				...
				se.decimalSymbol +
				']?[0-9]+' +
				(se.currencySuffix ? '\\' + se.currencySuffix + '?' : '') +
				'$',
			'i',
		);
		if (typeof val === 'string') {
	...
	function isDate(val, format, retDate) {
		...
		if (typeof val.getUTCFullYear === 'function') {
			// date object
			...
			if (new RegExp('mon', 'ig').test(format)) {
				...
				val = val
					...
					.toLowerCase();
				for (
					var m = 0, len = w2utils.settings.fullmonths.length;
					m < len;
					m++
				) {
					var t = w2utils.settings.fullmonths[m];
					val = val
						.replace(new RegExp(t, 'ig'), parseInt(m) + 1)
						.replace(
							new RegExp(t.substr(0, 3), 'ig'),
							parseInt(m) + 1,
						);
				}
			}
	...
	function age(dateStr) {
		...
		if (typeof dateStr.getUTCFullYear === 'function') {
			// date object
			d1 = dateStr;
	...
	function interval(value) {
		...
		} else if (value < 3.1536e10) {
			ret = Math.floor((value / 2628000000) * 10) / 10 + ' months';
		} else {
			ret = Math.floor(value / 3.1536e9) / 10 + ' years';
		}
		return ret;
	}
	...
	function date(dateStr) {
		if (
			dateStr === '' ||
			dateStr == null ||
			(typeof dateStr == 'object' && !dateStr.getMonth)
		)
			return '';
	...
	function formatSize(sizeStr) {
		...
		return (
			(Math.floor((sizeStr / Math.pow(1024, i)) * 10) / 10).toFixed(
				i === 0 ? 0 : 1,
			) +
			' ' +
			(sizes[i] || '??')
	...
	function formatDate(dateStr, format) {
		...
		return format
			...
			.replace(
				/yy/g,
				year > 2000
					? 100 + parseInt(String(year).substr(2))
	...
	function formatDateTime(dateStr, format) {
		...
		return (
			this.formatDate(dateStr, fmt[0]) +
			' ' +
			this.formatTime(dateStr, fmt[1])
		);
	}
	...
	function md5(input) {
		...
		function __pj_crypt_hex_md5(s) {
			return __pj_crypt_rstr2hex(
				__pj_crypt_rstr_md5(__pj_crypt_str2rstr_utf8(s)),
			);
		}
		function __pj_crypt_b64_md5(s) {
			return __pj_crypt_rstr2b64(
				__pj_crypt_rstr_md5(__pj_crypt_str2rstr_utf8(s)),
			);
		}
		function __pj_crypt_any_md5(s, e) {
			return __pj_crypt_rstr2any(
				__pj_crypt_rstr_md5(__pj_crypt_str2rstr_utf8(s)),
				e,
			);
		}
		function __pj_crypt_hex_hmac_md5(k, d) {
			return __pj_crypt_rstr2hex(
				__pj_crypt_rstr_hmac_md5(
					__pj_crypt_str2rstr_utf8(k),
					__pj_crypt_str2rstr_utf8(d),
				),
			);
		}
		...
		function __pj_crypt_rstr_md5(s) {
			return __pj_crypt_binl2rstr(
				__pj_crypt_binl_md5(__pj_crypt_rstr2binl(s), s.length * 8),
			);
		}
		...
		function __pj_crypt_rstr_hmac_md5(key, data) {
			...
			var hash = __pj_crypt_binl_md5(
				ipad.concat(__pj_crypt_rstr2binl(data)),
				512 + data.length * 8,
			);
			return __pj_crypt_binl2rstr(
				__pj_crypt_binl_md5(opad.concat(hash), 512 + 128),
			);
		}
		...
		function __pj_crypt_md5_cmn(q, a, b, x, s, t) {
			return __pj_crypt_safe_add(
				__pj_crypt_bit_rol(
					__pj_crypt_safe_add(
						__pj_crypt_safe_add(a, q),
						__pj_crypt_safe_add(x, t),
					),
					s,
				),
				b,
	...
	function transition(div_old, div_new, type, callBack) {
		...
		div_old.parentNode.style.cssText +=
			'perspective: 900px; overflow: hidden;';
		div_old.style.cssText +=
			'; position: absolute; z-index: 1019; backface-visibility: hidden';
		div_new.style.cssText +=
			'; position: absolute; z-index: 1020; backface-visibility: hidden';
		...
		switch (type) {
			...
			case 'slide-right':
				...
				window.setTimeout(function () {
					div_new.style.cssText +=
						'transition: ' +
						time +
						's; transform: translate3d(0px, 0, 0)';
					div_old.style.cssText +=
						'transition: ' +
						time +
						's; transform: translate3d(' +
			...
			case 'slide-down':
				// init divs
				div_old.style.cssText +=
					'overflow: hidden; z-index: 1; transform: translate3d(0, 0, 0)';
				div_new.style.cssText +=
					'overflow: hidden; z-index: 0; transform: translate3d(0, 0, 0)';
				$(div_new).show();
				// -- need a timing function because otherwise not working
				window.setTimeout(function () {
					div_new.style.cssText +=
			...
			case 'slide-up':
				...
				div_old.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0)';
				div_new.style.cssText +=
					'overflow: hidden; transform: translate3d(0, ' +
					height +
					'px, 0)';
				$(div_new).show();
				// -- need a timing function because otherwise not working
			...
			case 'flip-left':
				// init divs
				div_old.style.cssText +=
					'overflow: hidden; transform: rotateY(0deg)';
				div_new.style.cssText +=
			...
			case 'flip-down':
				// init divs
				div_old.style.cssText +=
					'overflow: hidden; transform: rotateX(0deg)';
				div_new.style.cssText +=
					'overflow: hidden; transform: rotateX(180deg)';
				$(div_new).show();
				// -- need a timing function because otherwise not working
				window.setTimeout(function () {
					div_new.style.cssText +=
			...
			case 'pop-in':
				...
				div_old.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0)';
				div_new.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0); transform: scale(.8); opacity: 0;';
				$(div_new).show();
				// -- need a timing function because otherwise not working
				window.setTimeout(function () {
					div_new.style.cssText +=
			...
			case 'pop-out':
				// init divs
				div_old.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0); transform: scale(1); opacity: 1;';
				div_new.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0); opacity: 0;';
				$(div_new).show();
				// -- need a timing function because otherwise not working
				window.setTimeout(function () {
					div_new.style.cssText +=
						'transition: ' + time + 's; opacity: 1;';
					div_old.style.cssText +=
						'transition: ' +
						time +
						's; transform: scale(1.7); opacity: 0;';
				}, 1);
				break;
			...
			default:
				...
				div_old.style.cssText +=
					'overflow: hidden; transform: translate3d(0, 0, 0)';
				div_new.style.cssText +=
					'overflow: hidden; translate3d(0, 0, 0); opacity: 0;';
				$(div_new).show();
				// -- need a timing function because otherwise not working
				window.setTimeout(function () {
					div_new.style.cssText +=
	...
	function lock(box, msg, spinner) {
		...
		if (options.spinner === true)
			options.msg =
				...
				(!options.msg ? 'style="width: 35px; height: 35px"' : '') +
				'></div>' +
				options.msg;
	...
	function message(where, options) {
		...
		var titleHeight = parseInt(
			$(where.box).find(where.title).css('height') || 0,
		);
		if (options.width > pWidth) options.width = pWidth - 10;
		if (options.height > pHeight - titleHeight)
			options.height = pHeight - 10 - titleHeight;
		options.originalWidth = options.width;
		options.originalHeight = options.height;
		if (parseInt(options.width) < 0)
			options.width = pWidth + options.width;
		if (parseInt(options.width) < 10) options.width = 10;
		if (parseInt(options.height) < 0)
			options.height = pHeight + options.height - titleHeight;
		...
		if (
			options.height == null ||
			options.height > poptions.height - titleHeight - 5
		) {
			options.height = poptions.height - titleHeight - 5; // need margin from bottom only
		}
		// negative value means margin
		if (options.originalHeight < 0)
			options.height = pHeight + options.originalHeight - titleHeight;
		if (options.originalWidth < 0)
			options.width = pWidth + options.originalWidth * 2; // x 2 because there is left and right margin
		...
		if (
			...
			if (msgCount == 1) {
				if (this.unlock) {
					if (where.param) this.unlock(where.param, 150);
					else this.unlock(150);
				}
			} else {
				$(where.box)
					.find('#w2ui-message' + (msgCount - 2))
					.css('z-index', 1500);
			}
			...
			if (
				...
				options.html =
					'<div class="w2ui-message-body">' +
					(options.body || '') +
					'</div>' +
					'<div class="w2ui-message-buttons">' +
					(options.buttons || '') +
					'</div>';
			}
			...
			if (display == 'none') {
				...
				if (msgCount === 0 && this.lock) {
					if (where.param) this.lock(where.param);
					else this.lock();
				}
				setTimeout(function () {
	...
	function getStrWidth(str, styles) {
		var w,
			html =
				'<div id="_tmp_width" style="position: absolute; top: -900px;' +
				(styles || '') +
				'">' +
				encodeTags(str) +
				'</div>';
	...
	function checkName(params, component) {
		...
		if (!params || params.name == null) {
			console.log(
				'ERROR: The parameter "name" is required but not supplied in $().' +
					component +
					'().',
			);
			return false;
		...
		if (w2ui[params.name] != null) {
			console.log(
				'ERROR: The parameter "name" is not unique. There are other objects already created with the same name (obj: ' +
					params.name +
					').',
			);
			return false;
		...
		if (!w2utils.isAlphaNumeric(params.name)) {
			console.log(
				'ERROR: The parameter "name" has to be alpha-numeric (a-z, 0-9, dash and underscore). ',
			);
			return false;
		}
		return true;
	...
	function checkUniqueId(id, items, itemsDecription, objName) {
		...
		for (var i = 0; i < items.length; i++) {
			if (items[i].id === id) {
				console.log(
					'ERROR: The parameter "id=' +
						id +
						'" is not unique within the current ' +
						itemsDecription +
						'. (obj: ' +
						objName +
						')',
...
w2utils.formatters = {
	...
	datetime: function (value, params) {
		...
		return (
			...
			dt +
			'">' +
			w2utils.formatDateTime(dt, params) +
			'</span>'
		);
	},
	...
	time: function (value, params) {
		...
		return (
			...
			dt +
			'">' +
			w2utils.formatTime(value, params) +
			'</span>'
		);
	},
...
w2utils.event = {
	on: function (edata, handler) {
		...
		if (typeof edata == 'string' && edata.indexOf(':') != -1) {
			var tmp = edata.split(':');
			if (['complete', 'done'].indexOf(edata[1]) != -1)
				edata[1] = 'after';
			edata = {
				type: tmp[0],
				execute: tmp[1],
		...
		if (!edata.type) {
			console.log(
				'ERROR: You must specify event type when calling .on() method of ' +
					this.name,
			);
			return;
		}
		if (!handler) {
			console.log(
				'ERROR: You must specify event handler function when calling .on() method of ' +
					this.name,
			);
			return;
		}
	...
	off: function (edata, handler) {
		...
		if (!edata.type && !scope) {
			console.log(
				'ERROR: You must specify event type when calling .off() method of ' +
					this.name,
			);
			return;
		}
		...
		for (var h = 0, len = this.handlers.length; h < len; h++) {
			...
			if (
				(t.edata.type === edata.type ||
					edata.type === '*' ||
					(t.edata.scope != null && edata.type == '')) &&
				(t.edata.target === edata.target || edata.target == null) &&
				(t.edata.execute === edata.execute ||
					edata.execute == null) &&
				(t.handler === handler ||
					handler == null ||
					(scope != null && t.edata.scope == scope))
			) {
				// match
			} else {
				newHandlers.push(t);
	...
	trigger: function (edata) {
		var $ = jQuery;
		var edata = $.extend(
			{ type: null, phase: 'before', target: null, doneHandlers: [] },
			edata,
			{
				isStopped: false,
				isCancelled: false,
		...
		for (var h = this.handlers.length - 1; h >= 0; h--) {
			...
			if (
				(item.edata.type === edata.type ||
					item.edata.type === '*') &&
				(item.edata.target === edata.target ||
					item.edata.target == null) &&
				(item.edata.execute === edata.phase ||
					item.edata.execute === '*' ||
					item.edata.phase === '*')
			) {
				...
				if (edata.isStopped === true || edata.stop === true)
					return edata; // back compatibility edata.stop === true
			}
		}
		// main object events
		var funName =
			'on' +
			edata.type.substr(0, 1).toUpperCase() +
			edata.type.substr(1);
		...
		if (edata.phase === 'after') {
			if (typeof edata.onComplete === 'function')
				edata.onComplete.call(this, edata);
...
(function ($) {
	...
	$.fn.w2marker = function () {
		...
		if (str.length === 0 || !str[0]) {
			...
			return $(this).each(function (index, el) {
				...
				function replaceValue(matched) {
					// mark new
					return (
						'<span class="w2ui-marker">' + matched + '</span>'
					);
				}
			});
		}
		...
		function clearMarkedText(index, el) {
			while (
				el.innerHTML.indexOf('<span class="w2ui-marker">') != -1
			) {
				el.innerHTML = el.innerHTML.replace(
	...
	$.fn.w2tag = function (text, options) {
		...
		if (options['class'] !== '' && options.inputClass === '')
			options.inputClass = options['class'];
		...
		return $(this).each(function (index, el) {
			...
			if (text === '' || text == null) {
				...
				// insert
				$('body').append(
					'<div onclick="event.stopPropagation()" style="display:none;" id="w2ui-tag-' +
						origID +
						'" ' +
						'       class="w2ui-tag ' +
						($(el).parents(
							'.w2ui-popup, .w2ui-overlay-popup, .w2ui-message',
						).length > 0
							? 'w2ui-tag-popup'
							: '') +
						'">' +
						'   <div style="margin: -2px 0px 0px -2px; white-space: nowrap;">' +
						'      <div class="w2ui-tag-body ' +
						options.className +
						'" style="' +
						(options.style || '') +
						'">' +
				...
				$tags = $('#w2ui-tag-' + tagID);
				$(el)
					.data('w2tag', $tags.get(0))
					.data('checkIfMoved', checkIfMoved);
			}
			...
			function checkIfMoved(checkOnly, instant) {
				...
				var posTop = parseInt(
					offset.top + (options.top ? options.top : 0),
				);
				...
				if (options.position == 'top') {
					...
					if (tagBody.data('posClass') !== posClass) {
						tagBody
							.removeClass(
								'w2ui-tag-right w2ui-tag-left w2ui-tag-top w2ui-tag-bottom',
							)
							.addClass(posClass)
				...
				if (
					$tags.data('position') !== posLeft + 'x' + posTop &&
					checkOnly !== true
				) {
					$tags.css(
						w2utils.cssPrefix({
							transition: instant ? '0s' : '.2s',
						}),
					)
						.css({
							left: posLeft + 'px',
	...
	$.fn.w2overlay = function (html, options) {
		...
		if (
			obj.length > 0 &&
			(obj[0].tagName == null ||
				obj[0].tagName.toUpperCase() == 'BODY')
		)
		...
		if (
			options.contextMenu &&
			(options.pageX == null || options.pageY == null)
		) {
			console.log(
				'ERROR: to display menu at mouse location, pass options.pageX and options.pageY.',
			);
		}
		// append
		$('body').append(
		...
		if (bc != null && bc !== 'rgba(0, 0, 0, 0)' && bc !== 'transparent')
			div1.css({ 'background-color': bc, 'border-color': bc });
		...
		function resize() {
			...
			if (div1.length > 0) {
				...
				if (options.tmp.contentHeight) {
					...
					if (h > $('.leaflet-container').height())
						h = $('.leaflet-container').height();
					div2.height(h);
					setTimeout(function () {
						var $div = div2.find('div.menu');
						if (h > $div.height()) {
							div2.find('div.menu').css(
								'overflow-y',
								'hidden',
							);
						}
				...
				switch (options.align) {
					...
					case 'right':
						boxLeft =
							w2utils.getSize($(obj), 'width') - w + 10;
						tipLeft = w - 40;
						break;
				}
				...
				if (
					(maxHeight > -50 && maxHeight < 210) ||
					options.openAbove === true
				) {
					var tipOffset;
					// show on top
					if (options.contextMenu) {
						...
						maxHeight =
							offset.top - $(document).scrollTop() - 7;
						tipOffset = 24;
					}
					...
					div1.css(
						'top',
						offsetTop - h - tipOffset + options.top + 'px',
					);
				...
				// don't show tip
				if (options.contextMenu || options.noTip) {
					// context menu
					div1.find('>style').html(
						'#w2ui-overlay' +
							name +
        ...
        */
		...
		if (menu === 'refresh') {
			...
			if (
				...
				options.onSelect = options.select;
			if (
				typeof options.onRender === 'function' &&
				typeof options.render !== 'function'
			)
				options.render = options.onRender;
			...
			$.fn.w2menuDown = function (event, index) {
				...
				if (options.type == 'check' || options.type == 'radio') {
					...
					if (item.checked) {
						...
						tmp.removeClass('w2ui-icon-empty').addClass(
							'w2ui-icon-check',
						);
					} else if (options.type == 'check') {
						tmp.removeClass('w2ui-icon-check').addClass(
							'w2ui-icon-empty',
						);
					}
				}
			...
			html +=
				'<div class="menu" style="position: absolute; top: ' +
				(options.search ? 40 : 0) +
			...
			setTimeout(function () {
				...
				if (options.search) {
					if (
						['text', 'password'].indexOf($(obj)[0].type) !=
							-1 ||
		...
		function mresize() {
			setTimeout(function () {
				// show selected
				$('#w2ui-overlay' + name + ' tr.w2ui-selected').removeClass(
					'w2ui-selected',
				);
				var cur = $(
					'#w2ui-overlay' +
				...
				if (options.tmp)
					options.tmp.contentHeight =
						$('#w2ui-overlay' + name + ' table').height() +
						(options.search ? 50 : 10);
				if (options.tmp)
					options.tmp.contentWidth = $(
						'#w2ui-overlay' + name + ' table',
					).width();
				if ($('#w2ui-overlay' + name).length > 0)
				...
				if (cur.length > 0) {
					...
					$('#w2ui-overlay' + name + ' div.menu').scrollTop(
						scrTop,
					);
					if (
						top < scrTop ||
						top + cur.height() > scrTop + height
					) {
						$('#w2ui-overlay' + name + ' div.menu').animate(
							{
								scrollTop:
									top -
									(height - cur.height() * 2) / 2,
							},
							200,
							'linear',
						);
		...
		function change(event) {
			...
			switch (key) {
				...
				case 27: // escape
					...
					break;
				case 38: // up
					options.index = w2utils.isInt(options.index)
						? parseInt(options.index)
						: 0;
					options.index--;
					while (
						options.index > 0 &&
						options.items[options.index].hidden
					)
						options.index--;
					if (
						options.index === 0 &&
				...
				case 40: // down
					...
					options.index++;
					while (
						options.index < options.items.length - 1 &&
						options.items[options.index].hidden
					)
						options.index++;
					if (
					...
					if (options.index >= options.items.length)
						options.index = options.items.length - 1;
					cancel = true;
					break;
			}
			...
			if (!cancel) {
				...
				for (var i = 0; i < options.items.length; i++) {
					...
					var suffix = '';
					if (
						['is', 'begins with'].indexOf(options.match) !==
						-1
					)
						prefix = '^';
					if (['is', 'ends with'].indexOf(options.match) !== -1)
						suffix = '$';
					try {
						var re = new RegExp(
		...
		function getMenuHTML() {
			if (options.spinner) {
				return (
					'<table class="w2ui-drop-menu"><tbody><tr><td style="padding: 5px 10px 10px 10px; text-align: center">' +
					'    <div class="w2ui-spinner" style="width: 18px; height: 18px; position: relative; top: 5px;"></div> ' +
					'    <div style="display: inline-block; padding: 3px; color: #999;">' +
			...
			var count = 0;
			var menu_html =
				'<table cellspacing="0" cellpadding="0" class="w2ui-drop-menu"><tbody>';
			var img = null,
				icon = null;
			for (var f = 0; f < options.items.length; f++) {
				...
				if (typeof mitem === 'string') {
					...
					if (mitem.text != null && mitem.id == null)
						mitem.id = mitem.text;
					if (mitem.text == null && mitem.id != null)
						mitem.text = mitem.id;
				...
				if (mitem.hidden !== true) {
					...
					if (typeof options.render === 'function')
						txt = options.render(mitem, options);
					if (img)
						imgd =
							'<td class="menu-icon"><div class="w2ui-tb-image w2ui-icon ' +
							img +
							'"></div></td>';
					if (icon)
						imgd =
							'<td class="menu-icon" align="center"><span class="w2ui-icon ' +
							icon +
							'"></span></td>';
					...
					if (txt != null && txt !== '' && !/^-+$/.test(txt)) {
						...
						var colspan = 1;
						if (imgd === '') colspan++;
						if (mitem.count == null && mitem.hotkey == null)
							colspan++;
						...
						menu_html +=
							...
							f +
							'" style="' +
							(mitem.style ? mitem.style : '') +
							'" ' +
							(mitem.tooltip
								? 'title="' +
									w2utils.lang(mitem.tooltip) +
									'"'
								: '') +
							'        class="' +
							bg +
							...
							(mitem.disabled === true
								...
								: 'false') +
							') return;' +
							"               jQuery.fn.w2menuClick(event, '" +
							f +
							'\');">' +
							imgd +
							'   <td class="menu-text" colspan="' +
							colspan +
							'">' +
							w2utils.lang(txt) +
							'</td>' +
						...
						// horizontal line
						menu_html +=
							'<tr><td colspan="3" style="padding: 6px; pointer-events: none"><div style="border-top: 1px solid silver;"></div></td></tr>';
					}
				}
			...
			if (count === 0) {
				menu_html +=
					'<tr><td style="padding: 13px; color: #999; text-align: center">' +
					options.msgNoItems +
					'</div></td></tr>';
			}
	...
	var generatePalette = function (paletteName, options) {
		...
		if (typeof customColorRow !== 'undefined') {
			...
			$.fn.w2colorPalette.push([
				'F2F2F2',
				'F2F2F2',
				'F2F2F2',
				'F2F2F2',
				'F2F2F2',
			]); // custom colors (up to 4)
		}
	...
	$.fn.w2color = function (options, callBack) {
		...
		function bindEvents(pal) {
			$('#w2ui-overlay .color')
				...
				.on('mousedown.w2color keydown.w2color', function (event) {
					...
					index = $(event.originalEvent.target)
						.attr('index')
						.split(':');
					var theme = $(event.originalEvent.target).attr(
						'theme',
					);
			...
			$('#w2ui-overlay input')
				...
				.on('change.w2color', function (event) {
					...
					var customColorRow =
						$.fn.w2colorPalette[pal.length - 2];
					if (customColorRow.indexOf(tmp) !== -1) {
						customColorRow.splice(
							customColorRow.indexOf(tmp),
							1,
						);
					}
			...
			$('#transparent-color-button').on(
				'mousedown.w2color keydown.w2color',
				function () {
					$(el).data('_color', '');
		...
		if (options.color)
			options.color = String(options.color).toUpperCase();
		...
		if ($('#w2ui-overlay').length === 0) {
			...
			// only refresh contents
			$('#w2ui-overlay .w2ui-color')
				.parent()
				.html(getColorHTML(pal, options));
		}
		...
		var updatePalette = function () {
			var palette = $(
				'#w2ui-overlay .color-palette-selector select option:selected',
			).get(0).value;
			...
			$('#w2ui-overlay .w2ui-color')
				...
				.html(getColorHTML(pal, options));
			bindEvents(pal);
			$('#w2ui-overlay .color-palette-selector select').on(
				'change',
				updatePalette,
			);
		};
		...
		function getColorHTML(pal, options) {
			...
			html +=
				'<button id="transparent-color-button" onmousedown="event.stopPropagation(); event.preventDefault()" class="jsdialog ui-pushbutton auto-color-button">' +
				buttonText +
				'</button>';

			html +=
				'<div class="jsdialog ui-listbox-container color-palette-selector">' +
			...
			for (var i in window.app.colorPalettes) {
				html +=
					...
					i +
					'" ' +
					(i === currentPalette ? 'selected="selected"' : '') +
					'>' +
					window.app.colorPalettes[i].name +
					'</option>';
			}
			html +=
				'</select><span class="jsdialog ui-listbox-arrow"/></div>';

			var detailedPalette =
				window.app.colorPalettes[currentPalette].colors;

			html +=
				'<div class="w2ui-color" onmousedown="event.stopPropagation(); event.preventDefault()">' + // prevent default is needed otherwiser selection gets unselected
			...
			for (var i = 0; i < pal.length - 2; i++) {
				...
				if (i < 2)
					html +=
						'<tr><td style="height: 8px" colspan="8"></td></tr>';
			}
			...
			html +=
				...
				'   <td><div class="color" tabIndex=0 style="background-color: #' +
				tmp1[0] +
				';" name="' +
				tmp1[0] +
				'" index="8:0">' +
				(options.color == tmp1[0] ? '&#149;' : '&#160;') +
				'</div></td>' +
				'   <td><div class="color" tabIndex=0 style="background-color: #' +
				tmp1[1] +
				';" name="' +
				...
				tmp1[3] +
				'" index="8:0">' +
				(options.color == tmp1[3] ? '&#149;' : '&#160;') +
				'</div></td>' +
				'</tr>' +
				'<tr><td style="height: 4px" colspan="8"></td></tr>' +
				'<tr><td style="text-align: left;" colspan="8"><span style="margin-left: 1px;">' +
				_('Recent') +
				'</span></td></tr>' +
				'<tr>' +
				'   <td><div class="color" tabIndex=0 style="background-color: #' +
	...
	w2field.prototype = {
		...
		init: function () {
			...
			if (
				...
				console.log(
					'ERROR: w2field could only be applied to INPUT or TEXTAREA.',
					this.el,
				);
				return;
			}
			...
			switch (this.type) {
				...
				case 'hex':
					defaults = {
						...
						currencyPrecision:
							w2utils.settings.currencyPrecision,
					...
					options.numberRE = new RegExp(
						'[' + options.groupSymbol + ']',
						'g',
					);
					options.moneyRE = new RegExp(
						'[' +
							options.currencyPrefix +
							options.currencySuffix +
							options.groupSymbol +
							']',
						'g',
					);
					options.percentRE = new RegExp(
						'[' + options.groupSymbol + '%]',
						'g',
					);
					// no keyboard support needed
					if (
						['text', 'alphanumeric', 'hex', 'bin'].indexOf(
							this.type,
						) != -1
					) {
				...
				case 'color':
					defaults = {
						...
						suffix:
							'<div style="width: ' +
							(parseInt($(this.el).css('font-size')) ||
								12) +
							'px">&#160;</div>',
						arrows: false,
						keyboard: false,
				...
				case 'combo':
					...
					if (this.type == 'list') {
						...
						if (
							!$.isPlainObject(options.selected) &&
							options.items
						) {
							for (
								var i = 0;
								...
								if (
									item &&
									item.id == options.selected
								) {
									options.selected = $.extend(
										true,
										{},
										item,
									);
					...
					if (options.selected.text != null)
						$(this.el).val(options.selected.text);
					break;
		...
		clear: function () {
			...
			if (['money', 'currency'].indexOf(this.type) != -1) {
				$(this.el).val(
					$(this.el).val().replace(options.moneyRE, ''),
				);
			}
			...
			if (tmp != null) {
				$(this.el).height('auto');
				if (tmp && tmp['old-padding-left'])
					$(this.el).css(
						'padding-left',
						tmp['old-padding-left'],
					);
				if (tmp && tmp['old-padding-right'])
					$(this.el).css(
						'padding-right',
						tmp['old-padding-right'],
					);
				if (tmp && tmp['old-background-color'])
					$(this.el).css(
						'background-color',
						tmp['old-background-color'],
					);
				if (tmp && tmp['old-border-color'])
					$(this.el).css(
						'border-color',
						tmp['old-border-color'],
					);
			...
			if (['list'].indexOf(this.type) != -1) {
				...
				setTimeout(function () {
					...
					if (!$.isEmptyObject(selected) && options.icon) {
						options.prefix =
							'<span class="w2ui-icon ' +
							options.icon +
							'"style="cursor: pointer; font-size: 14px;' +
							' display: inline-block; margin-top: -1px; color: #7F98AD;' +
							options.iconStyle +
							'">' +
							'</span>';
						obj.addPrefix();
					...
					if ($(focus).val() === '') {
						...
						$(obj.el).val('');
						setTimeout(function () {
							if (obj.helpers.prefix)
								obj.helpers.prefix.hide();
							var tmp =
								'position: absolute; opacity: 0; margin: 4px 0px 0px 2px; background-position: left !important;';
							if (options.icon) {
								...
								$(obj.helpers.focus)
									...
									.attr(
										'style',
										tmp +
											'width: 11px !important; opacity: 1; display: block',
									);
							} else {
								...
								$(obj.helpers.focus)
									...
									.attr(
										'style',
										tmp +
											'width: 0px !important; opacity: 0; display: none',
									);
							}
						}, 1);
					}
					...
					if (
						$(obj.el).prop('readonly') ||
						$(obj.el).prop('disabled')
					) {
						setTimeout(function () {
			...
			if (['enum', 'file'].indexOf(this.type) != -1) {
				...
				if (selected) {
					for (var s = 0; s < selected.length; s++) {
						...
						if (typeof options.renderItem == 'function') {
							ren = options.renderItem(
								it,
								s,
								'<div class="w2ui-list-remove" title="' +
									w2utils.lang('Remove') +
									'" index="' +
									s +
									'">&#160;&#160;</div>',
				...
				if (html !== '') {
					...
					var style =
						...
						'; ' +
						'box-sizing: ' +
						$(this.el).css('box-sizing') +
						'; ' +
						'line-height: ' +
						$(this.el).css('line-height') +
						'; ' +
						'font-size: ' +
						$(this.el).css('font-size') +
						'; ' +
						'font-family: ' +
						$(this.el).css('font-family') +
						'; ';
					div.prepend(
						'<div class="w2ui-enum-placeholder" style="' +
							style +
							'">' +
				...
				// ITEMS events
				div.off('scroll.w2field')
					.on('scroll.w2field', function (event) {
					...
					.on('click', function (event) {
						...
						// default behavior
						if (
							$(event.target).hasClass('w2ui-list-remove')
						) {
							if (
								$(obj.el).prop('readonly') ||
								$(obj.el).prop('disabled')
							...
							if (obj.type == 'file') {
								...
								var td1 =
									'style="padding: 3px; text-align: right; color: #777;"';
								...
								preview +=
									...
									td2 +
									'>' +
									'        <span style="width: 200px; display: block-inline; overflow: hidden; text-overflow: ellipsis; white-space: nowrap="nowrap";">' +
									item.type +
									'</span>' +
									'    </td></tr>' +
									'    <tr><td ' +
					...
					.on('mouseover', function (event) {
						...
						if ($(target).data('mouse') == 'out') {
							var item =
								selected[$(event.target).attr('index')];
							// trigger event
							var edata = obj.trigger({
								phase: 'before',
								type: 'mouseOver',
				...
				$(this.el).height('auto');
				var cntHeight =
					$(div).find('> div.w2ui-multi-items').height() +
					w2utils.getSize(div, '+height') * 2;
				if (cntHeight < 26) cntHeight = 26;
				if (cntHeight > options.maxHeight)
					cntHeight = options.maxHeight;
		...
		resize: function () {
			...
			if (suffix) {
				obj.options.suffix =
					'<div class="arrow-down" style="margin-top: ' +
					(parseInt($(obj.el).height()) - 6) / 2 +
					'px;"></div>';
				obj.addSuffix();
			}
			if (prefix) {
		...
		clean: function (val) {
			...
			// clean
			if (
				['int', 'float', 'money', 'currency', 'percent'].indexOf(
					this.type,
				) != -1
			) {
				if (typeof val == 'string') {
					if (
						options.autoFormat &&
						['money', 'currency'].indexOf(this.type) != -1
					)
						val = String(val).replace(options.moneyRE, '');
					if (options.autoFormat && this.type == 'percent')
						val = String(val).replace(options.percentRE, '');
					if (
						options.autoFormat &&
						['int', 'float'].indexOf(this.type) != -1
					)
		...
		click: function (event) {
			...
			if (
				['date', 'time', 'color', 'datetime'].indexOf(this.type) !=
				-1
			) {
				this.updateOverlay();
			}
		...
		focus: function (event) {
			...
			// color, date, time
			if (
				['color', 'date', 'time', 'datetime'].indexOf(obj.type) !==
				-1
			) {
				if (
					$(obj.el).prop('readonly') ||
			...
			if (['date', 'time', 'datetime'].indexOf(obj.type) != -1) {
				...
				if (val !== '' && !obj.inRange(obj.el.value)) {
					...
					if (
						obj.type == 'date' &&
						val !== '' &&
						!w2utils.isDate(obj.el.value, options.format)
					) {
						$(obj.el).val('').removeData('selected').change();
						if (options.silent === false) {
						...
						obj.type == 'datetime' &&
						val !== '' &&
						!w2utils.isDateTime(obj.el.value, options.format)
					) {
						$(obj.el).val('').removeData('selected').change();
						if (options.silent === false) {
							$(obj.el).w2tag('Not a valid date');
		...
		keyDown: function (event, extra) {
			...
			if (
				...
				var val =
					parseFloat(
						$(obj.el).val().replace(options.moneyRE, ''),
					) || 0;
			...
			if (obj.type == 'date') {
				...
				switch (key) {
					case 38: // up
						if (event.shiftKey) break; // no action if shift key is pressed
						var newDT = w2utils.formatDate(
							dt.getTime() + daymil,
							options.format,
						);
						if (inc == 10)
							newDT = w2utils.formatDate(
								new Date(
									dt.getFullYear(),
									dt.getMonth() + 1,
									dt.getDate(),
			...
			if (obj.type == 'datetime') {
				...
				switch (key) {
					case 38: // up
						if (event.shiftKey) break; // no action if shift key is pressed
						var newDT = w2utils.formatDateTime(
							dt.getTime() + daymil,
							options.format,
						);
						if (inc == 10)
							newDT = w2utils.formatDateTime(
								new Date(
									dt.getFullYear(),
									dt.getMonth() + 1,
									dt.getDate(),
		...
		keyUp: function (event) {
			...
			if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
				...
				if (
					[16, 17, 18, 20, 37, 39, 91].indexOf(event.keyCode) ==
					-1
				) {
		...
		request: function (interval) {
			...
			if (
				options.minLength !== 0 &&
				search.length < options.minLength
			) {
			...
			if (
				options.url &&
				$(obj.el).prop('readonly') !== true &&
				$(obj.el).prop('disabled') !== true &&
				((options.items.length === 0 && obj.tmp.xhr_total !== 0) ||
					(obj.tmp.xhr_total == options.cacheMax &&
						search.length > obj.tmp.xhr_search.length) ||
					(search.length >= obj.tmp.xhr_search.length &&
						search.substr(0, obj.tmp.xhr_search.length) !=
							obj.tmp.xhr_search) ||
					search.length < obj.tmp.xhr_search.length)
			) {
				// empty list
					...
					obj.tmp.xhr = $.ajax(ajaxOptions)
						.done(function (data, status, xhr) {
							...
							if (
								...
								console.log(
									'ERROR: server did not return proper structure. It should return',
									{
										status: 'success',
										records: [
											{ id: 1, text: 'item' },
							...
							if (
								...
								options.recId = options.recid; // since lower-case recid is used in grid
							if (options.recId || options.recText) {
								data.records.forEach(function (item) {
									if (
										typeof options.recId ==
										'string'
									)
										item.id = item[options.recId];
									if (
										typeof options.recId ==
										'function'
									)
										item.id = options.recId(item);
									if (
										typeof options.recText ==
										'string'
									)
										item.text =
											item[options.recText];
									if (
										typeof options.recText ==
										'function'
									)
										item.text =
											options.recText(item);
								});
							}
							// remember stats
			...
			if (obj.tmp.xhr_loading !== true) {
				...
				for (var i = 0; i < options.items.length; i++) {
					...
					if (options.compare != null) {
						...
						var prefix = '';
						var suffix = '';
						if (['is', 'begins'].indexOf(options.match) != -1)
							prefix = '^';
						if (['is', 'ends'].indexOf(options.match) != -1)
							suffix = '$';
						try {
							var re = new RegExp(
								prefix + search + suffix,
				...
				setTimeout(function () {
					...
					if (
						options.markSearch &&
						html.indexOf('$.fn.w2menuHandler') != -1
					) {
		...
		updateOverlay: function (indexOnly) {
			...
			if (this.type == 'date') {
				...
				if ($('#w2ui-overlay').length === 0) {
					$(obj.el).w2overlay(
						'<div class="w2ui-reset w2ui-calendar" onclick="event.stopPropagation();"></div>',
						{
				...
				(function refreshCalendar(month, year) {
					...
					$('#w2ui-overlay .w2ui-calendar-title').on(
						'mousedown',
						function () {
							if (
								$(this)
									.next()
									.hasClass('w2ui-calendar-jump')
							) {
								$(this).next().remove();
							} else {
								var selYear, selMonth;
								$(this).after(
									'<div class="w2ui-calendar-jump" style=""></div>',
								);
								$(this)
									.next()
									.hide()
									.html(obj.getYearHTML())
									.fadeIn(200);
								setTimeout(function () {
									$(
										...
										.find(
											'.w2ui-jump-month, .w2ui-jump-year',
										)
										.on('click', function () {
											if (
												$(this).hasClass(
									...
									$(
										'#w2ui-overlay .w2ui-calendar-jump >:last-child',
									).prop('scrollTop', 2000);
								}, 1);
							}
						},
			...
			if (this.type == 'time') {
				...
				if ($('#w2ui-overlay').length === 0) {
					$(obj.el).w2overlay(
						'<div class="w2ui-reset w2ui-calendar-time" onclick="event.stopPropagation();"></div>',
						{
				...
				if (
					this.options.noMinutes == null ||
					this.options.noMinutes === false
				) {
					$('#w2ui-overlay .w2ui-time').on(
						'mouseup',
						function () {
							...
							if ($('#w2ui-overlay').length > 0)
								$('#w2ui-overlay')[0].hide();
							$(obj.el).w2overlay(
								'<div class="w2ui-reset w2ui-calendar-time"></div>',
								{ css: { 'background-color': '#fff' } },
							);
							$('#w2ui-overlay > div').html(
								obj.getMinHTML(hour),
							);
			...
			if (this.type == 'datetime') {
				...
				if ($('#w2ui-overlay .w2ui-time').length > 0)
					$('#w2ui-overlay')[0].hide();
				if ($('#w2ui-overlay').length === 0) {
					$(obj.el).w2overlay(
						'<div class="w2ui-reset w2ui-calendar" onclick="event.stopPropagation();"></div>',
				...
				(function refreshCalendar(month, year) {
					$('#w2ui-overlay > div > div').html(
						obj.getMonthHTML(month, year, $(obj.el).val()) +
							(options.btn_now
								? '<div class="w2ui-calendar-now now">' +
									w2utils.lang(
										'Current Date & Time',
									) +
									'</div>'
						...
						.on('mouseup', function () {
							...
							$('#w2ui-overlay .w2ui-time').on(
								...
								function (event) {
									...
									var txt = w2utils.formatDateTime(
										selDate,
										obj.options.format,
									);
							...
							if (
								obj.options.noMinutes == null ||
								obj.options.noMinutes === false
							) {
								$('#w2ui-overlay .w2ui-time').on(
									'mouseup',
									function () {
					...
					$('#w2ui-overlay .now')
						.on('mousedown', function () {
							...
							var tmp = w2utils.formatDateTime(
								new Date(),
								obj.options.format,
							);
			...
			if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
				...
				if ($(input).is(':focus')) {
					...
					if (
						options.url != null &&
						$(input).val().length < options.minLength &&
						obj.tmp.emptySet !== true
					)
						msgNoItems =
							options.minLength +
							' ' +
							w2utils.lang('letters or more...');
					if (
						options.url != null &&
						$(input).val() === '' &&
						obj.tmp.emptySet !== true
					)
						msgNoItems = w2utils.lang('Type to search...');
					if (options.url == null && options.items.length === 0)
						msgNoItems = w2utils.lang('Empty list');
					if (options.msgNoItems != null)
						msgNoItems = options.msgNoItems;
					if (msgNoItems == 'function')
						msgNoItems = msgNoItems(options);
					$(el).w2menu(
						!indexOnly ? 'refresh' : 'refresh-index',
						$.extend(true, {}, options, {
							search: false,
			...
			if (this.type == 'date') {
				...
				if (dt) {
					...
					if (this.options.start || this.options.end) {
						var st =
							typeof this.options.start == 'string'
								? this.options.start
								: $(this.options.start).val();
						var en =
							typeof this.options.end == 'string'
								? this.options.end
								: $(this.options.end).val();
						var start = w2utils.isDate(
							st,
							this.options.format,
							true,
						);
						var end = w2utils.isDate(
							en,
							this.options.format,
							true,
						);
					...
					if (
						...
						$.inArray(str, this.options.blocked) != -1
					)
						inRange = false;
                    ...
                    */
					if (
						this.options.blockWeekDays != null &&
						this.options.blockWeekDays != undefined &&
				...
				if (dt) {
					...
					if (this.options.start || this.options.end) {
						...
						if (
							...
							this.options.start instanceof Date
						) {
						...
						if (
							typeof this.options.end == 'object' &&
							this.options.end instanceof Date
						) {
					...
					if (inRange && this.options.blocked) {
						for (
							var i = 0;
							i < this.options.blocked.length;
							i++
						) {
							var blocked = this.options.blocked[i];
							if (typeof blocked == 'string') {
								// convert string to Date object
								blocked = w2utils.isDateTime(
									blocked,
									this.options.format,
									true,
								);
							...
							if (
								...
								blocked instanceof Date &&
								blocked.getFullYear() ==
									dt.getFullYear() &&
								blocked.getMonth() == dt.getMonth() &&
								blocked.getDate() == dt.getDate()
							) {
								inRange = false;
								break;
							}
		...
		checkType: function (ch, loose) {
			...
			switch (obj.type) {
				case 'int':
					if (
						loose &&
						['-', obj.options.groupSymbol].indexOf(ch) != -1
					)
						return true;
					return w2utils.isInt(
						ch.replace(obj.options.numberRE, ''),
					);
				case 'percent':
					ch = ch.replace(/%/g, '');
				case 'float':
					if (
						...
						[
							...
							obj.options.groupSymbol,
						].indexOf(ch) != -1
					)
						return true;
					return w2utils.isFloat(
						ch.replace(obj.options.numberRE, ''),
					);
				case 'money':
				case 'currency':
					if (
						...
						[
							'-',
							obj.options.decimalSymbol,
							obj.options.groupSymbol,
							obj.options.currencyPrefix,
							obj.options.currencySuffix,
						].indexOf(ch) != -1
					)
						return true;
					return w2utils.isFloat(
						ch.replace(obj.options.moneyRE, ''),
					);
				case 'bin':
		...
		addPrefix: function () {
			...
			setTimeout(function () {
				...
				var helper;
				var tmp = $(obj.el).data('tmp') || {};
				if (tmp['old-padding-left'])
					$(obj.el).css('padding-left', tmp['old-padding-left']);
				...
				if (obj.options.prefix !== '') {
					...
					helper
						.css({
							...
							'padding-bottom': $(obj.el).css(
								'padding-bottom',
							),
							'padding-left': $(obj.el).css(
								'padding-left',
							),
							'padding-right': 0,
							'margin-top':
								parseInt(
									$(obj.el).css('margin-top'),
									10,
								) +
								2 +
								'px',
						...
						.on('click', function (event) {
							if (
								obj.options.icon &&
								typeof obj.onIconClick == 'function'
							) {
								// event before
								var edata = obj.trigger({
									phase: 'before',
					...
					$(obj.el).css(
						'padding-left',
						helper.width() +
							parseInt($(obj.el).css('padding-left'), 10) +
							'px',
					);
					// remember helper
					...
					helper
						.css({
							...
							height:
								$(obj.el).height() +
								parseInt(
									$(obj.el).css('padding-top'),
									10,
								) +
								parseInt(
									$(obj.el).css('padding-bottom'),
									10,
								) +
								'px',
							padding: 0,
		...
		addFocus: function () {
			...
			helper
				.css({
					...
					'margin-left':
						parseInt($(obj.el).css('margin-left')) +
						parseInt($(obj.el).css('padding-left')) +
						'px',
			...
			helper
				...
				.on('blur', function (event) {
					...
					if (pholder != null)
						$(obj.el).attr('placeholder', pholder);
				})
				.on('keydown', function (event) {
					var el = this;
					obj.keyDown(event);
		...
		addMulti: function () {
			...
			if (obj.type == 'enum') {
				...
				html =
					'<div class="w2ui-field-helper w2ui-list" style="' +
					margin +
					'; box-sizing: border-box">' +
					'    <div style="padding: 0px; margin: 0px; display: inline-block" class="w2ui-multi-items">' +
					'    <ul>' +
					'        <li style="padding-left: 0px; padding-right: 0px" class="nomouse">' +
					'            <input type="text" style="width: 20px; margin: -3px 0 0; padding: 2px 0; border-color: white" autocomplete="off"' +
					($(obj.el).prop('readonly')
						? ' readonly="readonly"'
						: '') +
					($(obj.el).prop('disabled')
		...
		addFile: function (file) {
			...
			if (selected) {
				for (var s = 0; s < selected.length; s++) {
					// check for dups
					if (
						selected[s].name == file.name &&
						selected[s].size == file.size
					)
						return;
			...
			if (
				...
				newItem.size > options.maxFileSize
			) {
				err =
					'Maximum file size is ' +
					w2utils.formatSize(options.maxFileSize);
			...
			if (
				...
				err =
					'Maximum total size is ' +
					w2utils.formatSize(options.maxSize);
				if (options.silent === false) $(obj.el).w2tag(err);
				console.log('ERROR: ' + err);
				return;
			}
			...
			if ($.isArray(menu)) {
				for (var m = 0; m < menu.length; m++) {
					if (typeof menu[m] == 'string') {
						...
						if (menu[m].text != null && menu[m].id == null)
							menu[m].id = menu[m].text;
						if (menu[m].text == null && menu[m].id != null)
							menu[m].text = menu[m].id;
						if (menu[m].caption != null)
							menu[m].text = menu[m].caption;
					}
				}
				return menu;
		...
		getMonthHTML: function (month, year, selected) {
			...
			if (this.type === 'datetime') {
				var dt_sel = w2utils.isDateTime(
					selected,
					options.format,
					true,
				);
				selected = w2utils.formatDate(
					dt_sel,
					w2utils.settings.dateFormat,
				);
			}
			for (var ci = 1; ci < 43; ci++) {
				...
				if (
					options.colored &&
					options.colored[tmp_dt_fmt] !== undefined
				) {
		...
		getYearHTML: function () {
			...
			for (var m = 0; m < months.length; m++) {
				mhtml +=
					'<div class="w2ui-jump-month" name="' +
					m +
					'">' +
					months[m] +
					'</div>';
		...
		getHourHTML: function () {
			...
			var options = this.options;
			if (options == null)
				options = { format: w2utils.settings.timeFormat };
			...
			for (var a = 0; a < 24; a++) {
				...
				tmp[Math.floor(a / 8)] +=
					'<div class="' +
					(this.inRange(tm1) || this.inRange(tm2)
						? 'w2ui-time '
						: 'w2ui-blocked') +
					'" hour="' +
					a +
		...
		getMinHTML: function (hour) {
			...
			for (var a = 0; a < 60; a += 5) {
				...
				tmp[ind] +=
					'<div class="' +
					(this.inRange(tm) ? 'w2ui-time ' : 'w2ui-blocked') +
					'" min="' +
					a +
					'">' +
					time +
...
(function ($) {
	...
	$.fn.w2toolbar = function (method) {
		if ($.isPlainObject(method)) {
			...
			for (var i = 0; i < items.length; i++) {
				object.items[i] = $.extend(
					{},
					w2toolbar.prototype.item,
					items[i],
				);
				// menus
				if (object.items[i].type == 'menu-check') {
					...
					if (Array.isArray(item.items)) {
						for (var j = 0; j < item.items.length; j++) {
							var it = item.items[j];
							if (it.checked && item.selected == null)
								item.selected = it.id;
							else it.checked = false;
							if (!it.checked && item.selected == it.id)
								it.checked = true;
							if (it.checked == null) it.checked = false;
						}
					}
	...
	w2toolbar.prototype = {
			...
			for (var o = 0; o < items.length; o++) {
				...
				if (id == null) {
					...
					this.items = this.items
						.slice(0, middle)
						.concat([it], this.items.slice(middle));
				}
				this.refresh(it.id);
				this.resize();
			}
		...
		remove: function () {
			...
			for (var a = 0; a < arguments.length; a++) {
				...
				if (!it || String(arguments[a]).indexOf(':') != -1)
					continue;
				removed++;
				// remove from screen
				$(this.box)
		...
		get: function (id, returnIndex) {
			if (arguments.length === 0) {
				...
				for (var i1 = 0; i1 < this.items.length; i1++)
					if (this.items[i1].id != null)
						all.push(this.items[i1].id);
				return all;
			}
			...
			for (var i2 = 0; i2 < this.items.length; i2++) {
				...
				// find a menu item
				if (
					['menu', 'menu-radio', 'menu-check'].indexOf(
						it.type,
					) != -1 &&
					tmp.length == 2 &&
					it.id == tmp[0]
		...
		click: function (id, event) {
			...
			if (it && !it.disabled) {
				...
				if (
					...
					if (it.checked) {
						...
						setTimeout(function () {
							...
							if (
								['color', 'text-color'].indexOf(
									it.type,
								) != -1
							) {
								if (it.transparent == null)
									it.transparent = true;
								$(el).w2color(
									{
										color: it.color,
				...
				// route processing
				if (it.route) {
					var route = String('/' + it.route).replace(
						/\/{2,}/g,
						'/',
					);
					...
					if (info.keys.length > 0) {
						for (var k = 0; k < info.keys.length; k++) {
							route = route.replace(
								...
								this.routeData[info.keys[k].name],
							);
						}
					}
					setTimeout(function () {
				...
				if (
					event &&
					['button', 'check', 'radio'].indexOf(it.type) != -1
				) {
		...
		scroll: function (direction) {
			...
			switch (direction) {
				...
				case 'right':
					...
					width2 = scrollBox.find(':first').outerWidth();
					scroll = scrollLeft + width1 - 50; // 35 is width of both button
					if (scroll >= width2 - width1)
						scroll = width2 - width1;
					scrollBox.animate({ scrollLeft: scroll }, 300);
					break;
			}
			setTimeout(function () {
		...
		render: function (box) {
			...
			if (box != null) {
				if (
					$(this.box).find('> table #tb_' + this.name + '_right')
						.length > 0
				) {
					$(this.box)
						.removeAttr('name')
			...
			// render all buttons
			var html =
				'<div class="w2ui-scroll-wrapper" onmousedown="var el=w2ui[\'' +
				this.name +
				'\']; if (el) el.resize();">' +
			...
			html +=
				'</tr>' +
				'</tbody></table></div>' +
				'<div class="w2ui-scroll-left" onmousedown="event.preventDefault()" onclick="var el=w2ui[\'' +
				this.name +
				"']; if (el) el.scroll('left');\"></div>" +
				'<div class="w2ui-scroll-right" onmousedown="event.preventDefault()" onclick="var el=w2ui[\'' +
				this.name +
				"']; if (el) el.scroll('right');\"></div>";
			$(this.box)
				.attr('name', this.name)
		...
		refresh: function (id) {
			...
			var el = $(this.box).find(
				'#tb_' + this.name + '_item_' + w2utils.escapeId(it.id),
			);
			...
			if (el.length === 0) {
				...
				if (
					...
					if ($('#w2ui-overlay-' + this.name).length > 0)
						$('#w2ui-overlay-' + this.name)[0].hide();
				}
				// refresh
				el.html(html);
				if (it.hidden) {
		...
		resize: function () {
			...
			var scrollBox = box.find('.w2ui-scroll-wrapper');
			if (
				scrollBox.find(':first').outerWidth() >
			...
			if (window.mode.isMobile()) {
				$(
					'#toolbar-wrapper.mobile .w2ui-scroll-right, #toolbar-wrapper.mobile .w2ui-scroll-left',
				).hide();
			}
		...
		getItemHTML: function (item) {
			var html = '';
			if (item.caption != null && item.text == null)
				item.text = item.caption; // for backward compatibility
			...
			if (item.img) {
				...
				var colorContainer =
					'<div class="selected-color-classic"></div>';
				...
				img =
					item.img == 'textcolor' ||
					item.img == 'backcolor' ||
					item.img == 'backgroundcolor'
						? '<td><div class="w2ui-tb-image w2ui-icon ' +
							item.img +
							'"></div>' +
							colorContainer +
							'</td>'
			...
			if (item.icon)
				img =
					'<td><div class="w2ui-tb-image"><span class="' +
					item.icon +
					'"></span></div></td>';
                    ...
                    */
					...
					case 'drop':
						html +=
							...
							'       onmouseover = "' +
							(!item.disabled
								? "jQuery(this).addClass('over'); w2ui['" +
									this.name +
									"'].tooltipShow('" +
									item.id +
									"', event);"
							...
							'       onmouseout  = "' +
							(!item.disabled
								? "jQuery(this).removeClass('over').removeClass('down'); w2ui['" +
									this.name +
									"'].tooltipHide('" +
									item.id +
									"', event);"
							...
							img +
							(text !== ''
								? '<td class="w2ui-tb-caption" nowrap="nowrap" style="' +
								...
								: '') +
							(item.count != null
								? '<td class="w2ui-tb-count" nowrap="nowrap"><span>' +
									item.count +
									'</span></td>'
								: '') +
							([
		...
		tooltipShow: function (id, event, forceRefresh) {
			...
			var $el = $(this.box).find(
				'#tb_' + this.name + '_item_' + w2utils.escapeId(id),
			);
			var item = this.get(id);
			...
			setTimeout(function () {
				if (
					...
					if (
						...
						return; // not for opened drop downs
		...
		menuClick: function (event) {
			...
			if (event.item && !event.item.disabled) {
				...
				if (typeof it.route == 'string') {
					...
					if (info.keys.length > 0) {
						for (var k = 0; k < info.keys.length; k++) {
							if (obj.routeData[info.keys[k].name] == null)
								continue;
							route = route.replace(
								new RegExp(
									':' + info.keys[k].name,
