...
(function () {
	...
	var undef_type = 'undefined',
		...
		request_JSON = function (uri) {
			if (browserless) return loadFromDisk(uri);
			...
			try {
				...
				console.log(
					'Localization Error: Unable to get localization data: ' +
						uri,
					e,
				);
			}
			...
			try {
				...
				setTimeout(function () {
					...
					var l10n_err = new Error(
						'Unable to load localization data: ' + uri,
					);
		...
		load = (String_ctr[$to_locale_string] = function (data) {
			...
			return Function.prototype[$to_locale_string].apply(
				String_ctr,
				arguments,
			);
		}),
		...
		localize = (String_ctr.prototype[$to_locale_string] = function () {
			var using_default = use_default,
				current_locale =
					String_ctr[using_default ? $default_locale : $locale],
			...
			do {
				...
				if (
					locale in localizations &&
					this_val in localizations[locale]
				) {
	...
	try {
		...
		if (global.nav) {
			...
			var nodeError =
				'Problem setting nav in L10N. You are most likely running in a non-browser environment like Node.' +
	...
	if (
		...
		try {
			...
			if (global.XMLHttpRequest) {
				...
				var nodeError =
					'Problem setting XHR in L10N. You are most likely running in a non-browser environment like Node.' +
	...
	if (!browserless || typeof document !== undef_type) {
		...
		while (i--) {
			...
			if (l10n_js_media_type.test(elt.type)) {
				if (array_index_of.call(rel, 'localizations') !== -1) {
					...
					array_index_of.call(rel, 'localization') !== -1
				) {
					// single localization
					localization = {};
					localization[
						(elt.getAttribute('hreflang') || '')[
							$to_lowercase
						]()
		...
		if (global.l10NLocalFilePath) {
			...
			var nodeError =
				'Problem loading localization file. You are most likely running in a non-browser environment like Node.' +
