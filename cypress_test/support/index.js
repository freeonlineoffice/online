...
Cypress.Commands.add('cGet', function(selector, options) {
	...
	else if (selector) // This is not lool frame and there is a selector.
		return cy.get(cy.cActiveFrame).its('0.contentDocument').find('#loolframe').its('0.contentDocument').then(cy.wrap).find(selector, options);
	else // Not lool frame without a selector.
		return cy.get(cy.cActiveFrame, options).its('0.contentDocument').find('#loolframe').its('0.contentDocument').then(cy.wrap);
});
