	...
	if (cy.cActiveFrame === '#loolframe')
		if (true) {
			...
			return cy.waitUntil(() => {
				...
				//
				// document.getElementById('loolframe') - returns nothing
				// Cypress.$('loolframe') - returns nothing
				var looldocument = window.top.document.getElementsByClassName('aut-iframe')[0].contentDocument.getElementById('loolframe').contentDocument;
				console.log('Try to get ' + selector + ' on ' + looldocument);
				if (!looldocument)
					return false;
				var elem = Cypress.$(looldocument).find(selector);
...
//		return cy.waitUntil(() => cy.iframe(cy.cActiveFrame).its('0.contentDocument').get(selector,options));
		...
		//return cy.iframe(cy.cActiveFrame).find('#loolframe').its('0.contentDocument').get(selector,options);
});
