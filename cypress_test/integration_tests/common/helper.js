...
function checkLoolFrameGlobal() {
	cy.log('checkLoolFrameGlobal - start.');
	cy.get('#loolframe').its('0.contentDocument').should('exist').its('body').should('not.be.undefined');
...
function checkIfDocIsLoaded(isMultiUser) {
	...
	if (isMultiUser) {
		...
		checkFirstLoolFrameGlobal();
		checkSecondLoolFrameGlobal();
		...
		cy.frameLoaded('#loolframe');
		checkLoolFrameGlobal();
