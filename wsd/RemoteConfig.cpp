                                               ...
                                               Poco::JSON::Object::Ptr remoteJson)
{
    try
    {
        ...
        //use feature_lock.locked_hosts[@allow] entry from loolwsd.xml if feature_lock.locked_hosts.allow key does not exist in json
                                                 ...
                                                 const std::string& assetType)
{
    ...
    if (assetType == "presentation")
    {
        for (const auto& it : assets)
            if (!it.second.active)
                removeTemplate(it.second.pathName, LOOLWSD::TmpPresntTemplateDir);
    }
    ...
    std::map<std::string, AssetData>& assets, const std::string& assetType)
{
    ...
    if (assetType == "fonts")
        ...
        assetFile = LOOLWSD::TmpFontDir + '/' + Util::encodeId(Util::rng::getNext()) + ".ttf";
    else if (assetType == "presentation")
        assetFile = removeTemplate(uri, LOOLWSD::TmpPresntTemplateDir);
    ...
    if (assetType == "fonts")
        LOOLWSD::sendMessageToForKit("addfont " + assetFile);
