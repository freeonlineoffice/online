                                                ...
                                                const std::shared_ptr<StreamSocket>& socket)
{
    ...
    else if (requestDetails.equals(2, "download"))
    {
        ...
        const Poco::Path filePath(LOOLWSD::ChildRoot + jailId + JAILED_DOCUMENT_ROOT + decoded);
                                                    ...
                                                    unsigned mobileAppDocId)
{
    ...
    try
    {
        ...
        auto rvs =
            std::make_shared<RequestVettingStation>(LOOLWSD::getWebServerPoll(), requestDetails);
...
std::string ClientRequestDispatcher::getDiscoveryXML()
{
    ...
    for (unsigned long it = 0; it < listNodes->length(); ++it)
    {
        ...
        {
            const std::string ext = elem->getAttribute("ext");
            if (LOOLWSD::ViewWithCommentsFileExtensions.insert(ext).second) // Skip duplicates.
