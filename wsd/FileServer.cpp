                             ...
                             const std::string& unittest)
    {
        ...
        if (serveBrowserSetttings)
        {
            ...
            const std::string& browserSettingPath =
                LOOLWSD::FileServerRoot + "test/data/presets/user/browsersetting.json";
            if (FileUtil::Stat(browserSettingPath).exists())
            {
                Poco::JSON::Array::Ptr browsersettingArray = new Poco::JSON::Array();
                Poco::JSON::Object::Ptr configEntry = new Poco::JSON::Object();
                std::string uri = LOOLWSD::getServerURL().append(prefix + browserSettingPath);
                Util::trim(uri);
                configEntry->set("uri", uri);
                configEntry->set("stamp", etagString);
    ...
    std::istream& message, const std::shared_ptr<StreamSocket>& socket)
{
    ...
    return ResourceAccessDetails(std::move(wopiSrc), urv[ACCESS_TOKEN], urv[NO_AUTH_HEADER], urv[DEBUG_WOPI_CONFIG_ID]);
}
                                                       ...
                                                       const std::shared_ptr<StreamSocket>& socket)
{
    ...
    httpSession->asyncRequest(httpRequest, LOOLWSD::getWebServerPoll(), true);
}
                                                   ...
                                                   const std::shared_ptr<StreamSocket>& socket)
{
    ...
    ossBrandJS << "<script src=\"" << responseRoot << "/browser/" LOOLWSD_VERSION_HASH "/"
               << BRANDING << ".js\"></script>";
    ...
    if constexpr (ConfigUtil::isSupportKeyEnabled())
    {
        ...
        if (!key.verify() || key.validDaysRemaining() <= 0)
        {
            ...
            oss << "<script src=\"" << SUPPORT_KEY_BRANDING_UNSUPPORTED
                << "/browser/" LOOLWSD_VERSION_HASH ".js\"></script>";
            brandJS = oss.str();
                                                    ...
                                                    const Poco::Util::AbstractConfiguration& config)
{
    ...
    const bool hasIntegrationTheme =
        !safeThemeStr.empty() &&
        FileUtil::Stat(LOOLWSD::FileServerRoot + "/browser/dist/" + safeThemeStr).exists();
    ...
    if constexpr (ConfigUtil::isSupportKeyEnabled())
    {
        ...
        if (!key.verify() || key.validDaysRemaining() <= 0)
        {
            ...
            ossBrandCSS << "<link rel=\"stylesheet\" href=\"" << responseRoot << "/browser/"
                        << LOOLWSD_VERSION_HASH << "/" << themePrefix
    ...
    if (brandCSS.empty())
    {
        std::ostringstream ossBrandCSS;
        ossBrandCSS << "<link rel=\"stylesheet\" href=\"" << responseRoot << "/browser/"
                    << LOOLWSD_VERSION_HASH << "/" << themePrefix << BRANDING << ".css\">";
        brandCSS = ossBrandCSS.str();
    }
