...
{
    if (!LOOLWSD::getConfigValue<bool>("quarantine_files[@enable]", false) ||
...
bool Quarantine::quarantineFile(const std::string& docName)
{
    ...
    {
        sourcefilePath = LOOLWSD::ChildRoot + "tmp/lool-" + _docBroker.getJailId() + "/user/docs/" +
                         _docBroker.getJailId() + '/' + docName;
    }
    else
    {
        sourcefilePath = LOOLWSD::ChildRoot + _docBroker.getJailId() + "/tmp/user/docs/" +
                         _docBroker.getJailId() + '/' + docName;
    }
    ...
    auto& fileList = LOOLWSD::QuarantineMap[_docBroker.getDocKey()];
    if (!fileList.empty())
    {
...
void Quarantine::removeQuarantinedFiles()
{
    ...
    for (const auto& file : LOOLWSD::QuarantineMap[docKey])
    {
        FileUtil::removeFile(file);
    }

    LOOLWSD::QuarantineMap.erase(docKey);
}
