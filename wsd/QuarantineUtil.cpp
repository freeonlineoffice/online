...
{
    if (!LOOLWSD::getConfigValue<bool>("quarantine_files[@enable]", false) || isQuarantineEnabled())
        return;
...
bool Quarantine::quarantineFile(DocumentBroker* docBroker, const std::string& docName)
{
    ...
    {
        sourcefilePath = LOOLWSD::ChildRoot + "tmp/lool-" + docBroker->getJailId() + "/user/docs/" +
