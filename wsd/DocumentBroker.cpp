...
DocumentBroker::DocumentBroker(ChildType type, const std::string& uri, const Poco::URI& uriPublic,
    ...
    , _saveManager((std::getenv("LOOL_NO_AUTOSAVE") != nullptr)
    ...
    [[maybe_unused]] std::unique_ptr<WopiStorage::WOPIFileInfo> wopiFileInfo)
{
    ...
    if (session)
    {
        LOG_DBG("Setting username ["
                << LOOLWSD::anonymizeUsername(session->getUserName()) << "] and userId ["
                << LOOLWSD::anonymizeUsername(session->getUserId()) << "] for session ["
                                          ...
                                          std::unique_ptr<WopiStorage::WOPIFileInfo> wopiFileInfo)
{
    ...
    const std::string watermarkText =
        (ConfigUtil::isSupportKeyEnabled() && !LOOLWSD::OverrideWatermark.empty())
                                 ...
                                 const std::string& extendedData)
{
    ...
    static const bool forceBackgroundEnv = !!getenv("LOOL_FORCE_BGSAVE");
                                                      ...
                                                      const std::string &tag, bool sendError)
{
    ...
    if (type != ClipboardRequest::CLIP_REQUEST_SET)
        clipFile = LOOLWSD::SavedClipboards->getClipboard(tag);
    if (clipFile)
    {
...
std::string DocumentBroker::getAbsoluteMediaPath(std::string localPath)
{
    ...
    return FileUtil::buildLocalPathToJail(LOOLWSD::EnableMountNamespaces,
                                          LOOLWSD::ChildRoot + _jailId, std::move(localPath));
#else // MOBILEAPP
