...
## Build Online itself
    ...
    ./autogen.sh
	emconfigure ./configure --disable-werror --with-lokit-path=/home/tml/lo/core-lool-wasm/include --with-lo-path=/home/tml/lo/core-lool-wasm/instdir --with-lo-builddir=/home/tml/lo/core-lool-wasm --with-zstd-includes=/opt/zstd.emsc.3.1.30/include --with-zstd-libs=/opt/zstd.emsc.3.1.30/lib --with-poco-includes=/opt/poco.emsc.3.1.30/include --with-poco-libs=/opt/poco.emsc.3.1.30/lib --host=wasm32-local-emscripten
    emmake make CC=emcc CXX=em++

## Running WASM Online

There are two modes in which you can use the Wasm binary, either served from a LOOL instance or
served stand-alone from a plain web server.

The below steps should get one up and running.

### Served from a LOOL instance
...
You will need to set "Enable WASM support" to true in loolwsd.xml.in, and you may need to increase
MaxFileSizeToCacheInBytes in wsd/FileServer.cpp to avoid
```
[ loolwsd ] ERR  Failed to read file [.../browser/dist/wasm/online.wasm] or is too large to cache and serve| wsd/FileServer.cpp:1285
```
failures.
...
browser/dist/lool.html`) and point your browser at that served lool.html page with a
...
<http://localhost:6931/lool.html?file_path=/test.odt> for the emrun example).
