...
public:
    ...
    void invokeWSDTest() override
    {
        ...
        http::Request httpRequest("lool/" + wopiSRC + "/ws?WOPISrc=/wopi/files" + wopiSRC + "&compat=/ws/open/open/1");
        httpRequest.set("ProxyPrefix", "https://substituted-into-lool.html-from-here/");
        ...
        http::Request loadRequest("lool/" + wopiSRC + "/ws?WOPISrc=/wopi/files" + wopiSRC + "&compat=/ws/" + accessToken + "/write/2");
        loadRequest.set("ProxyPrefix", "https://substituted-into-lool.html-from-here/");
        std::ostringstream os;
        os << "B" << std::hex << "0x" << 2 << "\n0x" << loadCmd.size() << '\n';
        const std::string str = os.str();

        loadRequest.setBody("B0x1\n0x2f\nloolclient 0.1 1758919064472 1325.9000000953674\n" +
                            os.str() + loadCmd + "\n" + ".");
