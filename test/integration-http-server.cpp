...
void HTTPServerTest::testLoolPost()
{
    ...
    fprintf(stderr, "%s\n", html.c_str());
    LOK_ASSERT(html.find(_uri.getHost()) != std::string::npos);
    LOK_ASSERT(html.find("window.versionPath = '" LOOLWSD_VERSION_HASH "';") != std::string::npos);
    LOK_ASSERT(html.find("window.loolwsdVersion = '" LOOLWSD_VERSION "';") != std::string::npos);
